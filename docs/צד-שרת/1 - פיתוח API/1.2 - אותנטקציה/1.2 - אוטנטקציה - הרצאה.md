...
![דיאגרמת זרימה: המשתמש שולח שם וסיסמה ל-/login ומקבל בחזרה JWT token; לאחר מכן שולח את הטוקן לנתיב /upload_trip, השרת בודק את הטוקן מול מסד הנתונים ומזהה את המשתמש לפני שהוא מבצע את הפעולה](../../../images/Pasted image 20251115101147.png)
...
![אימות סיסמה מול hash שמור: הסיסמה שהוזנה עוברת דרך bcrypt וה-hash שלה מושווה ל-hash שבמסד הנתונים, התאמה מחזירה token ואי-התאמה מחזירה 401](../../../images/illustrations/srv-1-2-password-hash.svg)
