...
![דיאגרמה ממוספרת: בקשת HTTP מגיעה ל-route (1), שמאמת מול schema (2), מעביר ל-controller (3), שפונה ל-model (4), שניגש למסד הנתונים (5)](../../../images/Pasted image 20260127160008.png)
...
4. הcontroller ניגש לmodel של מלון במסד הנתונים, ועושה query על כל המלונות דרך המודל.
...
חלק מההיתרונות של FastAPI היא היצירה האוטומטית של interface גרפי שמציג את כל ה־Routes שלכם בצורה אינטראקטיבית.  
...
![צינור אימות הקלט ב-FastAPI: גוף הבקשה נבדק מול הסכמה של Pydantic, קלט תקין ממשיך ליצירת המשתמש עם 201 וקלט שגוי מוחזר עם שגיאת 422](../../../images/illustrations/srv-1-1-validation-pipeline.svg)
...
וכאשר כל routing מבצע פעולות שמוגדרות בcontroller-ים
...
def get_product(id: int):
    product = None  # assume the product was not found
    if not product:
...
async def global_exception_handler(request: Request, exc: Exception):
    return JSONResponse(
        status_code=500,
        content={"error": str(exc), "message": "General server error"}
    )
```
