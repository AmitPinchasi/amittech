...
![איור המדגים את מסלול הבקשה ב-Express: הבקשה עוברת דרך ה-middleware, נמצא ה-route המתאים, נבדק אם ה-token תקין, ובמסלול המוצלח ה-controller פונה ל-database ומחזיר JSON, אחרת מוחזרת שגיאת 401](../../../images/illustrations/srv-3-3-express-lifecycle.svg)
