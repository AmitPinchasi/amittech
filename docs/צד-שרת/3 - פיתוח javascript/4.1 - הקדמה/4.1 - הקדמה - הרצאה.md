...
let person = { name: "Danny", age: 30 }; // object
let empty = null; // null
...
if (hour < 12) {
  ...
  console.log("Good afternoon!"); // printed
} else {
...
![איור המדגים את שרשרת ה-Promise של fetch: כאשר התשובה מגיעה בהצלחה רצים בזה אחר זה שני ה-then, ואם קורית שגיאה הזרימה קופצת ל-catch](../../../images/illustrations/srv-3-1-fetch-then-chain.svg)
