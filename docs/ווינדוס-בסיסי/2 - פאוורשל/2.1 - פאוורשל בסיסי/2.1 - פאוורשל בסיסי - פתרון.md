### פתרון לתרגילים מסכמים

1. **הפקודות הבסיסיות של PowerShell:**

- כדי ללמוד על הפקודה `Get-ChildItem`:

```powershell
Get-Help Get-ChildItem
```

- סוג האובייקט המוחזר הוא **FileInfo** או **DirectoryInfo**, תלוי אם מדובר בקובץ או בתיקייה.

- לסנן קבצים שבתיקייה הנוכחית שמכילים את המילה "log" בשם שלהם:

```powershell
Get-ChildItem | Where-Object {$_.Name -like "*log*"}
```

2. **יצירת משתנים ועבודה עם אובייקטים:**

- יצירת משתנה בשם `$greeting` והצגת אורך המחרוזת:

```powershell
$greeting = "Welcome to PowerShell!"
Write-Output $greeting
$greeting.Length
```

3. **שימוש ב-Pipe:**

- הצגת כל הprocesses במערכת:

```powershell
Get-Process
```

- לסנן processes שמכילים את המילה "chrome":

```powershell
Get-Process | Where-Object {$_.Name -like "*chrome*"}
```

- עצירת processes שמכילים את המילה "chrome":

```powershell
Get-Process | Where-Object {$_.Name -like "*chrome*"} | Stop-Process
```

4. **השתמשו ב-.NET Framework:**

- יצירת אובייקט `DateTime` והצגת התאריך והשעה בפורמט הרצוי:

```powershell
$dateTime = [System.DateTime]::Now
$dateTime.ToString("dd/MM/yyyy HH:mm")
```

5. **ניהול קבצים:**

- יצירת תיקייה חדשה בשם "PowerShellFiles":

```powershell
New-Item -Path "C:\PowerShellFiles" -ItemType Directory
```

- יצירת קובץ טקסט חדש בתיקייה:

```powershell
New-Item -Path "C:\PowerShellFiles\example.txt" -ItemType File
```

- לוודא שהקובץ נמצא בתיקייה:

```powershell
Get-ChildItem -Path "C:\PowerShellFiles"
```

6. **התממשקות עם PowerShell:**

- פתח את PowerShell דרך ה-Windows Terminal:
- חפש "Windows Terminal" בתפריט ההתחל, ואז בחר ב-"Windows Terminal" ופתח את PowerShell.
- הצגת כל הפקודות הזמינות ב-PowerShell:

```powershell
Get-Command
```

- סוגי פקודות: **Cmdlet** (לדוג' `Get-Process`) ו-**Alias** (לדוג' `echo` שהיא alias של `Write-Output`).
7. **עבודה עם Process:**

- הצגת כל הprocesses:

```powershell
Get-Process
```

- סינון processes בשם "explorer":

```powershell
Get-Process | Where-Object {$_.Name -like "*explorer*"}
```

- עצירת process ה-"explorer":

```powershell
Get-Process | Where-Object {$_.Name -like "*explorer*"} | Stop-Process
```
