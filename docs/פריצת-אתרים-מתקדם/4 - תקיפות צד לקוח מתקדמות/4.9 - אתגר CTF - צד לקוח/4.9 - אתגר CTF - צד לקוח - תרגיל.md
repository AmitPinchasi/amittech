...
## שלב 2: הpollution פרוטוטייפ להסלמה ל-Stored XSS
...
השתמשו בpollution פרוטוטייפ דרך metadata של תגובה כדי לשנות את `commentTemplate` ולהזריק XSS ל-stored context.

### רמזים

1. הפונקציה `mergeConfig` פגיעה לpollution פרוטוטייפ
...
## שלב 4: הinjection CSS לexfiltration CSRF Token של ה-admin
...
ה-CSP מגביל סקריפטים, אבל `style-src 'self'` מאפשר CSS מהאתר עצמו. יש נקודת injection ב-CSS:
...
1. השתמשו בinjection CSS כדי לחלץ את ה-CSRF token תו אחר תו
...
- איך מפעילים את הexfiltration מהדפדפן של ה-admin?
...
| מעבדה | נושא | רמה |
|-------|------|-----|
| [CSP bypass](https://portswigger.net/web-security/cross-site-scripting/content-security-policy) | הbypass CSP | Expert |
| [Prototype pollution to XSS](https://portswigger.net/web-security/prototype-pollution) | הpollution פרוטוטייפ | Practitioner |
| [DOM clobbering](https://portswigger.net/web-security/dom-based/dom-clobbering) | שיבוש DOM | Expert |
| [DOM XSS via postMessage](https://portswigger.net/web-security/dom-based/controlling-the-web-message-source) | הexploit postMessage | Practitioner |
...
| אתגר | תיאור |
|-------|--------|
| HackTheBox - CSP Bypass challenges | אתגרי bypass CSP ברמות שונות |
...
```
1. review the source code carefully
2. identify the CSP and find weaknesses
3. find all the sinks (innerHTML, eval, location, etc.)
4. find all the sources (URL params, postMessage, WebSocket, etc.)
5. map the data flow from source to sink
6. look for gadgets (prototype pollution, DOM clobbering)
7. build the exploitation chain step by step
8. test each step separately before combining them
```

### כלים שימושיים

```
- Burp Suite - intercepting and manipulating traffic
- DevTools Console - testing payloads
- DevTools Application > Service Workers - monitoring SW
- CSP Evaluator - analyzing the CSP policy
- DOMPurify test page - testing payloads against DOMPurify
```
