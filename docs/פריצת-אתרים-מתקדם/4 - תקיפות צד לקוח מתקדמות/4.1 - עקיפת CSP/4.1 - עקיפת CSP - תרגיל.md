...
נתונות המדיניות הבאות. עבור כל אחת, זהו את החולשה וכתבו payload לbypass.
...
### מעבדה 1: bypass CSP עם AngularJS
...
- רמה: Expert
- משימה: הריצו `alert(document.cookie)` תוך bypass CSP שמרשה `cdnjs.cloudflare.com`
- רמז: השתמשו ב-AngularJS sandbox escape

### מעבדה 2: bypass CSP עם DOM XSS
...
- רמה: Practitioner
- משימה: גנבו CSRF token תוך bypass CSP

### מעבדה 3: bypass CSP עם policy injection
...
ודאו שהמדיניות עמידה בפני כל 10 טכניקות הbypass שנלמדו.
...
## תרגיל 8: אתגר מתקדם - chaining bypasses
