...
## תרגיל 2: exploit postMessage ל-XSS
...
```javascript
// inside the iframe of banking.example.com
...
```javascript
window.addEventListener('message', function(event) {
  // "supports" local files and sandboxed iframes
...
- רמה: Practitioner
- משימה: הריצו `print()` תוך bypass בדיקת origin חלקית
