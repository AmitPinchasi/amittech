...
5. **Hackvertor** - encoding וdecoding
...
**חוק 1 - הוספת header X-Forwarded-For:**
- Type: Request header
- Match: (ריק)
...
**אימות:** גלשו לאתר שמחזיר headers CSP ו-X-Frame-Options. ודאו ב-Burp Proxy שהheaders הוסרו מהתגובה (השוו בין הבקשה המקורית בטאב HTTP History לבין מה שהדפדפן קיבל).
...
**שאלה:** האם נמצאו פרמטרים או headers שמשנים את התנהגות האפליקציה?

---

## חלק ז - Hackvertor

### משימה 10 - תרגול encoding payloads

1. שלחו בקשה ל-Repeater
2. תרגלו את תגיות Hackvertor הבאות:

הencoding payload XSS בדרכים שונות:
...
3. לכל encoding, שלחו את הבקשה ובדקו מה הגיע לשרת (ב-Logger++)
4. נסו ליצור chaining של encodings שעוקף את פילטר ה-XSS של DVWA ברמת High
...
3. תארו תרחיש שבו מאקרו עם extraction טוקן CSRF חיוני לביצוע בדיקה ב-Intruder
4. כיצד חוקי Match and Replace יכולים לעזור בבדיקת Clickjacking?
5. מה היתרון של Hackvertor על פני encoding ידני ב-Decoder?
