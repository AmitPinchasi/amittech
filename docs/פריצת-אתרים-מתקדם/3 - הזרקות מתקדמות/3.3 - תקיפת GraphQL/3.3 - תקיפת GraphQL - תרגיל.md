...
2. שלחו query introspection מלאה:
...
4. בנו query שמחלצת את המידע הרגיש
5. השלימו את המעבדה

---

## תרגיל 2 - bypass Introspection חסומה
...
## תרגיל 3 - bypass הגנת Brute Force עם Aliases
...
3. בנו query עם aliases לbypass ההגבלה:
...
4. כתבו סקריפט שמייצר את הquery מקובץ סיסמאות
...
1. התחברו לאפליקציה ובצעו query לפרופיל שלכם
2. שנו את ה-ID בquery כדי לגשת לפרופילים אחרים:
...
2. בנו query GraphQL שמכילה SQLi ב-variable:
...
4. מצאו ונצלו IDOR
5. מצאו ונצלו bypass הרשאות
6. נסו batching attack
7. בדקו האם יש הגבלת עומק לqueries
