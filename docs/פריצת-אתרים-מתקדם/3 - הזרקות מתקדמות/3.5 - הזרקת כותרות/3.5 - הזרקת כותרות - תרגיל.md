# הinjection headers - תרגיל

## תרגיל 1 - poisoning איפוס סיסמה
...
3. שנו את הheader של Host לכתובת Burp Collaborator:
...
## תרגיל 2 - poisoning איפוס סיסמה עם headers נוספות
...
3. מצאו איזו header עובדת
4. חלצו את ה-token והשלימו את המעבדה

---

## תרגיל 3 - bypass בקרת גישה עם Host Header
...
def set_lang():
    ...
    resp.headers['X-Language'] = lang  # Vulnerable!
    return resp
```

2. נסו להזריק headers:
...
## תרגיל 6 - bypass WAF עם X-Original-URL
...
1. נסו poisoning איפוס סיסמה - נסו את כל וריאציות ה-Host header
