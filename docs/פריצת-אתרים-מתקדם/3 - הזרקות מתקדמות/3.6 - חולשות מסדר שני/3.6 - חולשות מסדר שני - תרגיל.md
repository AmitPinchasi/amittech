...
## תרגיל 3 - זיהוי נקודות injection
...
מפו את כל נקודות הinjection האפשריות למתקפות מסדר שני:
...
```
Step 1: register with an XSS payload in the display name
Step 2: an admin views the user list
Step 3: XSS fires - the cookie is sent to the attacker
Step 4: the attacker uses the cookie to access the panel
Step 5: the attacker extracts sensitive information
```
