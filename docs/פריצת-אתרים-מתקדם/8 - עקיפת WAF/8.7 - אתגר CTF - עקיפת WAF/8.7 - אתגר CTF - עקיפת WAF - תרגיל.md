...
אתגר זה משלב את כל הטכניקות שנלמדו בפרק 8. תעברו חמישה שלבים, כל אחד דורש שילוב של טכניקות bypass שונות. כל שלב נותן דגל (flag) שמאפשר גישה לשלב הבא.
...
2. בצעו זיהוי ידני:
   ...
   # Send a malicious request and check the block page
...
נקודת injection XSS מוחזרת (Reflected XSS) בכתובת:
...
4. שלבו טכניקות:
   - תגית חלופית + event handler חלופי + encoding של הפונקציה
...
4. שלבו עם טכניקות encoding:
   ```
   # URL encoding of the address
   # Unicode characters
   # Mixing
   ```
...
השלב הסופי משלב את כל מה שלמדתם. נקודת injection פקודות:
...
- **wafw00f** - זיהוי WAF
- **SQLMap** - עם tamper scripts לbypass WAF:
  ```bash
  sqlmap -u "http://target/page?id=1" --tamper=space2comment,between,randomcase
  ```
- **Burp Suite** + Hackvertor - שרשראות encoding
- **XSStrike** - מציאת payloads XSS שעוקפים פילטרים
...
| שלב 2 | 20 נקודות | הbypass ל-XSS |
| שלב 3 | 25 נקודות | הbypass ל-SQLi |
| שלב 4 | 20 נקודות | הbypass ל-SSRF |
| שלב 5 | 25 נקודות | שילוב לכל טכניקות ל-RCE |
| **סהכ** | **100 נקודות** | |

### בונוסים

- שלב 2: הpayload XSS ללא תגיות כלל (בתוך הקשר JavaScript) - 5 נקודות בונוס
- שלב 3: הextraction כל הנתונים מכל הטבלאות - 5 נקודות בונוס
