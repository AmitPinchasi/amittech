...
## תרגיל 2 - Blind SSRF עם extraction מידע
...
3. נסו encoding כפול של הנתיב: `%25%36%31%25%36%34%25%36%64%25%36%39%25%36%65`
...
def generate_ssrf_bypasses(target_ip, target_port, target_path):
    """
    Input: internal IP, port, path
    Output: list of URLs that may bypass filters
    """
    bypasses = []
    # TODO: add all the techniques from the lecture
    # 1. Decimal representation
    # 2. Hexadecimal representation
    # 3. Octal representation
    # 4. IPv6
    # 5. Using @
    # 6. Domains pointing to 127.0.0.1
    # 7. URL encoding
...
def safe_fetch(url):
    ...
    # Step 2: sending the request (vulnerable to DNS Rebinding!)
