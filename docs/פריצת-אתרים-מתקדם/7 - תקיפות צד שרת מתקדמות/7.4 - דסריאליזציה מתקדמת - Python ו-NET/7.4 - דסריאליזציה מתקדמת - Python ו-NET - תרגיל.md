...
def detect_format(data_string):
    """
    Receives a string (possibly Base64) and identifies the serialization format
    """
    results = []

    # TODO: try to decode Base64
    try:
        decoded = base64.b64decode(data_string)
    except:
        decoded = data_string.encode() if isinstance(data_string, str) else data_string

    # TODO: check Java serialization (AC ED 00 05)

    # TODO: check Python pickle (0x80 0x02-0x05)

    # TODO: check .NET BinaryFormatter (00 01 00 00)

    # TODO: check PHP serialized (O:4:"User":2:{...})

    # TODO: check .NET ViewState

    # TODO: check Ruby Marshal (04 08)

    return results

def scan_http_request(request_text):
    """
    Scan a full HTTP request
    """
    findings = []

    # TODO: extract cookies, parameters, headers
    # TODO: check each value with detect_format
...
3. הצעת כלי exploit מתאים לכל פורמט שנמצא
4. הדפסת המלצות הגנה
...
```
POST /api/import - receives a pickle file (Content-Type: application/octet-stream)
POST /api/config - receives YAML
GET /api/export?format=pickle - returns data as pickle
