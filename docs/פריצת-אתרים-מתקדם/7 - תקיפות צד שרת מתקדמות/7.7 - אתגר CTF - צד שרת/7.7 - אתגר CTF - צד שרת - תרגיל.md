...
def scan_internal(ip, port):
    """Check whether an internal service exists"""
    ...
    try:
        ...
        # Short response time = service exists
        # Long response time = timeout = no service
...
# The SSRF sends a request to the internal service with the payload
ssrf_payload = {
...
```bash
# Try each one and check in the Collaborator
...
```bash
# Via reverse shell or command injection from the previous step
curl "http://10.0.0.10:5000/page?name={{7*7}}"
# If it returns 49 - there is SSTI
```
...
# Chain 2 (if the first is blocked)
...
```
Stage 1: Blind SSRF --> discover internal services
    |
    v
Stage 2: SSRF --> Metadata --> AWS credentials
    |
    v
Stage 3: SSRF --> internal Java service --> detect deserialization
    |
    v
Stage 4: ysoserial --> RCE on the Java server
    |
    v
Stage 5: SSTI in an internal service --> RCE --> flag
```
...
מכונות מומלצות עם chaining חולשות צד שרת:
...
```markdown
# Penetration Test Report - Server-Side CTF Challenge

## Executive Summary
[A short summary of the findings and risks]

## Findings

### Finding 1: Blind SSRF
- Severity: High
- Entry point: [URL and parameter]
- Proof of exploitation: [payload]
- Impact: [what can be done]
- Remediation recommendation: [how to fix]

### Finding 2: AWS Credential Theft
[...]
...
## Full Attack Chain
[Diagram and description of all the stages]
