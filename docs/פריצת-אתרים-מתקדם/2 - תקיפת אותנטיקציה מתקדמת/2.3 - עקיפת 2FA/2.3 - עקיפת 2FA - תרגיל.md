...
3. Create a Macro that includes:
   - GET /login
   - POST /login (עם credentials)
   - GET /2fa
4. Set the Rule to run for Intruder only
5. Set the Scope to the target address
```
...
## תרגיל 4: הbypass 2FA עם שינוי פרמטרים
...
## תרגיל 6: תרחיש מורכב - chaining טכניקות
