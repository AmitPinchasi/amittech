...
אתגר CTF רב-שלבי שמשלב את כל הטכניקות שנלמדו בפרק זה: תנאי race, פגמי לוגיקה עסקית, בלבול טיפוסים, והשמה המונית. כל שלב מספק רמז לשלב הבא. הדגל הסופי נמצא בפאנל הניהול.
...
## שלב 1 - תנאי race: הנחה מרובה

**תיאור:**
בחנות יש קוד הנחה `DISCOUNT20` שנותן 20% הנחה. הקוד אמור לעבוד פעם אחת בלבד, אבל מנגנון ההגנה פגיע לתנאי race.
...
## שלב 2 - bypass תהליך מרובה שלבים: דילוג על תשלום
...
```
Step 1: POST /api/checkout/start      -> {"cartId": "..."}
Step 2: POST /api/checkout/shipping   -> {"address": "..."}
Step 3: POST /api/checkout/payment    -> {"method": "credit", "amount": 9999}
Step 4: POST /api/checkout/confirm    -> {"orderId": "..."}
```
...
## שלב 3 - בלבול טיפוסים: הbypass אימות
...
-- attempt 2: boolean value (via JSON body or custom header)
-- the server may interpret the value as a boolean

-- attempt 3: numeric value
GET /api/premium/admin-tools HTTP/1.1
X-Admin-Token: 0

-- attempt 4: magic hash
GET /api/premium/admin-tools HTTP/1.1
...
4. **Root Me** - אתגרים בקטגוריית "Web - Server" הכוללים תנאי race ולוגיקה
