...
## תרגיל 4 - bypass בקרות גישה (Practitioner)
...
**רמז:** הבקשה המוברחת צריכה לכלול את הheader `Host` ואת `Content-Length` מתאים.

---

## תרגיל 5 - bypass בקרות גישה TE.CL (Practitioner)
...
**משימה:**
1. השרת החזיתי מוסיף headers לבקשה (כמו `X-Forwarded-For`). אתם צריכים לגלות אילו
2. השתמשו בהברחה כדי ללכוד את הheaders שהשרת החזיתי מוסיף
3. השתמשו במידע הזה כדי לגשת ל-`/admin`

**רמז:** הבריחו POST לנתיב שמציג את הקלט בתגובה (חיפוש, תגובה). הגוף של הבקשה הבאה יצורף לפרמטר ותוכלו לראות את הheaders.
...
**משימה:**
1. מצאו נקודת XSS מוחזר באפליקציה (בheader User-Agent)
...
**משימה:**
1. מצאו הפניה (redirect) שמשתמשת בheader Host
2. השתמשו בהברחה כדי להפנות את הבקשה הבאה לדומיין של exploit server
3. הגישו JavaScript זדוני דרך ה-redirect

---

## תרגיל 10 - cache poisoning דרך הברחה (Expert)

**מעבדה:** Exploiting HTTP request smuggling to perform web cache poisoning

**משימה:**
1. שלבו הברחת בקשות עם cache poisoning
2. גרמו לכך שמשאב JavaScript שנשמר בcache יכיל קוד זדוני
3. הפעילו `alert(document.cookie)` על כל מבקר באתר

**רמז:** הבריחו בקשה שנתיב ה-URL שלה מצביע על ה-JavaScript הסטטי, אבל ה-Host מצביע על שרת התוקף. התגובה מהשרת הזדוני תישמר בcache.

---

## תרגיל 11 - poisoning תור תגובות (Expert)
...
#!/usr/bin/env python3
"""
Write a script that receives a URL and checks:
1. Whether there is a CL.TE vulnerability (using timing)
2. Whether there is a TE.CL vulnerability (using timing)
3. Whether there is a TE.TE vulnerability (trying different obfuscations)

The script should:
- Send raw HTTP requests using a socket
- Measure the response time
- Decide based on timeout whether there is a vulnerability
- Print the findings
"""
```
