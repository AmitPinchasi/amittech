...
```bash
# Servers that respond
grep "\[200\]" live_subs.txt

# Servers with 403 (may contain hidden content)
grep "\[403\]" live_subs.txt

# Servers with 404 (potential for takeover)
...
```bash
# Look for CNAME that points to known services:
# *.github.io
# *.herokuapp.com
# *.s3.amazonaws.com
# *.azurewebsites.net
...
class TakeoverLabHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if host == "blog.vulnerable.lab":
            # simulates a GitHub Pages that doesn't exist
            ...
            # simulates a Shopify that doesn't exist
            self.send_response(404)
            self.send_header("Content-Type", "text/html")
            self.end_headers()
            self.wfile.write(b"""
...
```python
# subdomain_audit.py - script skeleton
import subprocess
import json
import requests
...
def enumerate_subdomains(domain):
    # TODO: enumerate from crt.sh
    pass

def check_cname(subdomain):
    # TODO: check the CNAME record
    pass

def check_takeover(subdomain, cname):
    # TODO: check against TAKEOVER_SIGNATURES
    pass
...
4. כיצד Subdomain Takeover יכול לשמש לbypass מנגנוני CORS?
