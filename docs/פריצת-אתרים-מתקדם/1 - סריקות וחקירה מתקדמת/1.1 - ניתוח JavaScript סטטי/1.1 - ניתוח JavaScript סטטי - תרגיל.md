...
בתרגיל זה תתרגלו טכניקות ניתוח JavaScript סטטי. התרגיל כולל ארבעה אתגרים שמכסים את כל הנושאים מההרצאה - מextraction נקודות קצה ועד שחזור קוד מקור מ-Source Maps.

---

## אתגר 1 - extraction נקודות קצה API מ-Bundle
...
2. השתמשו בסקריפט Python מההרצאה (או כתבו סקריפט משלכם) לextraction כל נתיבי ה-API מהקובץ
...
```bash
# make sure Juice Shop is running on port 3000
...
```bash
# find the main JS files
curl -s http://localhost:3000 | grep -oP 'src="[^"]*\.js"'

# run LinkFinder on each file
