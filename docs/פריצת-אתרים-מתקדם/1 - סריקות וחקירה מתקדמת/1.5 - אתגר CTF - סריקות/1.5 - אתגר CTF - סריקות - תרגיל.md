...
const server = http.createServer((req, res) => {
    ...
    if (path === "/api/health") {
        ...
        // Old version exposes additional information
        res.writeHead(200);
...
```bash
# Create all the directories and files, then:
...
## שלב 4 - extraction סודות מ-Source Maps ושימוש בטוקנים
...
2. **ניתוח JavaScript** - extraction נתיבי API, מפתחות, וטוקנים מקוד ה-bundle
...
5. **הexploit מידע** - שימוש בטוקנים שנחשפו לגישה לנקודות קצה מוגנות
