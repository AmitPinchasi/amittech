...
ביישום קיימת חולשת Blind SSRF בפיצ'ר "בדיקת זמינות שרת". ניתן לשלוח URL והמערכת מחזירה רק status code (200, 404, 500 וכו') - לא את תוכן התגובה.
...
1. חשיפת גרסת שרת בheaders HTTP: `Server: Apache/2.4.49`
...
2. נסו לעקוף את המנגנון בשיטות הבאות:
   - שינוי headers: `X-Forwarded-For`, `X-Real-IP`, `X-Originating-IP`
...
4. חשבו את ציון ה-CVSS:
   - של bypass ה-Rate Limiting לבדה
   - של השרשרת: הbypass + Brute Force + Account Takeover
