...
2. עקפו הגנות (אם קיימות) - DNS rebinding, redirect, או encoding URL
...
# Step 3 - exploit the admin endpoint
# TODO: complete
```
...
- נקודת הקצה מקבלת את ה-API key בheader `X-API-Key`
...
1. **Exploiting XSS to perform CSRF** - chaining XSS עם CSRF
   - כתובת: `https://portswigger.net/web-security/cross-site-scripting/exploiting/lab-perform-csrf`

2. **SSRF with filter bypass via open redirection** - chaining SSRF עם Open Redirect
   - כתובת: `https://portswigger.net/web-security/ssrf/lab-ssrf-with-filter-bypass-via-open-redirection`

3. **Stealing OAuth access tokens via an open redirect** - chaining OAuth עם Open Redirect
   - כתובת: `https://portswigger.net/web-security/oauth/lab-oauth-stealing-oauth-access-tokens-via-an-open-redirect`

### מעבדות מתקדמות

4. **Web cache poisoning to exploit XSS** - chaining Cache Poisoning עם XSS
5. **Multi-step exploitation** - כל מעבדת Expert שדורשת chaining
...
```python
# Required structure for the script
