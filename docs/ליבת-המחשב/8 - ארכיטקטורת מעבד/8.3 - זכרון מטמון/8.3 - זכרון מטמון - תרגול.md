...
### תרגיל 1 - חישוב פרמטרי cache

נתון cache L1 עם הפרמטרים הבאים:
- גודל כולל: 32 KB
- גודל שורה: 64 בתים
...
### תרגיל 2 - חיזוי התנהגות cache

נתון cache direct-mapped (1-way) עם:
- 4 שורות (4 sets)
- גודל שורה: 16 בתים
- כתובות של 16 ביט
...
3. שימו לב שכתובות 0 ו-64 ממופות לאותו set. מה קורה? איך cache 2-way set-associative היה פותר את הבעיה?
...
**גרסה B - לפי עמודות:**
```c
// version B: j after i (column-column)
for (int j = 0; j < N; j++)
    for (int i = 0; i < N; i++)
        matrix[i][j] = 0;
```
...
2. חשבו את שיעור ה-cache miss עבור L1D (32KB, שורה 64B):
   ...
   - גרסה B: כמה misses?
   (רמז: int הוא 4 בתים, שורת cache 64 בתים = 16 int-ים)
...
int main() {
    ...
    // --- experiment 1: without padding (false sharing) ---
    ...
    // --- experiment 2: with padding (no false sharing) ---
...
2. הסבירו בדיוק מה קורה בניסוי 1 (בלי padding):
   - על איזה שורת cache נמצאים counter0 ו-counter1?
