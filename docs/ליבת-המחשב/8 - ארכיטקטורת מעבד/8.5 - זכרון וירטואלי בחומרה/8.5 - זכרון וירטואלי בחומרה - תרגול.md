...
2. נניח שה-page table walk מצא שהpage ממופה למסגרת פיזית (page frame) `0x00000001234AB`. מהי הכתובת הפיזית הסופית?

3. אם במקום page רגיל (4KB) היינו משתמשים בpage ענק (2MB), איזה חלקים של הכתובת הוירטואלית היו משתנים? מהו הoffset בpage ענק?
...
תוכנית ניגשת לכתובות הבאות (page = 4KB = 0x1000):
...
| גישה | כתובת | מספר page (VPN) | TLB Hit/Miss | מצב TLB אחרי (VPNs) |
|---|---|---|---|---|
| 1 | 0x1000 | 1 | ? | ? |
| 2 | 0x1004 | 1 | ? | ? |
| ... | | | | |
...
4. אם הpages היו huge pages (2MB = 0x200000), האם היו פחות misses? חשבו - איזה כתובות עכשיו על אותו page?

---

### תרגיל 3 - תקורת טבלאות pages

נניח process שמשתמש ב-1GB של זכרון וירטואלי רציף (כתובות 0x0 עד 0x3FFFFFFF).

1. כמה pages של 4KB צריך?
...
6. חשבו את הזכרון הכולל שטבלאות הpages תופסות. כמה אחוז מה-1GB?

   רמז: כל טבלה היא עמוד (4KB) עם 512 כניסות של 8 בתים.

7. חזרו על החישוב עם pages ענקיים של 2MB. כמה זכרון חוסכים?

---

### תרגיל 4 - השוואת pages רגילים מול ענקיים
...
int main() {
    ...
    // warm-up - force all pages to be mapped
    memset(buf, 0, SIZE);
    ...
    long sum = 0;
    for (long i = 0; i < SIZE; i += 4096) {  // jumps of a page
        sum += buf[i];
    }
    ...
    // sequential scan with small stride
    clock_gettime(CLOCK_MONOTONIC, &start);
    sum = 0;
    for (long i = 0; i < SIZE; i += 64) {  // jumps of a cache line
        sum += buf[i];
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
...
1. הריצו את התוכנית. מדוע סריקה עם stride של 4096 (גודל page) איטית יותר יחסית מ-stride 64? (רמז: חשבו על TLB misses)
