...
double measure_stride(char *arr, long size, int stride) {
    ...
    long count = 0;
    volatile char sink;  // volatile so the compiler doesn't eliminate the reads
    ...
    return elapsed / count * 1e9;  // nanoseconds per access
}

int main() {
    char *arr = malloc(ARRAY_SIZE);
    if (!arr) { perror("malloc"); return 1; }
    memset(arr, 1, ARRAY_SIZE);  // warm-up - make sure all pages are mapped
...
int main() {
    ...
    // --- experiment 3 (bonus): branchless version ---
    clock_gettime(CLOCK_MONOTONIC, &start);
...
#define COUNT 200000000L  // 200 million

// --- version 1: without padding (false sharing) ---
struct shared_counters {
    volatile long c0;
    volatile long c1;
};

// --- version 2: with padding ---
struct padded_counters {
    volatile long c0;
    char pad[64 - sizeof(long)];  // padding for a separate cache line
    volatile long c1;
};
...
int main() {
    ...
    // experiment 3 (bonus): single thread as baseline
    struct timespec start, end;
...
two threads (shared):   2.500 sec  (6x slower than single!)
two threads (padded):   0.400 sec  (like single - both threads run in parallel)
...
```bash
# stage 1 - cache misses
perf stat -e cache-misses,cache-references,\
L1-dcache-load-misses,L1-dcache-loads \
./cache_stride

# stage 2 - branch mispredictions
...
# IPC (Instructions Per Cycle) for all programs
