...
(gdb) break strcmp
(gdb) run
# type admin and any key
# when stopped at strcmp:
(gdb) x/s $rdi   # the first argument
(gdb) x/s $rsi   # the second argument
```
...
call check_license
test eax, eax
je   invalid_label    ; if eax == 0, jump to "Invalid!"
```
...
call check_license
test eax, eax
jne  valid_label      ; if eax != 0, jump to "Valid!"
```
