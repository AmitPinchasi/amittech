...
```bash
(gdb) print $rax
(gdb) print/x $rdi         ; in hexadecimal format
(gdb) print/d $esi          ; in decimal format
(gdb) print/t $eax          ; in binary format
```
...
תוכניות שעושות fork יוצרות process חדש. כברירת מחדל, GDB עוקב אחרי ה-parent:

```bash
(gdb) set follow-fork-mode child
```

עכשיו GDB יעקוב אחרי process הבן אחרי fork.
...
0x40123b:  je      0x401260          ; if 0 -> "access denied"
...
![איור עקיפת בדיקת סיסמה ב-GDB: שינוי ערך האוגר rax מאפס לאחד גורם לפקודה je לא לקפוץ, כך שהתוכנית ממשיכה אל access granted במקום access denied](../../../images/illustrations/core-7-3-patch-branch.svg)
...
**כלי GEF** (GDB Enhanced Features) - interface נקי ופשוט יותר מpwndbg, מתאים גם הוא ל-RE.
...
verify:
        push    rbp
        mov     rbp, rsp
        mov     qword [rbp-8], rdi     ; saving the argument (the string)
...
.loop:
        ...
        lea     rcx, [rip+0x...]       ; rcx = address of the constant array
...
| `set follow-fork-mode child` | מעקב אחרי process בן |
