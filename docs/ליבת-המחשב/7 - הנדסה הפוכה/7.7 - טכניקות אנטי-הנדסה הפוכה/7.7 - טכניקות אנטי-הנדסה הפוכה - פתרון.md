...
undefined8 main(void) {
    char local_28[28];
    // ... the encrypted bytes are copied to local_28 ...
...
```bash
(gdb) break *0x4011XX   # the address we found
(gdb) continue
```
...
1. השוואת גדלים:
```
original:  ~16KB (depends on the system)
packed:    ~8KB
```
...
```bash
(gdb) # find check_timing in Ghidra
(gdb) break *<address_of_timing_comparison>
(gdb) continue
(gdb) set $rax = 0    # or change the ZF flag
(gdb) continue
```
...
(gdb) break main
(gdb) run
# find the call to check_timing
# put a breakpoint after the call
