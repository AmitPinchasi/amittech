...
; after dead code insertion:
mov eax, 5
push rbx          ; does nothing
mov rbx, 0x1234   ; not used for anything
xor rcx, rcx      ; has no effect
nop               ; nothing
pop rbx           ; cancels the push
add eax, 3
```
...
```c
// always true (because x^2 + x is always even)
if ((x * x + x) % 2 == 0) {
    // the real code here
} else {
    // fake code that will never run
}
```
...
```c
// normal - strings will find this:
printf("Password incorrect!\n");

// encrypted - strings won't find:
char encrypted[] = {0x29, 0x42, 0x52, 0x52, ...};
for (int i = 0; i < len; i++) {
    encrypted[i] ^= 0x37;  // decrypts at runtime
}
printf(encrypted);
```
...
```c
// the code on disk is gibberish
// at runtime, the code decrypts itself:
unsigned char *code = (unsigned char *)function_addr;
mprotect(code, 4096, PROT_READ | PROT_WRITE | PROT_EXEC);
for (int i = 0; i < code_len; i++) {
    code[i] ^= key;
}
// now the code is decrypted and can be run
```
...
### בדיקת ptrace
כמו שלמדנו, GDB משתמש ב-ptrace כדי לשלוט בprocess. תוכנית יכולה לבדוק אם מישהו מדבג אותה:
...
**איך זה עובד**: השדה `TracerPid` ב-`/proc/self/status` מכיל את ה-PID של הprocess שעוקב אחרינו. אם הערך שונה מ-0, מישהו מדבג.

**איך לעקוף**: ב-GDB, אפשר לשנות את ערך pid ל-0:
```bash
(gdb) break atoi
(gdb) run
# when stopped at atoi:
(gdb) finish
(gdb) set $rax = 0
(gdb) continue
```
...
void timing_check() {
    ...
    if (diff > 1000000) {  // more than a millisecond = suspicious
        exit(1);
    }
}
```
...
void trap_handler(int sig) {
    // if the handler runs = no debugger
    debugger_detected = 0;
}
...
void check_int3() {
    ...
    if (debugger_detected) {
        // the handler didn't run = debugger caught the int3
        exit(1);
    }
}
```
...
![איור מנגנון ה-breakpoint של הדיבאגר: הדיבאגר שומר את הבית המקורי ומחליף אותו בבית 0xCC שהוא הפקודה int3, וכשהמעבד מגיע אליו נוצר SIGTRAP שעוצר את התוכנית](../../../images/illustrations/core-7-7-int3-breakpoint.svg)
...
void handler(int sig) {
    // if we got here = no debugger
}

void check() {
    signal(SIGALRM, handler);
    alarm(1);  // SIGALRM in one second
    // debugger might catch the signal
}
```
...
void check_vm_hardware() {
    // checking MAC address - VMware, VirtualBox have known prefixes
    // VMware: 00:0C:29, 00:50:56
    // VirtualBox: 08:00:27

    // checking for the presence of VM tools
...
void timing_vm_check() {
    // certain operations (like I/O) are slower in a VM
...
```
Before packing:
[original code] [data] [imports]

After packing:
[small unpacker] [compressed/encrypted original code]
```
...
![איור פעולת packer בזמן ריצה: קובץ ארוז מכיל unpacker קטן וקוד מקורי דחוס, וכשמריצים אותו ה-unpacker פורש את הקוד המקורי בזיכרון ומעביר אליו את השליטה בנקודת ה-OEP](../../../images/illustrations/core-7-7-packer-unpack.svg)
...
# skipping a check (changing rip to the address after the check):
