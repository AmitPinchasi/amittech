...
```bash
readelf -h suspicious_binary    # ELF header
readelf -S suspicious_binary    # sections
readelf -l suspicious_binary    # segments
readelf -d suspicious_binary    # dynamic - libraries the program depends on
readelf -s suspicious_binary    # symbols (if any)
```

חפשו דברים חשודים:
- **ספריות חריגות** - ספריות שקשורות לרשת, הצפנה, או מניפולציה של processes
...
- **מניפולציה של processes**: `fork`, `exec`, `ptrace`, `kill`
...
- **יצירת processes**: `fork()`, `execve(...)` - מה התוכנה מריצה?
...
```bash
strace -e trace=network ./suspicious_binary    # only network syscalls
strace -e trace=file ./suspicious_binary       # only file syscalls
```
...
- **שמות processes**: processes שהתוכנה מריצה
- **מפתחות רגיסטרי** (ב-Windows)
