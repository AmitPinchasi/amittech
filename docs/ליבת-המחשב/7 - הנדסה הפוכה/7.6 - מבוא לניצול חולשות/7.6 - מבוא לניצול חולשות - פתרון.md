...
1. **החולשה**: שימוש ב-`gets(name)` לקריאת קלט לתוך buffer `name[32]`. הפונקציה gets לא מגבילה את כמות הבתים שהיא קוראת, ולכן אם המשתמש מקליד יותר מ-32 תווים, הבתים הנוספים גולשים מעבר ל-buffer ודורסים ערכים על המחסנית - כולל את כתובת החזרה.
...
| saved rbp (8 bytes)       |
+---------------------------+
| name[0..31] (32 bytes)    |  <-- gets writes here
+---------------------------+
Low addresses (rsp)
```
...
```
Before gets:                  After gets with long input:
+-------------------+         +-------------------+
| return address    |         | AAAA AAAA         |  <-- overwritten!
+-------------------+         +-------------------+
| saved rbp         |         | AAAA AAAA         |  <-- overwritten!
+-------------------+         +-------------------+
| buffer[0..N]      |         | AAAA AAAA AAAA... |  <-- full
+-------------------+         +-------------------+
```
...
```c
// gets doesn't know the size of the buffer
gets(buffer);

// fgets receives the buffer size as a parameter and won't write beyond that size
...
```c
// strcpy: copies all of src to dst, with no limit.
// if src is longer than dst - overflow!
strcpy(dst, src);

// strncpy: copies at most n bytes from src to dst.
// won't overflow beyond n bytes.
strncpy(dst, src, n);
```
...
| saved rbp (8 bytes)       |
+---------------------------+
| CANARY (8 bytes)          |  <-- random value checked before ret
+---------------------------+
| buffer[0..15] (16 bytes)  |  <-- gets writes here
+---------------------------+
Low addresses (rsp)
```
