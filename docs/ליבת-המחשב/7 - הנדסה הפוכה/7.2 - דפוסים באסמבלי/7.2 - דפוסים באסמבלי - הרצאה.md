...
```asm
cmp     edi, 5
jle     .else_branch       ; if x <= 5, jump to else
; --- if body ---
jmp     .end
.else_branch:
; --- else body ---
...
![איור שמראה כיצד התנאי x > 5 בשפת C מתורגם לקפיצה jle שמדלגת אל ה-else כאשר x קטן או שווה לחמש](../../../images/illustrations/core-7-2-inverted-jump.svg)
...
באסמבלי:
```asm
        mov     dword [rbp-4], 0       ; i = 0 (initialization)
...
.check:
        ...
        jl      .body                  ; if true, go back to loop body
```
...
![איור הצורה של לולאת for באסמבלי: אתחול, קפיצה ראשונית אל בדיקת התנאי, גוף הלולאה, קידום המונה וחזרה לבדיקה עד שהתנאי מפסיק להתקיים](../../../images/illustrations/core-7-2-for-loop-shape.svg)

עם אופטימיזציה (`-O2`), הקומפיילר עשוי לפשט:
```asm
        xor     ecx, ecx               ; i = 0 (xor with itself = zero)
.body:
        ; --- loop body ---
        inc     ecx                     ; i++
        cmp     ecx, 10                ; i < 10?
        jl      .body                  ; if true, go back
```
...
.check:
        ...
        jg      .body                  ; if true, go back to body
```
...
```asm
        ...
        cmp     edi, 100
        je      .case_100
        jmp     .end                   ; default (or end if there's no default)
.case_1:
        call    func_a
        jmp     .end
.case_5:
...
```asm
        cmp     edi, 3
        ja      .default               ; if x > 3 (unsigned), it's not a valid case
        lea     rax, [rip + .jump_table]
        movsxd  rdx, dword [rax + rdi*4]
        add     rax, rdx
        jmp     rax                    ; indirect jump through the table
...
![איור מימוש switch בעזרת טבלת קפיצות: בדיקת גבולות עם ja, חישוב כתובת מתוך הטבלה לפי הערך הנבדק, וקפיצה עקיפה אל ה-case המתאים](../../../images/illustrations/core-7-2-jump-table.svg)
...
```asm
        mov     edx, 30                ; third argument -> rdx
        mov     esi, 20                ; second argument -> rsi
        mov     edi, 10                ; first argument -> rdi
        call    calculate
        mov     [rbp-4], eax           ; save the return value (rax) to a local variable
```
...
**זיהוי printf:**
```asm
lea     rdi, [rip+0x...]       ; rdi = format string (address in .rodata)
mov     esi, [rbp-4]           ; rsi = first argument for the format
xor     eax, eax               ; al = 0 (number of SSE/float arguments)
call    printf@plt
```
...
**זיהוי malloc:**
```asm
mov     edi, 0x100             ; rdi = size of the allocation (256 bytes)
call    malloc@plt
mov     [rbp-8], rax           ; save the returned pointer
```
המפתח: ארגומנט מספרי אחד ב-rdi, וערך ההחזרה נשמר כמצביע.

**זיהוי strcmp:**
```asm
lea     rdi, [rbp-0x20]        ; rdi = first string
lea     rsi, [rip+0x...]       ; rsi = second string
call    strcmp@plt
test    eax, eax               ; check the return value
je      .strings_equal         ; if 0, the strings are equal
```
...
```asm
        mov     dword [rax], 1         ; p.x = 1  (offset 0 from the start of the struct)
        mov     dword [rax+4], 2       ; p.y = 2  (offset 4)
        lea     rcx, [rip+0x...]       ; rcx = address of the string "origin"
...
func:
        push    rbp
        mov     rbp, rsp
        sub     rsp, 0x40              ; allocation of 64 bytes on the stack
        lea     rsi, [rbp-0x40]        ; rsi = address of the buffer
...
```
```asm
sar     eax, 2              ; x / 4 (signed, preserves sign)
...
```asm
mov     eax, edi
sar     eax, 31             ; eax = 0 if positive, -1 if negative
shr     eax, 30             ; rounding correction (for division by 4)
add     eax, edi
sar     eax, 2              ; division by 4
```
...
```
```asm
test    eax, eax            ; AND of eax with itself, sets flags
...
```c
return x != 0;  // convert to bool
```
```asm
test    edi, edi
setne   al                  ; al = 1 if edi != 0, else 0
...
mystery_func:
        push    rbp
        mov     rbp, rsp
        mov     dword [rbp-4], edi     ; save first argument
        mov     dword [rbp-8], 0       ; local variable = 0
        mov     dword [rbp-12], 1      ; local variable = 1
        jmp     .check
.loop:
        mov     eax, [rbp-12]
        add     [rbp-8], eax           ; sum += i
...
.check:
        ...
        cmp     eax, [rbp-4]           ; i <= n?
        jle     .loop
        mov     eax, [rbp-8]           ; return value = sum
        pop     rbp
        ret
```
