...
func:
        xor     eax, eax       ; eax = 0
        test    edi, 1         ; 6 & 1 = 0 -> ZF=1 (6 is even)
        jnz     .odd           ; doesn't jump (ZF=1)
        mov     eax, 1         ; eax = 1
.odd:
        ret
```
...
func:
        mov     eax, edi       ; eax = 10
        xor     edx, edx       ; edx = 0 (upper part of the dividend)
...
.loop:
        mov     ecx, edi
        and     ecx, 1             ; ecx = x & 1 (the lowest bit)
...
.loop:
        cmp     ecx, esi           ; i >= n?
        jge     .end
        shl     dword [rdi + rcx*4], 1  ; arr[i] *= 2 (left shift = multiply by 2)
        inc     ecx                ; i++
        jmp     .loop
.end:
        ret
...
```asm
        ...
        shr     eax, 30                ; eax = 0 (positive) or 3 (negative)
        add     eax, edi               ; rounding correction
...
mystery:
        ...
        mov     rbp, rsp
        push    rbx
        mov     rbx, rdi               ; rbx = first argument (pointer)
        xor     eax, eax               ; eax = 0
        test    rbx, rbx               ; pointer == NULL?
        je      .end                   ; if so - return 0
        xor     eax, eax               ; eax = 0 (counter)
.loop:
        cmp     byte [rbx + rax], 0    ; the byte at position rax == 0?
        je      .end                   ; if so - done
        inc     rax                    ; rax++
        jmp     .loop
.end:
