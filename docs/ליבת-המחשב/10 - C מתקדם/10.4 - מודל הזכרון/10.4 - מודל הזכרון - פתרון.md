...
// change to "int flag = 0" to see the issue
volatile int flag = 0;
...
// change to "int counter = 0" to see a race condition
atomic_int counter = 0;

void *increment(void *arg) {
    for (int i = 0; i < INCREMENTS; i++) {
        atomic_fetch_add_explicit(&counter, 1, memory_order_relaxed);
        // for the version without atomics: counter++;
    }
    return NULL;
}
...
void *producer(void *arg) {
    for (int i = 0; i < NUM_MESSAGES; i++) {
        // waiting for the consumer to finish with the previous message
        while (atomic_load_explicit(&ready, memory_order_acquire) != 0)
            ;

        data = (i + 1) * 100;  // preparing the data
        printf("producer: sent %d\n", data);

        // publishing that the data is ready
...
void *consumer(void *arg) {
    for (int i = 0; i < NUM_MESSAGES; i++) {
        // waiting for the producer to publish data
        while (atomic_load_explicit(&ready, memory_order_acquire) != 1)
            ;

        printf("consumer: received %d\n", data);

        // notifying the producer that it's done
...
// version without a barrier
void set_values_no_barrier(void) {
    x = 1;
    y = 2;
}

// version with a barrier
...
הqueue עובד בצורת מערך מעגלי עם שני אינדקסים: head (מאיפה הצרכן קורא) ו-tail (לאן היצרן כותב). הqueue ריק כש-head == tail, ומלא כש-(tail + 1) % size == head.
...
כשהqueue מלא, היצרן פשוט מנסה שוב (busy-wait). בקוד אמיתי, אפשר להוסיף `sched_yield()` או לישון זמן קצר כדי לא לבזבז CPU.
