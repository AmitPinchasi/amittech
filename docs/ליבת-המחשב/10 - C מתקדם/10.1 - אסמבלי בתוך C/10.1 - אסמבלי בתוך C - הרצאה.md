...
```c
asm [volatile] (
    "template"       // assembly instructions with placeholders
    : outputs        // output operands
    : inputs         // input operands
    : clobbers       // registers that are affected
);
```
...
![איור המראה את מיפוי האופרנדים בבלוק extended asm, שבו הערכים של a ושל b נכנסים לרגיסטרים ותוצאת החיבור מועתקת אל משתנה הפלט](../../../images/illustrations/core-10-1-operand-mapping.svg)
...
asm("imull %1, %0"
    : "=r"(result)
    : "r"(x), "0"(x)  // "0" means: use the same register as operand 0
);
// result = x * x = 49
```
...
asm("cpuid"
    ...
    : "a"(function_id)
    : // no clobbers here because all registers that cpuid changes appear in the outputs
);
```
...
asm("xorl %%ecx, %%ecx\n\t"
    ...
    : "=r"(result)
    : "r"(input)
    : "eax", "ecx"  // the assembly changes eax and ecx
);
```
...
![איור המראה כיצד הפקודה rdtsc ממלאת את הרגיסטרים eax ו-edx בערכים, וכיצד הקוד מרכיב מהם מונה אחד באורך 64 ביט](../../../images/illustrations/core-10-1-rdtsc-registers.svg)
...
void get_cpu_vendor(char *vendor)
{
    ...
    // the vendor string is split across three registers
    *((uint32_t *)vendor)     = ebx;
...
// returns 1 if the swap succeeded, 0 if not
...
static inline long my_write(int fd, const void *buf, long count)
{
    ...
    asm volatile(
        ...
        : "rcx", "r11", "memory"  // syscall changes rcx and r11
    );
    return ret;
}
