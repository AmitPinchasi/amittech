...
```c
// the compiler may remove this check!
if (x + 1 > x) {
    // the compiler thinks: "this is always true, because overflow is UB and won't happen"
    do_something();
}
```
...
```c
// want to make sure we don't go out of bounds
if (ptr + offset < ptr) {
    // overflow check
    return ERROR;
}
...
```c
void foo(int *p) {
    int x = *p;          // dereference of p
    if (p == NULL) {     // check if p is NULL
        handle_null();
        return;
    }
    use(x);
...
![איור המראה את שרשרת ההסקה של הקומפיילר שמובילה למחיקת בדיקת NULL אחרי שכבר בוצע dereference למצביע](../../../images/illustrations/core-10-2-null-check-removed.svg)
...
```c
int arr[10];
arr[15] = 42; // UB! access out of bounds
```
...
*p = 42;
free(p);
printf("%d\n", *p); // UB! the memory was freed
```

אחרי `free`, הזיכרון עשוי להיות מוקצה לשימוש אחר. קריאה ממנו עלולה להחזיר ערך שונה, זבל, או לקרוס. כתיבה אליו עלולה לדרוס נתונים של allocation אחרת.

![איור המראה את מצב הזיכרון בשימוש אחרי שחרור, שבו הבלוק המשוחרר נתפס על ידי הקצאה חדשה והקריאה דרך המצביע הישן מחזירה ערך זר](../../../images/illustrations/core-10-2-use-after-free.svg)
...
```c
void process(int *p) {
    *p = 5;              // line 1: dereference p
    if (p == NULL)        // line 2: NULL check
        return;
    printf("value: %d\n", *p);
}
```
