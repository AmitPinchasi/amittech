...
unsigned int extract_bits(unsigned int n, int start, int len) {
    // creates a mask of len set bits
    unsigned int mask = (1u << len) - 1;
    // shifts right by start and applies the mask
    return (n >> start) & mask;
}
...
int main(void) {
    ...
    n = set_bits(n, 4, 4, 0xA); // inserts 0xA into bits 4-7
    printf("  set_bits(0, 4, 4, 0xA) = 0x%X\n", n); // 0xA0

    n = set_bits(n, 0, 4, 0x5); // inserts 0x5 into bits 0-3
