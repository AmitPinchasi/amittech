...
```c
int bit = (x >> k) & 1;  // returns 0 or 1 - the value of bit k
```
...
```c
int perms = FLAG_READ | FLAG_WRITE;  // 0011 - read and write
```
...
```c
perms |= FLAG_EXEC;  // now 0111
```

כיבוי דגל:

```c
perms &= ~FLAG_WRITE;  // now 0101
```

![איור המראה כיצד הדלקה וכיבוי של דגלים משנים ביט בודד בערך perms, בלי לפגוע בשאר הביטים](../../../images/illustrations/core-10-3-flag-mask.svg)
...
n     = 00010000
n - 1 = 00001111
n & (n-1) = 00000000  ->  0, so it's a power of 2!
```
...
n     = 00010100  (20)
n - 1 = 00010011  (19)
n & (n-1) = 00010000  ->  not 0, so it's not a power of 2
```
...
```
a ^= b  ->  a = 5^3 = 6
b ^= a  ->  b = 3^6 = 5  (the original value of a!)
a ^= b  ->  a = 6^5 = 3  (the original value of b!)
```
...
```c
int my_abs(int x) {
    int mask = x >> 31;         // 0 if x >= 0, -1 (all bits 1) if x < 0
    return (x ^ mask) - mask;
}
```
...
```
n = 01010100
n & (n-1) = 01010100 & 01010011 = 01010000  -> we turned off the low bit (bit 2)
n & (n-1) = 01010000 & 01001111 = 01000000  -> we turned off bit 4
n & (n-1) = 01000000 & 00111111 = 00000000  -> we turned off bit 6
```

3 איטרציות, 3 ביטים דלוקים.

![איור המראה את אלגוריתם קרניגהן, שבו כל איטרציה מכבה את הביט הדלוק הנמוך ביותר ומעלה את המונה עד שהמספר מתאפס](../../../images/illustrations/core-10-3-kernighan-popcount.svg)
...
```c
int pos = __builtin_ffs(x);   // returns the position of the lowest set bit (1-based), 0 if x==0
int tz = __builtin_ctz(x);    // count trailing zeros - the number of zeros on the right (0-based)
```

דוגמה:
```
x = 01010100  (84)
__builtin_ffs(x) = 3    (bit 2, 1-based = 3)
__builtin_ctz(x) = 2    (2 zeros on the right)
```

### ביט דלוק גבוה ביותר - find last set

```c
int lz = __builtin_clz(x);   // count leading zeros - the number of zeros on the left
```

דוגמה:
```
x = 01010100  (84, in 32 bits: 00000000 00000000 00000000 01010100)
__builtin_clz(x) = 25   (25 zeros on the left)
// the highest bit is bit 31 - 25 = 6
```
...
```c
struct permissions {
    unsigned int read    : 1;  // one bit
    unsigned int write   : 1;
    unsigned int execute : 1;
    unsigned int level   : 4;  // 4 bits (values 0-15)
    unsigned int flags   : 9;  // 9 bits
};
```
...
```c
uint32_t swapped = __builtin_bswap32(x);   // reverses byte order in 32 bits
uint64_t swapped = __builtin_bswap64(x);   // reverses byte order in 64 bits
```
