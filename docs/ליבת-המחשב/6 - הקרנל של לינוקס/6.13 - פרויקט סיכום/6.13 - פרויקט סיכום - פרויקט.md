...
- **VFS ומערכת הקבצים** (סעיף 6.4) - interface file_operations
...
dmesg | tail -3    # a message about successful registration should appear

# create the device file:
# (the major number appears in dmesg)
sudo mknod /dev/kvstore c <major> 0
sudo chmod 666 /dev/kvstore

# test:
cat /dev/kvstore      # a message should appear in dmesg
echo "test" > /dev/kvstore  # same

# unload:
sudo rmmod kvstore
dmesg | tail -3    # an unload message should appear
```
...
- הallocation זיכרון עם kmalloc ושחרור עם kfree
...
static LIST_HEAD(kv_list);         // the linked list of all pairs
...
// find an entry by key (returns NULL if not found)
                              ...
                              size_t count, loff_t *pos)
{
    ...
    *eq_pos = '\0';    // separates key from value
    eq_pos++;           // eq_pos now points to the value
                             ...
                             size_t count, loff_t *pos)
{
    ...
    if (total_len == 0 || *pos >= total_len)
        return 0;   // no data, or we've already read everything
...
static int __init kvstore_init(void)
{
    ...
    // create the class and device (automatically creates /dev/kvstore)
...
הגנו על מבנה הנתונים כך שמספר processes יוכלו לקרוא/לכתוב בו-זמנית בלי corruption.
...
4. בדיקה - הריצו מספר processes במקביל:

```bash
# in one terminal:
for i in $(seq 1 100); do echo "key$i=value$i" > /dev/kvstore; done

# in a second terminal (in parallel):
for i in $(seq 100 200); do echo "key$i=value$i" > /dev/kvstore; done

# test:
cat /dev/kvstore | wc -l   # should be 200
```

### רמזים

- אם allocation הזיכרון (kmalloc) נמצאת בתוך הנעילה, השתמשו ב-GFP_KERNEL (מותר כי mutex מאפשר שינה)
...
static int kvstore_proc_show(struct seq_file *m, void *v)
{
    ...
    // add: total memory in use, the longest key, etc.
...
- הוסיפו interface sysfs במקום (או בנוסף ל-) procfs
