...
ג. בדקו אילו capabilities יש לprocess הנוכחי שלכם:
```bash
...
```
בתוך ה-shell החדש, הריצו `ip addr`. מה קרה לinterfaces הרשת? האם אתם יכולים לגלוש לאינטרנט מתוך ה-shell הזה?

ג. הסבירו למה user namespaces חשובים לאבטחת קונטיינרים. מה קורה אם process בתוך קונטיינר רץ כ-root (UID 0)?

ד. Docker משתמש בשילוב של namespaces, cgroups, seccomp, ו-capabilities. לכל אחד מהתרחישים הבאים, זהו איזה מנגנון אחראי:
   1. קונטיינר לא יכול לראות processes של קונטיינרים אחרים
   2. קונטיינר מוגבל ל-512MB זיכרון
   3. קונטיינר לא יכול לקרוא ל-`reboot()` syscall
   4. קונטיינר יש לו כתובת IP משלו
   5. הprocess root בקונטיינר לא יכול לטעון מודולי קרנל
