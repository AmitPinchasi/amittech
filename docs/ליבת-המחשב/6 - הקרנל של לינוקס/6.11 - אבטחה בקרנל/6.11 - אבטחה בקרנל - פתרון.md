...
- **CapInh (inherited)** - capabilities שעוברות בירושה ל-exec(). אם capability נמצאת גם ב-inherited של הprocess וגם ב-permitted של הקובץ, היא תהיה פעילה בprocess החדש.
- **CapPrm (permitted)** - הסט המקסימלי של capabilities שהprocess **יכול** להפעיל. הוא יכול "להדליק" capability ב-effective רק אם היא קיימת ב-permitted.
- **CapEff (effective)** - ה-capabilities ש**פעילות כרגע**. אלה ה-capabilities שהקרנל באמת בודק כשהprocess מנסה לבצע פעולה פריבילגית.
...
```bash
# give nginx the ability to bind to low ports:
...
כל syscall אחר יגרום להריגת הprocess (SECCOMP_RET_KILL).

**ב. מה יקרה בשורה A (fopen)?**

`fopen` קורא בפנים ל-`open` syscall (או `openat`). ה-syscall הזה **לא מותר** במסנן. התוצאה: הקרנל שולח SIGKILL לprocess, והוא נהרג **מיד**.

**ג. האם שורה B תתבצע?**

**לא.** הprocess נהרג בשורה A. הוא לא מגיע לשורה B. seccomp עם SECCOMP_RET_KILL הורג את הprocess מיד כשה-syscall הלא מורשה נקרא.

**ד. PR_SET_NO_NEW_PRIVS:**

הדגל הזה מבטיח שהprocess (וצאצאיו) **לא יוכלו לקבל הרשאות חדשות**. למשל, exec() של קובץ setuid לא ייתן הרשאות root.

זה נדרש לפני seccomp כי בלעדיו, process יכול:
...
בתוך ה-shell החדש, `ps aux` מראה **רק את bash ו-ps עצמו**. כל שאר הprocesses במערכת לא נראים.

הסיבה: יצרנו PID namespace חדש. bash הוא PID 1 בתוך ה-namespace הזה (כמו init). הוא רואה רק processes שנוצרו בתוך ה-namespace. הדגל `--mount-proc` מרכיב /proc מחדש בתוך ה-namespace, כך ש-ps קורא מה-/proc החדש.
...
`ip addr` מראה **רק את lo (loopback)**. אין eth0, wlan0, או כל interface רשת אחר.

לא ניתן לגלוש לאינטרנט - אין interface רשת שמחובר לעולם החיצון. כדי שזה יעבוד, צריך ליצור veth pair ולחבר אותו ל-bridge ב-namespace הראשי (זה מה ש-Docker עושה).

**ג. User namespaces ואבטחת קונטיינרים:**

ב-user namespace, UID 0 בתוך הקונטיינר **ממופה** ל-UID אחר (למשל 65534) בחוץ. זה אומר:

- בתוך הקונטיינר: הprocess חושב שהוא root ויכול לעשות הכל (בתוך ה-namespace)
...
1. **קונטיינר לא רואה processes אחרים** - PID namespace
...
**ניצול:**
1. הprocess A פותח את ה-device
2. הprocess A סוגר את ה-device (kfree)
3. תוקף מקצה אובייקט slab באותו גודל - הוא מקבל את אותו הזיכרון!
4. הprocess A קורא מה-device - קורא את הנתונים של התוקף
...
```c
// fix - open with an atomic permission check:
struct file *f = filp_open(filename, O_RDONLY, 0);
if (IS_ERR(f))
    return PTR_ERR(f);
// now f points to a specific file, cannot be swapped
```
