...
4. **שכבת IP**: הקרנל מוסיף IP header באמצעות skb_push:
   ...
   **החלטת ניתוב**: הקרנל מחפש בטבלת הניתוב (`ip route`) את הנתיב לכתובת היעד. הוא מוצא את הdefault gateway (למשל 192.168.1.1) ואת interface הרשת (למשל eth0).
...
6. **הדרייבר**: הsk_buff נכנס לTX queue של interface eth0. הדרייבר לוקח אותו מהqueue ושם אותו בTX ring buffer של החומרה - מבנה נתונים בזיכרון שהNIC קורא ממנו.

7. **DMA**: הNIC משתמש בDMA כדי לקרוא את החבילה מהTX ring buffer בזיכרון ישירות, בלי מעורבות המעבד. אחרי ששלח את החבילה, הNIC יכול לשלוח interrupt לציין שהTX הושלם (כדי שהקרנל ישחרר את הsk_buff).
...
3. **הקשר בין proc/net/tcp ל-ss**:
   הפקודה ss קוראת את המידע מאותם מבני נתונים בקרנל (דרך Netlink socket, שהוא יעיל יותר מקריאת `/proc/net/tcp`). שני הכלים מציגים את אותו מידע, רק ש-ss מציג אותו בפורמט קריא יותר (כתובות עשרוניות, שמות states, שמות processes).
...
1. **משמעות הדגלים**:
   ...
   **הבדל 0.0.0.0 מול 127.0.0.1**:
   - `0.0.0.0:22` - SSH מאזין על **כל** הinterfaces. אפשר להתחבר מכל כתובת IP.
...
1. **מה יש בnamespace חדש**:
   - `ip addr show` - רואים רק interface `lo` (loopback), והוא במצב DOWN
   - `ip route show` - ריק לגמרי, אין טבלת ניתוב
   - `ping 8.8.8.8` - נכשל עם "Network is unreachable" כי:
     - אין interface רשת פעיל (lo הוא DOWN)
     - אין טבלת ניתוב
...
2. **איך Docker containers מתקשרים עם האינטרנט**:
   ...
   **veth pair** - זוג interfaces וירטואליים שמחוברים זה לזה. כמו pipe - מה שנכנס בצד אחד יוצא בצד השני. קצה אחד נמצא בnamespace של הcontainer, והקצה השני בnamespace של המארח.
...
4. **סוגי namespaces נוספים בלינוקס**:
   - **PID namespace** - בידוד מזהי processes. הprocess 1 בcontainer הוא לא process 1 במארח.
