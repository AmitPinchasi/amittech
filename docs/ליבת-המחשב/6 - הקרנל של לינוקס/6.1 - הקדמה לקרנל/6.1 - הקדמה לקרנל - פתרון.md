...
הסבר:
- `current` הוא מאקרו שמחזיר מצביע ל-`task_struct` של הprocess הנוכחי. דרכו אפשר לגשת ל-`pid` (מזהה הprocess) ול-`comm` (שם הprocess)
...
### 5. למה הקרנל ממופה בכל process?

הקרנל ממופה בטבלאות הpaging של כל process כדי שמעבר מיוזר מוד לקרנל מוד (בזמן syscall או interrupt) יהיה **מהיר ככל האפשר**.

אם הקרנל היה ממופה בטבלת paging נפרדת, בכל syscall היינו צריכים:
1. לשמור את ערך הCR3 הנוכחי (של הprocess)
2. לטעון CR3 חדש (של הקרנל)
3. לבצע את הsyscall
4. לטעון בחזרה את הCR3 של הprocess
...
לכן הפתרון הוא פשוט: ממפים את הקרנל בכל טבלת paging, ומסמנים את הpages שלו כ-Ring 0 only. כך המעבר ל-Ring 0 לא דורש החלפת CR3 בכלל - הקוד של הקרנל כבר נגיש, פשוט עכשיו ההרשאה מאפשרת לגשת אליו.
