...
דרייבר הוא הגשר בין החומרה לבין שאר הקרנל. הוא מתרגם בין הinterface הגנרי שהקרנל מספק (קריאה, כתיבה, פתיחה) לבין הפרטים הספציפיים של כל התקן חומרה.
...
סוג מיוחד - **לא מופיעים ב/dev/**. ניגשים אליהם דרך interface הsocket (שנלמד בפרק 6.9). כל כרטיס רשת מיוצג ע"י שם interface כמו `eth0`, `wlan0`, `enp3s0`.
...
![תרשים המסלול של קריאה מקובץ התקן תו מהקריאה read ביוזר מוד, דרך המספר הראשי שמזהה את הדרייבר, ועד copy_to_user שמחזירה את הבתים](../../../images/illustrations/core-6-8-chardev-read-path.svg)
...
```bash
# compilation (requires kernel headers installed)
make -C /lib/modules/$(uname -r)/build M=$(pwd) modules

# loading the module
sudo insmod mychardev.ko

# the driver prints the major it got - let's check dmesg
dmesg | tail -1
...
# all the devices on the PCI bus
ls /sys/bus/pci/devices/

# info about the network card
...
3. כשההתקן סיים, הוא שולח interrupt (IRQ) למעבד

הקרנל מספק API לדרייברים שצריכים DMA:

```c
// memory allocation suited for DMA (physically contiguous, noncacheable)
void *dma_alloc_coherent(struct device *dev, size_t size,
                         dma_addr_t *dma_handle, gfp_t flag);

// mapping a regular buffer to a DMA address
