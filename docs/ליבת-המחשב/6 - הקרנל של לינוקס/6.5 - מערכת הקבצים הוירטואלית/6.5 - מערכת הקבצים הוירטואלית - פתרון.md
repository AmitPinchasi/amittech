...
```
MemFree:  4092000 kB  (dropped a bit)
Buffers:    32000 kB  (unchanged)
Cached:   2052000 kB  (rose by roughly the file size)
```

ג. Cached גדל כי הקרנל שמר את תוכן הקובץ שקראנו ב-page cache. כשcat קרא את הקובץ, הקרנל טען את הpages מהדיסק לRAM. גם אחרי שcat סיים, הpages נשארו ב-cache.

ד. אם נקרא את אותו קובץ שוב, הוא לא ייקרא מהדיסק. הוא כבר ב-page cache! הread של VFS יבדוק את ה-page cache, ימצא את הpages שם, ויעתיק מהRAM ישירות לbuffer של הprocess. זה הרבה יותר מהיר מקריאה מדיסק (מיקרו-שניות מול מילי-שניות).
...
**2. פתרון "/" (root)** - הקרנל מתחיל מ-root dentry של הprocess (בדרך כלל השורש של מערכת הקבצים). הוא לוקח את ה-dentry של `/`.
...
- בדיקת הרשאות: האם לprocess יש הרשאת execute על `/`? (הרשאת execute על תיקייה פירושה הרשאה לחפש בתוכה).
...
**6. בדיקת הרשאות על הקובץ** - הקרנל בודק אם לprocess (לפי UID ו-GID שלו) יש הרשאת קריאה על notes.txt (כי ביקשנו O_RDONLY).
...
**8. הallocation file descriptor** - הקרנל מחפש את המספר הנמוך ביותר הפנוי בטבלת ה-file descriptors של הprocess (שלמדנו עליה ב-5.5). מכניס את ה-struct file לתוך הטבלה.
