...
איך זה עובד? התשובה היא VFS - ה-Virtual File System. זו שכבת הפשטה (abstraction layer) בתוך הקרנל שמספקת interface אחיד לכל מערכות הקבצים. היא מה שהופך את הפילוסופיה של "הכל הוא קובץ" ממשפט נחמד למציאות טכנית.
...
struct super_block {
    ...
    const struct super_operations *s_op; // operations on the superblock
    struct dentry *s_root;             // the dentry of the root directory
    // ...
};
```
...
struct inode {
    ...
    kgid_t i_gid;                      // group (group id)
    unsigned long i_ino;               // inode number (unique within the filesystem)
    ...
    const struct inode_operations *i_op;  // operations on inode
    const struct file_operations *i_fop;  // operations on files (default)
    struct super_block *i_sb;          // pointer to the superblock
    struct address_space *i_mapping;   // mapping to the page cache
...
- **i_mapping** - קישור ל-page cache. כשקוראים את תוכן הקובץ, הpages נשמרים כאן.
...
```c
struct dentry {
    struct dentry *d_parent;           // pointer to the parent directory's dentry
    struct qstr d_name;                // file/directory name
    struct inode *d_inode;             // pointer to the inode
    const struct dentry_operations *d_op; // operations on dentry
    struct super_block *d_sb;          // pointer to the superblock
    struct list_head d_child;          // list of siblings (dentries in the same directory)
    struct list_head d_subdirs;        // list of children (dentries inside this directory)
    // ...
};
```
...
הקרנל שומר dentries בcache (cache) שנקרא dcache. זה אחד הcaches הכי חשובים בקרנל. כשהקרנל צריך לפתור נתיב, הוא בודק קודם ב-dcache. אם ה-dentry כבר שם - אין צורך לקרוא מהדיסק.

### file - קובץ פתוח

ה-struct שנקרא `file` מייצג קובץ פתוח. הוא נוצר כשprocess קורא ל-`open()` ונמחק כש-`close()` נקרא (או כשהprocess מת). שימו לב - inode מייצג קובץ על הדיסק. file מייצג קובץ פתוח ע"י process מסוים.

```c
struct file {
    struct path f_path;                // the path (including dentry and mount)
    const struct file_operations *f_op;   // operations on the file
    atomic_long_t f_count;             // reference count
    unsigned int f_flags;              // flags (O_RDONLY, O_WRONLY, O_NONBLOCK...)
    fmode_t f_mode;                    // mode (read/write)
    loff_t f_pos;                      // current position in the file
    struct fown_struct f_owner;        // owner (for signals)
    void *private_data;                // the driver's private data
    // ...
};
```
...
2. הוא מוצא את ה-struct file המתאים (דרך טבלת ה-file descriptors של הprocess, שלמדנו עליה ב-5.5).
3. הוא קורא ל-`file->f_op->read()`.
4. הפונקציה שרצה היא הread של מערכת הקבצים הספציפית - ext4, procfs, או מה שלא יהיה.

זה בעצם פולימורפיזם בC - אותו interface, מימושים שונים. כשקוראים `read()` על קובץ ב-ext4, רץ הread של ext4. כשקוראים `read()` על `/proc/cpuinfo`, רץ הread של procfs שמייצר את התוכן דינמית. מנקודת מבט של הprocess, אין הבדל.

![תרשים המראה כיצד אותה קריאת read מגיעה דרך שכבת ה-VFS למימושים שונים לגמרי לפי מערכת הקבצים שהקובץ שייך אליה](../../../images/illustrations/core-6-5-vfs-dispatch.svg)
...
כשprocess קורא ל-`open("/home/user/file.txt", O_RDONLY)`, הקרנל צריך לפתור את הנתיב ולמצוא את ה-inode של הקובץ. התהליך הזה נקרא pathname resolution, וזה אחד הדברים שVFS עושה הכי הרבה.
...
בכל שלב, הקרנל בודק גם הרשאות - האם לprocess יש הרשאת execute (שפירושה "חיפוש" עבור תיקיות) על כל תיקייה בנתיב.
...
כשקוראים תוכן מקובץ, הוא לא נקרא ישירות מהדיסק אל הbuffer של הprocess. הנתונים עוברים דרך ה-page cache - הcache ברמת pages ש-VFS מנהל.

### איך זה עובד

1. הprocess קורא ל-`read()` על קובץ.
2. הקרנל בודק אם הpage המבוקש כבר ב-page cache.
3. אם כן - הנתונים מועתקים מה-cache לbuffer של הprocess. לא צריך לגשת לדיסק.
4. אם לא - הקרנל קורא את הpage מהדיסק, שומר אותו ב-page cache, ואז מעתיק לprocess.

כתיבה עובדת באופן דומה: הנתונים נכתבים ל-page cache, והpage מסומן כ-dirty. בהמשך, הpage נכתב חזרה לדיסק (על ידי pdflush/writeback threads).

### למה לינוקס "משתמש בהרבה RAM"

אם הרצתם `free` או `top` ושמתם לב שכמעט כל הRAM תפוס - זה בגלל ה-page cache. לינוקס משתמש בכל הRAM הפנוי לcache קבצים. זה לא בזבוז - זה אופטימיזציה. ברגע שprocess צריך זכרון, הקרנל זורק pages מה-page cache כדי לפנות מקום.
...
בלינוקס מודרני, כל process (או קבוצת processes) יכול לראות עץ mount שונה. זה נקרא mount namespace. זה הבסיס לcontainers - כל container רואה מערכת קבצים שונה, עם מערכות קבצים שונות מותקנות בנקודות שונות.
...
ה-VFS הוא שכבת ההפשטה שמאחדת את כל מערכות הקבצים בלינוקס תחת interface אחד. ראינו את ארבעת המבנים המרכזיים:
