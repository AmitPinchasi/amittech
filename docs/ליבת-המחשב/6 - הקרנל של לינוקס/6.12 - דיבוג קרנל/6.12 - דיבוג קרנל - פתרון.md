...
- `Out of memory: Killed process ...` - OOM killer הרג process
...
struct node *linked_list_search(struct list_head *head, int key)
{
    ...
    while (current != NULL) {       // NULL check
        if (current->key == key)    // access current + 0x10
            return current;
...
כל פעם שprocess מבצע exec(), נראה את ה-PID, שם הprocess, ושם הקובץ שמורץ.
...
בפלט נראה לכל process כמה פעמים כל syscall נקרא. שמות ה-syscalls מופיעים כמספרים - אפשר לתרגם עם `/usr/include/asm/unistd_64.h`.
...
- מקבצים לפי שם הprocess (comm) ומציגים כהיסטוגרמה
