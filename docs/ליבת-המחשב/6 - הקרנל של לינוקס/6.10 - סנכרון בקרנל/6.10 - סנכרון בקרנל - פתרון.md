...
**פעולות אטומיות (atomic_t)** - זה מונה פשוט שצריך רק increment. פעולה אטומית מספיקה ועובדת גם מהקשר interrupt. אלטרנטיבה טובה: **per-CPU** counter - כל CPU סופר בנפרד, ורק כשרוצים את הסכום הכולל קוראים מכולם. זה אפילו יעיל יותר כי אין cache line bouncing בין מעבדים.
...
// option 2 - per-CPU (preferred):
DEFINE_PER_CPU(unsigned long, packet_count);
// in the interrupt handler:
this_cpu_inc(packet_count);
```

**ב. עץ עם allocation זיכרון בעדכון**

**mutex** - כי העדכון כולל allocation זיכרון (kmalloc עם GFP_KERNEL), שיכולה לישון. ב-spinlock אסור לישון, אז mutex הוא הבחירה הנכונה. זה גם מתרחש רק ב-process context (יצירת קובץ ע"י משתמש).
...
**spinlock עם irqsave** - כי interrupt handler צריך לגשת לרשימה (אסור mutex), וגם process context ניגש (צריך להגן מפני interrupts). חייבים להשתמש ב-`spin_lock_irqsave` כדי למנוע דדלוק בין process context ל-interrupt handler.
...
```c
// Thread 2 - fixed:
spin_lock(&lock_a);    // same order as Thread 1
spin_lock(&lock_b);
// ... work ...
spin_unlock(&lock_b);
spin_unlock(&lock_a);
...
### קטע ב - דדלוק! (interrupt)

**כן, יש סכנת דדלוק.**

הסיטואציה:
1. `my_process_func` נועלת `data_lock`
2. בדיוק באותו רגע, מגיעה interrupt **על אותו CPU**
...
{
    unsigned long flags;
    spin_lock_irqsave(&data_lock, flags);   // disables interrupts!
    // ... read shared data ...
    spin_unlock_irqrestore(&data_lock, flags);
}
```

כעת interrupts מושבתות בזמן שהמנעול מוחזק, אז ה-handler לא יכול לרוץ באמצע.
...
`mutex_lock` יכול לישון, ו-interrupt handler **לא יכול לישון** (הוא לא שייך לprocess, אז אין לו process context שניתן להרדים). זה יגרום ל-kernel oops/panic.
...
static DEFINE_SPINLOCK(queue_lock);    /* spinlock because get_message is called from an interrupt */
...
int add_message(const char *data, int priority)
{
    ...
    /* GFP_ATOMIC because we may be called from a context that cannot sleep,
       or because we lock a spinlock right after. If add_message is called
       only from process context, we can use GFP_KERNEL and allocate before locking. */
...
- השתמשנו ב-`irqsave` כי `add_message` עלולה להיקרא מ-process context ואז interrupt שקוראת ל-`get_message` תיצור דדלוק
- השתמשנו ב-`GFP_ATOMIC` כי הallocation קורית בהקשר שצריך spinlock (אם הallocation תמיד מ-process context, אפשר להקצות לפני הנעילה עם GFP_KERNEL)
...
// callback to free after a grace period:
...
int get_timeout(void)
{
    ...
    if (cfg)
        ...
        timeout = -1;     // default if there is no config
    rcu_read_unlock();
...
int update_config(int max_conn, int timeout, const char *name)
{
    ...
    // free the old config after a grace period:
    if (old_cfg)
