...
ב. מבנה נתונים מורכב (עץ) שמעודכן כשמשתמש יוצר קובץ חדש. העדכון כולל הallocation זיכרון.
...
ה. רשימה מקושרת של חיבורי רשת פתוחים. interrupt handler צריך לחפש חיבור ברשימה, וprocess user space צריך להוסיף ולהסיר חיבורים.
...
/* TODO: define the appropriate synchronization mechanism */

int add_message(const char *data, int priority)
{
    struct message *msg;

    msg = kmalloc(sizeof(*msg), /* TODO: which flag? */);
    if (!msg)
        return -ENOMEM;
...
// Reading the configuration (called thousands of times per second):
int get_timeout(void)
{
    int timeout;
    /* TODO: safe read with RCU */
    return timeout;
}

// Updating the configuration (happens rarely):
