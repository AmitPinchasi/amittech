...
int main() {
    // attempt 1: a file that doesn't exist
    ...
    // attempt 2: a file without permissions
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (f == NULL) {
            int err_code = errno;  // save errno before making further calls
