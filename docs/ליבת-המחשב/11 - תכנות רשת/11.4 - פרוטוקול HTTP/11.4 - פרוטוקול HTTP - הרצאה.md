...
![איור מונפש של חילופי הודעות HTTP, שבו הלקוח שולח שורת בקשה וכותרות ואחריהן שורה ריקה, והשרת מחזיר שורת מצב, כותרות, וגוף](../../../images/illustrations/core-11-4-http-request-response.svg)
...
int main(int argc, char *argv[])
{
    ...
    /* read the response */
    char response[BUFFER_SIZE * 16];  /* large buffer for the response */
    int total = 0;
    ssize_t n;
...
#define PORT 8080
#define BUFFER_SIZE 4096
#define ROOT_DIR "./www"  /* root directory for files */
...
void handle_request(int client_fd)
{
    ...
    /* protection against path traversal: .. is not allowed in the path */
    if (strstr(path, "..")) {
...
![תרשים זרימה מונפש של החלטות שרת HTTP סטטי, שבודק את המתודה, מוודא שהנתיב לא מכיל נסיגה לתיקיית האב, ומנסה לפתוח את הקובץ](../../../images/illustrations/core-11-4-server-request-flow.svg)
