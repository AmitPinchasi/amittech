...
int main(void)
{
    ...
    while (1) {
        ...
        /* get the current time and send it */
...
int main(void)
{
    ...
    while (1) {
        ...
        if (pid == 0) {
            ...
            close(client_fd);  /* already duplicated, don't need the original */

            /* run shell */
            execvp("/bin/sh", (char *[]){"sh", NULL});
            perror("execvp");  /* we only get here if exec failed */
            exit(1);
        }
...
- `signal(SIGCHLD, SIG_IGN)` - אומר למערכת ההפעלה לנקות processes ילד אוטומטית כשהם מסיימים, כדי למנוע processes zombie. למדנו על זה בפרק 5.4.
