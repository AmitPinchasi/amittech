...
בפרק 5.5 למדנו על מתארי קבצים (file descriptors) וראינו שבלינוקס "הכל הוא קובץ". בפרק 5.6 למדנו על pipes שמאפשרים תקשורת בין processes על אותה מכונה. בפרק 6.9 ראינו איך הקרנל מנהל את מחסנית הרשת - שכבות הפרוטוקולים, ניתוב, וטיפול בחבילות.
...
| `AF_UNIX` | תקשורת מקומית בין processes על אותה מכונה (כמו pipe, אבל דו-כיווני) |
...
struct in_addr {
    uint32_t s_addr;            // IP address as a 32-bit number
};
```
...
memset(&server_addr, 0, sizeof(server_addr));   // zero everything
...
// wrong! the port will be garbled:
server_addr.sin_port = 8080;  // don't do this
```
...
// string to number (presentation to network)
struct in_addr addr;
inet_pton(AF_INET, "192.168.1.1", &addr);

// number to string (network to presentation)
...
שימו לב לcasting ל-`(struct sockaddr *)`. הפונקציה `connect` מקבלת מצביע ל-`struct sockaddr` הגנרי, אבל אנחנו מעבירים `struct sockaddr_in` הספציפי ל-IPv4. הcasting הזה נפוץ בכל interface הסוקטים.
...
![תרשים זרימה מונפש של מחזור החיים של סוקט שרת, מיצירת הסוקט דרך bind ו-listen ועד לולאת accept שמקבלת לקוח אחרי לקוח](../../../images/illustrations/core-11-1-server-socket-lifecycle.svg)
...
`bind` קושרת את הסוקט לכתובת ופורט מסוימים. `INADDR_ANY` (שזה בעצם 0) אומר "תאזין על כל interfaces הרשת של המכונה". אם נרצה להאזין רק על localhost, נשתמש ב-`inet_pton(AF_INET, "127.0.0.1", &server_addr.sin_addr)`.
...
`listen` אומרת למערכת ההפעלה: "הסוקט הזה הוא שרת, תתחיל לקבל חיבורים". הפרמטר השני (5) הוא ה-backlog - כמה חיבורים ממתינים מותר לצבור בqueue לפני שהשרת מגיע לטפל בהם.
...
![איור מונפש שמראה איך כל קריאה לaccept מחזירה מתאר קבצים חדש ללקוח בעוד המתאר המאזין נשאר פתוח וממשיך לקבל חיבורים](../../../images/illustrations/core-11-1-accept-new-fd.svg)
...
if (n > 0) {
    ...
    write(client_fd, buffer, n);  // send back (echo)
}
```
...
// go back to accept to get the next client
```
...
# in another terminal:
nc 127.0.0.1 8080
# or with the client we wrote earlier:
./client 127.0.0.1 8080
```
...
```c
/* after accept: */
dup2(client_fd, STDIN_FILENO);   // stdin comes from the socket
dup2(client_fd, STDOUT_FILENO);  // stdout goes to the socket
dup2(client_fd, STDERR_FILENO);  // stderr goes to the socket
close(client_fd);                // no longer need the original fd

/* now every printf goes to the client, and every scanf reads from the client! */
execvp("/bin/sh", (char *[]){"sh", NULL});
/* the client gets a remote shell */
```
