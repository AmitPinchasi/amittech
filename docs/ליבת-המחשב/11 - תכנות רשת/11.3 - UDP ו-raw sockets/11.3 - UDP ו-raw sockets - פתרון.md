...
int main(int argc, char *argv[])
{
    ...
    while ((bytes_read = fread(pkt.data, 1, CHUNK_SIZE, fp)) > 0) {
        ...
        usleep(1000);  /* short delay to prevent flooding */
    }

    /* send end packet (data_len = 0) */
    pkt.seq_num = htonl(seq);
    pkt.data_len = 0;
...
int main(int argc, char *argv[])
{
    ...
    /* timeout so we can check statistics */
    struct timeval tv = {1, 0};
    setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
    ...
    while (running) {
        ...
        /* check statistics every 10 seconds */
...
sudo ./sniffer        # all traffic
sudo ./sniffer 80     # only port 80
sudo ./sniffer 443    # only HTTPS
```
