...
int main(void)
{
    ...
    /* main loop - receive and send */
    char buffer[BUFFER_SIZE];
    while (1) {
        struct sockaddr_in client_addr;
        socklen_t client_len = sizeof(client_addr);

        /* receive message - recvfrom also fills in the sender's address */
        ...
        /* send response - sendto sends to a specific address */
...
- שליחת פרוטוקולים שאין להם interface socket רגיל (כמו ICMP)
...
uint16_t checksum(void *data, int len)
{
    ...
    /* if there's a leftover byte at the end */
    if (len == 1)
        sum += *(uint8_t *)buf;

    /* fold: add the upper 16 bits to the lower 16 bits */
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        /* parse the response - IP header then ICMP */
        ...
        sleep(1);  /* one second between pings */
    }
...
![איור מונפש של מחזור בקשת ping, שבו נשלחת חבילת ICMP Echo Request עם מספר סידורי וחוזרת תשובת Echo Reply שממנה מחושב זמן הלוך ושוב](../../../images/illustrations/core-11-3-ping-icmp-roundtrip.svg)

---

## האזנה לתעבורה - packet sniffing

סוקטים גולמיים מאפשרים לנו להאזין לכל התעבורה שעוברת בinterface הרשת. זה בדיוק מה ש-tcpdump ו-Wireshark עושים ברמה הבסיסית.
...
int main(void)
{
    /* AF_PACKET + SOCK_RAW + ETH_P_ALL = all packets, including ethernet */
...
1. `socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL))` - סוקט שמקבל כל חבילה שמגיעה לinterface הרשת, כולל ה-Ethernet header
...
![איור מונפש של פירוק חבילה מסוקט גולמי לשכבות, שבו מחשבים את כתובת כל שכבה לפי אורך השכבה שלפניה](../../../images/illustrations/core-11-3-packet-layers.svg)
