...
`signal(SIGCHLD, SIG_IGN)` אומר למערכת ההפעלה לנקות processes ילד אוטומטית. בלי זה, כל ילד שמסיים הופך לzombie (process שנגמר אבל עדיין תופס כניסה בטבלת הprocesses) עד שהאב קורא ל-`wait`.
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            if (clients[i] != -1 && FD_ISSET(clients[i], &read_fds)) {
                ...
                if (n <= 0) {
                    ...
                    /* add a prefix with the fd number */
...
int main(void)
{
    ...
    /* add the server socket - use data.fd since it's the server, no struct needed */
    ...
    while (1) {
        ...
        for (int i = 0; i < n; i++) {
            if (events[i].data.fd == server_fd) {
                ...
                /* create a data structure for the client */
                ...
                /* add to epoll with a pointer to the struct */
                ev.events = EPOLLIN;
                ev.data.ptr = cd;
...
בתוצאות תראו הבדלים ברורים: שרת fork יהיה האיטי ביותר (בגלל העלות של יצירת processes), select יהיה בינוני, ו-epoll יהיה המהיר ביותר - במיוחד עם מספר גבוה של לקוחות.
