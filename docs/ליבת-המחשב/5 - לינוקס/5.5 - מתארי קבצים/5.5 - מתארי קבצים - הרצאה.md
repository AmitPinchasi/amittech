...
בהרצאה הזו נצלול לעומק לתוך המנגנון הזה. נבין מה בדיוק הם מתארי קבצים, איך הם מאורגנים בתוך process, ונלמד לעבוד ישירות עם הsyscall-ים של לינוקס בלי לעבור דרך libc. בנוסף, נלמד על מנגנון חשוב שנקרא dup2 שמאפשר לנו לשלוט לאן הפלט של התוכנה שלנו הולך.

---

## מתארי קבצים - file descriptors

מתאר קובץ (file descriptor, בקיצור fd) הוא מספר שלם (int) שמייצג משאב פתוח בprocess. כשאנחנו פותחים קובץ, מערכת ההפעלה מחזירה לנו מספר - וזה המספר שנשתמש בו כדי לקרוא, לכתוב, ולסגור את המשאב.
...
## טבלת הfd-ים של process

לכל process בלינוקס יש **טבלה פרטית של מתארי קבצים**. הטבלה הזו ממפה מספרים (0, 1, 2, 3, ...) לאובייקטי קבצים בקרנל.
...
כבר ראינו את זה בפרק 4, אבל עכשיו נבין את זה לעומק. כל process בלינוקס מתחיל עם שלושה fd-ים פתוחים מראש:
...
שלושת הfd-ים האלו נפתחים אוטומטית על ידי מערכת ההפעלה כשprocess מתחיל לרוץ. הם קיימים עוד לפני ששורה אחת של הקוד שלנו רצה.
...
הנה השרשרת:
```
fopen("file.txt", "r")  -->  open("file.txt", O_RDONLY)  -->  syscall in the kernel  -->  returns fd
fwrite(buf, 1, n, f)    -->  write(fd, buf, n)           -->  syscall in the kernel
fclose(f)                -->  close(fd)                   -->  syscall in the kernel
```
...
דוגמאות:
```c
// open for read only (like fopen with "r")
int fd = open("data.txt", O_RDONLY);

// open for writing, create if missing, truncate content (like fopen with "w")
int fd = open("output.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644);

// open for appending to the end of the file (like fopen with "a")
...
## הredirect פלט - I/O redirection עם dup2
...
int main() {
    ...
    dup2(fd, 1);  // fd 1 (stdout) now points to output.txt

    // close the original fd - we no longer need it
    close(fd);

    // from now on, all output goes to the file!
    printf("This line goes to the file, not the screen\n");
...
![איור הפניית הפלט עם dup2: טבלת מתארי הקבצים של התהליך משתנה צעד אחר צעד, המתאר 1 מפסיק להצביע על המסך ומתחיל להצביע על הקובץ output.txt, ולכן הפלט של printf נכתב לקובץ](../../../images/illustrations/core-5-5-dup2-redirect.svg)

---

## מתארי קבצים וfork

בהרצאה 5.2 למדנו על `fork` - שכפול process. דבר חשוב שצריך לדעת: כאשר process קורא ל-`fork`, **process הבן יורש עותק של טבלת הfd-ים של האב**.
...
2. הshell קורא ל-`fork` - הבן יורש את הfd-ים, כולל הredirect
...
int main() {
    ...
    if (pid == 0) {
        // child process - the fd is still open!
...
אחרי ההרצה, הקובץ `shared.txt` יכיל את ההודעות משני הprocesses. שימו לב שלא היינו צריכים לעשות `open` בprocess הבן - הוא ירש את הfd מהאב.
...
בהרצאה הזו למדנו:
- מתאר קובץ (fd) הוא מספר שלם שמייצג משאב פתוח בprocess
- בלינוקס הכל הוא קובץ - קבצים, pipes, sockets, התקנים - הכל נגיש דרך fd
- לכל process יש טבלת fd-ים פרטית
...
- הsyscall-ים `dup` ו-`dup2` מאפשרים שכפול וredirect של fd-ים
- בעזרת `dup2` אפשר להפנות stdout לקובץ - ככה הshell מממש את האופרטור `>`
- כש-`fork` יוצר process בן, הבן יורש את כל הfd-ים של האב
