...
1. הספריות המשותפות לכולן:
   ...
   - `linux-vdso.so.1` - ספריה וירטואלית שהקרנל מזריק לכל process

2. הנתיב של הdynamic linker הוא `/lib64/ld-linux-x86-64.so.2`

3. הספריה `linux-vdso.so.1` (שמה המלא: Virtual Dynamic Shared Object) היא ספריה מיוחדת שהקרנל ממפה אוטומטית לזיכרון של כל process. היא מכילה מימושים מהירים של כמה syscall-ים נפוצים (כמו `gettimeofday`) שיכולים לרוץ בuser mode בלי מעבר לkernel mode - מה שמאיץ אותם משמעותית.
