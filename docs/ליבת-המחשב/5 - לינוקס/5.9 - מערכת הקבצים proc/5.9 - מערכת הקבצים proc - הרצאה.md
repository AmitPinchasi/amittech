...
עד עכשיו למדנו איך לעבוד עם קבצים, processes, מתארי קבצים ועוד.
אבל יש שאלה שעולה - איך אפשר לדעת מה קורה בתוך מערכת ההפעלה? איך כלים כמו `ps`, `top`, `htop` ו-`lsof` יודעים להראות לנו מידע על processes, זיכרון ומשאבי מערכת?

התשובה היא **מערכת הקבצים proc** - או בשמה המלא `/proc`.

לינוקס חושפת מידע על הקרנל ועל processes דרך מערכת קבצים וירטואלית שנקראת `/proc`. אלה לא קבצים אמיתיים על הדיסק - הקרנל מייצר את התוכן שלהם בזמן אמת כשאנחנו קוראים אותם. כלומר, כל פעם שאנחנו עושים `cat` על קובץ ב-`/proc`, הקרנל מרכיב את התוכן באותו רגע ממש.
...
## מידע על processes - proc/[pid]/

בתוך `/proc` יש ספריות שנקראות לפי מספרי PID. כל ספריה כזו מכילה מידע על process מסוים.
למשל, אם יש לנו process עם PID 1234, כל המידע עליו נמצא ב-`/proc/1234/`.

נעבור על הקבצים החשובים ביותר בספריה הזו.

### מיפויי זיכרון - proc/[pid]/maps

הקובץ `/proc/[pid]/maps` מכיל את כל מיפויי הזיכרון של הprocess - כלומר, אילו אזורי זיכרון ממופים לprocess, עם איזה הרשאות, ומאיפה הם באו.

זוכרים את paging מפרק 2? אלה בעצם הentries של הpage table שדיברנו עליהן! כל שורה מייצגת אזור זיכרון וירטואלי שממופה לpages פיזיים.
...
### סטטוס הprocess - proc/[pid]/status

הקובץ `/proc/[pid]/status` מכיל מידע כללי על הprocess בפורמט קריא. כל שורה היא key-value:
...
השדות החשובים:
- **Name** - שם הprocess (שם קובץ ההרצה)
- **State** - מצב הprocess (R=רץ, S=ישן, Z=זומבי, ועוד)
- **Pid** - מזהה הprocess
- **PPid** - מזהה process האב
- **Uid** - מזהה המשתמש שמריץ את הprocess
- **VmSize** - גודל הזיכרון הוירטואלי הכולל
- **VmRSS** - כמות הזיכרון הפיזי שהprocess באמת משתמש בו (Resident Set Size)
- **Threads** - מספר הthread-ים

### מתארי קבצים פתוחים - proc/[pid]/fd/

הספריה `/proc/[pid]/fd/` מכילה symlink-ים לכל מתארי הקבצים הפתוחים של הprocess. כל symlink נקרא לפי מספר הfd, ומצביע על הקובץ שהfd מייצג.

כזכור מהרצאה 5.5, לכל process יש לפחות שלושה fd-ים פתוחים: 0 (stdin), 1 (stdout), 2 (stderr).
...
הקובץ `/proc/[pid]/cmdline` מכיל את הארגומנטים של שורת הפקודה שהprocess הופעל איתה. הארגומנטים מופרדים בתו null (`\0`) במקום רווח.
...
הקובץ `/proc/[pid]/exe` הוא symlink שמצביע על קובץ ההרצה עצמו של הprocess. אפשר להשתמש בו כדי לדעת בדיוק איזה קובץ בינארי רץ:
...
הקובץ `/proc/[pid]/environ` מכיל את כל משתני הסביבה של הprocess, מופרדים ב-`\0`:
...
## proc/self/ - קיצור לprocess הנוכחי

במקום לכתוב `/proc/[pid]/` עם הPID שלנו, לינוקס מספקת קיצור נוח: `/proc/self/`.
זהו symlink שתמיד מצביע על ספריית ה-`/proc` של הprocess שקורא אותו.
...
בנוסף למידע על processes, `/proc` מכיל גם מידע כללי על המערכת:
...
### דוגמה 1 - קריאת סטטוס הprocess

תוכנית שקוראת את `/proc/self/status` ומדפיסה את שם הprocess, הPID, וגודל הזיכרון:
...
תוכנית שקוראת את `/proc/self/maps` ומציגה את כל אזורי הזיכרון שממופים לprocess:
...
int main() {
    ...
    char line[512];
    printf("Memory map of the process:\n");
    printf("-------------------------------------------\n");
...
מערכת הקבצים `/proc` חושפת מידע רגיש על processes - אפשר לראות דרכה מתארי קבצים פתוחים, משתני סביבה (שיכולים להכיל סיסמאות), ומיפויי זיכרון.

בהפצות לינוקס מודרניות, אפשר להגביל את הגישה ל-`/proc` של processes אחרים באמצעות אופציית ה-`hidepid` בזמן mount:
- **hidepid=0** - ברירת מחדל, כל משתמש רואה את כל הprocesses
- **hidepid=1** - משתמשים לא יכולים לגשת לספריות `/proc/[pid]/` של processes של משתמשים אחרים
- **hidepid=2** - ספריות `/proc/[pid]/` של processes של משתמשים אחרים לא מופיעות בכלל
