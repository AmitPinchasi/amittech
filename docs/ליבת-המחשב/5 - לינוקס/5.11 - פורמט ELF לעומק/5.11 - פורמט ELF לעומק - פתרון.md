...
int initialized_var = 100;      // initialized global variable
int uninitialized_var;          // uninitialized global variable

void greet(void) {              // additional function
    printf("Hello!\n");         // constant string
}
...
| Info | `000200000007` | מכיל את אינדקס הsymbol (0x2) ואת סוג הrelocation (0x7) |
| Type | `R_X86_64_JUMP_SLOT` | סוג הrelocation - "jump slot" - זה אומר שצריך למלא כתובת בGOT עבור PLT |
| Sym. Value | `0x0` | הכתובת של הsymbol עדיין לא ידועה (תימצא בזמן ריצה) |
| Sym. Name | `printf@GLIBC_2.2.5` | שם הsymbol שצריך למצוא - printf מגרסת GLIBC 2.2.5 |
