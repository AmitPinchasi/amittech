...
- **הprocesses** (פרק 5.2) - fork, execve, wait
- **סיגנלים** (פרק 5.4) - טיפול ב-SIGINT, SIGCHLD
- **מתארי קבצים** (פרק 5.5) - שימוש ב-dup2 לredirect קלט/פלט
- **הpipes** (פרק 5.6) - pipe להעברת נתונים בין processes

ה-shell שנבנה יוכל לקבל פקודות מהמשתמש, להריץ אותן, לתמוך בredirect קלט/פלט, pipes, וטיפול בסיגנלים - בדיוק כמו bash או zsh, רק בגרסה מינימלית.
...
- יצירת process בן עם fork
- בprocess הבן - הרצת הפקודה עם execvp
- בprocess האב - המתנה לסיום הבן עם waitpid
...
## שלב 2 - redirect קלט ופלט - I/O redirection

בשלב הזה נוסיף תמיכה בredirect קלט ופלט - בדיוק כמו שlמדנו בפרק 5.5 על מתארי קבצים ו-dup2.

**מה צריך לממש:**
- תמיכה ב-`>` - redirect stdout לקובץ (יצירה/דריסה)
- תמיכה ב-`>>` - redirect stdout לקובץ (הוספה)
- תמיכה ב-`<` - redirect stdin מקובץ
...
3. בprocess הבן (אחרי fork ולפני execvp):
...
בשלב הזה נוסיף תמיכה באופרטור `|` - חיבור הפלט של פקודה אחת לקלט של פקודה אחרת. זה ישירות מה שלמדנו בפרק 5.6 על pipes.

**מה צריך לממש:**
- זיהוי התו `|` בשורת הקלט
- חלוקת הפקודה לשתי פקודות - מה שלפני הpipe ומה שאחריו
- יצירת pipe עם `pipe()`
- יצירת שני processes בן
- בבן הראשון: redirect stdout לצד הכתיבה של הpipe
- בבן השני: redirect stdin לצד הקריאה של הpipe
...
**מה צריך לממש:**
- טיפול ב-SIGINT (Ctrl+C): כשהמשתמש לוחץ Ctrl+C, רוצים שזה יהרוג את הprocess שרץ כרגע, אבל **לא** את הshell עצמו
- טיפול ב-SIGTSTP (Ctrl+Z): כשהמשתמש לוחץ Ctrl+Z, רוצים לעצור (stop) את הprocess הנוכחי (בונוס)
...
3. בprocess הבן, לפני execvp, החזירו את הטיפול בSIGINT לברירת מחדל:
```c
...
**מה צריך לממש:**
- אם הפקודה מסתיימת ב-`&`, להריץ אותה ברקע - כלומר, לא לחכות (waitpid) לסיום הprocess
- הshell ממשיך לקבל פקודות מיד
- טיפול ב-SIGCHLD כדי לנקות processes רקע שסיימו (כדי למנוע zombie-ים)
...
```
4. הדגל WNOHANG אומר ל-waitpid: "אם אין process שסיים, אל תחכה - תחזור מיד"
...
הפקודה cd לא יכולה לרוץ כprocess בן - היא חייבת לשנות את הספריה של הshell עצמו. זו מה שנקרא "built-in command". ממשו אותה עם `chdir()`:
```c
...
- דורש יצירת מספר pipes ומספר processes בן
