...
![אנימציה של פקודת cmp על שלושה זוגות ערכים, שמראה את החיסור הפנימי שהמעבד מבצע ואת הדגלים zf ו-sf שנדלקים בהתאם לתוצאה](../../../images/illustrations/core-1-3-cmp-flags.svg)
...
mov ax, 10
mov bx, 20
cmp ax, bx      ; compares: 10 - 20

jl is_less      ; if ax < bx (signed)
je is_equal     ; if ax == bx
jg is_greater   ; if ax > bx (signed)

is_less:
  ; here we handle the case where ax < bx
  jmp end

is_equal:
  ; here we handle the case where they are equal
  jmp end

is_greater:
  ; here we handle the case where ax is greater
  jmp end

end:
```

![תרשים זרימה של שרשרת קפיצות מותנות אחרי cmp, שבו המעבד בודק כל תנאי לפי הדגלים עד שאחד מתקיים וקופץ אל הקוד המתאים](../../../images/illustrations/core-1-3-jump-branch.svg)
...
![תרשים זרימה מונפש של לולאה עם cmp ו-jne, שמראה כיצד הערך ברגיסטר cx יורד באחד בכל סיבוב עד שהוא מגיע לאפס והלולאה נעצרת](../../../images/illustrations/core-1-3-loop-cx.svg)
...
```asm
test al, al     ; checks if AL == 0
jz zero_case    ; if so, jump
```
