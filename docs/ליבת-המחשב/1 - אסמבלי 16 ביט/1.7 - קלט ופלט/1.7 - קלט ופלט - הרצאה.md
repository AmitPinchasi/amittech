...
למעבד אין "חיבור ישיר" להתקן – הוא **שולח וקולט נתונים דרך interfaces חשמליים** הקרויים **פורטי קלט/פלט (I/O Ports)**.
...
```asm
in AL, DX    ; read from the port written in DX, the result goes into AL (8 bit)
...
```asm
out DX, AL   ; send AL to the port written in DX (8 bit)
out DX, AX   ; send 16 bit to the port
```
...
```asm
mov dx, 0x60       ; address of the keyboard port (Keyboard Data Port)
in al, dx          ; read a character from the keyboard
mov ah, 0Eh        ; BIOS display service
int 10h            ; display the character you read
```
...
```asm
mov dx, 0x300
mov al, 0b10101010   ; light up only some of the LEDs
out dx, al
```
...
![איור המראה מה עובר בכל אחד משלושת ערוצי ה-bus בעת פקודת out, כאשר ערוץ הכתובות נושא את מספר הפורט, ערוץ הנתונים את הערך שנשלח, וערוץ הפיקוד את אות הכתיבה להתקן](../../../images/illustrations/core-1-7-out-bus.svg)
...
```asm
wait_key:
  in al, 0x64       ; checks whether a character is ready
  test al, 1        ; checking bit 0 - ready?
  jz wait_key       ; if not - wait
...
```asm
keyboard_loop:
    in al, 0x64         ; check the keyboard status
    test al, 1          ; if bit 0 is on - a character is ready
    jz keyboard_loop    ; if not - keep waiting
    in al, 0x60         ; get the character
    mov ah, 0Eh
