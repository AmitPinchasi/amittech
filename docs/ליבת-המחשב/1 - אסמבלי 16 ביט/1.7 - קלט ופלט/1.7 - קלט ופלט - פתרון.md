...
CODESEG
start:
    mov dx, 0x60       ; keyboard port address
    in al, dx          ; read a character from the port
...
start:
wait_key:
    in al, 0x64        ; check the keyboard status port
    test al, 1         ; is bit 0 on? (character ready?)
    jz wait_key        ; if not - keep waiting

    in al, 0x60        ; read the character from the keyboard port

    mov ah, 0Eh        ; BIOS TTY service
    mov bh, 0
    mov bl, 7
    int 10h            ; display the character
...
CODESEG
start:
    mov dx, 0x300           ; address of the LED card port

    mov al, 0b11001100      ; first value - turns on some of the LEDs
    out dx, al              ; send to the port

    mov al, 0b00110011      ; second value - turns on other LEDs
    out dx, al              ; send to the port
...
keyboard_loop:
    ...
    test al, 1          ; is bit 0 on? (is a character ready?)
    jz keyboard_loop    ; if not - go back to the start of the loop
    ...
    mov bh, 0
    mov bl, 7
    int 10h             ; display the character on the screen

    jmp keyboard_loop   ; go back to the loop - keep reading characters
END start
```
