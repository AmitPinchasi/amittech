...
```asm
loop_demo:
    ; factor * 3 is computed once before the loop
...
```asm
volatile_loop:
    movl    $1, -4(%rsp)       ; flag = 1 (on the stack)
    xorl    %eax, %eax         ; count = 0
.L2:
    movl    -4(%rsp), %edx     ; reads flag from memory every time!
    testl   %edx, %edx
    je      .L1
    addl    $1, %eax
    cmpl    $1000000, %eax
...
.L2:
    ...
    subl    $1, %edi       ; n = n - 1
    jmp     .L2            ; jmp! not call! - tail call optimization
.L1:
    movl    %esi, %eax
    ret
```
...
factorial_regular:
    cmpl    $1, %edi
    jle     .L1
    ; the compiler might perform TCO here too (gcc is smart)
    ; but in more complex cases, the multiplication after the call prevents TCO
    ...
```
...
int main() {
    ...
    // every recursive call consumes a frame on the stack
    // 1000000 frames * ~16 bytes = ~16MB - more than the usual stack size

    // factorial_tail(1000000, 1) with -O2:
    // the compiler turned it into a loop - a single frame only!
...
main:
    ...
    movl    $5, %edi
    call    square          ; call to a function
    ...
    movl    $3, %edi
    call    cube            ; call to a function
    ...
```
...
main:
    ...
    ; square(5) = 25, cube(3) = 27 - computed at compile time!
    movl    $25, %esi
    movl    $27, %edx
    leaq    .LC0(%rip), %rdi
    call    printf
