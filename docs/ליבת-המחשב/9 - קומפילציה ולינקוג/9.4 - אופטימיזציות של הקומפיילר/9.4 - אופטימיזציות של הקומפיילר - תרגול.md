...
int volatile_loop() {
    ...
    while (flag) {
        count++;
        if (count >= 1000000) {
            // flag can change from outside (e.g. by a signal handler)
            // "in reality" we would check an external condition
            flag = 0;
        }
    }
    return count;
...
// regular version (not tail call)
int factorial_regular(int n) {
    if (n <= 1) return 1;
    return n * factorial_regular(n - 1);  // multiplication after the call - not a tail call
}

// tail-call version
int factorial_tail(int n, int acc) {
    if (n <= 1) return acc;
    return factorial_tail(n - 1, n * acc);  // the call is the last thing
}
```
