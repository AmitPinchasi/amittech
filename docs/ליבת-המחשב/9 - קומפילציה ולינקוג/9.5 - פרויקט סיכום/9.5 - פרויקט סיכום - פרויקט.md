...
void _start(void) {
    // TODO: call my_exit with exit code 0
}
```
...
// ... paste my_exit from the previous step here ...
...
// TODO: write a my_strlen function that computes string length
static int my_strlen(const char *s) {
    // ...
}

// TODO: write a my_puts function that prints a string with a newline
static void my_puts(const char *s) {
    // hint: use my_write with fd=1 (stdout)
    // ...
}
...
## שלב 3 - allocation זכרון בלי malloc

**המטרה:** כתבו bump allocator פשוט שמשתמש ב-mmap syscall לallocation זיכרון.

**רקע:** bump allocator הוא המקצה הפשוט ביותר - הוא מחזיק מצביע לסוף הזיכרון המוקצה, וכל allocation פשוט מקדמת את המצביע. אין free - הזיכרון לא משתחרר (זה מספיק לתוכנית הפשוטה שלנו).
...
// ... paste my_exit, my_write, my_strlen, my_puts from the previous steps here ...
...
// TODO: write a bump allocator
// note: r10 needs special handling in the syscall (see explanation below)
...
// TODO: write a my_malloc function
static void *my_malloc(long size) {
    // if the heap hasn't been initialized, initialize it
    // check that there's enough room
    // advance heap_current and return a pointer
    // ...
}

// TODO: write a function that converts a number to a string (base 10)
static void print_number(long n) {
    // hint: convert digit by digit from the end to the start
    // ...
}

void _start(void) __attribute__((noreturn));
void _start(void) {
    // demo of the allocator
...
**המטרה:** כתבו linker script שמגדיר את הפריסה של התוכנית, כולל symbols שנגישים מהקוד.
...
// TODO: write a print_hex function that prints an address in hexadecimal
...
6. הריצו `nm freestanding` ובדקו את הsymbols - אילו symbols הגיעו מהlinker script?
