...
- הביטוי `ALIGN` מופיע עשרות פעמים. ערכי יישור נפוצים: `ALIGN(4096)` (גבול page), `ALIGN(8)`, `ALIGN(4)`, `ALIGN(64)` (גבול cache line).
...
בפלט של readelf נראה שהסקשנים מסודרים בסדר: `.startup`, `.text`, `.handlers` - בדיוק כפי שהגדרנו בlinker script. כתובות ההתחלה של כל סקשן יתאימו לsymbols שהגדרנו.
