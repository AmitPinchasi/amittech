...
### תרגיל 4 - הגדרת symbols בlinker script

שנו את הlinker script מתרגיל 3 כך שיגדיר symbols:
...
2. השתמשו ב-`extern char _text_start;` וכו' כדי לגשת לsymbols שהגדרתם.
3. הדפיסו את כתובת ההתחלה והסיום של כל סקשן.
4. חשבו את גודל סקשן `.text` מתוך הsymbols, וודאו שהוא תואם את מה שreadelf מראה.
...
5. הגדירו symbols בlinker script שמסמנים את תחילת וסוף כל סקשן.
