...
קובעת את **נקודת הכניסה** - הכתובת הראשונה שהמעבד מריץ. בדרך כלל זה `_start`, אבל אפשר לשים כל שם של symbol.
...
![אנימציה של מונה המיקום בסקריפט הלינקר, שמראה כיצד הכתובת מתחילה ב-0x400000 ומתקדמת בגודל של כל סקשן בתורו, כך שכל סקשן מתחיל בדיוק בכתובת שבה הסתיים הסקשן הקודם](../../../images/illustrations/core-9-3-location-counter.svg)
...
```
.text : {
    main.o(.text)      /* only .text from main.o */
    utils.o(.text)     /* then .text from utils.o */
...
## הגדרת symbols בlinker script

אחד הדברים החזקים בlinker scripts הוא היכולת להגדיר symbols שאפשר לגשת אליהם מקוד C:
...
עכשיו אפשר לגשת לsymbols האלה מקוד C:
...
void init_bss(void) {
    // reset the BSS section to 0 (required on embedded systems)
    char *p = &_bss_start;
    while (p < &_bss_end) {
        *p = 0;
        p++;
...
שימו לב: הsymbols `_text_start` וכו' לא מכילים ערך - הם פשוט מייצגים כתובות. לכן ניגשים אליהם עם `&` (כתובת הsymbol) ולא בלי.
...
![אנימציה של חלוקת הסקשנים בין אזורי הזיכרון במערכת embedded, שמראה כיצד סקשני הקוד נשמרים ורצים מ-FLASH, ואילו סקשן data נשמר ב-FLASH אך מועתק ל-RAM באתחול וסקשן bss קיים רק ב-RAM ומאופס על ידי קוד ההפעלה](../../../images/illustrations/core-9-3-memory-regions.svg)
...
SECTIONS {
    ...
    . = ALIGN(4096);  /* align to a page boundary (4KB) */
...
למה זה חשוב? כי סגמנטים עם הרשאות שונות חייבים להיות בpages זיכרון שונים. סגמנט הקוד הוא R-X וסגמנט הנתונים הוא RW-, ואי אפשר להגדיר הרשאות שונות על אותו page. לכן מיישרים לגבול 4096 (גודל page בx86).
...
# comparison to a regular hello world
gcc -static hello.c -o hello_static
ls -la hello_static
# hundreds of KB (because it includes all of libc)
...
```c
// in C code - place data in a custom section
...
זה שימושי כשרוצים לשים קוד או נתונים מסוימים במיקום מדויק בזיכרון - למשל טבלת וקטורי interrupts שחייבת להיות בכתובת 0x0 במערכות embedded.
...
```bash
# usage directly with ld
ld -T my_script.ld main.o -o program

# usage via gcc (passes through to ld)
...
- **הגדרת symbols** - יוצרים symbols שנגישים מקוד C
