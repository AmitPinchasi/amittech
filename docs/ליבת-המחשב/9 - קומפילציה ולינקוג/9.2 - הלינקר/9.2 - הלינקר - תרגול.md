...
1. קמפלו כל קובץ בנפרד ל-`.o` ובדקו את הsymbols של כל קובץ עם `nm`. לכל קובץ, רשמו אילו symbols הוא מגדיר (T/D) ואילו הוא צריך (U).
...
void my_strrev(char *s);  // reverse a string
#endif
```
...
6. הריצו `nm libmystring.a` כדי לראות את הsymbols בספריה.
...
// liba.c
#include <stdio.h>
int func_b(int x);  // defined in libb
...
```c
// libb.c
int func_a(int x);  // defined in liba
