...
```
```
                 U counter         # undefined - used (extern)
                 U increment       # undefined - used (declaration only)
0000000000000000 T main            # defined - defines
                 U print_counter   # undefined - used
```
...
```
```
0000000000000000 D counter         # defined in .data - initialized global variable
0000000000000000 T increment       # defined in .text - function
```
...
```
                 U counter         # undefined - used (extern)
0000000000000000 T print_counter   # defined in .text - function
...
גם `main.o` וגם `printer.o` צריכים את `counter`, ו-`main.o` צריך את `increment`. שני הsymbols האלה מוגדרים רק ב-`counter.o` שהשמטנו.
...
ב-`strong.c` המשתנה `value` מוגדר עם ערך התחלתי (`int value = 42;`) - זה symbol חזק. ב-`weak.c` המשתנה `value` לא מאותחל (`int x;`) - זה symbol חלש. הsymbol החזק מנצח, אז הערך ההתחלתי הוא 42. שני הקבצים חולקים את אותו משתנה.
...
עכשיו שני הקבצים מגדירים symbol חזק עם אותו שם - שגיאה.

3. במקרה הראשון: symbol חזק + symbol חלש = הsymbol החזק מנצח (בלי שגיאה). במקרה השני: שני symbols חזקים = שגיאת `multiple definition`. הכלל: אסור שיהיו שני symbols חזקים עם אותו שם.

4. פתרון - להשתמש ב-`static`:

```c
// strong.c
static int value = 42;  // local - not accessible from outside
```

```c
// weak.c
static int value = 99;  // local - doesn't conflict
```

עם `static`, כל קובץ מקבל עותק פרטי של `value`. הsymbols לא חשופים ללינקר ולכן אין התנגשות.
...
עם `--start-group`/`--end-group`, הלינקר עובר על הספריות שביניהם שוב ושוב עד שכל הsymbols נפתרים. בחזרה הראשונה הוא לוקח `func_a` מ-liba ו-`func_b` מ-libb. בחזרה השנייה הוא מוודא שהכל פתור.
...
הסקשן `.text` הוצב בכתובת 0x401000, הסקשן `.data` ב-0x404000 (על page זיכרון שונה, כי הרשאות שונות - הקוד הוא R-X והנתונים הם RW-).
