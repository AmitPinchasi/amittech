...
```c
#include <stdio.h>     // search in system directories (/usr/include)
#include "myheader.h"  // search current directory first, then system directories
```
...
```c
#pragma once              // alternative to include guard - supported by most compilers
#pragma pack(1)           // changes struct alignment (disables padding)
#pragma GCC optimize("O2") // GCC-specific optimization directive
```
...
```c
// way 1: include guards (the standard way)
#ifndef MY_HEADER_H
#define MY_HEADER_H
// header content
#endif

// way 2: pragma once (the short way)
#pragma once
// header content
```
...
```bash
gcc -O0 main.c   # no optimization (default) - suitable for debugging
gcc -O1 main.c   # basic optimizations
gcc -O2 main.c   # advanced optimizations (recommended for production)
gcc -O3 main.c   # aggressive optimizations (may increase code size)
gcc -Os main.c   # optimization for minimal code size
gcc -Og main.c   # optimizations that don't hurt debuggability
```
...
```bash
gcc -Wall main.c        # enables most warnings
gcc -Wextra main.c      # additional warnings
gcc -Werror main.c      # treats warnings as errors
gcc -Wall -Wextra -Werror main.c  # the recommended combination
```
...
- **symtab.** - טבלת הsymbols (שמות הפונקציות והמשתנים)
...
2. יוצר רשומת relocation שאומרת: "בoffset הזה, תמלא את הכתובת של הsymbol `printf`"
...
![אנימציה של מילוי רשומת relocation, שמראה כיצד האסמבלר משאיר במקום הקריאה כתובת אפס ורושם חור בסקשן rela.text, והלינקר מוצא את הכתובת הסופית של printf וממלא אותה במקום האפסים](../../../images/illustrations/core-9-1-relocation-patch.svg)
...
מה הלינקר עושה:
1. **פתרון symbols - symbol resolution** - מוצא את ההגדרה של כל symbol שמשתמשים בו
...
```bash
# steps 1+2+3: compile each file separately to an object file
gcc -c main.c -o main.o
gcc -c math_utils.c -o math_utils.o

# step 4: link all object files into an executable
...
2. **קימפול math_utils.c** - הpreprocessor מעתיק את `math_utils.h`. הקומפיילר רואה את ההגדרות של `add` ו-`multiply`. האסמבלר יוצר `math_utils.o` עם הsymbols `add` ו-`multiply` מוגדרים.
...
```bash
# checking symbols in the object file
nm main.o
# U printf        <- undefined (defined elsewhere)
# T main          <- defined in text section

# checking sections
readelf -S main.o

# checking relocation entries
readelf -r main.o
...
```
    ...
    .c file ---------> .i file ---------> .s file ---------> .o file ---------> executable
        ...
        (preprocessor)      (compiler)           (assembler)          (linker)
              |                   |                   |                   |
        processes #include  translates C      translates assembly  links .o files
        replaces #define    to assembly       to machine code      and libraries
        handles #ifdef      performs          creates ELF file     resolves symbols
        removes comments    optimizations     with relocations     determines addresses
```
...
- למה יש שגיאות "undefined reference"? כי הלינקר לא מצא הגדרה לsymbol
