...
5. הsymbols שצריכים relocation:

```bash
readelf -r stages.o
```

נראה שהsymbol `printf` (או `puts` אם הקומפיילר החליט לבצע אופטימיזציה) צריך relocation - כי ההגדרה שלו נמצאת ב-libc, לא בקובץ שלנו.
...
- **T** (אות גדולה) = symbol גלובלי מוגדר בסקשן `.text` - נגיש מקבצים אחרים
- **t** (אות קטנה) = symbol מקומי (static) בסקשן `.text` - נגיש רק בתוך הקובץ
- **D** (אות גדולה) = symbol גלובלי מוגדר בסקשן `.data`
- **d** (אות קטנה) = symbol מקומי (static) בסקשן `.data`

הפונקציה `internal_helper` והמשתנה `local_counter` הם `static`, ולכן symbols מקומיים (אות קטנה). הלינקר לא יחשוף אותם לקבצים אחרים.
...
הsymbols מסוג `U` (undefined) הם: `global_counter`, `increment`, ו-`printf`. הם undefined כי הקובץ `app.c` משתמש בהם אבל לא מגדיר אותם - ההגדרה שלהם נמצאת בקבצים אחרים (utils.o ו-libc).
...
```c
extern int local_counter;  // let's try to access it
```
...
כי `local_counter` הוגדר כ-`static` - הוא symbol מקומי (t/d) ולא נגיש מקבצים אחרים. הלינקר פשוט לא רואה אותו.
...
int main() {
    int x = 3;  // removed the decimal number
    int arr[5];
    arr[4] = 42;  // valid index

    printf("Result: %d\n", calculate(x, 2));
    printf("Value: %d\n", arr[4]);  // added the argument
...
dead_code:
    pushq   %rbp
    movq    %rsp, %rbp
    movl    $7, -4(%rbp)       # x = 3 + 4 = 7 (constant folding even at O0)
    movl    -4(%rbp), %eax
    addl    %eax, %eax         # y = x * 2
    movl    %eax, -8(%rbp)
