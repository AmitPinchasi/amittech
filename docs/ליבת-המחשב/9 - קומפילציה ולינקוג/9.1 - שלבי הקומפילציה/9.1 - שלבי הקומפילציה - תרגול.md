...
5. הריצו `readelf -r stages.o` - מהם הsymbols שצריכים relocation?
...
2. הריצו `nm utils.o` - מהם הsymbols שמופיעים? מה ההבדל בין symbols מסוג `T` לsymbols מסוג `t`? מה ההבדל בין `D` ל-`d`?
3. הריצו `nm app.o` - מהם הsymbols מסוג `U`? למה הם undefined?
...
int dead_code(void) {
    int x = 3 + 4;
    int y = x * 2;
    return 42;  // x and y are not used for anything
}
