...
איך Node.js מטפל באלפי חיבורים עם thread אחד? איך Redis, אחד ממסדי הנתונים המהירים בעולם, עובד בthread יחיד? איך Nginx מגיש אלפי בקשות בו-זמנית?
...
while (true) {
    ...
    for each event:
        handle(event);             // call the appropriate callback
}
```

זה הכל. אין threads מרובים, אין mutex, אין deadlock. הthread אחד שמחכה לאירועים ומטפל בהם אחד אחרי השני.

![תרשים זרימה מונפש של סיבוב בלולאת אירועים, שבו ממתינים לאירוע, מוצאים את הפונקציה המתאימה לפי המתאר, מריצים אותה, וחוזרים להמתנה](../../../images/illustrations/core-12-4-event-loop-cycle.svg)
...
int create_timer(int interval_sec) {
    ...
    struct itimerspec spec = {
        .it_interval = { .tv_sec = interval_sec, .tv_nsec = 0 },  // recurring
        .it_value    = { .tv_sec = interval_sec, .tv_nsec = 0 }   // first
    };
...
void on_timer(int fd, void *data) {
    ...
    read(fd, &expirations, sizeof(expirations));  // must read
    printf("timer fired! (%lu expirations)\n", expirations);
}

// usage with the event loop
int tfd = create_timer(1);  // every second
...
int create_signal_fd(void) {
    ...
    // block the signals so signalfd receives them
...
// (event_loop_t definition and all the functions from above)
...
```c
// hard to follow: the logic is scattered
void on_request_read(int fd, void *data) {
    // read the request...
    // when done, register a callback for processing
}
void on_request_processed(int fd, void *data) {
    // process...
    // when done, register a callback for writing
}
void on_response_written(int fd, void *data) {
    // written...
    // go back to reading a new request
}
```
...
void handle_connection(int fd, void *data) {
    ...
    case STATE_READING_REQUEST: {
        ...
        // check if we received a complete request
        if (strstr(conn->read_buf, "\r\n\r\n")) {
            conn->state = STATE_PROCESSING;
            // process the request (directly, since it's fast)
    ...
    case STATE_WRITING_RESPONSE: {
        ...
        if (conn->write_pos >= conn->write_len) {
            // finished writing, go back to reading
...
![תרשים זרימה מונפש של מכונת מצבים לחיבור בלולאת אירועים, שעוברת ממצב קריאה לעיבוד ואז לכתיבה וחוזרת לקרוא את הבקשה הבאה](../../../images/illustrations/core-12-4-connection-state-machine.svg)
...
```c
// split across 3 different callbacks, hard to follow
```
...
```
// pseudo-code with coroutines
async function handle_client(fd):
    request = await read(fd)       // "pause" until there's data
    response = process(request)
    await write(fd, response)      // "pause" until the write completes
```
...
שימו לב: שרת אחד, thread אחד, אפס mutex-ים, ומטפל בהרבה לקוחות במקביל. זו העוצמה של event loop.

---

## סיכום

| גישה | הthreads | מורכבות | ביצועים | שימושים |
|---|---|---|---|---|
| הthread לכל חיבור | רבים | נמוכה | בינוניים | שרתים פשוטים |
