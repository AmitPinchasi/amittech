...
int main(void) {
    ...
    // send a poison pill to each consumer
    for (int i = 0; i < NUM_CONSUMERS; i++) {
...
### פתרון לתרגיל 3 - cache עם reader-writer lock
...
void *worker(void *arg) {
    ...
    for (int phase = 0; phase < NUM_PHASES; phase++) {
        ...
        // wait for everyone to finish before swapping arrays
...
int main(void) {
    ...
    // after NUM_PHASES iterations, the result will be in array_a (if NUM_PHASES is even)
...
typedef struct {
    int remaining;   // how many more can pass
    pthread_mutex_t mutex;
...
// blocks until the gate is open, then passes
...
int main(void) {
    ...
    // create 10 threads waiting at the gate
    for (int i = 0; i < 10; i++) {
        ids[i] = i;
        pthread_create(&threads[i], NULL, thread_func, &ids[i]);
    }

    sleep(1);  // give all the threads time to reach the gate
