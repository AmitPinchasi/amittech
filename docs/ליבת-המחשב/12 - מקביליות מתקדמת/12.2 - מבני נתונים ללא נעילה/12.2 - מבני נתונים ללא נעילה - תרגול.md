...
צרו 8 threads שכל אחד מגדיל את המונה 100,000 פעמים. בסיום, הערך צריך להיות בדיוק 800,000.
...
ממשו את מחסנית Treiber המלאה (push ו-pop) ובדקו אותה עם מספר threads.

צרו 4 threads שכל אחד עושה push של 10,000 מספרים, ובו-זמנית 4 threads שעושים pop.

דרישות:
- ממשו את `stack_push` ו-`stack_pop` עם CAS.
- ספרו כמה פריטים כל thread pop הוציא.
...
ממשו queue SPSC מבוסס ring buffer עם C11 atomics.
...
- אם הqueue מלא, היצרן עושה spin-wait (נסה שוב).
- אם הqueue ריק, הצרכן עושה spin-wait.
...
התרחיש: 4 threads, כל אחד עושה 100,000 פעולות push ואז 100,000 פעולות pop.
...
2. הthread 1 מתחיל pop - קורא `top = C`, `next = B`, ואז נעצר (סמלצו עם `usleep`).
3. הthread 2 עושה pop של C, pop של B, ואז push של C בחזרה.
4. הthread 1 ממשיך - ה-CAS מצליח כי `top` עדיין C, אבל `next` (שהוא B) כבר שוחרר.
