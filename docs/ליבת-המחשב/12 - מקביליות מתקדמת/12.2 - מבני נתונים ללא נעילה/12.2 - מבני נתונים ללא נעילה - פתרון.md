...
void counter_increment(atomic_counter_t *c) {
    ...
    while (!atomic_compare_exchange_weak(&c->value, &old, old + 1)) {
        // old is automatically updated to the current value, try again
    }
}
...
int stack_pop(treiber_stack_t *s, int *value) {
    ...
    *value = old_top->data;
    // note: in a real program, free here is dangerous because of ABA
    // for the purpose of this exercise it's fine because our pop threads don't interleave with push
    free(old_top);
    return 1;
}
...
void *pop_thread(void *arg) {
    ...
    // try to pop until we succeed OPS_PER_THREAD times
...
int main(void) {
    ...
    // run the push threads first so there's something to read
...
#define QUEUE_SIZE 4096  // a power of 2
...
int main(void) {
    ...
    /*
     * note: the results depend heavily on the chip and the number of cores.
     * with high contention (many threads), lock-free usually wins.
     * with low contention (few threads), mutex is sometimes faster
     * because futex on the fast path is a single atomic operation in user-space.
     * the conclusion: always measure before deciding.
     */
...
// thread 1: tries to pop, but stops in the middle
void *thread1(void *arg) {
    (void)arg;

    // step 1: read top and next
    ...
    // the CAS will succeed because top still points to C (after thread2 put it back)
    // but new_top (=B) is no longer part of the stack!
...
// thread 2: pops C, pops B, pushes C - causes ABA
void *thread2(void *arg) {
    (void)arg;

    usleep(10000);  // give thread1 time to read top first
...
int main(void) {
    // build the stack: top -> C -> B -> A -> NULL
    node_A.name = 'A';
    node_A.next = NULL;
    node_B.name = 'B';
    node_B.next = &node_A;
    ...
    /*
     * the solution to the ABA problem: tagged pointers.
     * every time top changes, a counter (tag) is incremented.
     * the CAS checks both the pointer and the counter.
     * even if the pointer returns to the same value (C), the counter is different -> CAS fails.
     *
     * implementation:
     * typedef struct {
     *     node_t *ptr;
     ...
     * } tagged_ptr_t;
     *
     * in a CAS on 128 bits (cmpxchg16b on x86_64), you need to check
     * both ptr and tag. if tag changed - CAS fails, and the problem is avoided.
     */
