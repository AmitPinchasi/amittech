...
```c
// every call = syscall = context switch
...
```c
// syscall 1: wait for events
int nfds = epoll_wait(epfd, events, MAX_EVENTS, -1);
for (int i = 0; i < nfds; i++) {
    // syscall 2: read/write
...
1. קבל SQE פנוי מqueue ההגשה.
2. מלא את פרטי הפעולה (opcode, fd, buffer, ...).
3. שלח (submit) - אפשר עם `io_uring_enter()` syscall, או בלי syscall כלל במצב SQPOLL.
4. חכה להשלמה (או בדוק אם יש) - קרא מqueue ההשלמה.
5. עבד את התוצאה.

![איור מונפש של מחזור בקשה בio_uring, שבו הבקשה נכתבת לטבעת ההגשה המשותפת והתוצאה מוחזרת מהקרנל דרך טבעת ההשלמה](../../../images/illustrations/core-12-3-sq-cq-rings.svg)
...
// initialization - 256 = the queue size
...
// tag with user_data for identification
io_uring_sqe_set_data(sqe, my_context);

// submit all the accumulated requests
...
// process the result
int result = cqe->res;                          // return value
void *context = io_uring_cqe_get_data(cqe);     // our user_data
...
// event types - we'll store in user_data
enum event_type {
    EVENT_ACCEPT,
    EVENT_READ,
    EVENT_WRITE
...
![איור מונפש שמשווה את מספר קריאות המערכת לכל פעולת קלט פלט בקריאה חוסמת, בגישת epoll, בio_uring, ובמצב SQPOLL](../../../images/illustrations/core-12-3-syscall-count.svg)
...
במצב SQPOLL, הקרנל מריץ thread קרנל שסורק את ה-SQ באופן קבוע (polling). ברגע שהמשתמש כותב בקשה ל-SQ, thread הקרנל מזהה אותה ומבצע - בלי שום syscall מצד המשתמש.

```c
struct io_uring_params params = {0};
params.flags = IORING_SETUP_SQPOLL;
params.sq_thread_idle = 2000;  // the kernel thread sleeps after 2 seconds of inactivity
...
```c
// read from the file, then write to the socket
struct io_uring_sqe *sqe1 = io_uring_get_sqe(&ring);
io_uring_prep_read(sqe1, file_fd, buf, size, 0);
sqe1->flags |= IOSQE_IO_LINK;  // chain to the next operation

struct io_uring_sqe *sqe2 = io_uring_get_sqe(&ring);
io_uring_prep_send(sqe2, socket_fd, buf, size, 0);

io_uring_submit(&ring);
// the read from the file will happen first, and only after it the write to the socket
```
