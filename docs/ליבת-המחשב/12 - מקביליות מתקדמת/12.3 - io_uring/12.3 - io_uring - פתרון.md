...
int main(void) {
    ...
    // submit 10 read requests at once
    for (int i = 0; i < NUM_FILES; i++) {
        struct io_uring_sqe *sqe = io_uring_get_sqe(&ring);
        io_uring_prep_read(sqe, fds[i], buffers[i], BUF_SIZE - 1, 0);
        io_uring_sqe_set_data64(sqe, i);  // identify by index
    }
...
int main(int argc, char *argv[]) {
    ...
    while (block_idx < total_blocks || in_flight > 0) {
        ...
        // each read+write pair produces 2 CQEs
        // count only the write (odd) as "done"
