...
```c
int result = add(3, 5); // result will equal 8
```
...
```c
#include <stdio.h>
#include "math_utils.h" // note: double quotes for our own file
...
![תרשים המראה קימפול של פרויקט מכמה קבצים: קובץ ה-header מצהיר על הפונקציה, כל קובץ מקור מקומפל לקובץ אובייקט בנפרד, והלינקר מחבר את הכל לקובץ הרצה אחד](../../../images/illustrations/core-3-5-multifile-link.svg)
