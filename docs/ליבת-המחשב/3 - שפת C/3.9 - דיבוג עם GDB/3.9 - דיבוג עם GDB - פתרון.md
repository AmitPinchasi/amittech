...
(gdb) next
(gdb) print numbers
(gdb) x/5dw &numbers       # display 5 decimal values from the array
(gdb) step                  # step into sum_array
(gdb) info locals           # see total and i
(gdb) next
(gdb) next
(gdb) print total           # see intermediate value
(gdb) info registers        # see all the registers
(gdb) x/20xb $rbp           # looking at the stack frame
(gdb) disass sum_array      # see the assembly
(gdb) finish                # exit the function
(gdb) print result          # see the result
(gdb) continue
```
...
(gdb) print index
$1 = 10
(gdb) next                  # numbers[index] = 42;  -> the crash happens here
```
...
(gdb) print index           # see that it's 10
(gdb) x/5dw &numbers       # see that the array only has 5 elements
```
...
(gdb) display i
(gdb) display sum
(gdb) next                  # stepping through variable definitions
(gdb) next
(gdb) next                  # entering the loop
(gdb) next                  # i = 0, sum = 1
(gdb) next                  # i = 1, sum = 3
(gdb) next                  # i = 2, sum = 6
(gdb) next                  # i = 3, sum = 10
(gdb) next                  # i = 4, sum = 15
(gdb) next                  # i = 5, sum = ??? garbage value!
(gdb) print numbers[5]      # undefined value - out of bounds of the array
```
