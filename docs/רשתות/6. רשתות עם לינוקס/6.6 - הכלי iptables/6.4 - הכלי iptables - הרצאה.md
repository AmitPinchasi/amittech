...
הכלי **iptables** הוא כלי לבקרת תעבורת רשת במערכות **Linux**. הוא משמש ליצירת חוקים שמגדירים כיצד להתמודד עם packets רשת נכנסות, יוצאות ועוברות דרך המערכת. `iptables` עובד עם ה-Kernel (הגרעין של מערכת ההפעלה) של לינוקס ומשתמש במנגנון ה-NETFILTER כדי לסנן packets ולבצע פעולות שונות כמו **חסימה, routing, הפניית פורטים והגבלת חיבורים**.
...
1. **INPUT** – מטפלת בpackets נכנסות למחשב.
2. **OUTPUT** – מטפלת בpackets שיוצאות מהמחשב.
3. **FORWARD** – מטפלת בpackets שעוברות דרך המחשב (למשל, כאשר המחשב משמש כנתב).

![תרשים זרימה שמראה כיצד פאקטה עוברת בשרשראות netfilter: אחרי PREROUTING מתקבלת החלטת ניתוב, פאקטה ליעד מקומי ממשיכה ל-INPUT ואילו פאקטה שעוברת דרך המחשב ממשיכה ל-FORWARD ואז ל-POSTROUTING ביציאה](../../../images/illustrations/net-6-6-netfilter-chains.svg)
...
|**mangle**|שינוי שדות בpackets כמו TTL, QoS ועוד.|
...
- `-L` – מציג את החוקים.
- `-v` – מפרט מידע נוסף כמו מספר packets ובייטים.
...
- חוסם packets נכנסות וpackets שעוברות דרך המחשב, אך מאפשר packets יוצאות.
...
- `-s 192.168.1.100` – מתייחס לpackets שמגיעות מהכתובת הזו.
- `-j DROP` – מבצע חסימה (משליך את הpackets).
...
![תרשים זרימה שמראה כיצד iptables בודק פאקטה מול חוקי השרשרת לפי הסדר, והחוק הראשון שמתאים הוא זה שקובע: פאקטה לפורט 80 חולפת על פני חוקי החסימה ומתקבלת בזכות חוק ACCEPT](../../../images/illustrations/net-6-6-chain-rule-walk.svg)
...
## **חוקי Mangle (שינוי packets)**

### **הורדת TTL לpackets יוצאות**

```sh
iptables -t mangle -A POSTROUTING -j TTL --ttl-set 64
```

- משנה את ערך TTL ל-64 (מונע routing נוסף).
...
- חוסם packets עם דגלים חריגים (ניסיון סריקה).
...
- יוצר שרשרת `MYCHAIN` ומנתב packets דרכה.
