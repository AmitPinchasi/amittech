...
לקוח FTP הנפוץ ביותר עם GUI (interface גרפי) הוא **FileZilla**. הוא מאפשר לנו להעלות ולהוריד קבצים בצורה מאוד אינטואיטיבית.
...
ftp = FTP('ftp.dlptest.com')  # The server's address
ftp.login()  # Log in without a username and password
...
**RDP** הוא פרוטוקול של מיקרוסופט שמאפשר לנו להתחבר למחשב מרוחק ולקבל גישה לinterface הגרפי (GUI) של המחשב. עם RDP, אנחנו לא רק שולחים פקודות כמו ב-Telnet, אלא אנחנו למעשה "נכנסים" למחשב המרוחק ומבצעים כל פעולה כאילו היינו יושבים מולו.
...
לאחר מכן, תוכל להזין את כתובת ה-IP של המחשב המרוחק ולקבל גישה לinterface הגרפי שלו.
