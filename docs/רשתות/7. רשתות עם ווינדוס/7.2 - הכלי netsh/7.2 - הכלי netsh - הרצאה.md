...
הכלי **Netsh** (Network Shell) הוא כלי שורת פקודה עוצמתי של מערכת ההפעלה Windows, המאפשר ניהול והגדרה של רכיבי רשת שונים, כולל חומת אש (fire wall), פרופילי רשת, network interfaces, Wi-Fi ועוד. באמצעות Netsh ניתן לבצע פעולות מתקדמות כמו הגדרת חוקים ב-Firewall, קביעת כתובות IP, ניהול חיבורי Wi-Fi וגיבוי הגדרות רשת.

![איור המציג את חלוקת הכלי netsh להקשרים נפרדים: interface לניהול כתובות ו-network interfaces, advfirewall לחומת האש, wlan לרשתות Wi-Fi ו-winhttp להגדרות Proxy](../../../images/illustrations/net-7-2-netsh-contexts.svg)
...
### 1. ניהול network interfaces (Network Interfaces)

#### הצגת רשימת network interfaces:

```cmd
netsh interface show interface
```

#### שינוי שם של network interface:

```cmd
netsh interface set interface name="Ethernet" newname="MyNetwork"
```

#### הפעלת/כיבוי network interface:
...
הכלי Netsh הוא כלי עוצמתי שמאפשר ניהול מתקדם של רכיבי רשת, חומת אש, Wi-Fi והגדרות Proxy ב-Windows. ניתן להשתמש בו לצורך בדיקות רשת, אבטחת מחשב והגדרת תצורה מתקדמת של network interfaces. השימוש בו דורש הרשאות מנהל ומאפשר שליטה מלאה על התקשורת במערכת.