...
1. ספרייה נותנת כלים ספציפיים ואנחנו קוראים לה (אנחנו שולטים בזרימה). framework נותן מבנה שלם ומכתיב את הארכיטקטורה (ה-framework שולט בזרימה וקורא לקוד שלנו). ריאקט היא ספרייה - היא מטפלת רק ב-UI ומשאירה לנו את הבחירה בנושאים כמו routing וניהול סטייט.
