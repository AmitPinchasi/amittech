...
שימו לב לשימוש ב-`onBlur` כדי לסמן שהמשתמש ביקר בשדה, ול-early return לstate ההתחברות.
...
2. SyntheticEvent הוא אובייקט event של ריאקט שעוטף את event הדפדפן המקורי. הוא מספק interface אחיד בין דפדפנים ומאפשר לריאקט לנהל events ביעילות. אפשר לגשת לevent המקורי דרך `e.nativeEvent`.
