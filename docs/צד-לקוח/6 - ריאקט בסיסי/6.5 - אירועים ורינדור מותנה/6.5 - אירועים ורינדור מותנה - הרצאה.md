...
## הevents בריאקט - Events

ריאקט מטפלת בevents בצורה דומה ל-DOM, אבל עם כמה הבדלים:

- שמות events ב-**camelCase**: `onClick`, `onChange`, `onSubmit` (לא `onclick`, `onchange`)
- מעבירים **פונקציה** כ-handler, לא מחרוזת: `onClick={handleClick}` (לא `onclick="handleClick()"`)
- הevent שמתקבל הוא **SyntheticEvent** של ריאקט - עוטף את הevent המקורי של הדפדפן
...
#### אובייקט הevent - Event Object

ה-handler מקבל אובייקט event עם מידע שימושי:
...
זה שימושי במיוחד לstates טעינה (loading), שגיאות (error), וחוסר נתונים (empty state).
...
זה טוב כשיש יותר משני states - ternary מקונן הופך ללא קריא.
...
## שילוב - events עם רינדור מותנה

דוגמה שמשלבת events ורינדור מותנה:
...
- הevents בריאקט משתמשים ב-camelCase ומקבלים פונקציה (לא מחרוזת)
