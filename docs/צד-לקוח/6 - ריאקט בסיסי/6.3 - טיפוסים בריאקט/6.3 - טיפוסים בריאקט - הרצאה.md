...
## טיפוסי events - Event Types

כשמטפלים בevents בריאקט, כל event מגיע עם טיפוס מתאים:
...
הטיפוס הגנרי (כמו `HTMLButtonElement`) מציין את סוג האלמנט שעליו הevent מתרחש. זה נותן גישה מטויפסת ל-properties הספציפיים של האלמנט.
...
- לevents יש טיפוסים ייעודיים כמו `React.MouseEvent`, `React.ChangeEvent`
