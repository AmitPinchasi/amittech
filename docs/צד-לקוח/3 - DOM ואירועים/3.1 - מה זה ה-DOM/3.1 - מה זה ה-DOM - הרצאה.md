...
- כל tag HTML הופכת ל-node (צומת) בעץ
...
זה הסוג הכי נפוץ. כל tag HTML הופכת לצומת אלמנט:
...
זה השורש של כל העץ. הוא לא מייצג tag HTML, אלא את המסמך עצמו.
...
- אפשר ללחוץ על כל אלמנט ולראות את הproperties שלו
...
הדפדפן הוסיף `<html>`, `<head>`, `<body>` וסגר את tags ה-`<p>`.
...
כשהדפדפן טוען דף, זה לא קורה ברגע אחד. יש שני events חשובים:

#### הevent DOMContentLoaded
...
![תרשים של סדר טעינת דף: בניית עץ ה-DOM, אירוע DOMContentLoaded, טעינת המשאבים ולבסוף אירוע load](../../../images/illustrations/client-3-1-page-load-events.svg)
...
- הattribute `defer` אומרת לדפדפן: "תוריד את הקובץ עכשיו, אבל תריץ אותו רק אחרי שה-DOM מוכן"
