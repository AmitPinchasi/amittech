...
- `textContent` מחזיר את כל הטקסט בלי tags HTML
...
4. **textContent מול innerHTML** - `textContent` מחזיר ומגדיר טקסט רגיל בלי HTML. `innerHTML` מחזיר ומגדיר HTML כמחרוזת, כולל tags. `textContent` בטוח מפני התקפות XSS, ו-`innerHTML` עלול להיות מסוכן עם קלט ממשתמשים.

5. **closest מול matches** - `closest` מחפש הורה (כולל האלמנט עצמו) שמתאים לסלקטור, ומחזיר אלמנט או null. `matches` רק בודק אם האלמנט עצמו מתאים לסלקטור, ומחזיר true/false. `closest` שימושי בטיפול בevents כשרוצים למצוא הורה מסוים, ו-`matches` שימושי לסינון.
