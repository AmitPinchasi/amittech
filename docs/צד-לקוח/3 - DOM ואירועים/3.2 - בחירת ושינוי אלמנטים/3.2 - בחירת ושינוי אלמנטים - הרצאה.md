...
- שימושי מאוד כשעובדים עם events ורוצים למצוא הורה מסוים
...
אפשר לשנות CSS ישירות על אלמנט דרך הproperty `style`:
...
![אנימציה שמראה איך קריאות classList משנות את רשימת המחלקות של האלמנט: add מוסיף מחלקה ו-toggle מוסיף ומסיר לסירוגין](../../../images/illustrations/client-3-2-classlist-state.svg)
...
אפשר לקרוא ולשנות attributes HTML:
...
לרוב הattributes אפשר לגשת ישירות כ-property של האלמנט:
...
הattributes `data-*` מאפשרות לנו לשמור מידע מותאם אישית על אלמנטים:
...
- `getAttribute` לקריאת attributes
...
- `element.dataset` - attributes data-*
- גישה ישירה: `element.id`, `element.className`, `element.src`, `element.href`
- הstate: `element.value`, `element.checked`, `element.disabled`, `element.hidden`
