...
כשלוחצים על כפתור submit בטופס (או לוחצים Enter בתוך input), הדפדפן מפעיל את event ה-submit על אלמנט ה-form.
...
- הevent submit מופעל על אלמנט ה-form, לא על הכפתור
...
![תרשים זרימה של אירוע submit: עם preventDefault הנתונים מטופלים ב-JavaScript, ובלעדיו הדפדפן שולח את הטופס והדף נטען מחדש](../../../images/illustrations/client-3-6-submit-preventdefault.svg)
...
## הevent input מול event change

שני events חשובים לעבודה עם שדות טופס:
...
הevent input מופעל בכל פעם שהערך משתנה - כל הקשה, כל הדבקה, כל מחיקה.
...
הevent change מופעל רק כשהמשתמש עוזב את השדה (blur) - ורק אם הערך באמת השתנה.
...
![אנימציה של ולידציה בזמן אמת: על כל הקשה נבדק אורך הערך, והשדה מסומן כשגוי עד שהקלט נעשה תקין](../../../images/illustrations/client-3-6-live-validation.svg)
...
- הevent submit מופעל על ה-form - חובה `event.preventDefault()` כדי לטפל ב-JS
...
- הevent `input` מופעל על כל שינוי; event `change` מופעל כשעוזבים את השדה
