...
- הevent `input` מתעדכן על כל הקשה
...
- כל שדה מקבל ולידציה בזמן אמת על event `input`
...
- בשמירה מעדכנים גם את ה-originalProfile כך שהאיפוס יחזיר לstate שנשמר
...
- חשוב לנקות את ה-customValidity על event `input`, אחרת השדה תמיד ייחשב לא תקין
