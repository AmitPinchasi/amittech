...
בנו אפליקציית רשימת משימות מלאה עם interface מעוצב ושמירה מקומית.
...
- השתמשו בדלגציית events - handler אחד על ה-ul לכל הלחיצות על משימות
...
בנו מחשבון עם interface גרפי שעובד עם עכבר ומקלדת.
...
- השתמשו בדלגציית events - handler אחד על ה-div של הכפתורים
- כל כפתור עם `data-value` attribute שמציין את הערך שלו
- הevent keydown על document לתמיכת מקלדת
...
- שמרו את state המחשבון במשתנים: currentNumber, previousNumber, operator
...
- השתמשו בדלגציית events לכפתורי התשובות
...
- שמרו את state המשחק במשתנים: currentQuestionIndex, score, answers (מערך של תשובות המשתמש)
...
- השתמשו בדלגציית events - handler אחד על הלוח כולו
