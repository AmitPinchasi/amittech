...
## מודל התפשטות events - Event Propagation

בשיעור הקודם למדנו איך לרשום events על אלמנטים עם addEventListener.
אבל מה בעצם קורה כשלוחצים על כפתור שנמצא בתוך div, שנמצא בתוך body?
האם הevent מופעל רק על הכפתור? התשובה היא לא - והמנגנון הזה הוא אחד הדברים הכי חשובים להבין ב-DOM.

כשevent מתרחש על אלמנט, הוא לא נשאר רק שם. הוא עובר מסלול שלם דרך עץ ה-DOM.
המסלול הזה מורכב משלושה שלבים:

1. **שלב הלכידה - Capturing Phase** - הevent יורד מ-document דרך כל ההורים עד האלמנט שעליו לחצנו
2. **שלב המטרה - Target Phase** - הevent מגיע לאלמנט עצמו (ה-target)
3. **שלב הbubbling - Bubbling Phase** - הevent עולה חזרה מהאלמנט דרך כל ההורים עד document
...
ברוב המוחלט של המקרים, אנחנו עובדים עם שלב הbubbling. שלב הלכידה הוא נדיר ונדבר עליו בהמשך.

---

## הbubbling events - Event Bubbling

הbubbling events אומר שכשevent מתרחש על אלמנט, הוא "bubbles" כלפי מעלה - מהאלמנט, להורה שלו, להורה של ההורה, וכן הלאה עד ה-document.
...
הevent קרה על הכפתור, אבל הוא bubbled למעלה - קודם להורה, ואז לסבא.
...
כי הbubbling מתחיל מהאלמנט שעליו לחצנו ועולה למעלה.

![תרשים שמראה את סדר הבעבוע: ה-handler של הכפתור רץ קודם, אחריו ההורה ולבסוף ההורה של ההורה](../../../images/illustrations/client-3-5-bubbling-order.svg)

---

## event.target מול event.currentTarget

כשevent bubbles, חשוב להבדיל בין שני דברים:

- **event.target** - האלמנט שעליו התרחש הevent במקור (מי שלחצנו עליו)
...
## עצירת הbubbling - event.stopPropagation

לפעמים אנחנו לא רוצים שהevent ימשיך לbubble. אפשר לעצור את ההתפשטות עם `event.stopPropagation()`:
...
עכשיו כשלוחצים על הכפתור, רק "child clicked" יודפס. הevent לא ימשיך לbubble ל-parent.
...
שימוש מוגזם ב-stopPropagation יכול ליצור באגים קשים לאיתור. אם מישהו רשם handler על document ולא מבין למה הוא לא נקרא - זה כי מישהו אחר עצר את הbubbling באמצע הדרך.
...
`event.stopPropagation()` עוצר את הbubbling להורים, אבל handlers נוספים על אותו אלמנט עדיין ירוצו.
...
## דלגציית events - Event Delegation

זה הנושא הכי חשוב בשיעור הזה. דלגציית events היא טכניקה שמנצלת את מנגנון הbubbling כדי לטפל בevents בצורה יעילה.
...
למה זה עובד? בגלל הbubbling. כשלוחצים על li, הevent bubbles ל-ul, ושם ה-handler תופס אותו. אנחנו בודקים את event.target כדי לדעת על מי בדיוק לחצנו.
...
![תרשים של דלגציית אירועים: לחיצה על פריט מבעבעת ל-handler היחיד שעל הרשימה, שמאתר את הפריט עם closest ומטפל בו](../../../images/illustrations/client-3-5-delegation-flow.svg)
...
אמרנו שevent עובר שלושה שלבים. כברירת מחדל, addEventListener רושם handler שמופעל בשלב הbubbling.
אפשר לרשום handler שמופעל בשלב הלכידה - כלומר לפני שהevent מגיע ל-target:
...
- כשרוצים ליירט event לפני שהוא מגיע ליעד (למשל לחסום לחיצות מסוימות ברמת ה-document)
...
- כשevent מתרחש, הוא עובר שלושה שלבים: לכידה (למטה), מטרה, bubbling (למעלה)
- הbubbling events - הevent עולה מהאלמנט דרך כל ההורים עד ה-document
- `event.target` - האלמנט שעליו התרחש הevent; `event.currentTarget` - האלמנט שה-handler רשום עליו
- `event.stopPropagation()` - עוצר את הbubbling; `event.stopImmediatePropagation()` - עוצר גם handlers נוספים על אותו אלמנט
- **דלגציית events** - רושמים handler על ההורה במקום על כל ילד. יעיל, עובד על אלמנטים דינמיים
