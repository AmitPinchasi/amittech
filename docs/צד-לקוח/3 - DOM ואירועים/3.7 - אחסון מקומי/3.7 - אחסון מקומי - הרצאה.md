...
![תרשים שמראה ששמירת אובייקט בלי JSON.stringify מאבדת את המידע, ועם stringify ו-parse האובייקט חוזר שלם](../../../images/illustrations/client-3-7-json-roundtrip.svg)
...
## הevent storage - תקשורת בין טאבים

כשמשנים נתונים ב-localStorage, הדפדפן מפעיל event `storage` בכל הטאבים האחרים (של אותו domain) שפתוחים. זה מאפשר לטאבים לתקשר ביניהם.
...
הevent לא מופעל בטאב שביצע את השינוי - רק בטאבים אחרים.
...
- **הevent storage** - מאפשר לטאבים לתקשר כשנתונים משתנים
