...
- הevent `input` על ה-form מתפשט מכל שדה ילד (bubbling)
...
- `lastMessage` key נפרד משמש להפעלת event storage (כי chatHistory עשוי להיות ארוך)
- `Date.now()` כ-id מבטיח שהערך תמיד משתנה (אחרת storage event לא מופעל אם הערך זהה)
- הevent `storage` מופעל רק בטאבים אחרים, לא בטאב ששינה את הערך
