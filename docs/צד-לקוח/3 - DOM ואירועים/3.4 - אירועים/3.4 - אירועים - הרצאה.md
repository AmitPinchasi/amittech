...
הevent הוא כל דבר שקורה בדף - המשתמש לחץ על כפתור, הזיז את העכבר, הקליד על המקלדת, שינה את גודל החלון, או שהדף סיים להיטען. JavaScript מאפשר לנו "להקשיב" לevents האלה ולהגיב אליהם.

- בלי events, אתרים היו סטטיים ולא אינטראקטיביים
- הevents הם הבסיס לכל אינטראקציה עם המשתמש
- כמעט כל דבר שקורה בדפדפן יוצר event

## addEventListener - הדרך הנכונה

המתודה `addEventListener` מאפשרת לנו להוסיף listener (מאזין) שירוץ כשevent מסוים קורה:
...
- הפרמטר הראשון הוא סוג הevent כמחרוזת
- הפרמטר השני הוא הפונקציה שתרוץ כשהevent קורה (נקראת handler או callback)
- אפשר להוסיף כמה listeners לאותו event על אותו אלמנט

![תרשים זרימה של אירוע: המשתמש לוחץ, הדפדפן יוצר אובייקט event, ה-listener מופעל והפונקציה handler רצה עם האובייקט](../../../images/illustrations/client-3-4-event-flow.svg)
...
כל handler מקבל אובייקט event שמכיל מידע על מה שקרה:
...
#### stopPropagation - עצירת bubbling

כשevent קורה על אלמנט, הוא "מבעבע" (bubbles) למעלה לכל ההורים. אפשר לעצור את זה:
...
נלמד על bubbling לעומק בשיעור הבא.
...
יש דרך ישנה לטפל בevents ישירות ב-HTML:
...
- לא ניתן להוסיף כמה handlers לאותו event
...
- `CustomEvent` מאפשר ליצור events עם מידע מותאם אישית
- `detail` מכיל את המידע שרוצים להעביר עם הevent
- `dispatchEvent` שולח את הevent
...
## סיכום אובייקט הevent

| הproperty/מתודה | תיאור |
|--------------|-------|
| `event.type` | סוג הevent |
| `event.target` | האלמנט שהפעיל את הevent |
| `event.currentTarget` | האלמנט שה-listener רשום עליו |
| `event.preventDefault()` | ביטול התנהגות ברירת מחדל |
| `event.stopPropagation()` | עצירת bubbling |
...
- אובייקט הevent מכיל את כל המידע שצריך על מה שקרה
- `preventDefault` מבטל את ההתנהגות הרגילה (שליחת טופס, ניווט לקישור)
- `stopPropagation` עוצר את bubbling הevent להורים (נלמד בהרחבה בשיעור הבא)
