...
שימו לב: events `focus` ו-`blur` לא bubble (don't bubble), אז כדי לתפוס אותם על ה-container צריך להשתמש ב-capture phase (פרמטר שלישי `true`) או להשתמש ב-`focusin`/`focusout` שכן bubble.

#### תשובות לשאלות

1. **event.target מול event.currentTarget** - `event.target` הוא האלמנט שבאמת הפעיל את הevent (מה שהמשתמש לחץ עליו). `event.currentTarget` הוא האלמנט שה-listener רשום עליו. הם שונים כשהevent bubbled מאלמנט ילד.

2. **event.preventDefault()** מבטל את ההתנהגות הרגילה של הדפדפן. למשל: מניעת שליחת טופס (ורענון דף), מניעת ניווט בלחיצה על קישור, מניעת תפריט קליק ימני. משתמשים בו כשרוצים לטפל בevent בעצמנו במקום לתת לדפדפן לעשות את ברירת המחדל.

3. **addEventListener עדיף על onclick attribute** כי: אפשר להוסיף כמה handlers לאותו event, יש הפרדה בין HTML ל-JS, אין בעיות אבטחה של CSP, ואפשר להסיר listeners ספציפיים. onclick attribute מאפשר רק handler אחד ומערבב HTML עם JS.
...
6. **once: true** גורם ל-listener לרוץ פעם אחת בלבד. אחרי שהevent מתרחש ו-handler רץ, הוא מוסר אוטומטית. שימושי לevents חד-פעמיים כמו אנימציית כניסה, לחיצה ראשונה, או טעינה ראשונית.
