...
דרישות:
1. Username - לפחות 3 תווים, ולידציה בevent `input`
2. Email - חייב להכיל @ ונקודה, ולידציה בevent `blur`
3. Password - לפחות 8 תווים, לפחות אות גדולה אחת ומספר אחד, ולידציה בevent `input`
4. Confirm Password - חייב להיות זהה ל-Password, ולידציה בevent `input`
...
5. הוסיפו אפשרות לסנן את הפריטים לפי טקסט - שדה input שמסתיר פריטים שלא מתאימים (event `input`)
...
השתמשו בevents mousedown, mousemove ו-mouseup כדי לממש גרירה:
...
צרו "לוג events" שמציג בזמן אמת כל event שקורה בדף:
...
1. האזינו ל-test-area לevents הבאים: click, dblclick, mouseenter, mouseleave, keydown, focus, blur, input
2. לכל event, הוסיפו שורה ל-event-log שמציגה: זמן, סוג הevent, ה-target, וערך (אם יש)
3. צבעו כל סוג event בצבע אחר
...
4. מה ההבדל בין event `input` לevent `change`?
