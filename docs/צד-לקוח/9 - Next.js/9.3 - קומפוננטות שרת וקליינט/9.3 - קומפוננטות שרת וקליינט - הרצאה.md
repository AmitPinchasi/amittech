...
export default async function UsersPage() {
  // can read directly from the database
  const users = await db.user.findMany();

  // can access sensitive environment variables
  const apiKey = process.env.SECRET_API_KEY;

  // console.log will appear in the server's terminal, not in the browser
...
- להאזין לevents (onClick, onChange וכו')
...
![תרשים שמראה איך ההנחיה use client קובעת את הגבול בין קוד שנשאר בשרת ונשלח כ-HTML בלבד לבין קוד שנארז ונשלח לדפדפן ועובר hydration](../../../images/illustrations/client-9-3-use-client-boundary.svg)

#### מה אפשר לעשות ב-Client Components

- להשתמש ב-hooks (useState, useEffect, useRef, useContext)
- להאזין לevents (onClick, onChange, onSubmit)
- להשתמש ב-Browser APIs (window, document, localStorage)
...
export default function Page() {
  ...
  return (
    <ClientComponent
      ...
      instance={myClass}     // classes - not serializable
      data={myMap}           // Map/Set - not serializable
    />
  );
}
```
...
![תרשים זרימה שבודק כל prop שעובר מקומפוננטת שרת לקומפוננטת קליינט ומראה שערכים פשוטים ותאריכים עוברים כ-JSON בעוד שפונקציה גורמת לשגיאה](../../../images/illustrations/client-9-3-props-serialization.svg)
...
```tsx
// Server Component - this code is not sent to the browser
...
```tsx
// Server Component - direct access to the database
...
```tsx
// Server Component - the HTML is ready with all the content
...
```tsx
// this will fail! useState is not available in a Server Component
import { useState } from "react";
...
```tsx
// not recommended - you lose the benefits of Server Components
"use client";

export default function Page() {
  // there are no hooks or events here - no reason for this to be a Client Component
  return <h1>שלום</h1>;
}
```
...
import ServerComponent from "./ServerComponent"; // this automatically becomes a Client Component!

export default function ClientWrapper() {
  return <ServerComponent />; // will not run on the server
}
```
...
export default function Layout() {
  return (
    <ClientWrapper>
      <ServerComponent /> {/* this will still run on the server */}
    </ClientWrapper>
  );
}
```
...
- Client Components נדרשים ל-hooks, events ו-Browser APIs
