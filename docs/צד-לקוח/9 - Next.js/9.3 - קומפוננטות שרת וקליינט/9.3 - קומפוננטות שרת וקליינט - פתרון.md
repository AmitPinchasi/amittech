...
5. אקורדיון - **Client Component** - צריך useState למעקב אחרי state פתוח/סגור ו-onClick
...
```tsx
// The solution: pass the data and define the function in a Client Component
// Server Component
...
```tsx
// The solution: convert to a regular object
export default function Page() {
...
2. **למה Client Components עוברים SSR**: גם Client Components מרונדרים תחילה בשרת כ-HTML כדי שהמשתמש יראה תוכן מיד. אחר כך מתבצע hydration - React מחבר את הevents וה-state ל-HTML הקיים. זה שונה מ-Server Components שרצים רק בשרת.
