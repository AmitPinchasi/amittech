...
# variables available in the browser - must start with NEXT_PUBLIC_
...
![תרשים שמראה איך נקסט מחליט בזמן הבנייה אם משתנה סביבה נשאר בשרת בלבד או מוטמע בקוד ונשלח לדפדפן לפי הקידומת של השם](../../../images/illustrations/client-9-9-env-public.svg)

#### קבצי סביבה

```
.env                # default for all environments
.env.local          # local - overrides .env
...
```tsx
// Server Component - access to regular variables
