...
// Server variables - not available in the browser
...
export default function Page() {
  const apiKey = serverEnv.API_KEY; // works
...
export const logger = {
  ...
  error(message: string, error?: any) {
    ...
    // Here you could send to Sentry or a similar service
  },
};
```
...
4. **global-error.tsx לעומת error.tsx**: `error.tsx` תופס שגיאות בתוך ה-layout - הלייאאוט עצמו עדיין מוצג. `global-error.tsx` תופס שגיאות בלייאאוט הראשי עצמו - כשהלייאאוט נשבר. global-error חייב להכיל tags html ו-body כי הלייאאוט לא זמין.
