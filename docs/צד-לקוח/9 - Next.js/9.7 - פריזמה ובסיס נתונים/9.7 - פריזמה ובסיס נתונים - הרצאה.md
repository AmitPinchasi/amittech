...
![תרשים שמראה את הדרך שעושה שאילתת פריזמה מקוד TypeScript דרך התרגום לשאילתת SQL ופנייה לבסיס הנתונים ועד לתוצאה שחוזרת כאובייקטים עם טיפוסים](../../../images/illustrations/client-9-7-orm-query-flow.svg)
...
- מיגרציות אוטומטיות
- הinterface גרפי לניהול הנתונים (Prisma Studio)
...
# generate Prisma Client without a migration
npx prisma generate

# direct schema sync (development only)
npx prisma db push
...
- `prisma studio` פותח interface ווב בפורט 5555 לצפייה ועריכת נתונים
...
// single record by a unique field
const user = await prisma.user.findUnique({
...
export async function createPost(formData: FormData) {
  ...
  await prisma.post.create({
    data: {
      ...
      content,
      slug,
      authorId: 1, // will be replaced by authentication
    },
  });
...
הinterface גרפי לצפייה ועריכת הנתונים:
...
- הסכמה מוגדרת בקובץ `schema.prisma` עם models, שדות וקשרים
...
- Prisma Studio מספק interface גרפי לניהול הנתונים
