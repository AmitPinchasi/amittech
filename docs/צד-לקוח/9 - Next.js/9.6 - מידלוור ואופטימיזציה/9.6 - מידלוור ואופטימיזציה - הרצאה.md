...
```tsx
// middleware.ts (at the project's root level)
...
export function middleware(request: NextRequest) {
  ...
  // Rewrite - the URL doesn't change in the browser
...
![תרשים שמשווה בין redirect שגורם לדפדפן לפנות שוב לכתובת חדשה ולעדכן את שורת הכתובת לבין rewrite שמגיש נתיב אחר באותה בקשה בלי לשנות את הכתובת](../../../images/illustrations/client-9-6-redirect-vs-rewrite.svg)
...
```tsx
// Image that needs to load immediately (above the fold)
<Image
  src="/hero.jpg"
  alt="כותרת"
  width={1200}
...
export const metadata: Metadata = {
  title: "אודות", // Produces: "אודות | האתר שלי"
