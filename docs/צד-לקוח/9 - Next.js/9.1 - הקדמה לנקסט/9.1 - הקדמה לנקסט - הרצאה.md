...
- ריאקט לבדו הוא ספרייה לבניית interface משתמש - לא פריימוורק שלם
...
```tsx
// This is how CSR works - the browser receives almost empty HTML
// <div id="root"></div>
// then React builds all the content in JS
function App() {
...
```
Request from browser ──> Server runs the code ──> Ready HTML is sent ──> Browser displays immediately
```

![תרשים המשווה בין מסלול הרינדור בצד הלקוח שבו הדפדפן מקבל HTML ריק ובונה את הדף בעצמו לבין מסלול הרינדור בצד השרת שבו מגיע HTML מלא והמשתמש רואה תוכן מיד](../../../images/illustrations/client-9-1-csr-vs-ssr.svg)
...
```tsx
// ISR example - the page is revalidated every 60 seconds
async function Page() {
...
![תרשים זרימה של דף ISR מהיצירה בזמן ה-build דרך בקשות שמקבלות את הדף מהקאש ועד לבקשה שבה פג הזמן והדף נבנה מחדש ברקע](../../../images/illustrations/client-9-1-isr-revalidate.svg)
...
- הלייאאוט הראשי חייב להכיל tags `<html>` ו-`<body>`
...
export default function Page() {
  console.log("This will run on the server, not in the browser!");
