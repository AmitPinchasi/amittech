...
```
User ──> Authentication (login) ──> Authorization (what's allowed?) ──> Access to resource
```
...
![תרשים זרימה של התחברות עם אימייל וסיסמה מהחיפוש של המשתמש בבסיס הנתונים דרך השוואת ה-hash ועד ליצירת טוקן הסשן ושמירת העוגייה בדפדפן](../../../images/illustrations/client-9-8-credentials-login.svg)
...
export default auth((request) => {
  ...
  // Logged-in user trying to access the login page
...
![תרשים זרימה של שרשרת הבדיקות לפני כניסה לפאנל הניהול שבו בקשה בלי סשן מופנית להתחברות ומשתמש ללא תפקיד מנהל מופנה לדשבורד](../../../images/illustrations/client-9-8-auth-guard.svg)
