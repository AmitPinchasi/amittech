...
![עץ החלטה לבחירת אסטרטגיית הקאשינג של fetch לפי השאלה אם הנתונים משתנים כל רגע ואם מספיק לרענן אותם מדי פעם](../../../images/illustrations/client-9-4-fetch-cache-choice.svg)
...
![תרשים זרימה של Server Action משליחת הטופס בדפדפן דרך הרצת הפונקציה בשרת והכתיבה לבסיס הנתונים ועד רענון הקאש והצגת הרשימה המעודכנת](../../../images/illustrations/client-9-4-server-action-flow.svg)
...
export async function updatePost(id: number, data: { title: string }) {
  ...
  // Refresh the cache of the specific route
  revalidatePath("/posts");

  // You can also refresh a specific dynamic route
  revalidatePath(`/posts/${id}`);

  // Or refresh the entire layout
...
## הinterface טעינה וסטרימינג - Loading UI and Streaming
...
![תרשים שמראה סטרימינג של דשבורד שבו שלד הדף נשלח מיד וכל חלק שעטוף ב-Suspense מגיע לדפדפן בנפרד ברגע שסיים לשלוף את הנתונים](../../../images/illustrations/client-9-4-suspense-streaming.svg)
...
```tsx
// Usage on the client with error handling
"use client";
