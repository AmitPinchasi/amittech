    ...
    start(controller) {
      ...
      const notificationInterval = setInterval(() => {
        const notification = {
          ...
          message: `Update at ${new Date().toLocaleTimeString('he-IL')}`,
...
function ChatWithRooms({ username, serverUrl }: { username: string; serverUrl: string }) {
  ...
  useEffect(() => {
    ...
    s.on('room:user-joined', ({ room, user }: { room: string; user: string }) => {
      setSystemMessages((prev) => [...prev, `${user} joined room ${room}`]);
    });

    s.on('room:user-left', ({ room, user }: { room: string; user: string }) => {
      setSystemMessages((prev) => [...prev, `${user} left room ${room}`]);
    });
              ...
              >
                ...
                {task.lockedBy && (
                  <small style={{ color: '#999' }}>
                    {task.lockedBy === username ? 'You are moving this' : `${task.lockedBy} is moving this`}
                  </small>
                )}
              </div>
            ))}
  ...
  return (
    ...
    >
      <div>
        ...
        {countdown > 0 && (
          <span> - next attempt in {countdown} seconds</span>
        )}
        {messageQueue.length > 0 && (
          <span> | {messageQueue.length} messages pending</span>
        )}
      </div>
...
Socket.io מוסיף שכבת אבסטרקציה מעל WebSocket: חיבור מחדש אוטומטי, fallback ל-HTTP long-polling אם WebSocket לא זמין, תמיכה בחדרים ו-namespaces, שליחת events מותאמים (לא רק הודעות), acknowledgments לאישור קבלה, ותמיכה בבינארי. WebSocket רגיל דורש מימוש ידני של כל אלה.

**4. הודעות בזמן ניתוק:**

הדפוס הנפוץ הוא message queue - הודעות שנשלחות בזמן ניתוק נשמרות במערך מקומי (queue). כשהחיבור חוזר, כל ההודעות בqueue נשלחות לשרת לפי הסדר. חשוב להגביל את גודל הqueue ולהוסיף timestamp כדי שהשרת יוכל לדעת מתי ההודעה נכתבה במקור.

**5. emit מול broadcast:**

`emit` שולח event ללקוח ספציפי או לכל הלקוחות. `broadcast` שולח event לכל הלקוחות חוץ מהשולח. למשל, כשמשתמש שולח הודעת צ'אט, השרת משתמש ב-`socket.broadcast.emit` כדי לשלוח את ההודעה לכל שאר המשתמשים בחדר, אבל לא בחזרה לשולח (כי הוא כבר רואה את ההודעה שלו).
