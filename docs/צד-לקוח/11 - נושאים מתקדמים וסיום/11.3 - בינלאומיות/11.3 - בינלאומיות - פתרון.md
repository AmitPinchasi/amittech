...
function LanguageSwitcher() {
  ...
  // Loading saved language on initialization
  useEffect(() => {
...
/* Layout that works in RTL and LTR */
.page-layout {
  display: flex;
  gap: 24px;
}
...
export function middleware(request: NextRequest) {
  ...
  // Checking if there's a locale in the URL
...
הtag hreflang מודיעה למנועי חיפוש שקיימות גרסאות של הדף בשפות אחרות. היא עוזרת ל-Google להציג את הגרסה הנכונה למשתמשים לפי שפתם ומיקומם. ללא hreflang, מנוע החיפוש עלול להציג את הגרסה הלא נכונה, או אפילו לראות את הגרסאות השונות כתוכן כפול (duplicate content) ולפגוע בדירוג.
