...
my-monorepo/
  ...
  packages/
    ...
    config/        # ESLint, TS, Prettier config
...
Web Workers מאפשרים להריץ JavaScript בthread נפרד, מה שמונע חסימה של ה-UI.

![תרשים זרימה של Web Worker: הthread הראשי שולח נתונים לworker בהודעה, החישוב הכבד רץ ברקע בזמן שהUI ממשיך להגיב, והתוצאה חוזרת בהודעה ומעדכנת את התצוגה](../../../images/illustrations/client-11-7-web-worker.svg)
...
function VirtualList({ items }: VirtualListProps) {
  ...
  const virtualizer = useVirtualizer({
    ...
    overscan: 5, // number of extra items to render outside the screen
  });
...
// Example with 10,000 items - still smooth!
function App() {
...
function SearchResults() {
  ...
  const handleHover = (id: string) => {
    // Preload the product page on hover
...
```tsx
// next.config.js - limit bundle size
const nextConfig = {
  experimental: {
  ...
  webpack: (config, { isServer }) => {
    if (!isServer) {
      // Warn if the bundle is too large
      config.performance = {
        hints: 'warning',
...
- **Web Workers** - הרצת חישובים כבדים בthread נפרד
