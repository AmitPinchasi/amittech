      ...
      - name: Format Results
        ...
        with:
          script: |
            ...
            // Fail if it doesn't meet the thresholds
            if (performance < 80) {
...
jobs:
  ...
  test:
    ...
    steps:
      ...
      # Comment on the PR with coverage
...
בדיקות יחידה בודקות פונקציות בודדות בבידוד, אך לא מוודאות שהמערכת כולה עובדת יחד. בדיקות E2E מדמות משתמש אמיתי - לחיצות, ניווט, מילוי טפסים - ותופסות באגים שבדיקות יחידה מפספסות: שגיאות אינטגרציה, בעיות CSS, routing שבור. הרצה ב-CI מבטיחה שכל PR נבדק בצורה מקיפה.
