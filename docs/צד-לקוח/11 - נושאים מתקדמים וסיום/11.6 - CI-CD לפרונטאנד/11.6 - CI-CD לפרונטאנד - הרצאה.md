...
בGitHub Actions מאפשר להריץ workflows אוטומטיים בתגובה לevents ב-repository.

#### מבנה בסיסי

```yaml
# .github/workflows/ci.yml
name: CI Pipeline       # the workflow's name
...
- **Trigger** - event שמפעיל את ה-workflow (push, PR, schedule)
...
jobs:
  ...
  test:
    name: Tests
    runs-on: ubuntu-latest
    needs: lint-and-typecheck  # runs only after lint
  ...
  build:
    name: Build
    runs-on: ubuntu-latest
    needs: [lint-and-typecheck, test]  # runs after both
...
![תרשים זרימה של pipeline ב-CI: דחיפת קוד מפעילה בדיקות איכות, טסטים ובנייה בזה אחר זה, ורק אם כל השלבים עוברים מתבצעת פריסה לפרודקשן, אחרת נשלחת התראה](../../../images/illustrations/client-11-6-ci-pipeline.svg)
...
```yaml
# Vercel handles this automatically, but checks can be added before
jobs:
  checks:
    runs-on: ubuntu-latest
    steps:
...
jobs:
  lighthouse:
    ...
    runs-on: ubuntu-latest
    needs: deploy-preview  # after the preview is up
...
jobs:
  pr-validation:
    ...
    steps:
      ...
      # Checking that the PR includes a description
