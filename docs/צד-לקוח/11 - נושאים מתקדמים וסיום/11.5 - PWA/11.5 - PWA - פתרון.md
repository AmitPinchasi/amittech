...
self.addEventListener('fetch', (event) => {
  ...
  // HTML pages - Network First with fallback to offline
...
#### פתרון תרגיל 3 - זיהוי state רשת ותמיכה אופליין
...
export async function processQueue() {
  ...
  for (const action of queue) {
    try {
      ...
      // Still offline - we'll keep trying later
      break;
    }
  }
}
...
const withPWA = require('next-pwa')({
  ...
  runtimeCaching: [
    // Google Fonts - Cache First, a year
    {
    ...
    // Images - Cache First, 30 days, maximum 100
    {
...
function useServiceWorkerUpdate() {
  ...
  useEffect(() => {
    ...
    navigator.serviceWorker.ready.then((reg) => {
      ...
      // Check for update on every visit
      reg.update();
...
```tsx
// Inside the Service Worker - handling the SKIP_WAITING message
// public/sw.js
