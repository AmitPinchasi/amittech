...
// public/manifest.json
{
  "name": "My Application",
  "short_name": "App",
  "description": "Advanced web app for task management",
  "start_url": "/",
  "display": "standalone",
...
- **fullscreen** - מסך מלא, ללא שום interface דפדפן
...
// Install event - save static files
  ...
  event.respondWith(
    caches.match(event.request).then((cachedResponse) => {
      ...
      return fetch(event.request).then((response) => {
        // Store a valid response in the cache
        ...
        // Offline - return the offline page
...
![תרשים זרימה של Service Worker שמיירט בקשות רשת: אם התשובה נמצאת ב-cache היא מוחזרת מיד, אחרת נשלחת בקשה לרשת, וכאשר אין חיבור מוחזר דף האופליין](../../../images/illustrations/client-11-5-sw-fetch-cache.svg)

---

## אסטרטגיות cache - Caching Strategies

#### Cache First - cache קודם

```tsx
// Suitable for static assets that don't change (images, fonts)
self.addEventListener('fetch', (event) => {
...
```tsx
// Suitable for dynamic content (API, HTML pages)
async function networkFirst(request) {
  try {
    ...
    // If the network isn't available - fall back to the cache
...
```tsx
// Suitable for content that needs to be fast but also up to date
async function staleWhileRevalidate(request) {
  ...
  // Return the cache if available, otherwise wait for the network
...
// Precache - files generated at build time
...
export default function OfflinePage() {
  return (
    <div style={{ textAlign: 'center', padding: '48px 16px' }}>
      ...
      <p>Looks like you're offline. Check your network connection and try again.</p>
...
function NetworkStatus() {
  ...
  return (
    <div role="alert" style={{ background: '#f44336', color: 'white', padding: '8px', textAlign: 'center' }}>
      You are offline. Changes will be saved once the connection returns.
    </div>
  );
}
```
...
```tsx
// In the Service Worker - handling Push notifications
  ...
  return (
    <div style={{ padding: '16px', backgroundColor: '#e8f5e9', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <div>
        ...
        <p>Quick access from the home screen, works offline too</p>
      </div>
...
module.exports = withPWA({
  // The rest of the Next.js config
});
```
...
- **אסטרטגיות cache** - Cache First, Network First, Stale While Revalidate
