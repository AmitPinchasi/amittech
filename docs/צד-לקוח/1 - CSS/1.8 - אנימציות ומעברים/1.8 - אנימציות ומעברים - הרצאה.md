...
#### הproperties של transition

```css
transition: property duration timing-function delay;
```

- **property** - איזה property CSS לעבור (background, color, transform, וכו)
- **duration** - כמה זמן המעבר (0.3s, 500ms)
- **timing-function** - עקומת המהירות (ease, linear, וכו)
- **delay** - עיכוב לפני תחילת המעבר

אפשר גם לכתוב כל property בנפרד:
...
`all` יחיל מעבר על כל property שמשתנה. זה נוח אבל פחות יעיל מבחינת ביצועים - עדיף לציין את הproperties הספציפיים.
...
לא כל properties CSS שווים מבחינת ביצועים. חשוב לדעת מה לאנימציה ומה לא.
...
שני הproperties האלו מעובדים ב-GPU (כרטיס המסך) ולא גורמים לדפדפן לחשב מחדש את ה-layout. התוצאה: אנימציה חלקה ב-60fps.
...
שינוי של הproperties האלו מכריח את הדפדפן לחשב מחדש את המיקום של כל האלמנטים בעמוד - מה שגורם לגמגום.
...
![איור זרימה המראה שכשמנפישים תזוזה עם left ממירים אותה ל-transform, וכך האנימציה רצה ישירות על ה-GPU בצורה חלקה ב-60fps במקום לגרום לחישוב layout מחדש](../../../images/illustrations/client-1-8-transform-performance.svg)
...
- **transitions** - שינויים חלקים בין שני states, בדרך כלל על hover
