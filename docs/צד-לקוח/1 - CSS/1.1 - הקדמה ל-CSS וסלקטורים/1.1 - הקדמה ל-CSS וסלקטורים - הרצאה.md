...
הוספת עיצוב ישירות על אלמנט באמצעות הattribute `style`:
...
הוספת tag `<style>` בתוך ה-`<head>` של הדף:
...
הסלקטור הכי פשוט - בוחרים לפי שם הtag:
...
- class הוא attribute שאנחנו מוסיפים לאלמנטים ב-HTML
...
#### סלקטור attributes - attribute selectors

בוחרים אלמנטים לפי הattributes שלהם ב-HTML. זה מאוד חזק ויש כמה וריאציות:
...
סיכום סלקטורי attributes:

| סלקטור | משמעות |
|---------|--------|
| `[attr]` | לאלמנט יש את הattribute |
...
<div class="article">
    ...
    <div>
        <p>Paragraph deeper inside article</p>
    </div>
</div>
...
```html
<div class="menu">
    <p>Direct child of menu</p>
    <div>
        <p>Grandchild of menu (not a direct child)</p>
    </div>
</div>
```
...
![איור המשווה בין סלקטור אח סמוך שבוחר רק את הפסקה שמיד אחרי הכותרת לבין סלקטור אח כללי שבוחר את כל הפסקאות שאחריה](../../../images/illustrations/client-1-1-sibling-selectors.svg)
