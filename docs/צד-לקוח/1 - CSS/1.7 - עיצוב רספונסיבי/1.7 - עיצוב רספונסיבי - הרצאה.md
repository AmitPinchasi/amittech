...
כשפותחים אתר בטלפון בלי tag viewport, הדפדפן מנסה להציג את האתר כאילו הוא מסך רחב ומקטין הכל - מה שגורם לטקסט זעיר שלא ניתן לקרוא.

הtag הזו צריכה להיות בכל עמוד HTML:
...
בלי הtag הזו, כל ה-CSS הרספונסיבי שנכתוב פשוט לא יעבוד כמו שצריך במובייל.
...
![איור המראה כיצד בגישת mobile-first הבסיס הוא עמודה אחת וכל שאילתת min-width שהתנאי שלה מתקיים דורסת את הכללים הקודמים ומוסיפה עמודות ככל שהמסך מתרחב](../../../images/illustrations/client-1-7-mobile-first-queries.svg)
...
#### הtag picture - תמונות שונות לגדלים שונים
...
![איור המראה כיצד clamp שומר את גודל הפונט בין מינימום למקסימום, וכשהערך המועדף 4vw קטן מהמינימום או גדול מהמקסימום הגבול הוא שקובע](../../../images/illustrations/client-1-7-clamp-fluid-size.svg)
...
Container queries הם פיצ'ר חדש יחסית שמאפשר לעצב component לפי גודל המיכל שלו, ולא לפי גודל המסך.
...
container queries פותרות בדיוק את הבעיה הזו - הcomponent מתאים את עצמו לפי הגודל של המיכל שלו.
...
- תמיד הוסיפו את tag `viewport` ב-HTML
...
- container queries הם הפיצ'ר הבא - מאפשרים לcomponents להתאים למיכל שלהם
