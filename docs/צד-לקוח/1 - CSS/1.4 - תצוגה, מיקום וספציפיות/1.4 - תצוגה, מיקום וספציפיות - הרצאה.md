...
הproperty `display` הוא אחד הproperties הכי חשובים ב-CSS. הוא קובע איך אלמנט מתנהג בדף ביחס לאלמנטים אחרים.
...
הproperty `position` קובע איך אלמנט ממוקם בדף. בשילוב עם `top`, `right`, `bottom`, `left` אנחנו יכולים למקם אלמנטים בדיוק איפה שנרצה.
...
2. סגנונות inline (attribute style ב-HTML)
...
![איור המראה חישוב ציון ספציפיות מסלקטור אלמנט ששווה 1 דרך מחלקה ששווה 10 ו-id ששווה 100 ועד סלקטור משולב שמגיע לציון 121](../../../images/illustrations/client-1-4-specificity-score.svg)

#### הורשה - inheritance

חלק מהproperties עוברים אוטומטית מהורה לילדים:

הproperties שעוברים בירושה (בדרך כלל קשורים לטקסט):
- `color`
...
- `unset` - אם הproperty בר-ירושה, מתנהג כמו inherit, אחרת כמו initial
