...
כתבו פונקציה `memoize` שמקבלת פונקציה ומחזירה גרסה ממואיזית שלה. הגרסה הממואיזית שומרת תוצאות קודמות ומחזירה אותן מהcache אם אותם ארגומנטים כבר חושבו.
...
בונוס: הוסיפו מתודה `cache.clear()` שמנקה את הcache.
