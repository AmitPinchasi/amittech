...
- אם פעולה כבדה חוסמת את ה-thread, כל הדף קופא - כולל interface המשתמש
- אז איך fetch, setTimeout ו-event listeners עובדים בלי לחסום? התשובה: לולאת הevents
...
## הinterfaces הדפדפן - Web APIs

הדפדפן מספק APIs שרצים מחוץ ל-thread הראשי של JS:

- `setTimeout` / `setInterval` - טיימרים
- `fetch` - בקשות רשת
- `addEventListener` - האזנה לevents
- `requestAnimationFrame` - אנימציות
- `geolocation`, `IntersectionObserver` ועוד

כשקוראים ל-`setTimeout(fn, 1000)`, הדפדפן מתחיל טיימר ברקע. ה-JS ממשיכה הלאה. כשהטיימר מסתיים, הדפדפן מכניס את `fn` לqueue.

---

## הqueue המשימות - Macrotask Queue

כשה-Web API מסיים את העבודה שלו, הקולבק לא רץ מיד. הוא נכנס לqueue המשימות (macrotask queue).

דוגמאות למשימות שנכנסות לqueue:
- קולבקים של `setTimeout` / `setInterval`
- קולבקים של events DOM (click, input...)
...
למרות שה-timeout הוא 0 מילישניות, הקולבק לא רץ מיד. הוא נכנס לqueue ומחכה שהמחסנית תתרוקן.

---

## הqueue המיקרו-משימות - Microtask Queue

יש queue נוסף עם עדיפות גבוהה יותר - queue המיקרו-משימות.

מה נכנס לqueue המיקרו-משימות:
- קולבקים של Promises (`then`, `catch`, `finally`)
- `queueMicrotask()`
- `MutationObserver`

הכלל: לולאת הevents מרוקנת את כל המיקרו-משימות לפני שהיא עוברת למשימה הבאה בqueue הרגיל.
...
![איור המדגים איך לולאת האירועים מרוקנת קודם את הקוד הסינכרוני, אחר כך את כל המיקרו-משימות, ורק בסוף משימה מהמאקרו-תור, ולכן הפלט הוא Start, End, Promise, Timeout](../../../images/illustrations/client-4-7-event-loop-order.svg)

---

## מחזור לולאת הevents - Event Loop Cycle

לולאת הevents עובדת בלולאה אינסופית לפי הסדר הזה:

1. הרצת כל הקוד הסינכרוני (עד שהמחסנית ריקה)
2. ריקון כל queue המיקרו-משימות
3. ביצוע משימה אחת מqueue המאקרו-משימות
4. חזרה לשלב 2
...
`setTimeout(fn, 0)` לא אומר "הרץ עכשיו". זה אומר "הרץ ברגע שהמחסנית תתרוקן וqueue המיקרו-משימות יתרוקן".
...
אם קוד סינכרוני רץ יותר מדי זמן, הוא חוסם את הכל - ה-UI, events, אנימציות.
...
תכנות פונקציונלי (FP) הוא פרדיגמה שבה בונים תוכנה מפונקציות טהורות, נמנעים משינוי state, ומשתמשים בהרכבה של פונקציות.
...
- קל להבין אותן (לא צריך לעקוב אחרי state חיצוני)
- קל להריץ אותן במקביל (אין שינוי של state משותף)
...
- יש שני queues: מיקרו-משימות (Promises) ומאקרו-משימות (setTimeout, events)
- מיקרו-משימות תמיד רצות לפני מאקרו-משימות
- `setTimeout(fn, 0)` לא מיידי - מחכה שהמחסנית וqueue המיקרו-משימות יתרוקנו
