...
לכל Promise יש שלושה states:
- **pending** - ממתין (עדיין לא הסתיים)
- **fulfilled** - הצליח (יש תוצאה)
- **rejected** - נכשל (יש שגיאה)

![איור המדגים את שלושת המצבים של Promise - הוא מתחיל ב-pending ועובר פעם אחת בלבד ל-fulfilled שמפעיל את then, או ל-rejected שמפעיל את catch](../../../images/illustrations/client-4-5-promise-states.svg)
...
![איור המשווה ריצה סדרתית לריצה מקבילית - שלוש בקשות סדרתיות לוקחות שלוש שניות, ואילו Promise.all מריץ אותן יחד ומסיים בשנייה אחת](../../../images/illustrations/client-4-5-sequential-vs-parallel.svg)
...
| לולאת events | צריך `asyncio.run()` | מובנית בסביבה |
| Promises | `asyncio.Future` (פחות נפוץ) | `Promise` (בסיס השפה) |

ההבדל המרכזי: ב-JS כל דבר אסינכרוני בנוי סביב Promises ולולאת הevents רצה תמיד. בפייתון, `asyncio` הוא תוספת שצריך להפעיל מפורשות. בפרקטיקה, קוד אסינכרוני הרבה יותר נפוץ ב-JS מאשר בפייתון.
