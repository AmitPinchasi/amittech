...
איך זה עובד:
1. `enqueue` יוצרת Promise חדש ושומרת את `resolve`/`reject` שלו בqueue
2. `#processNext` בודקת אם יש משימה בqueue ואם לא רצה כבר משימה
3. כשמשימה מסתיימת, `finally` קורא ל-`#processNext` שוב כדי לעבד את הבאה
4. ה-Promise שהוחזר ל-caller מתקיים כשהמשימה שלו (לא הראשונה בqueue) מסתיימת
