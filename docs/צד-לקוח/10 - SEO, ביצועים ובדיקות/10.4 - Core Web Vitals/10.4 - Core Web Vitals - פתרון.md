...
// Hero data is loaded on the server
...
const ITEMS_PER_PAGE = 50; // Virtualization - show only 50 items

export default function SearchPage() {
  const [query, setQuery] = useState('');
  const deferredQuery = useDeferredValue(query); // Technique 1: deferred value
  const [visibleCount, setVisibleCount] = useState(ITEMS_PER_PAGE);

  // Technique 2: memoization of the filtering
  ...
  // Technique 3: virtualization - show only part of the items
    ...
    <div>
      <input
        ...
        onChange={(e) => {
          ...
          setVisibleCount(ITEMS_PER_PAGE); // Reset when searching
        }}
        placeholder="חיפוש..."
      />
...
// Server Component - data is loaded on the server
...
export default async function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main>
      ...
      {/* 1. Reserving space for the ad - height is always allocated */}
      ...
      {/* 2. Image with defined dimensions */}
      <Image
        src={article.image}
      ...
      {/* 3. Comments with a fixed-height skeleton */}
...
```tsx
// app/layout.tsx - loading fonts without FOUT
...
function getColor(name: string, value: number): string {
  ...
  if (value <= poor) return '#f59e0b'; // orange
  return '#ef4444'; // red
}
...
export function printPerformanceReport() {
  ...
  window.addEventListener('load', () => {
    // Wait a bit more so all layout shifts get recorded
    setTimeout(() => {
      ...
      if (report.slowResources.length > 0) {
        console.group(`Slow resources (${report.slowResources.length})`);
  ...
  return (
    <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px 16px' }}>
      <article style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
        {/* Image with defined dimensions - prevents CLS */}
        ...
        <div style={{ flex: '1 1 400px' }}>
          ...
          {/* Client Component only for the button */}
          <AddToCartButton productId={product.id} price={product.price} />
        </div>
      </article>

      {/* Reviews - lazy loaded with skeleton */}
      ...
      {/* Related products - lazy loaded with skeleton */}
...
// Separate Server components for secondary data
...
export function AddToCartButton({ productId, price }: { productId: string; price: number }) {
  ...
  async function handleAddToCart() {
    ...
    // Simple logic - no heavy computation on the main thread
