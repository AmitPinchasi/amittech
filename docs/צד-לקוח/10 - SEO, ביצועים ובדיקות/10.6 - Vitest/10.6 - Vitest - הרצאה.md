...
![אנימציה של מחזור TDD: כותבים בדיקה שנכשלת, כותבים קוד מינימלי שמעביר אותה, משפרים את הקוד וחוזרים על המחזור עם תכונה חדשה](../../../images/illustrations/client-10-6-tdd-cycle.svg)
...
function calculateTotal(price: number, taxRate: number): number {
  ...
  return Math.round((price + tax) * 100) / 100; // round to 2 decimal places
}
```
...
export default defineConfig({
  ...
  test: {
    ...
    css: true,            // support for importing CSS
  },
});
```
...
```tsx
// src/test/setup.ts
import '@testing-library/jest-dom'; // additional matchers like toBeInTheDocument
```
...
expect(0.1 + 0.2).toBeCloseTo(0.3); // for floating point issues
```
...
// object contains some of the keys
...
```tsx
// async/await - the recommended way
...
it('should call callback with data', (done) => {
  fetchUserCallback(1, (error, user) => {
    ...
    expect(user.name).toBe('דני');
    done(); // mark that the test has finished
  });
});
```
...
describe('Timer functions', () => {
  beforeEach(() => {
    vi.useFakeTimers(); // using fake timers
  });
  ...
  it('should call function after delay', () => {
    ...
    vi.advanceTimersByTime(1000); // jump forward by 1000ms
...
# run tests in watch mode (reruns on every change)
npx vitest --watch
