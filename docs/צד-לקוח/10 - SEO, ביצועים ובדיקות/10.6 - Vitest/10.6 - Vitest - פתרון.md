...
```tsx
// cart.test.ts - tests were written first (TDD)
...
```tsx
// cart.ts - code written after the tests
interface CartItem {
  id: string;
  name: string;
  price: number;
...
**2.** TDD: (1) אדום - כותבים בדיקה שנכשלת כי הקוד עוד לא קיים. (2) ירוק - כותבים את הקוד המינימלי שגורם לבדיקה לעבור. (3) שיפור - משפרים את הקוד בלי לשבור את הבדיקות. היתרון: הבדיקה מגדירה את הדרישה לפני הקוד, מה שמבטיח שכל קוד שנכתב באמת נבדק ושהinterface מעוצב מנקודת המבט של המשתמש בקוד.
