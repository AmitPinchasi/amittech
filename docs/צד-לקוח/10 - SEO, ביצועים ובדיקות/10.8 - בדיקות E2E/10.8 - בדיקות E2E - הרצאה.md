...
# Installs Playwright, browsers, and creates config files
```
...
export default defineConfig({
  ...
  use: {
    ...
    trace: 'on-first-retry', // keeps trace for failed tests
  ...
  // Run the dev server before the tests
  webServer: {
    command: 'npm run dev',
...
```tsx
// When you need to simulate real typing (with events)
...
![אנימציה של מנגנון ההמתנה האוטומטית של Playwright: לפני לחיצה הוא בודק שוב ושוב אם האלמנט גלוי ופעיל, ורק כשהאלמנט מוכן הפעולה מתבצעת](../../../images/illustrations/client-10-8-auto-wait.svg)
...
test('should match with threshold', async ({ page }) => {
  await page.goto('/');
  await expect(page).toHaveScreenshot('homepage.png', {
    maxDiffPixelRatio: 0.01, // up to 1% difference
  });
});
```
...
```tsx
// e2e/auth.spec.ts - grouping related tests
import { test, expect } from '@playwright/test';
