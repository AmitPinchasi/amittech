...
<meta name="description" content="Relevant description of the page, up to 155 characters" />

<!-- Fix alt text -->
<img src="hero.jpg" alt="Precise description of the image" />
...
/* Bad - contrast 2.5:1 */
.text-light {
  color: #aaa; /* on a white background */
}

/* Good - contrast 4.5:1+ */
.text-light {
  color: #595959; /* on a white background */
}

/* Button - sufficient contrast */
.button {
  background-color: #2563eb;
  color: #ffffff; /* contrast 7.4:1 */
}
```
...
```tsx
// hero image: 3MB JPEG -> next/image with WebP
...
<Image
  ...
  quality={80}
/>
// next/image will convert to WebP and shrink to ~200KB
```

**עדיפות 4 - ביצועים - JavaScript (2 שעות):**

```tsx
// code splitting - dynamic loading of heavy modules
...
lint + typecheck (30s)
     |
unit tests + coverage (1-2min)  ← fails if coverage is below 80%
     |
build (1-2min)
     |
 +---+---+
...
<body>
  ...
  console.log('Report saved to reports/test-report.html');
}
