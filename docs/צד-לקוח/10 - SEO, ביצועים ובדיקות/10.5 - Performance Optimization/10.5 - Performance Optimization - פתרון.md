...
// every module loads dynamically only when needed
...
const MarkdownEditor = dynamic(() => import('./MarkdownEditor'), {
  ...
  ssr: false, // the editor doesn't need SSR
});
...
// stats component wrapped in memo
...
function TodoApp() {
  ...
  // useMemo - filtering computed only when dependencies change
  ...
  // useMemo - stats computed only when the list changes
  const stats = useMemo(() => {
  ...
  // useCallback - functions aren't recreated
...
module.exports = {
  async headers() {
    return [
      {
        // JS/CSS files with a hash - cache for a year, immutable
        source: '/_next/static/(.*)',
        headers: [
          {
            key: 'Cache-Control',
      ...
      {
        // optimized images - cache for a month
        source: '/_next/image(.*)',
        headers: [
          {
            key: 'Cache-Control',
      ...
      {
        // static images - cache for a month
        source: '/images/(.*)',
        headers: [
          {
            key: 'Cache-Control',
...
```tsx
// app/page.tsx - homepage - refresh every 5 minutes
...
```tsx
// app/articles/[slug]/page.tsx - article page - refresh every hour
...
```tsx
// app/api/data/route.ts - API with stale-while-revalidate
