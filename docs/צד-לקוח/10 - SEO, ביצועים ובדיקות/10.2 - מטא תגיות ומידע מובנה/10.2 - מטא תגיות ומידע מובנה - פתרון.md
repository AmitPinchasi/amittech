## פתרון - מטא tags ומידע מובנה - Meta Tags and Structured Data
...
export function auditMetaTags(html: string): AuditResult {
  ...
  if (!title) {
    ...
    issues.push({ severity: 'warning', message: `Title is too short (${title.length} characters). 50-60 characters recommended.` });
    score -= 5;
  } else if (title.length > 60) {
    issues.push({ severity: 'warning', message: `Title is too long (${title.length} characters). Up to 60 characters recommended.` });
    score -= 5;
  }
  ...
  if (!description) {
    issues.push({ severity: 'error', message: 'Missing meta description tag' });
    score -= 15;
  } else if (description.length < 70) {
    issues.push({ severity: 'warning', message: `Description is too short (${description.length} characters). 120-155 characters recommended.` });
    score -= 5;
  } else if (description.length > 155) {
    issues.push({ severity: 'warning', message: `Description is too long (${description.length} characters). Up to 155 characters recommended.` });
    score -= 3;
  }
  ...
  if (!hasOgTitle) {
    issues.push({ severity: 'warning', message: 'Missing og:title tag' });
    score -= 5;
  }
  if (!hasOgDesc) {
    issues.push({ severity: 'warning', message: 'Missing og:description tag' });
    score -= 5;
  }
  if (!hasOgImage) {
    issues.push({ severity: 'warning', message: 'Missing og:image tag' });
    score -= 5;
  }
  ...
  if (!hasTwitterCard) {
    issues.push({ severity: 'info', message: 'Missing twitter:card tag' });
    score -= 3;
  }
  ...
  if (!hasViewport) {
    issues.push({ severity: 'error', message: 'Missing viewport tag - the site will not be mobile-friendly' });
    score -= 15;
  }
  ...
  if (jsonLdMatch) {
    try {
      ...
      issues.push({ severity: 'error', message: 'JSON-LD exists but is invalid (JSON error)' });
      score -= 10;
    }
  } else {
    issues.push({ severity: 'info', message: 'No JSON-LD structured data' });
    score -= 5;
  }
...
// Score: 52/100
// Issues: [
//   { severity: 'warning', message: 'Title is too short...' },
//   { severity: 'error', message: 'Missing meta description tag' },
...
**1.** הtag `title` מופיעה בכרטיסיית הדפדפן ובתוצאות החיפוש של Google. הtag `og:title` משמשת כשמשתפים את הקישור ברשתות חברתיות. הן לא חייבות להיות זהות - לפעמים כדאי שה-og:title תהיה קצרה ותמציתית יותר, בעוד ה-title כוללת גם את שם האתר.
...
**4.** Metadata API של Next.js מספק type safety עם TypeScript, ירושה אוטומטית של מטא-דאטה מ-layout לדפים, תמיכה מובנית במטא-דאטה דינמית עם generateMetadata, ו-deduplication אוטומטי (מונע tags כפולות). בנוסף, הוא מטפל אוטומטית ב-metadataBase לנתיבים יחסיים.

**5.** כשמשתפים קישור בוואטסאפ ללא tags Open Graph, וואטסאפ מנסה לחלץ את ה-title ואת השורה הראשונה של התוכן מה-HTML. התצוגה המקדימה תהיה בסיסית מאוד, בדרך כלל רק ה-URL עצמו או כותרת הדף ללא תמונה. זה פוגע בשיעור הקליקים כי התצוגה לא מושכת.
