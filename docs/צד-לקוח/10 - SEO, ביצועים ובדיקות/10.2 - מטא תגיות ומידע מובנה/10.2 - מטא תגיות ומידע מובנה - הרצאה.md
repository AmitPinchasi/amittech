## מטא tags ומידע מובנה - Meta Tags and Structured Data

בשיעור זה נצלול לעולם של מטא-tags ומידע מובנה. נלמד כיצד לספק למנועי חיפוש ולרשתות חברתיות מידע מדויק על התוכן שלנו, וכיצד ליישם את כל זה עם Next.js.

---

## מטא-tags חיוניות - Essential Meta Tags

מטא-tags הן אלמנטי HTML שנמצאים בתוך ה-`<head>` ומספקים מידע על הדף למנועי חיפוש ולדפדפנים.

#### כותרת - title

- הtag החשובה ביותר ל-SEO
...
- ללא tag זו, האתר לא ייחשב mobile-friendly
...
<!-- prevent showing in Google's cache -->
...
- טוויטר משתמש גם בtags OG כ-fallback
...
```html
<!-- all of these should point to the same canonical -->
...
![איור שמראה כיצד ארבע כתובות שונות של אותו מוצר מתאחדות באמצעות תגית canonical לכתובת קנונית אחת](../../../images/illustrations/client-10-2-canonical-url.svg)

#### דוגמה לדף pagination

```html
<!-- page 1 - it is the canonical -->
<link rel="canonical" href="https://blog.com/articles" />

<!-- page 2 - canonical of itself -->
...
## הinterface Metadata של Next.js - Next.js Metadata API
...
```bash
# check structured data from the CLI
...
- חפשו בתוך ה-`<head>` את כל tags ה-meta
...
- מטא-tags הן אלמנטים ב-`<head>` שמספקים מידע למנועי חיפוש ולדפדפנים
- הtags החיוניות: title, description, viewport, charset, robots
