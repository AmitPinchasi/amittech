...
![תרשים זרימה של שלושת שלבי מנוע החיפוש: הבוט סורק דפים דרך קישורים, התוכן מנותח ונשמר באינדקס, וכשמשתמש מחפש התוצאות מדורגות ומוצגות](../../../images/illustrations/client-10-1-search-pipeline.svg)
...
- הוא בודק את הטקסט, התמונות, הסרטונים, ומטא-tags
...
- כולל: כותרות, מטא-tags, מבנה כותרות (H1-H6), טקסט חלופי לתמונות, קישורים פנימיים
...
```tsx
// Example: Head component in Next.js with basic meta tags
...
```tsx
// Example: blog page with focused keywords
function BlogPost() {
  return (
    <>
      <Head>
        {/* Primary keyword: "React hooks guide" */}
      ...
      <article>
        ...
        {/* Relevant content with natural keyword usage */}
...
```bash
# Step 1: sign up at search.google.com/search-console
# Step 2: verify ownership of the site - one of the ways:

# Add a meta tag to the homepage:
# <meta name="google-site-verification" content="your-verification-code" />

# or add a DNS record of type TXT
# or upload an HTML file to the root directory
```

```tsx
// In Next.js - adding Search Console verification
// app/layout.tsx
...
```html
<!-- This is what a regular SPA's HTML looks like - problematic for SEO -->
<!DOCTYPE html>
<html>
<head>
  <title>My App</title>
...
- מטא-tags זמינות מיד ללא צורך בהרצת JavaScript

```html
<!-- This is what SSR's HTML looks like - excellent for SEO -->
<!DOCTYPE html>
<html lang="he" dir="rtl">
<head>
...
![אנימציה שמשווה את מסלול הסריקה של דף SSR שנכנס לאינדקס מיד לעומת דף SPA שמחזיר div ריק ונכנס לאינדקס רק אחרי רינדור JS](../../../images/illustrations/client-10-1-spa-ssr-crawl.svg)

#### אסטרטגיות רינדור - Rendering Strategies

```tsx
// Next.js App Router - server-side rendering (default)
// app/blog/[slug]/page.tsx

// Server Component - HTML is created on the server
...
```tsx
// Static Site Generation - pages are built ahead of time at build time
// app/blog/[slug]/page.tsx
...
// The page is pre-built for every slug returned from generateStaticParams
export default async function BlogPost({ params }: { params: { slug: string } }) {
  const post = await fetchPost(params.slug);
