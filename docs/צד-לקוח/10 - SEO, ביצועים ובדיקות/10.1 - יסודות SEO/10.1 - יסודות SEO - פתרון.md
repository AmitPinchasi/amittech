...
5. אין `meta charset`
6. שימוש ב-`<div>` במקום tags סמנטיות (header, main, h1, button)
...
- תיאור: `למדו TypeScript מאפס. מדריך מקיף שמכסה טיפוסים, interfaces, generics ועוד עם דוגמאות קוד מעשיות.`
...
```tsx
// in React using react-helmet-async
...
// URL changes from /#/projects/1 to /projects/1
```
...
**3. בעיית SPA ו-SEO:**
- ב-SPA, ה-HTML שנשלח לבוט הוא ריק - רק div ריק וקובץ JavaScript. הבוט צריך להריץ JS כדי לראות תוכן, מה שלא תמיד קורה בצורה תקינה. ב-SSR, ה-HTML נוצר בשרת ונשלח מלא עם כל התוכן והמטא-tags, כך שהבוט רואה הכל מיד.
