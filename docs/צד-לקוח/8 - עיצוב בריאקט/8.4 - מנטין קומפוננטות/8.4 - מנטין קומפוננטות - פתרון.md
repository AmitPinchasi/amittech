...
- שלושה states: טעינה (Skeleton), ריק (Alert), עם מוצרים
...
3. **Notification לעומת Alert** - Notification היא הודעה זמנית שבדרך כלל נעלמת אחרי כמה שניות (toast). Alert הוא component שמוצג כחלק מהדף ונשאר עד שהמשתמש סוגר אותו או עד שהמצב משתנה. נשתמש ב-Notification לevents חולפים (שמירה הצליחה) וב-Alert למידע שצריך להישאר (אזהרת אבטחה).
