...
ההוק הנפוץ ביותר - מנהל state בוליאני (פתוח/סגור) עם פעולות open, close, toggle.
...
form.validate(); // Trigger manual validation
form.isValid(); // Check if the form is valid
form.isDirty(); // Check if the form has changed
...
## useIntersection - זיהוי component בתצוגה
...
## useScrollIntoView - גלילה לcomponent
...
function ScrollToExample() {
  const { scrollIntoView, targetRef } = useScrollIntoView<HTMLDivElement>({
    offset: 60, // offset for a fixed header
    duration: 500,
  });
...
#### useClickOutside - לחיצה מחוץ לcomponent
...
- useDisclosure מנהל state פתוח/סגור בצורה נקייה, שימושי ל-Modal, Drawer וכל toggle
...
- useIntersection מזהה מתי component נכנס לתצוגה
- useScrollIntoView מאפשר גלילה חלקה לcomponent ספציפי
