...
- הטיפוס React.CSSProperties מבטיח שכל הproperties תקינות
...
2. **למה styled-components עלולה לפגוע בביצועים** - styled-components מייצרת CSS בזמן ריצה (runtime). בכל רנדור, הספריה צריכה לחשב את הסגנונות, ליצור מחרוזות CSS, ולהזריק אותן ל-DOM דרך tags style. CSS Modules, לעומת זאת, מייצרות קבצי CSS סטטיים בזמן build, והדפדפן מטפל בהם ישירות ללא עלות JavaScript.
