...
const theme = createTheme({
  ...
  // Overriding component default props
  components: {
    Button: {
      defaultProps: {
        radius: "md",
...
function App() {
  return (
    <MantineProvider theme={theme}>
      {/* All components will use this theme */}
    </MantineProvider>
  );
}
```
...
function App() {
  return (
    <MantineProvider theme={theme} cssVariablesResolver={resolver}>
      {/* Now you can use var(--sidebar-width) anywhere */}
    </MantineProvider>
  );
}
```
...
/* Using a postcss-preset-mantine mixin */
.sidebar {
  display: none;
...
function PolymorphicExamples() {
  return (
    <>
      ...
      {/* <Button component={Link} to="/dashboard">Navigation</Button> */}
...
// Navigation button that combines Button with Link
...
```css
/* Styles based on data attributes */
