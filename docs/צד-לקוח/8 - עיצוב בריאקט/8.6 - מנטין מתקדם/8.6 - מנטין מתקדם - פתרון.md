...
// Custom component (simple - no factory)
function StatCard({
  title,
  value,
  change,
...
5. **polymorphic components** - במקום לעטוף קומפוננטה (כמו `<a><Button>...</Button></a>`) שיוצר DOM מיותר ובעיות סגנון, polymorphic component משנה את אלמנט ה-HTML הבסיסי עצמו. `<Button component="a">` מרנדר tag a עם כל הסגנונות של Button ישירות. זה שומר על DOM נקי, סמנטיקה נכונה, ו-TypeScript בודק את ה-props של האלמנט החדש.
