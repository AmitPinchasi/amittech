...
function ThemeSwitch() {
  ...
  // computedColorScheme is always "light" or "dark"
  // even when colorScheme is "auto"
...
![תרשים זרימה של חישוב ערכת הצבעים בפועל, מהבחירה שנשמרת ב-localStorage דרך בדיקת העדפת המערכת ועד הצביעה של כל הקומפוננטות](../../../images/illustrations/client-8-8-color-scheme.svg)
...
function App() {
  return (
    <MantineProvider
      ...
      // Mantine automatically saves the choice in localStorage
      // The default key is "mantine-color-scheme-value"
    >
      <TheApp />
    </MantineProvider>
  );
...
```css
/* Alternative without CSSVariablesResolver */
.card {
