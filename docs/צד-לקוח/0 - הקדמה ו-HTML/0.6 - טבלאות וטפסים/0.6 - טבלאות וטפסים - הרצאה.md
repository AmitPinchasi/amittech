...
- הtag table יוצרת את הטבלה.
- הtag tr (table row) יוצרת שורה חדשה.
- הtag td (table data) יוצרת תא רגיל בתוך שורה.
...
- הtag th (table header) יוצרת תא כותרת. הדפדפן מציג אותו מודגש ומרוכז כברירת מחדל.
...
- הattribute colspan מגדירה על כמה עמודות התא משתרע.
...
- הattribute rowspan מגדירה על כמה שורות התא משתרע.
...
- הtag caption מוסיפה כותרת/כיתוב לטבלה.
- היא חייבת להיות מיד אחרי tag table הפותחת.
...
טפסים מאפשרים למשתמש להזין מידע ולשלוח אותו. כל טופס התחברות, טופס הרשמה, טופס חיפוש או טופס צור קשר שאתם מכירים - בנוי עם tags form בHTML.
...
- הattribute action מגדירה לאן המידע נשלח (הURL של השרת שיקבל את הנתונים).
- הattribute method מגדירה את שיטת השליחה.
...
![תרשים של שליחת טופס בשתי השיטות: בשיטת GET הנתונים מופיעים בכתובת ה-URL עצמה, ובשיטת POST הם נשלחים בגוף הבקשה ולא נראים בכתובת](../../../images/illustrations/client-0-6-get-vs-post.svg)

---

## סוגי שדות קלט - input types

הtag input היא הtag הנפוצה ביותר בטפסים. סוג השדה מוגדר battribute type:
...
- הattribute checked מסמנת את התיבה כברירת מחדל.
...
- הtag label מקשרת תווית לשדה קלט באמצעות הattribute for.
...
## הattributes חשובות של שדות קלט
...
- select לרשימה נפתחת.
- textarea לטקסט ארוך.
- הattributes: required, placeholder, pattern.
- labels לכל שדה.
