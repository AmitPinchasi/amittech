...
## גישה לproperties - Accessing Properties
...
## הוספה, עדכון ומחיקה של properties
...
שימו לב שאפשר להוסיף ולמחוק properties גם אם האובייקט הוגדר עם `const`. ה-`const` מגן רק על ההפניה (reference), לא על התוכן. זה בדיוק כמו בפייתון.

---

## קיצור שמות properties - Shorthand Property Names
...
## שמות properties מחושבים - Computed Property Names

אפשר להשתמש בביטוי כשם של property:
...
מאפשר להעתיק properties מאובייקט אחד לאחר:
...
הארגומנט הראשון הוא האובייקט היעד. כל שאר הארגומנטים מועתקים אליו. הproperties עם אותו שם נדרסים על ידי האובייקט האחרון.
...
![איור שמראה מיזוג אובייקטים עם spread שבו הערך של color מ-userPrefs דורס את הערך שהגיע קודם מ-defaults](../../../images/illustrations/client-2-7-spread-merge.svg)
...
ההבדל: `freeze` לא מאפשר שום שינוי. `seal` מאפשר לעדכן ערכים קיימים אבל לא להוסיף או למחוק properties.
...
ברוב המקרים `in` מספיק, אבל `hasOwnProperty` שימושי כשרוצים לבדוק רק את הproperties הישירים של האובייקט.
...
מה קורה כשמנסים לגשת לproperty שלא קיים?
...
![תרשים זרימה של שרשור אופציונלי שמראה שכאשר המאפיין phone לא קיים מוחזר undefined במקום לזרוק שגיאה](../../../images/illustrations/client-2-7-optional-chaining.svg)
...
- ניתן להוסיף, לעדכן ולמחוק properties בכל רגע
...
- `?.` (optional chaining) למניעת שגיאות בגישה לproperties מקוננים
