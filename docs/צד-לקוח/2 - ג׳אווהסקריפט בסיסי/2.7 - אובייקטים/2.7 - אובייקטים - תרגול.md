...
צרו אובייקט `person` שמייצג אדם עם הproperties הבאים:
- שם (name)
- גיל (age)
- עיר (city)
...
צרו אובייקט `bankAccount` עם הproperties:
- שם הבעלים (owner)
...
3. הוסיפו property `sidebar` (אובייקט מקונן עם `width: 250` ו-`visible: true`) ל-`mergedSettings`
