...
כל הפרויקטים עובדים ב-console בלבד - בלי interface משתמש. ההתמקדות היא בלוגיקה.
...
1. **צרו אובייקט `shoppingList`** עם property `items` (מערך ריק) וproperty `nextId` שמתחיל מ-1
...
2. **פונקציות CRUD:**
   ...
   - `updateItem(id, updates)` - מעדכנת properties של פריט (למשל שם, כמות). משתמשת ב-spread. מחזירה את הפריט המעודכן.
