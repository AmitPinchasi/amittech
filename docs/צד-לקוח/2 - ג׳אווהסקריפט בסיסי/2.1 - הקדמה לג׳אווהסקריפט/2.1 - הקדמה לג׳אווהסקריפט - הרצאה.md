...
אפשר לכתוב קוד JS ישירות בתוך tag `<script>`:
...
- הtag מופיעה בדרך כלל בסוף ה-`body`
...
כשמוסיפים tag `<script>` ב-`<head>`, הדפדפן עוצר את בניית הדף כדי להוריד ולהריץ את ה-JS. זה יכול להאט את טעינת הדף.

הפתרון - הattribute `defer`:
...
בפייתון אין מקבילה ישירה - הדבר הקרוב ביותר הוא `print()`, אבל alert חוסם את interface המשתמש.
...
- מוסיפים JS ל-HTML עם tag `<script>` - מומלץ עם `defer`
