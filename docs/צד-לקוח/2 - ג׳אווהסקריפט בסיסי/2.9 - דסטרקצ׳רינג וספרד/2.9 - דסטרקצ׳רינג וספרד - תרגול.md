...
1. צרו העתק של אובייקט עם property נוסף:
...
2. השתמשו ב-rest כדי להסיר property מאובייקט (בלי delete):
...
2. כתבו פונקציה `pluck(arr, key)` שמחלצת property מסוים ממערך של אובייקטים:
...
2. צרו מערך חדש שבו לכל עובד יש property `bonus` שווה ל-10% מהמשכורת (בלי לשנות את המקור)
