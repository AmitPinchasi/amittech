...
ב. כתבו פונקציה `renderResponse` שמקבלת `ApiResponse` ומחזירה מחרוזת HTML מתאימה לכל state:
...
#### תרגיל 7 - שילוב הכל - מערכת events
...
ג. כתבו פונקציה `describeEvent` שמקבלת `AppEvent` ומחזירה תיאור טקסטואלי של הevent. השתמשו ב-exhaustive checking.

ד. כתבו פונקציה `filterEvents` שמקבלת מערך של `AppEvent` ומחרוזת `eventType`, ומחזירה רק את הevents מהסוג המבוקש. השתמשו ב-type predicate.
