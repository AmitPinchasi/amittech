...
זה מאפשר לשתף "properties" בין טיפוסים שונים בלי ירושה:
...
## טיפוסי state - state machine types

discriminated unions מתאימים מצוין למידול מכונת states (state machine):
...
היתרון: אי אפשר ליצור state לא חוקי. למשל, אי אפשר שיהיה `status: "error"` בלי שדה `error`. הטיפוסים מבטיחים שכל state מכיל בדיוק את הנתונים הרלוונטיים אליו.

![תרשים מכונת מצבים של בקשת רשת המראה מעבר בין idle, loading, success ו-error כאשר לכל מצב יש רק את השדות השייכים לו](../../../images/illustrations/client-5-5-request-state-machine.svg)
...
- state machine types - discriminated unions models states בצורה בטוחה
- דפוס Result - הצלחה או כישלון כ-discriminated union
- דפוס Redux - actions כ-discriminated union, reducer עם switch
- שילוב union עם intersection ליצירת models עשירים ובטוחים
