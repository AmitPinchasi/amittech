...
#### תרגיל 7 - שילוב גנריקס - מערכת events מוטייפת

בנו מערכת events גנרית:

א. הגדירו אינטרפייס `EventDefinitions` שמתאר את מפת הevents של האפליקציה:
...
- `emit<K extends keyof T>(event: K, data: T[K]): void` - שליחת event
...
- `emit("unknownEvent", {})` - שגיאה (event לא קיים)
