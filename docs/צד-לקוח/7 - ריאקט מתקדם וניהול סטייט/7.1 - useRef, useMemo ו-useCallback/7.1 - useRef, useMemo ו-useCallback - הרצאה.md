...
![תרשים שמראה איך useMemo משווה את מערך התלויות לרינדור הקודם, מחזיר את הערך השמור כשהתלויות זהות ומריץ את החישוב מחדש רק כשהערך num משתנה](../../../images/illustrations/client-7-1-usememo-deps.svg)
...
![תרשים שמראה שבלי useCallback נוצרת בכל רינדור פונקציה עם הפניה חדשה והילד העטוף ב-memo מרונדר מחדש, ואילו עם useCallback ההפניה נשמרת וריאקט מדלג על הרינדור](../../../images/illustrations/client-7-1-usecallback-identity.svg)
...
// not needed - the component isn't wrapped in memo
const handleClick = useCallback(() => {
  setCount((c) => c + 1);
}, []);
// if Child isn't wrapped in memo, there's no point in useCallback

// not needed - the dependencies change on every render
const data = useMemo(() => processData(items), [items]);
// if items is a new array on every render, useMemo won't help
```
