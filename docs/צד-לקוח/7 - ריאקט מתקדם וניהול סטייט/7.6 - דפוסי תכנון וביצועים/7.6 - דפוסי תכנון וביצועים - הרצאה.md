...
// Attach the components to the namespace
...
![תרשים שמראה שכשההורה מרונדר מחדש הילד מרונדר איתו, אלא אם הוא עטוף ב-memo וה-props זהים, ואילו אובייקט חדש שנוצר בכל רינדור מבטל את החיסכון](../../../images/illustrations/client-7-6-memo-props-check.svg)
...
const ListItem = memo(
  ...
  (prevProps, nextProps) => {
    // Returns true if the props are equal (no render needed)
    return (
...
```tsx
// Problem - a new object on every render
function Parent() {
  return <Child style={{ color: "red" }} />;
}

// Solution - move it outside the component
const childStyle = { color: "red" };
function Parent() {
  return <Child style={childStyle} />;
}

// Problem - a new function on every render
function Parent() {
