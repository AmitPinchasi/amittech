...
- במקום לקרוס, היא מציגה interface חלופי (fallback UI)
...
class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    // Here you could send the error to a tracking service
  }
...
- הevents עדיין עולים (bubble) דרך עץ הריאקט, לא דרך ה-DOM
...
function NavLink() {
  const preload = () => {
    // When the user hovers over the link, start loading
