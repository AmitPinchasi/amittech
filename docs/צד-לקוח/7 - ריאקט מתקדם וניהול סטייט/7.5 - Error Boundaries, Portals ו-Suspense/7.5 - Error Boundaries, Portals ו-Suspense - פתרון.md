...
הסبر:
- האנימציה מבוססת על שני states: isVisible (האם ב-DOM) ו-isAnimating (האם מוצג)
...
2. **Event bubbling ב-Portals:** למרות שב-DOM האלמנט נמצא במקום אחר (למשל ב-body), בריאקט events ממשיכים לעלות דרך עץ הקומפוננטות המקורי. אז onClick על אלמנט בתוך Portal יעלה לקומפוננטת ההורה הריאקטית, לא להורה ב-DOM.
