...
const useStore = create<StoreState>()(
  devtools(
    (set) => ({
      ...
      // The third parameter of set is the action name in devtools
    }),
    { name: "MyStore" }
  )
);
...
const useTodoStore = create<TodoStore>()(
  immer((set) => ({
    ...
    addTodo: (text) =>
      set((state) => {
        // You can mutate state directly - immer handles immutability
        state.todos.push({
          id: Date.now(),
          text,
          done: false,
...
> = (set) => ({
  ...
  logout: () => set({ user: null, cart: [] }), // also accesses the cart slice
});
...
```tsx
// Not recommended - returns the entire store
const store = useStore();

// Recommended - selects only what's needed
