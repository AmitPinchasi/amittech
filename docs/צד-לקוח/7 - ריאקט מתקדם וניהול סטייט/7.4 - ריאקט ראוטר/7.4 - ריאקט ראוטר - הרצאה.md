...
בשיעור הזה נלמד על ניווט בצד הלקוח (Client-Side Routing) באמצעות React Router - הספרייה הפופולרית ביותר לrouting באפליקציות ריאקט.

---

## הrouting בצד הלקוח - Client-Side Routing
...
- `BrowserRouter` עוטף את כל האפליקציה ומספק את ההקשר של הrouting
...
- `Link` מחליף את tag `<a>` ומונע טעינה מחדש של הדף
...
- מאפשר שמירת state ב-URL (שיתוף לינקים, refresh)
...
// Assumes there's a useAuth hook that manages authentication
...
- React Router מספק routing בצד הלקוח ללא טעינה מחדש של הדף
