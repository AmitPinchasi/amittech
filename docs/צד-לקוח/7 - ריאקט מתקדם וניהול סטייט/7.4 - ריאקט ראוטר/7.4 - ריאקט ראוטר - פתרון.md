...
1. **Link מול NavLink:** Link הוא קישור בסיסי שמונע refresh. NavLink מוסיף מודעות לstate פעיל - יודע אם הנתיב שלו תואם ל-URL הנוכחי. נשתמש ב-NavLink בתפריטי ניווט שרוצים להדגיש את הדף הנוכחי, וב-Link בקישורים רגילים.

2. **navigate() מול Navigate:** `navigate()` (מ-useNavigate) הוא פונקציה שנקראת מתוך event handlers או logic. `Navigate` הוא קומפוננטה שמבצעת ניווט כחלק מהרנדר (declarative). נשתמש ב-navigate() ב-onClick, handleSubmit וכדומה, וב-Navigate בתוך JSX לrouting מותנה (כמו ProtectedRoute).

3. **useSearchParams מול useState:** יתרונות URL: אפשר לשתף את הלינק עם הפילטרים, refresh שומר את הstate, כפתורי חזרה/קדימה בדפדפן עובדים, ו-SEO טוב יותר. חסרונות: מוגבל לנתונים שניתן לייצג כ-string ב-URL, ופחות מתאים לstates מורכבים.
