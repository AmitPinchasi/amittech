...
![תרשים זרימה המראה כיצד השרת מייצר CSRF Token ומטמיע אותו בטופס, ואיך הבקשה מתקבלת רק אם הטוקן שחוזר מהדפדפן תואם לזה שנשמר בסשן](../../../images/illustrations/web-6-2-csrf-token-flow.svg)
...
![תרשים המראה שכבות הגנה מצטברות מפני CSRF - בדיקת CSRF Token, SameSite Cookie, Secure Cookie והגבלת CORS - כאשר כל שכבה חוסמת דרך תקיפה אחרת עד שרק בקשה לגיטימית מגיעה לאפליקציה](../../../images/illustrations/web-6-2-csrf-defense-layers.svg)
