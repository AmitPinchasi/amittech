...
![תרשים שלוש השכבות של אתר: הדפדפן פונה לשרת ה-Backend, ה-Backend שולח שאילתת SQL למסד הנתונים ומחזיר את המידע כ-JSON לדפדפן, כך שמסד הנתונים לעולם אינו חשוף ישירות ללקוח](../../../images/illustrations/web-0-8-web-stack.svg)
...
- כדי לשמור מידע בDB אנחנו נצטרך לצור טבלה, הנה query SQL שיוצרת טבלה
```sql
...
![תרשים שאילתת SELECT עם WHERE: מנוע ה-SQL עובר על כל שורה בטבלה, בודק את התנאי username שווה Amit, ומחזיר רק את השורה של Amit בעוד השאר מסוננות](../../../images/illustrations/web-0-8-select-where.svg)
...
#### מחקית מידע מהDB
- כדי למחוק מידע נכתוב query שליפה רגילה כמו מקודם, רק שבמקום SELECT נכתוב DELETE
```sql
...
```
![דיאגרמה: INNER JOIN בין טבלה עם id 1,2,3 לטבלה עם id 2,3,4 מחזיר רק את הערכים המשותפים 2,3](../../../images/Pasted image 20250331223603.png)
...
מאפשר לchain תוצאות מqueries שונות:
```sql
SELECT username FROM admins
UNION
SELECT username FROM customers;
```
![דיאגרמה: UNION בין טבלה עם id 1,2,3 לטבלה עם id 2,3,4 מחזיר את איחוד הערכים ללא כפילויות - 1,2,3,4](../../../images/Pasted image 20250331223633.png)
