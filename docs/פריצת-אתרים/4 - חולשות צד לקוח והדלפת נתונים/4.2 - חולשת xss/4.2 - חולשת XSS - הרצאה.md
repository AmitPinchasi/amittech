...
**(הinjection קוד זדוני המאוחסן בשרת)**
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Saving to the database without filtering
...
foreach ($comments as $comment) {
    echo "<div class='comment'>" . $comment['text'] . "</div>"; // XSS vulnerability
}
?>
```
...
**(הinjection קוד זדוני המוחזר מיד מהשרת)**
...
<?php
$searchQuery = $_GET['q'];
echo "תוצאות חיפוש עבור: " . $searchQuery; // XSS vulnerability
?>
```
...
- הinjection קוד המאפשר גניבת נתוני התחברות. (גניבת עוגיות)
...
**(הinjection קוד באמצעות שינוי ה-DOM בדפדפן)**
...
document.getElementById("output").innerHTML = userInput; // DOM XSS vulnerability
</script>
```
...
- הbypass הגנות מבוססות שרת, כי הקוד מתבצע ישירות בצד הלקוח.
...
✅ דרך אחת, **Escape של פלט (Output Encoding)** – שימוש ב-HTML entities למניעת injection קוד. (כמו הפונקציה שהצגנו בפתרונות למעלה.)  
...
![תרשים זרימה כללי של XSS: קלט לא מהימן ממקורות שונים - URL, טופס או מסד נתונים - מגיע לנקודת פלט בעמוד, ובקרת Escaping קובעת אם הקוד ירוץ בדפדפן הקורבן או יוצג כטקסט בלבד](../../../images/illustrations/web-4-2-xss-source-sink.svg)

## דוגמאות מתקדמות ל-XSS Payloads ודרכי bypass
...
## **Payloads מתקדמים עם bypass הגנות**

### 1. הbypass htmlspecialchars (כאשר לא משתמשים ב-ENT_QUOTES):
```javascript
...
?>
```
**הbypass**: אם הפלט משמש בתוך מאפיין HTML:
**Payload**:
```javascript
" onmouseover="alert(1)
```
