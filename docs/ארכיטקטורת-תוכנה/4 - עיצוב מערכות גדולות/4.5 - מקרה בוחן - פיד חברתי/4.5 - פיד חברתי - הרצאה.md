...
def publish_post(user_id: int, content: str):
    ...
    for follower_id in followers:
        ...
        feed_cache.ltrim(f"feed:{follower_id}", 0, 999)  # keep only the last 1000
```

**קריאת פיד (מהירה מאוד):**
```python
def get_feed(user_id: int) -> list:
    post_ids = feed_cache.lrange(f"feed:{user_id}", 0, 19)  # 20 posts
...
def get_feed(user_id: int) -> list:
    ...
    # fetch recent posts from each one
    all_posts = []
...
**חסרון:** קריאת פיד של מישהו שעוקב אחרי 500 איש = 500 queries! איטי מאוד.
...
![איור מודל היברידי ל-fan-out בפיד חברתי: פוסט חדש נבדק לפי מספר העוקבים, כאשר משתמש רגיל דוחף את הפוסט לפידים בכתיבה וסלב עם מיליוני עוקבים נמשך בזמן קריאה](../../../images/illustrations/arch-4-5-fanout-hybrid.svg)
...
def get_post(post_id: int) -> dict:
    ...
    redis.setex(f"post:{post_id}", 3600, json.dumps(post))  # cache for an hour
    return post
```
