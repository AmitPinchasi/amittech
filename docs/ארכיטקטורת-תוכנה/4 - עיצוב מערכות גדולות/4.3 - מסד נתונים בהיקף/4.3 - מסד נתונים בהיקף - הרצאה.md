...
```
Write   →  [Primary DB]  →  replication  →  [Replica 1]  ← Read
                        →  replication  →  [Replica 2]  ← Read
                        →  replication  →  [Replica 3]  ← Read
```
...
class DatabaseRouter:
    ...
    def get_read_connection(self):
        """Read from a replica using Round Robin"""
...
**הבעיה:** גם עם replicas, אם יש 10 מיליארד שורות - queries איטיות, הכנסת נתונים איטית.
...
**יתרון:** כל query ידועה ל-shard אחד.

![איור פיצול נתונים ב-Hash Sharding: ה-owner_id עובר חישוב modulo לפי מספר ה-shards, והתוצאה קובעת לאיזה shard הנתון נשמר](../../../images/illustrations/arch-4-3-sharding.svg)

**חסרון:** הquery על כל המשתמשים (admin report) צריכה לפנות לכל ה-shards.
...
האינדקס הנכון יכול להפוך query מ-10 שניות ל-1 מילישנייה.

```sql
-- Common query: "All tasks of a specific user, sorted by date"
SELECT * FROM tasks WHERE owner_id=? ORDER BY created_at DESC LIMIT 20;

-- An index that covers this query
CREATE INDEX idx_tasks_owner_created ON tasks(owner_id, created_at DESC);
```

**כלל:** הוסיפו אינדקס על כל עמודה שמופיעה ב-`WHERE` ו-`ORDER BY` בqueries נפוצות.

**שימו לב:** אינדקסים מאטים כתיבות (צריך לעדכן גם את האינדקס). לא הוסיפו אינדקס על כל עמודה.

### Denormalization - מלאכת פשט

בDB מנורמל, נתונים מפוצלים לטבלאות רבות. הqueries מורכבות דורשות joins.

לפעמים - בשביל ביצועים - מכפילים נתונים:

```sql
-- Normal: needs a JOIN between tasks, projects, users
SELECT t.title, p.name AS project, u.username AS owner
FROM tasks t
JOIN projects p ON t.project_id = p.id
JOIN users u ON t.owner_id = u.id;

-- Denormalized: we saved the project name directly in the tasks table
...
| Index Design | מהירות queries | בינונית |
| Denormalization | מהירות queries מורכבות | בינונית |
