...
הScalability היא היכולת של מערכת להתמודד עם עומס גדל יותר.
...
```
Small server: 2 CPU, 4GB RAM  →  Large server: 32 CPU, 128GB RAM
```
...
```
One large server  →  10 small servers + Load Balancer
```
...
**Redundancy:** אין נקודת כשל יחידה. כל component קריטי כפול.
...
![איור מצבי Circuit Breaker: המפסק עובר מ-Closed ל-Open לאחר סף כשלים, אחרי timeout עובר ל-Half-Open לבדיקה, ובהצלחה חוזר ל-Closed](../../../images/illustrations/arch-4-1-circuit-breaker.svg)
