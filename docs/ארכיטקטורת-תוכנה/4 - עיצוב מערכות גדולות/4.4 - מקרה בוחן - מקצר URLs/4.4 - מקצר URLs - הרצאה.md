...
def generate_short_code(long_url: str) -> str:
    ...
    return encoded[:7]  # 7 characters = 62^7 = 3.5 trillion combinations
```
...
async def redirect(short_code: str):
    ...
    if not long_url:
        ...
        long_url = row[0]
        # Step 3: store in cache for 24 hours
        await redis.setex(f"url:{short_code}", 86400, long_url)

    # Step 4: update counter (async, non-blocking)
    background_tasks.add_task(increment_click_count, short_code)

    return RedirectResponse(url=long_url, status_code=302)
```

![איור זרימת redirect עם שכבת cache: הבקשה בודקת קודם את Redis, פגיעה חוזרת מיד, והחטאה פונה ל-Read Replica ואז שומרת את הערך ב-cache](../../../images/illustrations/arch-4-4-cache-aside.svg)
