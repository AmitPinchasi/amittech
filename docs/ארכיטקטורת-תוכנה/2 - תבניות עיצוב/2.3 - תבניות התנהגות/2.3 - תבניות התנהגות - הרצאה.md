...
תבניות התנהגות עוסקות בתקשורת ואחריות בין אובייקטים - מי מחליט מה, ואיך אובייקטים מגיבים לevents.
...
**הפתרון:** האובייקט "מפרסם" events, והמעוניינים "נרשמים" לevents שמעניינים אותם.
...
class TaskService:
    def complete_task(self, task_id: int):
        ...
        # TaskService knows everyone who needs to be notified - a problem!
...
class EmailNotificationListener(TaskEventListener):
    def on_task_event(self, event: TaskEvent):
        if event.event_type == "task_completed":
            print(f"Sending email: task '{event.task['title']}' completed")


class SlackNotificationListener(TaskEventListener):
    def on_task_event(self, event: TaskEvent):
        print(f"Slack: task '{event.task['title']}' changed - {event.event_type}")
...
class TaskService:
    ...
    def complete_task(self, task_id: int):
        task = self._tasks.get(task_id, {"id": task_id, "title": f"Task {task_id}"})
        task["status"] = "done"
        # TaskService doesn't know who's listening - it just publishes
...
![תבנית Observer: השירות מפרסם event ל-EventBus, וזה מודיע בזה אחר זה לכל הרשומים](../../../images/illustrations/arch-2-3-observer-notify.svg)
...
# usage
tasks = [
    {"title": "Fix bug", "priority": "high", "due_date": "2026-04-10"},
    {"title": "Write documentation", "priority": "low", "due_date": "2026-04-05"},
    {"title": "review code", "priority": "normal", "due_date": "2026-04-08"},
]

task_list = TaskList(tasks)

task_list.set_sort_strategy(SortByDueDate())
print("By date:", [t["title"] for t in task_list.get_sorted_tasks()])

task_list.set_sort_strategy(SortByPriority())
print("By priority:", [t["title"] for t in task_list.get_sorted_tasks()])

task_list.set_sort_strategy(SortByTitle())
print("By title:", [t["title"] for t in task_list.get_sorted_tasks()])
```

![תבנית Strategy: החלפת אסטרטגיית המיון בזמן ריצה משנה את סדר המשימות בתוצאה](../../../images/illustrations/arch-2-3-strategy-swap.svg)
...
class CreateTaskCommand(Command):
    ...
    def description(self) -> str:
        return f"Creating task: {self.title}"
...
class UpdateTaskStatusCommand(Command):
    ...
    def description(self) -> str:
        return f"Updating status of task {self.task_id} to {self.new_status}"
...
history.execute(CreateTaskCommand(db, "Fix bug", owner_id=1))
history.execute(UpdateTaskStatusCommand(db, 1, "in_progress"))
history.execute(UpdateTaskStatusCommand(db, 1, "done"))

print("History:", history.history())

history.undo()  # undoing "done"
history.undo()  # undoing "in_progress"

print("History after undo:", history.history())
```
