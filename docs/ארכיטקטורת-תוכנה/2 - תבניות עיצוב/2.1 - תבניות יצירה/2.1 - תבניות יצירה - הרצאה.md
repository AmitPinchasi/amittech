...
**הפתרון:** במקום לקרוא ישירות ל-constructor, קוראים לפונקציית "Factory" שמחליטה מה ליצור.
...
def notify(task_id: int, channel: str):
    if channel == "email":
        ...
        notifier.send(...)
    # every time we add a channel, we need to change this here
```
...
![תבנית Factory: הפונקציה מחפשת את הערוץ במילון ומחזירה את המחלקה המתאימה](../../../images/illustrations/arch-2-1-factory-select.svg)
...
task = Task("Test", "Description", 1, "high", True, None, "red", 3, datetime.now(), None)
```
...
# Usage - very readable
task = (
    TaskBuilder("Fix bug in login", owner_id=5)
    .with_description("Users report a problem logging in via Safari")
    .with_priority("high")
...
![תבנית Builder: כל מתודה מחזירה את הבונה עצמו כדי לאפשר שרשור קריאות עד לקריאת build](../../../images/illustrations/arch-2-1-builder-chain.svg)
