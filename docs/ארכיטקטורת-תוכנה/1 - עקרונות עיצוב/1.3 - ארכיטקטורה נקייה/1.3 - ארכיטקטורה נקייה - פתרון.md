...
class CreateProjectUseCase:
    ...
    def execute(self, name: str, description: str, owner_id: int) -> Project:
        ...
        if len(active_projects) >= self.MAX_PROJECTS_PER_USER:
            raise ValueError(f"Cannot create more than {self.MAX_PROJECTS_PER_USER} active projects")
...
def test_cannot_create_more_than_10_projects():
    ...
    # Create 10 projects
    for i in range(10):
        use_case.execute(f"Project {i + 1}", "description", owner_id=1)

    # The 11th attempt should fail
    with pytest.raises(ValueError, match="Cannot create"):
        use_case.execute("Project 11", "description", owner_id=1)
...
def test_can_create_project_with_valid_data():
    ...
    project = use_case.execute("My Project", "My project description", owner_id=1)
...
def test_cannot_create_project_with_empty_name():
    ...
    with pytest.raises(ValueError, match="empty"):
        use_case.execute("", "description", owner_id=1)
```
