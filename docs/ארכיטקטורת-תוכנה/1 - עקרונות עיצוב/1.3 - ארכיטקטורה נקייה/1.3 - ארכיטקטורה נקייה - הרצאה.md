...
- **הלוגיקה העסקית** נפרדת לחלוטין מ-frameworks, מסדי נתונים וinterface המשתמש
...
![ארכיטקטורה נקייה: התלות תמיד מצביעה פנימה, משכבה חיצונית אל שכבה פנימית](../../../images/illustrations/arch-1-3-dependency-direction.svg)
...
class TaskRepository:
    """Abstract interface - doesn't know which DB"""
...
class NotificationPort:
    """Abstract interface - doesn't know how to send"""
...
class CompleteTaskUseCase:
    ...
    def execute(self, task_id: int, requesting_user_id: int) -> Task:
        ...
        if task.owner_id != requesting_user_id:
            raise PermissionError("Only the owner can complete the task")
        task.complete()
...
## שכבה 3: Interface Adapters - מתאמי interface
...
def test_cannot_complete_task_of_another_user():
    repo = InMemoryTaskRepository()  # fake database for tests
    use_case = CompleteTaskUseCase(repo)

    task = repo.save(Task(id=0, title="test", ..., owner_id=1))

    with pytest.raises(PermissionError):
        use_case.execute(task.id, requesting_user_id=2)  # another user
```
