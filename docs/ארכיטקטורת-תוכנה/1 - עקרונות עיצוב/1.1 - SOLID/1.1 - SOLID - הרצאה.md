...
class Task:
    ...
    def save_to_db(self):
        # The Task class shouldn't know about the database
        import sqlite3
    ...
    def send_notification(self, email: str):
        # The Task class shouldn't know about sending emails
        import smtplib
        server = smtplib.SMTP("smtp.gmail.com", 587)
        server.sendmail("system@taskflow.com", email, f"Task {self.title} created")

    def to_json(self):
        # This is fine - converting itself
...
class NotificationService:
    def notify_task_created(self, task: Task, email: str):
        import smtplib
        server = smtplib.SMTP("smtp.gmail.com", 587)
        server.sendmail("system@taskflow.com", email, f"Task {task.title} created")
```
...
![עקרון הפתוח-סגור: השירות תלוי בממשק NotificationChannel וכל ערוץ הוא מחלקה נפרדת שמממשת אותו](../../../images/illustrations/arch-1-1-ocp-extension.svg)
...
class ReadOnlyStorage(Storage):
    def save(self, key: str, value: str):
        # Violates LSP - a subclass that doesn't implement what it promises
        raise NotImplementedError("Not allowed to save in ReadOnlyStorage")

    def delete(self, key: str):
        raise NotImplementedError("Not allowed to delete in ReadOnlyStorage")
```
...
class RedisCache(WritableStorage):
    ...
    def delete(self, key: str):
        print(f"Deleting from Redis: {key}")
...
## I - עקרון הפרדת הinterfaces - Interface Segregation Principle

**ההגדרה:** עדיף הרבה interfaces קטנים ומדויקים על פני interface אחד גדול.
...
class TaskService:
    def __init__(self):
        # Direct dependency on SQLite - can't be swapped!
...
class TaskService:
    def __init__(self, repository: TaskRepository):
        # Depends on the abstraction, not a specific implementation
