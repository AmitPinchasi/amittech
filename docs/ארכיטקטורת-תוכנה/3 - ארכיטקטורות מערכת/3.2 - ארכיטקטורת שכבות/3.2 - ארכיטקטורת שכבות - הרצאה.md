...
```
+---------------------------+
|   Presentation Layer      |  שכבת interface המשתמש (Routes, HTTP)
+---------------------------+
           |
           v
+---------------------------+
...
![איור זרימת קריאה היורדת דרך שכבת Presentation, שכבת Business, שכבת Data ומסד הנתונים, כאשר כל שכבה פונה רק לשכבה שמתחתיה](../../../images/illustrations/arch-3-2-layered-flow.svg)

---

## שכבה 1: Presentation Layer - שכבת interface
...
```python
# Not good - Task is empty of logic
class Task:
    def __init__(self, id, title, status): ...

class TaskService:
    def is_overdue(self, task): ...  # Logic of Task lives in the service?


# Good - Task contains logic related to it
class Task:
