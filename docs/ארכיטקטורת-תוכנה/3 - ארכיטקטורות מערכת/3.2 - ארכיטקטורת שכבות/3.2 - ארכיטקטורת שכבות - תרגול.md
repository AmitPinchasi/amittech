...
def create_task(title: str, owner_id: int):
    ...
    # Check 2: no more than 50 open tasks
    open_tasks = db.execute(
        "SELECT count(*) FROM tasks WHERE owner_id=? AND status != 'done'",
        (owner_id,)
    ).fetchone()[0]
    if open_tasks >= 50:
        return {"error": "Too many open tasks"}
    ...
    server.sendmail("system@app.com", user[2], f"New task: {title}")
...
3. `task_repository.find_overdue(owner_id: int)` - SQL query
