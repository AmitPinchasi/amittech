...
class TaskService:
    ...
    def create_task(self, title: str, owner_id: int) -> dict:
        ...
        if open_count >= self.MAX_OPEN_TASKS:
            raise ValueError(f"Cannot create more than {self.MAX_OPEN_TASKS} open tasks")

        task = self._task_repo.save(title, owner_id)
        self._email.send(user["email"], f"New task: {title}")
        return task
...
```python
# repositories/task_repository.py - addition
...
# services/task_service.py - addition
