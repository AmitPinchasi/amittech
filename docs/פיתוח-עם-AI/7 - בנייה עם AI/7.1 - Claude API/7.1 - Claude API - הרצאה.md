...
![איור המתאר את מחזור החיים של קריאת Messages API: הבקשה נשלחת עם model, max_tokens, system ו-messages, המודל מעבד ומחזיר תשובה עם content, stop_reason ו-usage, ובכל תור חדש שולחים שוב את כל ההיסטוריה כי ה-API חסר מצב](../../../images/illustrations/ai-7-1-messages-api-roundtrip.svg)
