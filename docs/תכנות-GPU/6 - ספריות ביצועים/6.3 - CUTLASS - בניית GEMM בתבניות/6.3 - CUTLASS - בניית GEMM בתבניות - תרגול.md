...
cmake .. -DCUTLASS_NVCC_ARCHS=90a          # 90a = H100; השתמשו ב-80 ל-A100, 75 ל-T4
make cutlass_profiler -j$(nproc)           # לוקח כמה דקות
```
...
2. במארח, הקצו `cutlass::HostTensor` ל-A, B, C, D בגודל 4096x4096. מלאו את A ו-B בערכים אקראיים קטנים (למשל בעזרת `cutlass::reference::host::TensorFillRandomUniform` או ידנית), וקראו ל-`sync_device()`.
...
3. אמתו את המיזוג: חשבו רפרנס בשני שלבים נפרדים - GEMM רגיל (CUTLASS או cuBLAS) ל-`D_tmp`, ואז kernel נפרד או קוד מארח שמחשב `ReLU(0.5*D_tmp)`. השוו יסוד-יסוד ל-D הממוזג.
...
4. נתחו: מדוע ה-kernel שלכם רחוק מ-cuBLAS/CUTLASS? מה חסר בו (double buffering, צנרת רב-שלבית, Tensor Cores)? מדוע CUTLASS ב-FP16/Tensor Core מנצח את שניהם ב-FP32?
5. כתבו פסקה: באיזה מצב הייתם בוחרים בכל אחד מהשלושה בעבודה אמיתית?

רמז: השוואה הוגנת דורשת אותו טיפוס נתונים ואותה צורה. ה-kernel שלכם מ-3.5 הוא ב-FP32 על CUDA Cores, ולכן השוו אותו ל-cuBLAS/CUTLASS ב-FP32 (שיא FP32 של H100 ≈ 67 TFLOPS, ראו 1.5), ובנפרד הראו כמה Tensor Cores ב-FP16 קופצים (שיא ≈ 990 TFLOPS, פי ~15). הפער בין ה-kernel שלכם ל-cuBLAS ב-FP32 מגלה כמה שווה הצנרת הרב-שלבית וה-double buffering שאין לכם.
