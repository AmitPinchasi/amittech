...
מדוע בכלל צריך את זה? כי להגיע לביצועי-שיא על Tensor Cores הוא קשה להחריד. צריך לתזמן ידנית את טעינת ה-tiles מ-global memory (זיכרון גלובלי) ל-shared memory (זיכרון משותף) בצנרת רב-שלבית (multi-stage pipeline) שמסתירה latency (השהיה), לנהל את ה-double buffering, למפות בדיוק את ה-threads ל-fragments של רגיסטרים, ולהזין את יחידות ה-MMA (Matrix Multiply-Accumulate) בקצב שלא מרעיב אותן. CUTLASS אורזת את כל המומחיות הזו כתבניות C++ שאפשר להרכיב מחדש. זו הסיבה שהיא **header-only** (ספרייה בכותרות בלבד): כל הקוד הוא תבניות שמתמחות (specialize) בזמן קומפילציה לטיפוסים, לצורות ולדור ה-SM (מעבד רב-זרמי) הקונקרטיים שלכם.

הטכניקה שמאפשרת את זה נקראת **פולימורפיזם פרמטרי - parametric polymorphism**: מימוש מתבנת יחיד שמתמחה בזמן קומפילציה למאות טיפוסים, צורות וארכיטקטורות. אתם כותבים `Gemm<half_t, RowMajor, ...>` והמהדר מייצר מזה kernel קונקרטי; מחליפים טיפוס אחד, ומקבלים kernel אחר לגמרי - אבל מאותו קוד מקור. זה ההפך הגמור מ-cuBLAS, שבה הבחירה נעשית בזמן ריצה ומוסתרת מכם.
...
```text
   ┌──────────────────────────────────────────────────────────┐
   │  device        רמת ה-GPU כולו                            │
   │  ─ cutlass::gemm::device::Gemm<...>                        │
   │  ─ ה-API שהמארח קורא לו: מקצה, משיק את ה-grid, מסנכרן    │
   │  ┌────────────────────────────────────────────────────┐  │
   │  │  kernel      מה שרץ כ-grid על ה-SM-ים               │  │
   │  │  ─ ה-__global__ עצמו: swizzle, לולאה על ה-tiles     │  │
   │  │  ┌──────────────────────────────────────────────┐  │  │
   │  │  │  collective   בדרך כלל ה-thread block          │  │  │
   │  │  │  ─ mainloop + epilogue                         │  │  │
   │  │  │  ─ כאן קורה כל הכפל-צבירה על ה-Tensor Cores    │  │  │
   │  │  └──────────────────────────────────────────────┘  │  │
   │  └────────────────────────────────────────────────────┘  │
   └──────────────────────────────────────────────────────────┘
```

- **רמת ה-device** היא ה-API שהמארח (host, ה-CPU) קורא לו. אובייקט כמו `cutlass::gemm::device::Gemm<...>` הוא כמו קריאת cuBLAS מבחוץ: מעבירים לו מצביעים, גדלים וסקלרים, והוא מקצה זיכרון-עזר, מחשב את תצורת ה-grid ומשיק את ה-kernel. זו הרמה שנעבוד בה ברוב השיעור, כי היא הכי קרובה למה שכבר מוכר לנו.
...
using Gemm = cutlass::gemm::device::Gemm<
    cutlass::half_t, cutlass::layout::RowMajor,      // A: FP16, שורות
    cutlass::half_t, cutlass::layout::ColumnMajor,   // B: FP16, עמודות
    cutlass::half_t, cutlass::layout::RowMajor,      // C/D: FP16, שורות
    float,                                           // accumulator: FP32
    cutlass::arch::OpClassTensorOp,                  // השתמש ב-Tensor Cores
    cutlass::arch::Sm80,                             // דור החומרה (Ampere; רץ גם על H100)
    ...
    cutlass::epilogue::thread::LinearCombination<
        cutlass::half_t,                             // טיפוס הפלט
        128 / cutlass::sizeof_bits<cutlass::half_t>::value,  // רוחב וקטור = 8
        float, float>,                               // accumulator, compute
    cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<>,
    3>;                                              // מספר שלבי הצנרת (stages)
```

עכשיו הצד המארח שמריץ אותו. שימו לב שאין `<<<...>>>` - אובייקט ה-`Gemm` הוא שמשיק את ה-kernel פנימית:
...
int main() {
    ...
    // HostTensor מקצה גם על המארח וגם על ההתקן ומסנכרן ביניהם
    ...
    // ... מילוי A,B בערכים, ואז A.sync_device(), B.sync_device() ...

    Gemm gemm_op;
    Gemm::Arguments args(
        {M, N, K},              // גודל הבעיה
        A.device_ref(),         // TensorRef כולל leading dimension
        B.device_ref(),
        C.device_ref(),
        D.device_ref(),         // פלט
        {alpha, beta});         // פרמטרי ה-epilogue

    cutlass::Status status = gemm_op(args);   // כאן מושק ה-kernel
    ...
    D.sync_host();              // מעתיק את התוצאה חזרה למארח
    return 0;
}
```
...
```bash
export CUTLASS=$HOME/cutlass          # שורש ה-repo שקלונתם
...
- **ThreadblockShape** - ה-tile של הפלט שכל thread block מחשב. במקטע שלנו `<128, 128, 32>`: כל block מחשב אריח 128x128 של D, ומעבד את ממד ה-K בפרוסות של 32.
...
```text
   warps ל-block = (TB_M / Warp_M) x (TB_N / Warp_N)
                 = (128 / 64)     x (128 / 64)
...
```text
   לכל stage:  (128*32 + 32*128) יסודות * 2 בתים
            =  (4096 + 4096) * 2  =  16384 בתים = 16 KiB
   ב-3 stages:  3 * 16 KiB = 48 KiB shared memory ל-block
```

ה-H100 מאפשר עד כ-228 KiB shared memory ל-SM (ראו 1.6), כך שיש מרווח גדול; אפשר להעלות ל-4 או 5 stages כדי להעמיק את הצנרת ולהסתיר טוב יותר את ה-latency של טעינת ה-tiles הבאים. הנה טבלת האיזון:
...
GemmReLU::Arguments args(
    ...
    {alpha, beta});          // ה-ReLU מוחל אוטומטית אחרי alpha*acc + beta*C
...
```text
   acc  = הצבירה מה-mainloop (ב-FP32, ברגיסטרים)
   x    = alpha * acc + beta * C[i][j]        (LinearCombination)
   D[i][j] = max(x, threshold)                (ה-ReLU שנוסף)   -> נכתב פעם אחת
```
...
using TileShape    = Shape<_128, _256, _64>;   // CTA tile: layout של CuTe
using ClusterShape = Shape<_2, _1, _1>;        // אשכול thread blocks (חדש ב-Hopper)

// ה-CollectiveBuilder בונה את ה-mainloop (WGMMA + TMA) אוטומטית
using CollectiveMainloop = typename cutlass::gemm::collective::CollectiveBuilder<
    ...
    cutlass::gemm::collective::StageCountAuto,      // מספר stages נבחר אוטומטית
    cutlass::gemm::collective::KernelScheduleAuto   // warp-specialization אוטומטי
>::CollectiveOp;
```
...
הנתיב הזה משתמש גם ב-**warp specialization**: חלק מה-warps ב-block מוקדשים ל-producer (טעינה עם TMA) וחלק ל-consumer (חישוב עם WGMMA), בצנרת עמוקה שמסתירה latency טוב יותר מכל מה שאפשר על Ampere. זו הסיבה ש-NVIDIA ממליצה על CUTLASS דווקא ל-Transformers על Hopper ו-Blackwell (כפי שציינו ב-6.2 על cuDNN): את ה-kernels הכי מהירים לחומרה הכי חדשה בונים ב-CUTLASS 3.x, לא קוראים מספרייה סגורה.
...
cmake .. -DCUTLASS_NVCC_ARCHS=90a       # 90a = ארכיטקטורת H100
make cutlass_profiler -j$(nproc)
# מודד את כל וריאנטי ה-GEMM ב-FP16 לגודל נתון, מדווח GFLOP/s לכל אחד
...
- הספרייה בנויה בשלוש רמות - device (ה-API שהמארח קורא לו), kernel (ה-grid שרץ על ה-SM-ים), ו-collective (בדרך כלל ה-thread block) - המקבילות להיררכיית החומרה.
