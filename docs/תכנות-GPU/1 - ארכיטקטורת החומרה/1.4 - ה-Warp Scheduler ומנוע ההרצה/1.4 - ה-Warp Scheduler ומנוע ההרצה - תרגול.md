...
// kernel עם הרבה משתנים חיים בו-זמנית -> לחץ אוגרים מכוון
...
עכשיו נראה את המהדר מדווח על האוגרים.
...
רמז: `-Xptxas -v` מעביר את הדגל `-v` ל-ptxas (המרכיב ש-nvcc מפעיל כדי להפוך PTX ל-SASS). כש-`-maxrregcount` נמוך מדי, ה-kernel לא נכנס במגבלה והמהדר "שופך" משתנים ל-local memory (שיושב ב-global) - זה ה-spill, וזה עצמו פוגע בביצועים גם אם ה-occupancy עולה.
