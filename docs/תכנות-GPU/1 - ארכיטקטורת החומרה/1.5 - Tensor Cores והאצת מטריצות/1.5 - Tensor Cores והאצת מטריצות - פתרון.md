...
כאן נעבור על הפתרון המלא של כל תרגיל, עם הקוד, הפקודות והפלט המדויקים. שימו לב: פלטי ה-SASS, שמות ההוראות ומספרי ה-registers הם דוגמאות מהרצה על H100 (`sm_90a`) עם גרסת CUDA Toolkit מסוימת, והם ישתנו מעט בין ארכיטקטורות וגרסאות מהדר - אבל המבנה, ההיגיון והמסקנות זהים. אם אתם על T4 ב-Colab, החליפו `sm_90a` ב-`sm_75`.
...
int main() {
    ...
    float alpha = 1.0f, beta = 0.0f;   // עבור CUBLAS_COMPUTE_32F הסקלרים הם float
    // C = alpha * A*B + beta*C ; cuBLAS היא column-major, המטריצות מרובעות
...
**מה רואים כאן.** ה-`wmma::mma_sync` הבודד על אריח `m16n16k16` התפרק לשתי הוראות `HMMA.16816.F32` בקומפילציה ל-Hopper. כל `HMMA.16816` היא אריח חומרה `m16 n8 k16 = 16*8*16 = 2,048` MAC; שתיים מהן מכסות `2 * 2,048 = 4,096` MAC, וזה בדיוק גודל ה-`wmma` הלוגי `m16n16k16` (כי `16*16*16 = 4,096`). על ארכיטקטורות ישנות יותר (למשל Turing / `sm_75`) אותו kernel מתפרק לארבע הוראות `HMMA.1688.F32` (אריח `m16 n8 k8 = 1,024` MAC כל אחת, `4 * 1,024 = 4,096`) - שתי צורות שונות של אותו פירוק, לפי מה שהחומרה של הדור מציעה.
...
**למה זה עבד:** קימפלנו לארכיטקטורה עם ליבות טנזור (`sm_90a`), והשארנו את התוצאה נכתבת לזיכרון (`store_matrix_sync`), כך שהמהדר לא חיסל את החישוב כ-dead code. ה-`grep -i hmma` תפס את הוראות ליבת הטנזור בקוד המכונה. ראינו במו עינינו את מה שההרצאה טענה: הוראת `wmma` אחת ברמה הגבוהה **אינה** הוראת SASS אחת אלא מתפרקת לכמה הוראות `HMMA` קטנות.
...
ההוראה `HMMA16.16816.F32`: אריח `m=16, n=8, k=16`.
...
הערך גדול מאפס - הוכחה שהוראות עברו דרך צינור ליבת הטנזור. לשם השוואה, הרצת kernel של כפל מטריצות תמים על CUDA Cores תחת אותה מטריקה:
...
**למה זה עבד:** המטריקה `sm__inst_executed_pipe_tensor.sum` סופרת כמה הוראות בוצעו בצינור ליבת הטנזור על פני כל ה-SMs. ערך חיובי אומר חד-משמעית שהחומרה הפעילה את ליבות הטנזור; אפס אומר שהחישוב רץ כולו על ה-CUDA Cores. שימו לב ששם ה-kernel בפלט הוא kernel של CUTLASS - cuBLAS מממשת את ה-GEMM שלה מעל תבניות CUTLASS, בדיוק כפי שההרצאה תיארה: אתם קוראים לספרייה, והיא מנפיקה את הוראות ה-`HMMA`.

**איך להכליל:** זו טכניקת האימות הכללית ל"האם המנוע היקר באמת רץ". `sm__pipe_tensor_cycles_active.avg.pct_of_peak_sustained_active` נותנת את אחוז ניצול צינור ליבת הטנזור (כמה קרוב לשיא), ו-`ncu --set full` מציגה טבלת ניצול צינורות מלאה שבה שורת "Tensor" גלויה. כשתבנו GEMM ידני בפרויקט ותשפרו אותו שלב-שלב עד לליבות טנזור, המטריקות האלה יראו לכם בדיוק מתי המעבר קרה ובכמה אתם רחוקים מהשיא.


### פתרון תרגיל 5 - TMA מול זיכרון טנזור: הסבר מושגי

**1. מהו ה-TMA?** ה-Tensor Memory Accelerator הוא מנוע העתקה אסינכרוני ייעודי (Hopper ו-Blackwell) שמעביר נתונים **מזיכרון ה-GPU / global memory אל ה-shared memory / מטמון L1**, תוך **עקיפה מלאה של קובץ האוגרים**. הוא מחשב בחומרה כתובות affine (`addr = width * base + offset`) של גישות רב-ממדיות גדולות, ובכך מפנה גם אוגרים וגם CUDA Cores.
...
**3. המלכוד.** המשפט "ה-TMA מאיץ את זיכרון הטנזור" **שגוי** כי חרף השמות הדומים, ה-TMA **אינו** נוגע בזיכרון הטנזור כלל. ה-TMA טוען אך ורק אל ה-shared memory / מטמון L1. הנתונים מגיעים אל זיכרון הטנזור בדרך אחרת לגמרי - **רק דרך ליבות הטנזור / הוראות MMA** (או טעינה מ-shared memory ומאוגרים ברמת warpgroup). ה-TMA הוא מנוע *תנועה* אל ה-shared memory; זיכרון הטנזור הוא *מקום אחסון* שרק ליבות הטנזור ממלאות.
...
```text
   HBM (זיכרון ה-GPU)
        |
        |  <=== ה-TMA קופץ רק את החץ הזה (אל shared memory)
        v
   shared memory / L1
        |
        |  <=== דרך ליבות הטנזור / הוראות MMA בלבד (לא ה-TMA!)
        v
   Tensor Memory (TMEM, רק Blackwell)
```
...
**איך להכליל:** אותה משמעת - "מה הרכיב ולאן הוא כותב" - פותרת את רוב מלכודי השמות בהיררכיית הזיכרון של ה-GPU: `libcuda.so` מול `libcudart.so`, מטמון L1 מול shared memory (אותו SRAM פיזי, שני שמות), ו-`__global__` (מגדיר kernel) מול global memory (זיכרון). כשתגיעו ל-CUTLASS ול-CuTe בפרקים 6 עד 8, מסלול הנתונים HBM -> TMA -> shared -> ליבת טנזור -> TMEM יהיה בדיוק הצינור שתכתבו ותכווננו.


### פתרון תרגיל 6 (בונוס) - מ-wmma ל-wgmma

**1. למה Hopper הוסיפה `wgmma`?** ה-`wmma` הרגיל שיתופי ברמת warp בודד (32 threads), והאריח שלו קטן מכדי לרווֹת את רוחב הפס האריתמטי העצום של ליבות הטנזור בכרטיסי מרכזי-נתונים. ה-`wgmma.mma_async` פותר זאת בשני אופנים: (א) **אריח גדול יותר** - הוא שיתופי ברמת warpgroup (128 threads), ואריח גדול יותר ממלא את ליבת הטנזור ומונע רעב; (ב) **אסינכרוניות** - ההוראה משוגרת וממשיכה לרוץ ברקע בזמן שהthreads עושים עבודה אחרת, ובכך מבטלת את תקורת הסנכרון המפורש בין ארבעת ה-warps ומאפשרת לחפוף תנועת זיכרון עם חישוב.
...
**למה זה עבד:** אותה נוסחה, `m*n*k`, רק על אריח גדול בהרבה. הקפיצה מ-2,048 ל-262,144 MAC בהוראה אחת היא בדיוק הרעיון של ה-warpgroup: לרווֹת את ליבות הטנזור עם כמה שיותר עבודה לכל שליפת הוראה, ולעשות זאת אסינכרונית כדי לחפוף עם תנועת נתונים.

**איך להכליל:** המגמה בכל דור היא אריחים גדולים יותר, אסינכרוניים יותר, וקרובים יותר לחומרה: Volta הביאה `wmma` (warp), Hopper הביאה `wgmma` (warpgroup) ואת ה-TMA להזנתו, ו-Blackwell הביאה את `tcgen05.mma` ואת זיכרון הטנזור. הקרנלים המהירים ביותר של ימינו - Flash Attention, ה-GEMM של CUTLASS - בנויים כולם סביב שרשור אריחי ה-warpgroup האסינכרוניים עם ה-TMA. את הבנייה המעשית של הצינור הזה נלמד בפרקים 6 עד 8.
