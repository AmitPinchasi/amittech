...
![איור מונפש של טעינה שיתופית של tile: כל thread מעתיק איבר אחד מ-global memory אל ה-shared memory של ה-block](../../../images/illustrations/gpu-3-3-tile-cooperative-load.svg)
...
```cpp
__shared__ float As[TILE_WIDTH][TILE_WIDTH + 1];   // +1 shifts the bank mapping
```
...
- ב-matmul מרוצף כל block צובר tile של `C` בשלבים, כשבכל שלב נטענים tile של `A` וtile של `B` ל-shared memory; כל איבר טעון מנוצל `TILE_WIDTH` פעמים, וarithmetic intensity קופצת מ-1 ל-`TILE_WIDTH` FLOPs לכל קריאה.
...
- נדרשים בדיוק שני `__syncthreads()`: מחסום הטעינה (מונע קריאה-לפני-כתיבה) לפני החישוב, ומחסום הניצול-חוזר (מונע כתיבה-לפני-קריאה, דריסת הtile) אחריו; השמטת השני היא הבאג הקלאסי שנותן תוצאות שגויות ולא דטרמיניסטיות.
- `__syncthreads()` חייב במסלול שכל ה-threads של ה-block מגיעים אליו, אחרת ה-block נתקע ב-deadlock.
- ה-shared memory שכל block צורך מגביל את מספר ה-blocks התושבים על SM ולכן את ה-occupancy; בtiles קטנים המגביל הוא בדרך כלל מכסת ה-threads, אך tile גדול הופך את ה-shared memory לצוואר (הרחבה בפרק 8.1, החלוקה הסטטית מוגבלת ל-48 KiB לכל block).
