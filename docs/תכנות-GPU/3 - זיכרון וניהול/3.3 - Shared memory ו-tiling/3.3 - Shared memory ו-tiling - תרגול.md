...
// תרגיל 1: השלימו את ה-kernel המרוצף כאן.
__global__ void mmTiled(const float* A, const float* B, float* C, int N) {
    // ...
}

// בדיקת נכונות פשוטה על ה-CPU (איטית - השתמשו ב-N קטן לאימות בלבד)
...
int main(int argc, char** argv) {
    ...
    // חימום + מדידה (החליפו mmNaive ל-mmTiled כשתשלימו אותו)
...
רמז: הטעינה השיתופית היא `As[ty][tx] = (row < N && aCol < N) ? A[row*N + aCol] : 0.0f;` כאשר `aCol = t*TILE_WIDTH + tx`, ובאופן דומה `Bs[ty][tx]` עם `bRow = t*TILE_WIDTH + ty`. הלולאה הפנימית היא `for (int k = 0; k < TILE_WIDTH; k++) sum += As[ty][k] * Bs[k][tx];`. אם קיבלתם תוצאה שגויה רק בקצוות, כנראה שכחתם את ריפוד האפסים לאריחים חלקיים.
...
3. חשבו לכל ערך את העצימות האריתמטית התיאורטית (FLOPs לכל קריאת global) - היא אמורה לצאת `TILE_WIDTH`.
4. איזה ערך היה הכי מהיר על הכרטיס שלכם? האם `32` תמיד מנצח את `16`? הסבירו.

רמז: `TILE_WIDTH = 8` נותן block של 64 threads (שני warps) - עצימות אריתמטית נמוכה ומעט ניצול-חוזר. `TILE_WIDTH = 32` נותן block של 1024 threads (מקסימום) ו-8 KiB shared. לפעמים `32` מנצח ולפעמים `16`, כי `32` צורך יותר משאבים ומוריד occupancy (ראו 8.1). בתי ה-shared הם `2 * TILE_WIDTH * TILE_WIDTH * 4`.

### תרגיל 4 - המרה ל-shared memory דינמי

1. כתבו גרסה `mmTiledDyn(const float* A, const float* B, float* C, int N, int tile)` שבה גודל האריח מגיע כפרמטר בזמן ריצה, וה-shared memory מוכרז `extern __shared__ float smem[];`.
...
רמז: הפרמטר השלישי של `<<< >>>` הוא **מספר הבתים** של ה-shared הדינמי, לא מספר האיברים - אל תשכחו את `sizeof(float)`. אם קיבלתם `invalid argument` בהשקה, כנראה ביקשתם יותר shared ממה שמותר; לחריגה מ-48 KiB צריך `cudaFuncSetAttribute(mmTiledDyn, cudaFuncAttributeMaxDynamicSharedMemorySize, bytes)`. הביצועים אמורים להיות כמעט זהים לסטטי - האינדוקס הידני אינו יקר יותר.
...
רמז: השמטת המחסום השני יוצרת סכנת כתיבה-לפני-קריאה (write-after-read): warp מהיר דורס את האריח בשלב הבא בעוד warp איטי עדיין קורא ממנו. התלות בתזמון ה-warps היא שהופכת את השגיאה ללא-יציבה בין הרצות. ל-`N` קטן שנכנס באריח יחיד (`N <= TILE_WIDTH`) יש רק שלב אחד, ואז הבאג עלול לא להתגלות - השתמשו ב-`N` גדול בהרבה מ-`TILE_WIDTH` כדי לכפות כמה שלבים.
...
1. הריצו את הגרסה המרוצפת תחת `ncu` והשוו לנאיבי, למשל:
   ...
   ```
2. השוו בין הנאיבי למרוצף את המדדים: `DRAM Throughput`, `Compute (SM) Throughput`, ו-`Achieved Occupancy`. ראו איך הנאיבי חסום-זיכרון (DRAM גבוה, SM נמוך) בעוד המרוצף מזיז את המשקל לכיוון החישוב.
