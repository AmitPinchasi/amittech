...
בתרגול הזה תמפו בעצמכם את היררכיית הזיכרון על קוד אמיתי. תסמנו היכן חי כל משתנה ב-kernel נתון, תגרמו למשתנה לגלוש ל-local memory ותראו את הגלישה בעיניים בעזרת ה-`ptxas`, תוכיחו במדידה ש-shared memory הוא פרטי ל-block, ולבסוף תנתחו איזה תיאום בין blocks חוקי ואיזה מוביל ל-deadlock. חלק מהתרגילים הם ניתוחיים (קריאת פלט מהדר, נימוק על נכונות) וחלק דורשים הרצה על GPU. עבדו לפי הסדר. הריצו על GPU אמיתי; אם אין לכם, השתמשו ב-Google Colab (T4 חינמי) או במכונת ענן, כפי שראינו בשיעור 0.2. ה-H100 הוא כרטיס הייחוס, אך כל התרגילים ירוצו על כל GPU של NVIDIA.
...
__global__ void heavy(const int* idx, const float* in, float* out, int n) {
    float scratch[64];                       // גדול; מועמד לגלישה
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
...
__global__ void privacy(int* out) {
    ...
    if (threadIdx.x == 0) marker = blockIdx.x;   // כל block כותב את הזהות שלו
    __syncthreads();
    int gid = blockIdx.x * blockDim.x + threadIdx.x;
    out[gid] = marker;                            // כל thread מדווח מה הוא ראה
}
```
...
```cpp
// גרסה A: כל block מוסיף למונה גלובלי
__global__ void versionA(const float* in, int* counter, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n && in[i] > 0.0f) atomicAdd(counter, 1);
}

// גרסה B: block 0 ממתין עד ש-block אחרון יסמן שסיים
__global__ void versionB(volatile int* flag) {
    if (blockIdx.x == gridDim.x - 1 && threadIdx.x == 0)
        *flag = 1;                       // ה-block האחרון מסמן
    if (blockIdx.x == 0 && threadIdx.x == 0)
        while (*flag == 0) { /* spin */ }// block 0 מסתובב בהמתנה
}
```
...
4. חשבו: כמה `ld.global` יש בגוף, וכמה `st.global`? האם זה תואם לכך שהחישוב חסום-זיכרון?

רמז: ה-PTX חושף בדיוק את מה שהיה בלתי-נראה ב-CUDA C - הרגיסטרים. חפשו שורות כמו `.reg .f32 %f<N>;` ו-`.reg .b32 %r<N>;`. פקודת `ld.global.f32` היא הקריאה מ-global, `st.global.f32` היא הכתיבה. תראו קריאה אחת (`in[i]`) וכתיבה אחת (`out[i]`) - יחס גישות זיכרון גבוה לפעולה אריתמטית בודדת, בדיוק הסימן לחישוב חסום-זיכרון.
