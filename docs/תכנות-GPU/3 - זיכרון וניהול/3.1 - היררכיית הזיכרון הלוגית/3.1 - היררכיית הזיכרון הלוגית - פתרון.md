...
הערה: מספרי האוגרים, גדלי ה-stack frame ופלטי ה-`ptxas` שלהלן הם דוגמאות מ-H100 עם `sm_90a` וגרסת CUDA מסוימת. הערכים המדויקים משתנים בין ארכיטקטורות וגרסאות מהדר - מה שקבוע הוא ה**דפוס** (איזה kernel גולש ל-local memory, מי מגביל את התפוסה), לא הספרה האחרונה. הריצו אצלכם וראו את המספרים שלכם.
...
```text
A  coeff[4]           -> constant   (__constant__, לקריאה בלבד, נכתב מהמארח)
B  tile[128]          -> shared     (__shared__, משותף ל-block)
C  i                  -> register   (סקלר מקומי)
D  acc                -> register   (סקלר מקומי מצטבר)
E  buf[8]             -> local       (מערך מקומי באינדוקס דינמי)
F  in                 -> global      (מצביע מ-cudaMalloc)
G  coeff[k]/tile[..]  -> constant + shared (קריאה משני מרחבים)
H  buf[k % n]         -> local       (כתיבה למערך שב-local memory)
I  out                -> global      (מצביע מ-cudaMalloc)
```
...
**(3) `coeff` מול `in`:** שניהם בהיקף grid (כל הthreads רואים אותם), אבל `coeff` הוא constant memory - לקריאה בלבד מה-kernel, נכתב מהמארח לפני ההשקה, ממוטמן וממוטב לשידור כשכל ה-warp קורא את אותה כתובת. `in` הוא global memory רגיל - קריא וכתיב, הוקצה ב-`cudaMalloc`, וללא הטבת השידור.
...
**(3) הדרך הנכונה - שני kernels.** גבול ה-kernel הוא נקודת סנכרון גלובלית טבעית. מפצלים את הלוגיקה: kernel ראשון עושה את "שלב הכותב", המארח ממתין לסיומו (`cudaDeviceSynchronize` או פשוט ההשקה הבאה באותו stream), ואז kernel שני עושה את "שלב הקורא":

```cpp
producer<<<grid, block>>>(d_data);   // כל ה-blocks מסיימים כאן
consumer<<<grid, block>>>(d_data);   // מתחיל רק אחרי שהקודם הסתיים לגמרי
```
...
**איך להכליל:** הכלל: תיאום **בתוך** block - `__syncthreads()`; תיאום נתונים **בין** blocks - atomics; סנכרון **מלא** בין כל ה-grid - סיום kernel והשקת kernel חדש (או `grid.sync()` של cooperative groups עם השקה מיוחדת שמבטיחה תושבוּת). לעולם אל תבנו ממתין-מסתובב בין blocks של אותו kernel.
...
```text
threads תושבים מקסימלי מבחינת אוגרים = 65536 / 32 = 2048
מקסימום פיזי של H100 לכל SM               = 2048
=> קובץ האוגרים לא מגביל: 2048 = 2048, מגיעים לתפוסה מלאה.
```
...
```text
65536 / 16 = 4096  >  2048
=> בוודאי לא מגביל; הגבול הוא מספר ה-threads המקסימלי (2048), לא האוגרים.
```
...
```text
65536 / 64 = 1024  <  2048
=> האוגרים חותכים את התפוסה לחצי: רק 1024 threads תושבים במקום 2048.
   פחות warps תושבים -> פחות יכולת להסתיר latency.
```
...
**למה זה עבד:** ה-PTX חושף בדיוק את מה שהיה בלתי-נראה ב-CUDA C - הרגיסטרים הווירטואליים בני 32 סיביות. הצהרות ה-`.reg` הן ההכרזות עליהם, ופקודות ה-`ld.global`/`st.global` מסמנות במפורש כל מגע עם global memory. יחס של שתי גישות זיכרון לפעולת חשבון אחת הוא בדיוק החתימה של חישוב חסום-זיכרון.

**איך להכליל:** קריאת PTX היא הדרך לראות את היררכיית הזיכרון "מבפנים": `.reg` = רגיסטרים, `ld/st.global` = global, `ld/st.shared` = shared, `ld.const` = constant, ו-`ld/st.local` = local (הסימן לגלישה). כשרוצים להבין למה kernel איטי, ספירת פקודות ה-`ld.global` מול הפעולות האריתמטיות נותנת אומדן מיידי לעצימות האריתמטית - הכלי שנפַתח פורמלית בפרק על ביצועים.
