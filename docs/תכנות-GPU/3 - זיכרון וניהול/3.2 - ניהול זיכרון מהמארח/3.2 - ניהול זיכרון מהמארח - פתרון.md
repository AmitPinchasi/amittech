...
static float timeH2D(void* dst, const void* src, size_t bytes, int reps) {
    ...
    CUDA_CHECK(cudaMemcpy(dst, src, bytes, cudaMemcpyHostToDevice)); // חימום
    float best = 1e30f;
    for (int r = 0; r < reps; r++) {
        ...
        if (ms < best) best = ms;   // הזמן הטוב ביותר = הכי פחות רעש
    }
...
int main(void) {
    ...
    float *a, *b, *c;
    CUDA_CHECK(cudaMallocManaged(&a, bytes));   // מצביע יחיד לשני העולמות
    CUDA_CHECK(cudaMallocManaged(&b, bytes));
    CUDA_CHECK(cudaMallocManaged(&c, bytes));

    for (int i = 0; i < n; i++) { a[i] = 1.0f; b[i] = 2.0f; }  // כתיבה מהמארח

    int bs = 256, nb = (n + bs - 1) / bs;
    vecAdd<<<nb, bs>>>(a, b, c, n);              // אותו מצביע ל-kernel
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());          // חובה לפני קריאה מהמארח

    double maxErr = 0.0;
    for (int i = 0; i < n; i++) {                 // קריאה מהמארח - בלי cudaMemcpy
        double d = (double)c[i] - 3.0; if (d < 0) d = -d;
        if (d > maxErr) maxErr = d;
    }
    printf("managed vecadd: max error = %f, cudaMemcpy calls = 0\n", maxErr);

    CUDA_CHECK(cudaFree(a));                       // cudaFree, לא cudaFreeHost
...
למה זה עבד: `cudaMallocManaged` מחזיר מצביע יחיד שתקף גם על המארח וגם על ההתקן. כשהמארח כותב ל-`a[i]`, page fault ממקם את העמוד ב-RAM; כשה-kernel ניגש אליו, page fault של ה-GPU מהגר אותו ל-HBM3; וכשהמארח קורא את `c[i]`, העמוד מהגר בחזרה. כל ההעברות קורות אוטומטית, ולכן אין ולו `cudaMemcpy` אחד. ה-`cudaDeviceSynchronize` הכרחי כי הקריאה מהמארח חייבת לחכות שה-kernel יסיים לכתוב.
...
static float runManaged(int n, bool prefetch) {
    ...
    for (int i = 0; i < n; i++) { a[i] = 1.0f; b[i] = 2.0f; }  // הנתונים ב-RAM
    ...
    CUDA_CHECK(cudaEventRecord(s));
    if (prefetch) {
        CUDA_CHECK(cudaMemPrefetchAsync(a, bytes, device));    // דחוף ל-GPU מראש
        CUDA_CHECK(cudaMemPrefetchAsync(b, bytes, device));
    }
    vecAdd<<<nb, bs>>>(a, b, c, n);
    if (prefetch)
        CUDA_CHECK(cudaMemPrefetchAsync(c, bytes, cudaCpuDeviceId)); // חזרה למארח
    CUDA_CHECK(cudaEventRecord(e));
    CUDA_CHECK(cudaEventSynchronize(e));
    volatile float sink = c[n - 1];  // מכריח את התוצאה להגיע למארח
    (void)sink;
...
int main(void) {
    ...
    size_t bytes = (size_t)n * sizeof(float);
    float *h_a, *h_b, *h_c;
    CUDA_CHECK(cudaMallocHost(&h_a, bytes));   // pinned - הכרחי לחפיפה
    ...
    serial(d_a, d_b, d_c, h_a, h_b, h_c, n);              // חימום
...
למה זה עבד: פירוק ל-8 נתחים על שני streams לא-default נותן ל-GPU להריץ במקביל פעולות מ-streams שונים. בזמן שה-DMA engine מעתיק H2D של נתח אחד, מנוע אחר יכול להעתיק D2H של נתח קודם, וה-CUDA Cores מריצים את ה-vecAdd של נתח שלישי. ה-`cudaMemcpyAsync` באמת אסינכרוני **רק** מפני שזיכרון המארח pinned - עם pageable הוא היה מסתנכרן ואיבדנו את החפיפה. חיבור וקטורים חסום-זיכרון וזול-חישוב, ולכן החפיפה כאן היא בעיקר בין ההעברות בכיוונים ההפוכים, ומכאן ה-speedup המתון של כ-1.5x.
...
int main(void) {
    ...
    vecAdd<<<nb, bs>>>(d_a, d_b, d_c, n);   // חימום
...
// חלק ב - kernel כבד: הרבה חישוב לכל איבר -> חפיפה טובה יותר
__global__ void heavy(const float* a, const float* b, float* c, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        float v = a[i] + b[i];
        for (int k = 0; k < 256; k++) v = v * 1.0001f + 0.5f;   // עומס אריתמטי
        c[i] = v;
    }
}
```
...
touch kernel over-subscribed: 412.7 ms   (איטי - החלפת עמודים פנימה/החוצה)
```
...
למה זה עבד: בחלק א, `cudaMallocManaged` מצליח להקצות 30 GiB על כרטיס עם 24 GiB כי הזיכרון המנוהל הוא וירטואלי - ה-runtime מחליף עמודים בין ה-HBM3 ל-RAM המארח לפי דרישה, בדיוק כמו swap. `cudaMalloc` היה נכשל כאן ב-`out of memory`. המחיר: ה-kernel איטי מאוד כי הוא מחכה כל הזמן להחלפות עמודים. בחלק ב, ה-kernel הכבד מבצע 256 פעולות לכל איבר, כך שזמן החישוב לנתח מתקרב לזמן ההעתקה שלו; לפי מודל ה-`max(copy, compute)` מההרצאה, זה בדיוק המצב שבו החפיפה מקסימלית, ומכאן speedup כמעט פי 2 לעומת ה-1.5x של חיבור הווקטורים הקל.
