בשיעור 0.3 כתבנו את חיבור הווקטורים הראשון וראינו את כוריאוגרפיית חמשת השלבים - `cudaMalloc`, `cudaMemcpy` H2D, השקה, `cudaMemcpy` D2H, `cudaFree` - ובשיעור 3.1 מיפינו את היררכיית הזיכרון של ההתקן מבפנים (registers, shared memory, global memory) מנקודת המבט של ה-kernel. בשיעור הזה אנחנו חוזרים אל צד המארח (host, כלומר ה-CPU) ושואלים את השאלה המעשית שמלווה כל תוכנית CUDA רצינית: איך מנהלים את הזיכרון של ההתקן ביעילות מהמארח? נעמיק בכל אחד מכלי הניהול - ההקצאה וההעתקה הבסיסיות, זיכרון מארח נעול-עמודים (pinned) שפותח את הדלת להעברות מהירות ואסינכרוניות, זיכרון מנוהל (managed / unified) שמטשטש את הגבול בין המרחבים ומעביר עמודים לפי דרישה, וה-streams (זרמים) שמאפשרים לחפוף העתקה עם חישוב במקום לשלם עליהם בזה אחר זה. את ה-streams נציג כאן ברמת המבוא ההכרחית כדי לחפוף העברות, ואת הטיפול המלא בהם - תלויות, עדיפויות, callbacks - נפרוש בשיעור 5.3. המסר המרכזי: לרוב, צוואר הבקבוק של תוכנית CUDA איננו ה-kernel אלא תנועת הנתונים סביבו, וכאן נלמד לשלוט בה.

## הבסיס - ההקצאה, ההעתקה והכיוונים - allocate, copy, directions

נחזור בקצרה על שלישיית הפונקציות מ-CUDA Runtime API (הממשק ברמה הגבוהה עם התחילית `cuda`, כפי שראינו בשיעור 0.4), הפעם עם דגש על הפרטים שדילגנו עליהם. `cudaMalloc(void** ptr, size_t bytes)` מקצה בלוק רציף ב-global memory של ההתקן ומחזיר קוד שגיאה; הכתובת נכתבת לתוך המצביע שלכם. `cudaFree(ptr)` משחרר אותו. `cudaMemcpy(dst, src, bytes, kind)` מעתיק בתים בין המרחבים, כשהפרמטר `kind` קובע את הכיוון:

| ערך `kind` | כינוי | כיוון |
|---|---|---|
| `cudaMemcpyHostToDevice` | H2D | מארח -> התקן (מעלים קלט) |
| `cudaMemcpyDeviceToHost` | D2H | התקן -> מארח (מורידים תוצאה) |
| `cudaMemcpyDeviceToDevice` | D2D | התקן -> התקן (בתוך אותו GPU) |
| `cudaMemcpyHostToHost` | H2H | מארח -> מארח (נדיר, כמו `memcpy`) |

העתקת D2D שימושית כשמעתיקים בין שני buffers על אותו כרטיס בלי לצאת אל המארח - היא רצה על רוחב הפס של ה-HBM3 (ב-H100, בסדר גודל של 3.35 TB/s) ולא על ה-PCIe האיטי. יש גם `cudaMemcpyDefault`, שמסיק את הכיוון לבד מסוג המצביעים באמצעות Unified Virtual Addressing - נוח, אבל עדיף לציין כיוון מפורש כדי שהקוד יתעד את כוונתו.
...
**נקודה קריטית שנחזור אליה מיד:** `cudaMemcpy` הרגילה היא **חוסמת (blocking / synchronous)** מנקודת מבט המארח - היא לא חוזרת עד שההעתקה הסתיימה. זה נוח וקל להבנה, אבל זה גם אומר שבזמן ההעברה ה-CPU עומד ומחכה, וה-GPU אינו יכול לחפוף אליה חישוב. כדי לשבור את המחסום הזה נצטרך שני דברים חדשים: זיכרון מארח מסוג pinned, ו-copy אסינכרוני על stream. נתחיל מהזיכרון.

## זיכרון נעול-עמודים - pinned (page-locked) memory

כדי להבין למה זיכרון pinned מהיר יותר, צריך להיזכר איך מערכת ההפעלה מנהלת את זיכרון המארח. הקצאה רגילה ב-`malloc` מחזירה זיכרון **בר-החלפה (pageable)**: מערכת ההפעלה רשאית להזיז את העמודים הפיזיים שלו, אפילו לגרש אותם אל הדיסק (swap), ולכן הכתובת הפיזית של buffer יכולה להשתנות מתחת לרגליים. ה-DMA engine (מנוע ההעברה הישירה) שעל ה-GPU, לעומת זאת, עובד מול **כתובות פיזיות** וחייב שהן יישארו קבועות לאורך כל ההעברה.

מכאן נובע מנגנון ה-bounce buffer (חוצץ ביניים) שקורה מאחורי הקלעים כשמעתיקים מזיכרון pageable:

```text
   cudaMemcpy מזיכרון pageable (הדרך האיטית):

   [ h_a - pageable ]                    [ d_a - GPU ]
        │                                     ▲
        │ 1. הדרייבר מעתיק (CPU memcpy)         │ 2. DMA
        ▼    לחוצץ נעול פנימי                    │    מהחוצץ הנעול
   [ pinned staging buffer ]  ──────────────────┘

   שתי העתקות: קודם CPU-copy אל החוצץ, ואז DMA אל ה-GPU.
```

הדרייבר מקצה חוצץ נעול פנימי, מעתיק אליו את הנתונים בעותק CPU רגיל, ורק ממנו מבצע את ה-DMA אל ההתקן. עותק כפול זה מבזבז רוחב פס ומחייב את ה-CPU. אם נקצה את זיכרון המארח מלכתחילה כ**נעול-עמודים (page-locked, המכונה pinned)**, מערכת ההפעלה מבטיחה שהעמודים לא יזוזו ולא יגורשו, כך שה-DMA engine יכול לקרוא ישירות מזיכרון המארח - העתקה אחת בלבד:

```text
   cudaMemcpy מזיכרון pinned (הדרך המהירה):

   [ h_a - pinned ]  ───── DMA ─────►  [ d_a - GPU ]
   העתקה אחת בלבד, ישירה, בלי מעורבות ה-CPU.
```
...
```cpp
float* h_a;
CUDA_CHECK(cudaMallocHost(&h_a, bytes));      // הדרך הפשוטה
// ... שימוש ...
CUDA_CHECK(cudaFreeHost(h_a));

// גרסה עשירה יותר עם דגלים:
float* h_b;
CUDA_CHECK(cudaHostAlloc(&h_b, bytes, cudaHostAllocDefault));
CUDA_CHECK(cudaFreeHost(h_b));
```

הפונקציה `cudaMallocHost` היא הקיצור הנוח. `cudaHostAlloc` חושפת דגלים מתקדמים: `cudaHostAllocPortable` (הזיכרון נעול עבור כל ה-contexts), `cudaHostAllocMapped` (ממפה את הזיכרון גם למרחב הכתובות של ההתקן - zero-copy), ו-`cudaHostAllocWriteCombined` (מטמון write-combining, מהיר יותר להעברות H2D אבל איטי מאוד לקריאה מה-CPU). לרוב המקרים `cudaMallocHost` מספיקה. **חובה** לשחרר זיכרון pinned עם `cudaFreeHost` ולא עם `free` - אלה מקצים ממאגרים שונים לגמרי.
...
**מחיר ה-over-pinning (מלכוד).** זיכרון pinned אינו קסם חינם. עמודים נעולים אינם ניתנים להחלפה, ולכן הם גוזלים באופן קשיח מזיכרון ה-RAM הפיזי הזמין למערכת ההפעלה ולשאר התהליכים. אם תנעלו נתחים ענקיים - נניח רוב ה-RAM - תחניקו את המערכת כולה: תהליכים אחרים ייכנסו ל-swap, וההקצאה עצמה תואט (נעילת עמודים היא קריאת מערכת יקרה). לכן הכלל: נעלו רק את ה-buffers שבאמת חוזרים ומעורבים בהעברות (למשל buffers של קלט/פלט בלולאה חמה), לא כל הקצאת מארח. הקצאת pinned גם איטית יותר מ-`malloc`, ולכן מקצים אותה פעם אחת ומחזירים אותה שימוש, לא בכל איטרציה.

## זיכרון מנוהל - unified / managed memory

עד עכשיו ניהלנו שני מרחבי זיכרון נפרדים ידנית: מצביע מארח, מצביע התקן, והעתקות מפורשות ביניהם. **זיכרון מנוהל (managed memory, המכונה גם unified memory)** מציע מודל אחר: הקצאה אחת שמצביע יחיד עליה תקף גם על המארח וגם על ההתקן, וה-CUDA runtime מזיז את הנתונים ביניהם אוטומטית לפי הצורך.

```cpp
float* a;
CUDA_CHECK(cudaMallocManaged(&a, bytes));   // מצביע יחיד, שני העולמות
for (int i = 0; i < n; i++) a[i] = 1.0f;    // כתיבה מהמארח - חוקי!
vecAdd<<<blocks, threads>>>(a, b, c, n);    // אותו מצביע ל-kernel - חוקי!
CUDA_CHECK(cudaDeviceSynchronize());
printf("%f\n", c[0]);                        // קריאה מהמארח - חוקי!
CUDA_CHECK(cudaFree(a));                      // כן, cudaFree (לא cudaFreeHost)
```

שימו לב כמה זה קצר לעומת שרשרת חמשת השלבים. אין `cudaMalloc` נפרד למארח ולהתקן, אין `cudaMemcpy`. איך זה עובד? על ארכיטקטורות Pascal ומעלה (וה-H100 בכללן), הזיכרון המנוהל נשען על **migration של עמודים לפי דרישה (on-demand page migration)** באמצעות page faults של החומרה:

```text
   הרצף עם managed memory:

   1. cudaMallocManaged  ─► הקצאה לוגית, טרם ממוקמת פיזית
   2. המארח כותב a[i]    ─► page fault -> העמוד ממוקם ב-RAM המארח
   3. ה-kernel קורא a[i] ─► page fault של ה-GPU -> העמוד מהגר ל-HBM3
   4. המארח קורא c[i]    ─► page fault -> העמוד מהגר בחזרה ל-RAM
```

כשה-kernel ניגש לעמוד שנמצא כרגע בזיכרון המארח, ה-GPU מייצר page fault, ה-runtime עוצר, מעתיק את העמוד (בדרך כלל 4 KiB או 2 MiB) אל ה-HBM3, וממשיך. ההגירה אוטומטית לחלוטין. **היתרון: פרודוקטיביות.** מבנים מקוננים עם מצביעים, פורטינג מהיר של קוד CPU קיים, ופיתוח ראשוני - כל אלה נעשים פשוט הרבה יותר. **יתרון שני: oversubscription (הקצאת-יתר).** אפשר להקצות עם `cudaMallocManaged` יותר זיכרון ממה שנכנס פיזית ב-80 GiB של ה-HBM3; ה-runtime יחליף עמודים פנימה והחוצה לפי הצורך, בדיוק כמו זיכרון וירטואלי. עם `cudaMalloc` רגיל הקצאה כזו פשוט הייתה נכשלת ב-`out of memory`.
...
```cpp
int device = 0;
// לפני ההשקה: מזיזים את a,b אל ה-GPU בבת אחת (העברה גדולה ורציפה)
...
// אחרי ההשקה: מזיזים את התוצאה c בחזרה למארח מבעוד מועד
CUDA_CHECK(cudaMemPrefetchAsync(c, bytes, cudaCpuDeviceId));
CUDA_CHECK(cudaDeviceSynchronize());
```

ה-prefetch הופך את ההגירה מ"הרבה faults קטנים בזמן ה-kernel" ל"העברה אחת גדולה, מקונבעת, לפני ה-kernel" - וזה מחזיר את הביצועים קרוב לרמת הניהול הידני, בלי לוותר על נוחות המצביע היחיד. הקבוע `cudaCpuDeviceId` מציין את המארח כיעד.

**רמזים עם `cudaMemAdvise`.** כשהגישה לנתונים אינה חד-כיוונית, אפשר לרמוז ל-runtime על דפוס השימוש כדי שינהל את העמודים חכם יותר:

| רמז (advice) | משמעות |
|---|---|
| `cudaMemAdviseSetReadMostly` | הנתונים נקראים בעיקר ולא נכתבים; ה-runtime רשאי לשכפל אותם (עותק לקריאה בכל התקן) |
| `cudaMemAdviseSetPreferredLocation` | קבע "בית" מועדף לעמודים; הגירות ינסו להשאיר אותם שם |
| `cudaMemAdviseSetAccessedBy` | התקן זה ייגש לנתונים; מפה אותם למרחבו במקום להגר בכל fault |
...
הגרסה האסינכרונית של ההעתקה היא `cudaMemcpyAsync`, שמקבלת פרמטר stream נוסף וחוזרת למארח **מיד**, בלי לחכות לסיום:

```cpp
CUDA_CHECK(cudaMemcpyAsync(d_a, h_a, bytes, cudaMemcpyHostToDevice, stream));
myKernel<<<blocks, threads, 0, stream>>>(d_a, ...);   // הפרמטר הרביעי הוא ה-stream
CUDA_CHECK(cudaMemcpyAsync(h_c, d_c, bytes, cudaMemcpyDeviceToHost, stream));
```

שימו לב לתצורת ההשקה בעלת ארבעה שדות `<<<grid, block, sharedBytes, stream>>>` - השדה הרביעי הוא ה-stream (השלישי, כאן 0, הוא shared memory דינמי). **תנאי הכרחי:** כדי ש-`cudaMemcpyAsync` תהיה באמת אסינכרונית, זיכרון המארח **חייב להיות pinned.** אם תעבירו לה מצביע pageable, הקריאה תתנהג כמעט כמו `cudaMemcpy` חוסמת - הדרייבר נופל בחזרה ל-bounce buffer ולסנכרון. זו הסיבה ששני הסעיפים הקודמים מתחברים כאן: pinned + async + streams הם השלישייה שמאפשרת חפיפה.
...
**מדידת זמן מדויקת.** בניגוד ל-`std::chrono` שמודד על ה-CPU (וממדוד גם את תקורת הסנכרון), event מוקלט על ה-GPU עצמו ומודד את הזמן שבו ה-GPU באמת עבד:
...
CUDA_CHECK(cudaEventSynchronize(stop));             // חכה שה-stop יוקלט
float ms = 0.0f;
...
for (int c = 0; c < nChunks; c++) {
    ...
    cudaStream_t st = streams[c % nStreams];   // round-robin בין הזרמים
...
for (int s = 0; s < nStreams; s++)
    CUDA_CHECK(cudaStreamSynchronize(streams[s]));   // חכה לסיום
```
...
הכלל המנחה לאורך הקורס: **צוואר הבקבוק לרוב הוא תנועת הנתונים, לא ה-kernel.** ניהול נכון של הזיכרון מהמארח - מזעור העברות, נעילת ה-buffers הנכונים, וחפיפת מה שנשאר עם חישוב - הוא לעיתים קרובות מה שמפריד בין קוד CUDA איטי לקוד CUDA מהיר, יותר מכל אופטימיזציה בתוך ה-kernel עצמו.

## סיכום

- `cudaMemcpy` הרגילה חוסמת את המארח; הכיוונים הם H2D, D2H, D2D (על רוחב הפס של ה-HBM3) ו-H2H, וכולם שייכים ל-CUDA Runtime API שהוא עטיפה מעל ה-Driver API.
- זיכרון מארח pinned (נעול-עמודים) שמוקצה ב-`cudaMallocHost` או `cudaHostAlloc` מדלג על ה-bounce buffer ומאפשר DMA ישיר, ומכאן פי 2-3 מהירות בהעברות והתנאי ההכרחי להעברות אסינכרוניות.
