...
מרגע ההקצאה ועד ההשקה נדרשו חמש קריאות `cuda*`: שלוש `cudaMalloc`, שתי `cudaMemcpy`, ואז ההשקה `<<<>>>` (שהיא עצמה סוכר תחבירי מעל קריאת Runtime). ה-context של ההתקן נוצר בשקט בקריאה הראשונה שנוגעת ב-GPU - כאן, `cudaMalloc` הראשון. אין בקוד שום קריאת אתחול מפורשת: זה בדיוק היופי (והמלכוד) של ניהול ה-context המרומז של ה-Runtime.

למה זה עבד: ה-Runtime API יוצר ומאתחל את ה-primary context של ההתקן באופן עצל (lazily), בקריאה הראשונה שדורשת GPU. המפתח לעולם אינו רואה את השלב הזה.
...
static std::vector<char> readFile(const char* path) {
    ...
    fread(buf.data(), 1, sz, f); buf[sz] = '\0';   // PTX חייב להסתיים ב-null
    fclose(f);
    return buf;
}
...
int main() {
    ...
    CU_CHECK(cuInit(0));                              // אתחול מפורש - אין קסם מרומז
    ...
    CUmodule mod;   CU_CHECK(cuModuleLoadData(&mod, ptx.data()));  // טעינה ידנית + JIT
...
למה זה עבד: השרשרת המפורשת של ה-Driver API עשתה ידנית בדיוק את מה שה-Runtime עשה בשקט בתרגיל 1 - אתחול (`cuInit`), יצירת context (`cuCtxCreate`), טעינת קוד ה-kernel (`cuModuleLoadData`, שכאן מפעיל את ה-JIT של הדרייבר מ-PTX ל-SASS של sm_90), והשקה (`cuLaunchKernel`). התוצאה זהה כי זה אותו kernel על אותה חומרה.
...
2. **כפל מטריצות בגודל קבוע - Runtime API מספיק.** ה-kernel ידוע בזמן הידור, אין צורך בשליטה מפורשת ב-context. זה בדיוק המקרה הנפוץ שה-Runtime נועד לו; ה-Driver API רק יוסיף מאות שורות boilerplate ללא תועלת.
...
### פתרון תרגיל 6 (בונוס) - אינטרופ בין שתי הממשקים

```cpp
// interop.cu  -  קימפול: nvcc -arch=sm_90 interop.cu -o interop -lcuda
#include <cuda.h>
...
int main() {
    ...
    CUdevice dev; CU_CHECK(cuDeviceGet(&dev, 0));
    CUcontext primary;
    CU_CHECK(cuDevicePrimaryCtxRetain(&primary, dev));   // אותו context של ה-Runtime
    CU_CHECK(cuCtxSetCurrent(primary));

    // הקצאה דרך ה-Runtime (cuda*)
    float *da, *db, *dc;
    ...
    // השקה דרך ה-Driver (cu*) על אותם מצביעים.
    // אסור פשוט לעשות cast של סמל ה-Runtime ל-CUfunction - זה handle אטום
    // שמתקבל רק מ-cuModuleGetFunction. משיגים CUfunction חוקי מהסמל של ה-Runtime
    // דרך cudaGetFuncBySymbol (cudaFunction_t ו-CUfunction הם אותו טיפוס בסיסי).
    CUfunction fn;
