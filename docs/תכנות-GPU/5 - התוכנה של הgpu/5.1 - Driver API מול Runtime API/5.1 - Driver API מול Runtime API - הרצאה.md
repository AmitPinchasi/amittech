בשיעור 0.4 ציירנו את מפת שרשרת התוכנה כולה, ובתחתית נתיב ההרצה ראינו שלוש שכבות שמפרידות בין היישום שלכם לבין הסיליקון: ה-Runtime API, ה-Driver API, ו-nvidia.ko. בשיעור 4.3, כשבנינו kernel (קרנל - פונקציית ה-GPU) בזמן ריצה עם nvrtc, כבר טעמנו את ה-Driver API מקרוב - קראנו ל-`cuModuleLoadDataEx` ול-`cuLaunchKernel` בלי להסביר לעומק מדוע. השיעור הזה פותח את פרק 5, פרק מחסנית ה-host (מארח - צד ה-CPU), והוא מוקדש כולו לשתי הממשקים שהיישום שלכם מדבר דרכם אל ה-GPU (מעבד גרפי / כרטיס מסך): ה-CUDA Runtime API הגבוה והנפוץ, וה-CUDA Driver API הנמוך והמפורש. נבין בדיוק איך הם מסודרים בשכבות, מי עוטף את מי, מהו ההבדל בניהול ה-context ובהשקת ה-kernel, מהי הבטחת התאימות קדימה שמאפשרת לכם לשלוח בינארי היום ולהריץ אותו על דרייבר של מחר, ומתי באמת - אם בכלל - כדאי לרדת ישירות ל-Driver API. את מה שיושב מתחת לשתי הספריות האלה, כלומר את הדרייבר עצמו על שני חלקיו, נפרק בשיעור 5.2.

## שתי הממשקים ושכבות המחסנית - the two host APIs

נתחיל מהתמונה הכוללת. כל בקשה שהיישום שלכם שולח ל-GPU עוברת דרך מדרג (stack) קבוע של שכבות, כל אחת מפשטת (abstracts) את זו שמתחתיה. שתי השכבות העליונות הן ממשקי תכנות - APIs - ושתיהן ממומשות כספריות user mode (מרחב המשתמש) שרצות בתוך התהליך שלכם:

```text
  +---------------------------------------------------------+
  |  היישום (application)  -  קוד ה-host שלכם                |   user mode
  +---------------------------------------------------------+
        |  קורא ל-API. "לפי הסדר" (הסדר של NVIDIA):
        v
  +---------------------------------------------------------+
  |  CUDA Runtime API   cuda*    libcudart.so   <- גבוה     |   user mode
  |  ניהול context מרומז, ארגונומי, הכי נפוץ                 |
  +---------------------------------------------------------+
        |  ה-Runtime עוטף (wraps) את ה-Driver
        v
  +---------------------------------------------------------+
  |  CUDA Driver API    cu*      libcuda.so     <- נמוך     |   user mode
  |  context מפורש, טעינת modules מפורשת, שליטה עדינה        |
  +---------------------------------------------------------+
   -------------------- גבול user / kernel ------------------
        |  (syscall / ioctl)
        v
  +---------------------------------------------------------+
  |  nvidia.ko   מודול ליבה, מצב מיוחס (privileged)          |   kernel mode
  +---------------------------------------------------------+
        |  נוגע ישירות בחומרה
        v
  +---------------------------------------------------------+
  |  חומרת ה-GPU (device):  132 SMs של H100, HBM3 80 GiB    |   silicon
  +---------------------------------------------------------+
```

שתי עובדות יסוד לגבי המבנה הזה. ראשית, ה-Runtime API אינו ממשק עצמאי היושב ליד ה-Driver API - הוא **יושב מעליו ועוטף אותו**. כמעט כל קריאת Runtime מתורגמת, בסופו של דבר, לקריאת Driver אחת או יותר. שנית, הביטוי של NVIDIA "הממשקים לדרייבר הם, לפי הסדר, ה-Runtime API וה-Driver API" מקפל בתוכו את שני הדברים גם יחד: גם את סדר ההעדפה (התחילו ב-Runtime, רדו ל-Driver רק כשצריך) וגם את סדר השכבות (ה-Runtime מעל ה-Driver). ה-Driver API הוא "פתח המילוט" ברמה הנמוכה, לא ברירת המחדל.
...
## ה-Runtime API - הממשק הגבוה

ה-CUDA Runtime API הוא הממשק שרוב מוחלט של תוכניות CUDA, וכמעט כל הספריות (cuBLAS, cuDNN, PyTorch), נכתבות מולו. את כל הקוד שכתבנו עד כה בקורס - `cudaMalloc`, `cudaMemcpy`, `cudaDeviceSynchronize`, והתחביר `<<<>>>` - כתבנו מול השכבה הזו. מה שהופך אותו לנוח כל כך הוא בדיוק מה שהוא **מסתיר** מכם:

- **ניהול context מרומז.** אינכם קוראים לשום פונקציית אתחול. בקריאת ה-Runtime הראשונה שנוגעת ב-GPU (למשל `cudaMalloc` או `cudaSetDevice`), ה-Runtime יוצר עבורכם באופן עצל (lazily) את ה-context של ההתקן ומאתחל אותו. זהו ה-primary context, ונרחיב עליו בסעיף נפרד.
...
- **התחביר `<<<>>>`.** תחביר ההשקה המשולש הוא סוכר תחבירי (syntactic sugar) של CUDA C++ שהמהדר nvcc מתרגם לרצף קריאות Runtime (`cudaConfigureCall` / `cudaLaunchKernel` בגרסאות המודרניות). הוא זמין רק דרך ה-Runtime, לא דרך ה-Driver.
...
int main() {
    ...
    float *da, *db, *dc;
    CUDA_CHECK(cudaMalloc(&da, bytes));            // <- כאן נוצר ה-primary context, בשקט
    ...
    vecAdd<<<grid, block>>>(da, db, dc, n);        // <- הסוכר התחבירי של ה-Runtime
...
| `cuCtxCreate(&ctx, 0, dev)` | יצירת context מפורש על ההתקן ודחיפתו למחסנית ה-contexts של ה-thread. |
...
| `cuLaunchKernel(fn, gx,gy,gz, bx,by,bz, shmem, stream, args, extra)` | השקת ה-kernel - המקבילה המפורשת של `<<<>>>`. |

שימו לב במיוחד לשני שלבים שאין להם מקבילה גלויה ב-Runtime. **`cuModuleLoad` / `cuModuleLoadData`** הוא הטעינה הידנית של קוד ה-GPU: מה שה-Runtime עשה אוטומטית מה-fatbin המוטמע, כאן אתם עושים במפורש, ומתוך מקור שאתם בוחרים - קובץ `.ptx`, קובץ `.cubin`, מחרוזת PTX שנוצרה בזמן ריצה (בדיוק כפי שעשינו עם nvrtc בשיעור 4.3), או אפילו PTX שהורדתם מהרשת. **`cuLaunchKernel`** מקבל את הארגומנטים ל-kernel כמערך של מצביעים (`void** args`), ולא כרשימת פרמטרים מסודרת עם בדיקת טיפוסים בזמן הידור - זו אחת הנקודות שבהן ה-Driver API "בטיחותי" פחות אך גמיש יותר. שימו לב גם שהמאקרו `<<<>>>` פשוט **לא זמין** כאן; הוא סוכר של CUDA C++ מעל ה-Runtime בלבד.
...
ה-context של CUDA הוא המקבילה על ה-GPU לתהליך (process) על ה-CPU: הוא מחזיק את מרחב הכתובות של ההתקן, את ההקצאות, את ה-modules הטעונים ואת ה-streams. כאן טמון ההבדל ההתנהגותי הראשון והחשוב בין שתי הממשקים, זה שה-glossary מפנה אליו במפורש כ"caveat סביב ניהול context".

- **Primary context - ה-context הראשי.** לכל התקן יש context ראשי אחד, משותף, שה-**Runtime API** יוצר ומנהל עבורכם באופן מרומז. כשאתם קוראים ל-`cudaMalloc` הראשון, ה-Runtime מאחורי הקלעים "שולף" (retains) את ה-primary context של ההתקן ומשתמש בו. כל קריאות ה-Runtime חולקות את ה-context היחיד הזה.
...
מכאן נובע **caveat אינטרופ (interop) חשוב**: אם אתם מערבבים קריאות Driver API וקריאות Runtime API באותו thread, שני העולמות חייבים לחלוק את אותו context - אחרת הקצאה שעשיתם בצד אחד לא תהיה נראית בצד השני. הדרך הנכונה לערבב היא לא ליצור context מפורש עם `cuCtxCreate`, אלא לשלוף את ה-primary context עם `cuDevicePrimaryCtxRetain` - בדיוק אותו context שה-Runtime משתמש בו - וכך שתי הממשקים מדברים על אותו מרחב זיכרון:

```cpp
CUdevice dev;
cuDeviceGet(&dev, 0);
CUcontext primary;
cuDevicePrimaryCtxRetain(&primary, dev);   // אותו context שה-Runtime ישתמש בו
cuCtxSetCurrent(primary);                   // מעכשיו cudaMalloc ו-cuMemAlloc חולקים מרחב
// ... אפשר לערבב cu* ו-cuda* בבטחה ...
cuDevicePrimaryCtxRelease(dev);
```

המסקנה המנטלית: ה-Runtime תמיד עובד על ה-primary context; ה-Driver יכול לעבוד עליו (עם `PrimaryCtxRetain`) או על context מפורש משלו (`cuCtxCreate`). זהו לב ההבדל בניהול ה-context בין שתי הממשקים.
...
תחילה, קובץ ה-kernel לבדו, וההידור שלו ל-PTX (ארכיטקטורה וירטואלית `compute_90` של Hopper):

```cpp
// kernel.cu  -  רק ה-kernel, בלי main
...
```bash
nvcc -arch=compute_90 -ptx kernel.cu -o kernel.ptx   # CUDA C++ -> PTX בלבד
```
...
// קוראים קובץ טקסט (ה-PTX) לתוך מחרוזת מסתיימת ב-'\0'
static std::vector<char> readFile(const char* path) {
    FILE* f = fopen(path, "rb");
    if (!f) { perror("fopen"); exit(EXIT_FAILURE); }
    fseek(f, 0, SEEK_END); long sz = ftell(f); fseek(f, 0, SEEK_SET);
    std::vector<char> buf(sz + 1);
    fread(buf.data(), 1, sz, f); buf[sz] = '\0';   // חשוב: PTX חייב להסתיים ב-null
    fclose(f);
    return buf;
}
...
int main() {
    ...
    CUmodule mod;   CU_CHECK(cuModuleLoadData(&mod, ptx.data()));   // טעינה ידנית + JIT ל-SASS
    ...
    int block = 256, grid = (n + block - 1) / block;
    void* args[] = { &da, &db, &dc, (void*)&n };   // ארגומנטים כמערך מצביעים
    CU_CHECK(cuLaunchKernel(fn, grid, 1, 1, block, 1, 1,
                            0, nullptr, args, nullptr));  // המקבילה של <<<grid,block>>>
...
```text
   זמן build (אצלכם)                         זמן ריצה (אצל הלקוח)

   היישום נקשר דינמית      ───שולחים───►     ה-loader טוען את libcuda.so
   מול libcuda.so הישן                        ה*מותקן* (חדש יותר) של הלקוח
   (רק כותב את התלות בשם)                     ומקשר אליו את הבינארי
                                                     │
                                                     ▼
                                        ה-PTX המוטמע עובר JIT ל-SASS
                                        של ה-GPU החדש בזמן טעינה
```
...
2. **תאימות ABI קדימה.** NVIDIA מבטיחה שממשק ה-`libcuda.so` (ה-ABI - הממשק הבינארי) נשאר תואם קדימה: כל סמל (symbol) שהיה קיים בגרסה ישנה קיים גם בחדשה, עם אותה סמנטיקה. לכן בינארי ישן מוצא את כל מה שהוא צריך בספרייה החדשה.
...
```bash
nvcc vecadd.cu -o vecadd                 # ברירת מחדל: cudart *סטטי*
...
- שתי הממשקים שהיישום מדבר דרכם ל-GPU הם ה-CUDA Runtime API (גבוה, `cuda*`, `libcudart.so`, הנפוץ) וה-CUDA Driver API (נמוך, `cu*`, `libcuda.so`); ה-Runtime **עוטף** את ה-Driver ויושב מעליו.
...
- ה-Runtime תמיד עובד על ה-primary context המשותף של ההתקן; ה-Driver יכול לעבוד עליו (`cuDevicePrimaryCtxRetain`, נדרש כדי לערבב עם Runtime) או ליצור context מפורש ומבודד (`cuCtxCreate`).
