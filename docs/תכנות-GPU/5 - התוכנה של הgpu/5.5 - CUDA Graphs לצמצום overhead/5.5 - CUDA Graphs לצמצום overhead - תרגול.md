...
1. הקצו `float* d` בודד על הdevice עם `cudaMalloc` ואפסו אותו עם `cudaMemset`.
2. צרו stream לא-default עם `cudaStreamCreate`.
3. כתבו לולאה שמשגרת את `tiny<<<1,1,0,stream>>>(d)` בדיוק `N = 512` פעמים, ואחריה `cudaStreamSynchronize`. עטפו את כל הלולאה (512 הlaunches) במדידת `cudaEvent`.
4. הריצו איטרציית חימום אחת של כל הלולאה לפני המדידה, ואז מדדו את החציון (או המינימום) על פני 50 חזרות.
5. חשבו ודווחו את הoverhead **לכל launch**: `זמן_כולל_במיקרו / 512`.
...
1. לכדו בדיוק את אותו רצף של 512 launches `tiny` לתוך גרף: `cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal)`, הלולאה של 512 הlaunches על ה-`stream`, ואז `cudaStreamEndCapture(stream, &graph)`.
...
1. קחו את הגרף בר-ההרצה מתרגיל 2 (שלכד launches על `d`). ודאו ש-`replay` אחד עובד ומעדכן את `d[0]`.
2. עכשיו **שחררו** את `d` עם `cudaFree(d)`, והקצו buffer חדש `cudaMalloc(&d2, sizeof(float))`. אפסו אותו.
3. הריצו שוב `cudaGraphLaunch(graphExec, stream)` והעתיקו את `d2[0]` לhost. האם `d2` התעדכן? הסבירו את מה שקרה (או לא קרה).
...
2. הסבירו למה מדידת ה-CPU של לולאת ההנפקה (בלי סנכרון) יכולה להיות **קטנה** מזמן ה-GPU: הhost מנפיק את הlaunches ורץ הלאה בזמן שה-GPU עדיין עובד.
3. עכשיו הוסיפו `cudaStreamSynchronize` בסוף ומדדו שוב עם ה-CPU. איך זה משתנה?
4. הסיקו מה מודד כל כלי: זמן ההנפקה של הhost מול זמן ההרצה בפועל על הdevice.
...
3. הסבירו במילים למה צעד decode של LLM (אותו forward pass של השכבות, שוב ושוב לכל טוקן) הוא מקרה אידאלי לגרף, וקשרו זאת לחשבון ה-480 launches לטוקן מההרצאה.
4. הסבירו מתי גרף **לא** יעזור: איזה מאפיין של העומס גורם לoverhead להיות זניחה מלכתחילה.
