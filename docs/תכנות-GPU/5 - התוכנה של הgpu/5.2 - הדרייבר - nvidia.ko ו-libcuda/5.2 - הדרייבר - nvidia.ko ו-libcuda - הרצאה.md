...
| מודול | תפקיד |
| --- | --- |
| `nvidia.ko` | הליבה: ניהול ההתקן, זיכרון, שיגור עבודה לחומרה |
...
```bash
sudo modprobe nvidia          # פותר תלויות וטוען את nvidia.ko ואת התלויים בו
```
...
השורה השנייה מאשרת שזו הגרסה הפתוחה (Open Kernel Module), והאחרונה מראה ש-`nvidia-uvm` רשם לעצמו major device number 235 - נחזור לזה כשנדבר על קובצי ההתקן.
...
- **קישור דינמי.** לעולם לא מקשרים את `libcuda.so` סטטית; היא תמיד מקושרת דינמית ונטענת בזמן ריצה. הסיבה עמוקה: הספרייה חייבת להתאים לכרטיס ולגרסת הדרייבר של המכונה שעליה רצים בפועל, ואלה ידועים רק בזמן ריצה, לא בזמן הידור.
...
```text
  קוד המשתמש
     |  cudaMalloc(&p, n)
     v
  libcudart.so   (Runtime API, user mode)   -- ממיר לקריאת Driver API
     |  cuMemAlloc(...)
     v
  libcuda.so     (Driver API,  user mode)   -- אורז בקשה ל-nvidia.ko
     |  ioctl(fd, NV_ESC_..., &params)
  === גבול user / kernel (מלכודת syscall) ===
     |
     v
  nvidia.ko      (core, kernel mode)        -- מבצע מול החומרה
     |  MMIO / DMA / interrupts
     v
  חומרת ה-GPU (device):  80 GiB HBM3 של H100
```

הצומת המעניין הוא המעבר מ-`libcuda.so` אל `nvidia.ko`. הוא אינו קריאת פונקציה רגילה - הוא חוצה גבול הרשאות, ולכן חייב לעבור דרך syscall. המנגנון שבו NVIDIA משתמשת הוא `ioctl` (input/output control) - אותה קריאת מערכת גנרית לשליטה בהתקנים שהכרתם בליבת-המחשב. הרעיון: הדרייבר חושף קובצי התקן (device nodes) תחת `/dev`, קוד המשתמש פותח אותם ומנפיק אליהם `ioctl` עם קוד פקודה ומבנה פרמטרים, והליבה מפענחת ומבצעת.

קובצי ההתקן שהדרייבר יוצר:
...
```cpp
float* data;
CUDA_CHECK(cudaMallocManaged(&data, n * sizeof(float)));  // מצביע יחיד לשני הצדדים
for (int i = 0; i < n; ++i) data[i] = 1.0f;               // ה-CPU כותב
vecAdd<<<blocks, threads>>>(data, data, data, n);          // ה-GPU קורא/כותב - אותו מצביע
CUDA_CHECK(cudaDeviceSynchronize());
printf("%f\n", data[0]);                                    // ה-CPU קורא שוב
CUDA_CHECK(cudaFree(data));
```

כאן `data` הוא מצביע יחיד שה-CPU וה-GPU משתמשים בו בלי `cudaMemcpy` מפורש. איך זה עובד מתחת למכסה: כשה-GPU ניגש לדף שנמצא כרגע בזיכרון ה-host, החומרה יורה page fault, המטופל על ידי `nvidia-uvm.ko`. המודול משהה את ה-warp (קבוצת 32 threads) הרלוונטי, מהגר את הדף אל ה-HBM של ה-GPU, מעדכן את טבלאות הדפים, וממשיך. זו בדיוק אותה מכניקה של demand paging מ-CPU שהכרתם בליבת-המחשב, אלא שכאן ה"פסיקה" מגיעה מה-GPU דרך המודול הזה. הנוחות עולה בהשהיה (latency): page fault הוא יקר, ולכן בקוד ביצועים גבוה נעדיף לרוב הקצאה מפורשת עם `cudaMalloc` והעתקה מפורשת, או נכוון את ההגירה מראש עם `cudaMemPrefetchAsync`. את שיקולי הביצועים האלה נעמיק בפרק על ניהול הזיכרון; כאן העיקר הוא לזהות את `nvidia-uvm.ko` כרכיב דרייבר נבדל עם קובץ התקן משלו.

## ה-JIT של הדרייבר - PTX to SASS at load

לדרייבר יש תפקיד נוסף שקל לשכוח שהוא שם: הוא גם מהדר. בשיעורי פרק 4 ראינו ש-fat binary יכול לשאת PTX (שפת ביניים וירטואלית) ו/או SASS (אסמבלי מקורי לכל גרסת SM). מה קורה כשהיישום מנסה להריץ kernel שעבורו יש בקובץ רק PTX, ואין SASS מתאים ל-GPU הפיזי? כאן נכנס ה-JIT (Just-In-Time) של הדרייבר.

בזמן טעינת המודול - `cuModuleLoad` בשפת ה-Driver API, או אוטומטית בהפעלת ה-kernel הראשונה תחת ה-Runtime API - `libcuda.so` לוקחת את ה-PTX המוטמע, קוראת ל-`ptxas` הפנימי שלה, ומהדרת אותו ל-SASS של ה-GPU הפיזי הנוכחי. זו בדיוק הסיבה שהטמעת PTX נותנת תאימות קדימה: קובץ שהודר עם PTX של `compute_90` יוכל לרוץ בעתיד על Blackwell גם בלי הידור מחדש, כי הדרייבר יַהְדֵּר את ה-PTX ל-SASS של הכרטיס החדש בזמן הריצה.
...
מכיוון שהידור ב-JIT עולה זמן בהפעלה, הדרייבר שומר את התוצאה ב-cache על הדיסק. ברירת המחדל היא התיקייה `~/.nv/ComputeCache`, ואפשר לשלוט בה במשתני סביבה:

```bash
export CUDA_CACHE_DISABLE=1        # מכבה את ה-cache לגמרי
export CUDA_CACHE_PATH=/tmp/mycache  # מיקום חלופי
export CUDA_FORCE_PTX_JIT=1        # מכריח JIT מ-PTX גם אם יש SASS מתאים
```

אפשר לראות את זה בפעולה. נהדר תוכנית עם PTX בלבד (בלי SASS), ואז נריץ אותה ונצפה ב-cache מתמלא:

```bash
nvcc -gencode arch=compute_90,code=compute_90 vecadd.cu -o vecadd_ptx  # PTX בלבד
rm -rf ~/.nv/ComputeCache
./vecadd_ptx                                                            # מפעיל JIT
...
| --- | --- | --- |
| מה נותן | הרצת GPU בזמן ריצה | פיתוח והידור |
...
הדרייבר מספק את החצי שמריץ בפועל - מודול הליבה שנוגע בחומרה, וספריית ה-Driver API. ה-Toolkit מספק את החצי שבונה - המהדר nvcc וספריית ה-Runtime API שרוב הקוד קורא לה. אפשר להתקין דרייבר בלי Toolkit (מכונת הרצה גרידא) ואפשר, פחות שכיח, ההפך.
...
int main() {
    ...
    CUDA_CHECK(cudaDriverGetVersion(&driverVer));    // המקסימום שהדרייבר תומך בו
    CUDA_CHECK(cudaRuntimeGetVersion(&runtimeVer));  // גרסת ה-Toolkit שקומפלנו מולה
...
- נתיב הבקשה הוא קוד משתמש -> `libcudart.so` -> `libcuda.so` -> `ioctl` על קובצי `/dev/nvidia*` -> `nvidia.ko` -> חומרה; ה-`ioctl` (magic `0x46` = 'F') הוא הצינור הצר של כל האינטראקציה, ואפשר לראותו עם `strace`.
...
- הדרייבר גם מהדר: ה-JIT שלו מתרגם PTX ל-SASS בזמן טעינה (זה ה-hardware driver, לא nvcc), מה שנותן תאימות קדימה, ושומר את התוצאה ב-`~/.nv/ComputeCache` (נשלט ב-`CUDA_CACHE_DISABLE`, `CUDA_FORCE_PTX_JIT`).
