...
### פתרון תרגיל 1 - nsys על אפליקציה חסומת-תקורה
...
int main() {
    ...
    for (int k = 0; k < 10000; ++k)
        tiny_add<<<(N + 255) / 256, 256>>>(d, N);   // 10,000 שיגורים זעירים
...
ה-CPU בילה ~105 מילישנייה בקריאות `cudaLaunchKernel` בעוד שה-GPU עבד בסך הכל ~18 מילישנייה. יחס של כמעט פי 6 לטובת התקורה - האפליקציה **חסומת-תקורה** מובהקת.

למה זה עבד: כל `cudaLaunchKernel` עולה ~10 מיקרו-שנייה של תקורת CPU (שיעור 7.4), ללא קשר לכמה עבודה ה-kernel עושה. 10,000 שיגורים כפול 10,488 ננו-שנייה בממוצע = ~105 מילישנייה, שכולן תקורה טהורה. ה-GPU בטל את רוב הזמן ומחכה לקבל עבודה - בציר הזמן של ה-GUI זה נראה כרצף פערים בין kernels זעירים. איך להכליל: כשזמן ה-CUDA API עולה על זמן ה-GPU, אתם חסומי-תקורה, וזה בדיוק המקרה שבו CUDA Graphs (שיעור 5.5) עוזר - הוא אוסף את 10,000 השיגורים לשיגור host יחיד ומצניח את התקורה. תמיד השוו את שתי הטבלאות (API מול Kernel) לפני שמתחילים לאפטמז את ה-kernel עצמו.
...
```cpp
// pipeline.cu (עיקר - הגרסה הטובה)
const int CHUNKS = 8, CN = 1 << 22;        // 8 אצוות של 4M floats
...
for (int c = 0; c < CHUNKS; ++c) {
    ...
    heavy_kernel<<<CN/256, 256, 0, st[c]>>>(d+off, CN);      // חופף להעתקה של אצווה אחרת
...
```text
$ nsys stats rep_serial.nsys-rep     # הגרסה הגרועה (default stream)
** CUDA GPU MemOps + Kernels timeline (מקוצר):
   HtoD  chunk0  [  0.0 ms -  1.4 ms ]
   krnl  chunk0  [  1.4 ms -  3.2 ms ]
   DtoH  chunk0  [  3.2 ms -  4.6 ms ]
   HtoD  chunk1  [  4.6 ms -  6.0 ms ]   <- מתחיל רק אחרי שאצווה 0 סיימה לגמרי
   ...
   Total: 36.8 ms   (הכל בטור, אפס חפיפה)

$ nsys stats rep_overlap.nsys-rep    # הגרסה הטובה (streams + pinned)
   HtoD  chunk0  [  0.0 ms -  1.4 ms ]
   krnl  chunk0  [  1.4 ms -  3.2 ms ]
   HtoD  chunk1  [  1.4 ms -  2.8 ms ]   <- חופף ל-kernel של אצווה 0!
   krnl  chunk1  [  3.2 ms -  5.0 ms ]
   HtoD  chunk2  [  2.8 ms -  4.2 ms ]   <- חופף שוב
   ...
   Total: 17.1 ms   (חפיפה מלאה - פי ~2.1 מהיר יותר)
```
...
נשתמש ב-SAXPY כדוגמה הקנונית לחסום-זיכרון:
...
$ ncu -i rep_saxpy.ncu-rep --page details
  ...
  Section: GPU Speed Of Light Throughput
    ...
    Memory Throughput                  [%]         91.24     <- זיכרון קרוב לשיא
  ...
  Section: GPU Speed Of Light Roofline Chart
    ...
    -> נקודה על הגג האלכסוני, עמוק משמאל לרכס (295)
  ...
  Section: Warp State (Stall Reasons)
    Stall Long Scoreboard         [cycles/inst]     8.42     <- הדומיננטי
    Stall Wait                    [cycles/inst]     0.31
```

למה זה עבד: SAXPY מבצע 2 FLOP על כל 12 בייט (טעינת `x`, טעינת `y`, כתיבת `y`) - arithmetic intensity של 1/6, שהוא **0.167 FLOP/byte**, הרבה מתחת לנקודת הרכס 295 של ה-H100 (שיעור 7.2). לכן קטע ה-Speed of Light מראה ניצולת זיכרון ~91% מול ניצולת חישוב ~3%, וה-roofline מצייר את הנקודה על הגג האלכסוני - **חסום-זיכרון** חד-משמעי. סיבת העצירה הדומיננטית היא **long scoreboard** (8.42 מחזורים להוראה) - המתנה ל-`LDG` שחוזר מ-HBM, בדיוק כפי ש-8.3 חוזה לקוד חסום-זיכרון. ה-occupancy גבוה (~90%) כי SAXPY משתמש רק ב-16 registers לthread. איך להכליל: `--set full` נותן בהרצה אחת את כל התמונה. הסדר לקריאה: קודם Speed of Light/roofline (חישוב או זיכרון?), אז occupancy (מספיק warps?), אז stalls (מה עוצר?). שלושתם עקביים - חסום-זיכרון => DRAM בשיא => long scoreboard.
...
$ ncu --set full -k "rowsum_bad|rowsum_good" ./rowsum
  ...
  rowsum_bad:
    ...
    Duration                       4.81 ms
    Roofline: על הגג האלכסוני, רחוק מהרכס

  rowsum_good:
    Memory Throughput [%]          88.6      Sectors/Req     4.0
    Stall Long Scoreboard           2.1 cyc/inst
    Duration                       0.36 ms      (x13.3 מהיר יותר)
    Roofline: על הגג האלכסוני, קרוב יותר לשיא הזיכרון השימושי
```

למה זה עבד: `rowsum_bad` נותן ל-threads שכנים כתובות במרחק `N*4` בייט, וכל warp מפעיל 32 sectors נפרדים לבקשה - מתוכם רק 1/8 שימושי. `rowsum_good` נותן כתובות רצופות ולכן 4 sectors לבקשה, כולם שימושיים. שלושת המדדים מספרים את אותו סיפור: 32 sectors => יותר `LDG` => יותר long scoreboard stalls (11.8 מול 2.1 מחזורים) => יותר זמן. ה-roofline של שתיהן על הגג האלכסוני (שתיהן חסומות-זיכרון), אבל הרעה מבזבזת פי 8 בייטים. איך להכליל: `sectors_per_request` הוא המדד ה**ישיר** ביותר ל-coalescing, וקטע ה-Stalls מאשר אותו מזווית שנייה - גישה לא מאוחדת מייצרת יותר long scoreboard stalls כי היא מפעילה יותר עסקאות global. כשרואים 32 sectors לבקשה, מיד יודעים לחפש היכן `threadIdx.x` לא רץ על הממד הרציף (שיעור 8.4).
...
$ ncu --set full -k transposeNaive ./transpose
  ...
  Section: Memory Workload Analysis
    ...
    Shared Memory Bank Conflicts        16,252,928     <- הבעיה, ב-shared
    Global Load Sectors/Req              4.0           <- ה-global נקי!
    Global Store Sectors/Req             4.0

  Section: Warp State (Stall Reasons)
    Stall Short Scoreboard    [cyc/inst]    6.9        <- המתנה ל-LDS מ-shared
...
```text
[B------:R-:W3:-:S02]  /*1e00*/  LDG.E.SYS R6, [R2] ;      // כותב scoreboard 3
[B---3--:R-:W-:Y:S04]  /*1e10*/  FADD R4, R4, R6 ;         // חוסם עד ש-3 מתנקה
```

למה זה עבד: תצוגת ה-Source (שאפשרית רק בזכות `-lineinfo`, שמטמיע את מיפוי SASS-למקור בלי להאט - להבדיל מ-`-G`) חושפת שהשורה `acc += A[row*N+col]` אוכלת 74% מדגימות ה-stall. הוראת ה-SASS שלה, `LDG.E.SYS`, כותבת scoreboard 3 וממתינה לזיכרון שחוזר מ-HBM; ה-`FADD` שצורך את הערך חוסם על scoreboard 3 (`B---3--`) - זהו ה-**long scoreboard stall**. השרשרת המלאה סגורה: השורה החמה במקור -> הוראת `LDG` ב-SASS -> long scoreboard stall -> נקודה על הגג האלכסוני ב-roofline -> חסום-זיכרון. איך להכליל: כשקטע ה-Stalls אומר "long scoreboard", תצוגת ה-Source מראה **איזו שורה בדיוק** אחראית, וכמעט תמיד זו גישת global לא-מאוחדת (`LDG`) או, אם ה-stall הוא short, גישת shared עם conflict (`LDS`). המיפוי הזה הוא אותה יכולת `nvdisasm` משיעור 4.4, עכשיו עם מוני stall מוצמדים לכל שורה.
...
```text
איטרציה | כלי   | מה מצאנו                          | תיקון                    | speedup
--------+-------+----------------------------------+--------------------------+--------
   1    | nsys  | copy ו-compute בטור, אפס חפיפה     | streams + pinned (5.3)   | x2.1
   2    | ncu   | ה-kernel חסום-זיכרון, 32 sec/req   | תיקון coalescing (8.4)   | x11.0
   3    | nsys  | עכשיו חסום-תקורה: הרבה שיגורים     | CUDA Graphs (5.5)        | x1.4
--------+-------+----------------------------------+--------------------------+--------
                                            speedup מצטבר מקצה-לקצה:  ~x32
```

הפלט בכל צעד:

```text
# צעד 1 - nsys: הבעיה מערכתית (חוסר חפיפה)
Total (serial):   36.8 ms    ->    Total (overlap):   17.1 ms

# צעד 2 - ncu על ה-kernel אחרי סגירת החפיפה
  Roofline: memory-bound;  Sectors/Req: 32.0;  Long Scoreboard: 11.8 cyc
  אחרי תיקון דפוס הגישה:   Sectors/Req: 4.0;   Duration: x11 מהיר יותר

# צעד 3 - nsys שוב: הצוואר החדש
  cudaLaunchKernel  קופץ ל-40% מהזמן -> חסום-תקורה
  אחרי CUDA Graphs:  התקורה צונחת, x1.4 נוסף
```

למה זה עבד: בכל איטרציה השתמשנו בכלי הנכון למפלס הנכון. nsys ("איפה") גילה קודם את הבעיה המערכתית - חוסר חפיפה - שהיא בעיה ש-ncu לעולם לא היה רואה, כי הוא מסתכל על kernel בודד ולא על ציר הזמן. רק אחרי שסגרנו את החפיפה עברנו ל-ncu ("למה") כדי לתקן את ה-kernel עצמו, וכשהצוואר הזה נפתר, הצוואר **החדש** (תקורת שיגור) שוב הצריך את nsys. איך להכליל: זו מתודולוגיית ה-Theory of Constraints משיעור 7.1 - מצא, הרם, חזור על הצוואר החדש - כשכל איטרציה מתחילה ב-nsys ("איפה") ורק אז יורדת ל-ncu ("למה") אם הצוואר הוא kernel. אם היינו מתחילים מ-ncu, היינו מבזבזים שעות על אופטימיזציה של kernel בזמן שרוב הזמן בכלל הלך לחוסר החפיפה - צוואר שהיה נשאר בלתי-נראה. תמיד מהמערכתי אל הפרטני, לא ההפך. זהו בדיוק זרימת העבודה שנפעיל בפרויקטים 9.3-9.4.
