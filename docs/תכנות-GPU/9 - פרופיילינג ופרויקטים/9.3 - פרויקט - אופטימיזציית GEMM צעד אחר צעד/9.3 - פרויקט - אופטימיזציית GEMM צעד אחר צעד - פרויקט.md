...
```text
   שלב   kernel               מה מתקן                    כלי ההצדקה (ncu)          יעד %peak (FP32)
   ────────────────────────────────────────────────────────────────────────────────────────────
   1     naive                בסיס ההשוואה               -                          ~3%
   2     coalesced            מיפוי thread->נתון          sectors/request נופל 32->4  ~10%
   3     tiled (shared)       ניצול-חוזר דרך shared        DRAM throughput נופל        ~26%
   4a    1D blocktiling       כל thread מחשב עמודת פלט     shared throughput / AI      ~48%
   4b    2D blocktiling       כל thread מחשב אריח 8x8      FMA pipe util, registers    ~70%
   ────────────────────────────────────────────────────────────────────────────────────────────
   תקרה  cuBLAS SGEMM (FP32)  reference ceiling          -                           ~86%
   ────────────────────────────────────────────────────────────────────────────────────────────
   5     wmma (FP16/TC)       מעבר ל-Tensor Cores         tensor pipe util            (peak אחר!)
   תקרה  cublasGemmEx (FP16)  tensor reference ceiling   -                           (peak אחר!)
```

מטריצות ההרצה ריבועיות `N x N` מסוג `float` (FP32), עם `N = 4096` כברירת מחדל למדידה. שימו לב ש-`4096` מתחלק ב-`64`, ב-`128` וב-`8` - כל גדלי האריחים בפרויקט - כך שנוכל להשמיט בדיקות גבול בתוך ה-kernels המתקדמים ולהתמקד באלגוריתם. ה-GPU הרץ לדוגמה הוא H100 (SXM), כרגיל.
...
```cpp
// שני ה-peaks התיאורטיים של H100 (SXM), ב-GFLOP/s:
...
// מדווח N, זמן, GFLOP/s ואחוז מ-peak נתון. peak שונה ל-CUDA Cores מול Tensor Cores.
...
**שלב 3 - tiled (shared memory).** מבצעים staging של אריחי `A` ו-`B` ל-shared memory (ראו קוד מלא ב-3.5, `TILE=32`), וכל איבר משמש ב-`TILE` פעולות FMA לפני שנזרק. ההצדקה ב-ncu היא נפילת התעבורה ל-DRAM, שנמדדת ב"GPU Speed Of Light" כאחוז מרוחב הפס:
...
```text
  ...
  mmCoalesced        ~18%                 ~85%       <- חסום-זיכרון: DRAM רווי, SM מורעב
  mmTiled            ~40%                 ~30%       <- הזזנו את הצוואר מ-DRAM אל ה-SM
```
...
#define BK 8
#define TM 8
// מניחים ש-N מתחלק ב-BM, BN, BK (נכון ל-N=4096). אין בדיקות גבול.

__global__ void mm1DBlocktiling(const float* A, const float* B, float* C, int N) {
    __shared__ float As[BK][BM];    // אריח A מאוחסן משוחלף (transposed): As[k][m]
    __shared__ float Bs[BK][BN];    // אריח B: Bs[k][n]

    const int blockRow = blockIdx.y, blockCol = blockIdx.x;

    // מיקום ה-thread בתוך אריח הפלט 64x64:
    const int threadCol = threadIdx.x % BN;   // 0..63  -> עמודת פלט
    const int threadRow = threadIdx.x / BN;   // 0..7   -> קבוצת TM שורות

    // אינדקסים לטעינה שיתופית (כל 512 הthreads טוענים אריח ביחד):
    ...
    const float* Aptr = A + blockRow * BM * N;              // ראש פאנל השורות של A
    const float* Bptr = B + blockCol * BN;                  // ראש פאנל העמודות של B
    float* Cptr = C + blockRow * BM * N + blockCol * BN;

    float threadResults[TM] = {0.0f};                       // TM מצברים ב-registers

    for (int bk = 0; bk < N; bk += BK) {
        // טעינת האריחים ל-shared memory (A נשמר משוחלף כדי שהגישה בהמשך תהיה יעילה):
        As[innerColA][innerRowA] = Aptr[innerRowA * N + innerColA];
        Bs[innerRowB][innerColB] = Bptr[innerRowB * N + innerColB];
        __syncthreads();

        Aptr += BK;                                         // מתקדמים לאריח K הבא
        Bptr += BK * N;

        for (int k = 0; k < BK; k++) {
            float Btmp = Bs[k][threadCol];                  // load אחד מ-shared...
            for (int m = 0; m < TM; m++)                    // ...ל-TM פעולות FMA
...
**החשבון של העצימות האריתמטית.** בגרסת ה-tiled, הלולאה הפנימית הריצה FMA אחד לכל שני loads מ-shared memory. עכשיו, בלולאה הפנימית, טוענים `Btmp` פעם אחת ומריצים איתו `TM = 8` פעולות FMA (מול `TM` ערכי `As` שכבר ב-shared). היחס FMA-ל-load מ-shared זינק פי 8. פחות לחץ על ה-shared memory משמעו שה-warp scheduler מוצא יותר עבודה אריתמטית מוכנה בין הגישות, וה-FMA pipe מתמלא.
...
הגענו ל-48% - כמעט פי 2 מ-tiled. אבל ה-FMA pipe עדיין ב-55% בלבד, וה-shared memory throughput עדיין גבוה יחסית. הצוואר הבא: אפשר להעלות את העצימות שלב נוסף, ולנצל-חוזר ערכים **בשני הצירים** בבת אחת. זה שלב 4ב.


### שלב 4ב - register tiling דו-ממדי - 2D blocktiling

בשלב 4א thread ניצל-חוזר ערך של `B` מול `TM` ערכי `A`. עכשיו נעשה זאת סימטרית: כל thread יחשב **אריח פלט מלא** של `TM x TN = 8 x 8 = 64` איברים. לכל צעד `k`, ה-thread טוען `TM` ערכי `A` ו-`TN` ערכי `B` ל-registers, ומ-`TM + TN = 16` loads מ-shared memory הוא מפיק `TM * TN = 64` פעולות FMA. יחס של 4 FMA לכל load מ-shared - שוב קפיצה בעצימות. זו הצורה שבה kernels אמיתיים (וגם cuBLAS) בנויים ביסודם.
...
__global__ void mm2DBlocktiling(const float* A, const float* B, float* C, int N) {
    __shared__ float As[BK][BM];    // A משוחלף: As[k][m]
    __shared__ float Bs[BK][BN];    // Bs[k][n]

    const int blockRow = blockIdx.y, blockCol = blockIdx.x;
    const int numThreads = (BM * BN) / (TM * TN);          // 256

    // מיקום ה-thread ברשת 16x16 של אריחי-thread:
    const int threadCol = threadIdx.x % (BN / TN);         // 0..15
    const int threadRow = threadIdx.x / (BN / TN);         // 0..15

    // אינדקסי טעינה + strides (256 threads טוענים 128x8 ו-8x128 בכמה מעברים):
    const int innerRowA = threadIdx.x / BK, innerColA = threadIdx.x % BK;
    const int strideA   = numThreads / BK;                 // 32 שורות למעבר
    const int innerRowB = threadIdx.x / BN, innerColB = threadIdx.x % BN;
    const int strideB   = numThreads / BN;                 // 2 שורות למעבר

    const float* Aptr = A + blockRow * BM * N;
    const float* Bptr = B + blockCol * BN;
    float* Cptr = C + blockRow * BM * N + blockCol * BN;

    float threadResults[TM * TN] = {0.0f};                 // 64 מצברים ב-registers
    float regM[TM], regN[TN];                              // מטמון registers לצעד k

    for (int bk = 0; bk < N; bk += BK) {
        for (int off = 0; off < BM; off += strideA)        // טעינת אריח A (משוחלף)
            As[innerColA][innerRowA + off] = Aptr[(innerRowA + off) * N + innerColA];
        for (int off = 0; off < BK; off += strideB)        // טעינת אריח B
        ...
        for (int k = 0; k < BK; k++) {
            ...
            for (int m = 0; m < TM; m++)                   // מכפלה חיצונית 8x8
                for (int n = 0; n < TN; n++)
...
**כאן register pressure הופך למרכזי - קישור ישיר ל-8.5.** ספרו את ה-registers שהצהרנו במפורש: `threadResults[64] + regM[8] + regN[8] = 80` floats, ומעליהם אינדקסים ומצביעים. נאמת את המספר האמיתי מהמהדר:
...
__global__ void mmWMMA(const half* A, const half* B, float* C, int N) {
    ...
    for (int k = 0; k < N; k += WK) {                   // צוברים על ציר K באריחי 16
...
ההשקה - חשוב לזכור שהיחידה היא warp, ולכן ציר ה-`x` מחולק ב-`warpSize`:

```cpp
dim3 block(128, 4);                                    // 128*4 = 512 threads = 16 warps
int warpsX = block.x / 32;                             // 4 warps לאורך x
dim3 grid((N + WM * warpsX  - 1) / (WM * warpsX),
          (N + WN * block.y - 1) / (WN * block.y));
auto launch = [&]{ mmWMMA<<<grid, block>>>(dA_h, dB_h, dC, N); };
float ms = timeKernel(launch);
report("wmma (FP16/TC)", N, ms, H100_FP16_TC_PEAK_GFLOPS);   // peak של Tensor Cores!
```
...
```cpp
#include <cublas_v2.h>
const float alpha = 1.0f, beta = 0.0f;                 // alpha/beta ב-FP32 (compute type)

auto launch = [&]{
    cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                 N, N, N, &alpha,
                 dB_h, CUDA_R_16F, N,                  // "A" של cuBLAS = ה-B שלנו (FP16)
                 dA_h, CUDA_R_16F, N,                  // "B" של cuBLAS = ה-A שלנו (FP16)
                 &beta,
                 dC,   CUDA_R_32F, N,                  // פלט FP32
                 CUBLAS_COMPUTE_32F,                   // צבירה ב-FP32
                 CUBLAS_GEMM_DEFAULT_TENSOR_OP);       // חייב Tensor Cores
};
float ms = timeKernel(launch);
...
- **coalesced -> tiled:** ה-DRAM היה ב-85% (חסום-זיכרון); shared memory חתך את התעבורה ל-global memory, וה-DRAM ירד ל-30%.
...
```text
   [ ] כל השלבים 1-4 עוברים checkResult (rtol=1e-3) מול הייחוס; שלב 5 עובר עם rtol=2e-2
   [ ] טבלה 1 (CUDA Cores) מלאה לכל 5 ה-kernels + cuBLAS: N, ms, GFLOP/s, %peak, speedup
   [ ] טבלה 2 (Tensor Cores) מלאה ל-wmma ול-cublasGemmEx, מול ה-peak הנכון (989 TFLOPS)
   [ ] שלב 2: ncu מראה sectors/request יורד מ-~32 (naive) ל-~4 (coalesced)
   [ ] שלב 3: ncu מראה dram__throughput יורד מ-~85% ל-~30%
   [ ] שלב 4ב: 2D blocktiling מגיע ל->=80% מ-cuBLAS (זמן_cuBLAS/זמן_2D >= 0.8)
   [ ] שלב 4ב: מדווח registers/thread מ-Xptxas -v ומאושר spills=0
   [ ] שלב 5: wmma עובר בבירור את 66.9 TFLOPS (תקרת CUDA Cores)
   [ ] פסקת הסבר לכל מעבר, כשההצדקה היא מטריקת ncu קונקרטית (לא "הרגשה")
   [ ] כל קריאות CUDA/cuBLAS עטופות ב-CUDA_CHECK
```

פקודות הפרופיילינג המרכזיות שתריצו (מ-9.1), ריכוז:

```bash
# בנייה עם דוח registers:
nvcc -O3 -arch=sm_90a -Xptxas -v -lcublas -o gemm gemm.cu

# roofline אוטומטי לכל ה-kernels (ncu מזהה איפה כל אחד יושב מתחת לגג):
ncu --set roofline -o gemm_roofline ./gemm

# דוח מלא לניתוח צוואר בקבוק ידני (מדלגים על ה-warmup עם launch-skip):
ncu --set full --launch-skip 1 --launch-count 1 -o gemm_full ./gemm

# מטריקות ממוקדות למעבר בין שני שלבים ספציפיים:
