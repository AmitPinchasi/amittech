...
__global__ void heavy(const float* __restrict__ in, float* __restrict__ out, int n) {
    ...
    for (int k = 0; k < 48; k++) {
        #pragma unroll
        for (int i = 0; i < 64; i++)
            a[i] = a[i] * a[i] + a[(i + 1) & 63];   // כל הצוברים נשארים חיים
    }
    float s = 0.0f;
    #pragma unroll
...
heavy_lb(const float* __restrict__ in, float* __restrict__ out, int n) { /* גוף זהה ל-heavy */ }
```
