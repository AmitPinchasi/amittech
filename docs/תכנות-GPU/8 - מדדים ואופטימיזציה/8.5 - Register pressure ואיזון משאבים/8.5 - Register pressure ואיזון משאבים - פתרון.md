...
```text
   light:  16 אוגרים, 0 smem, 0 דליפה
           16 אוגרים -> 32*16=512, מעוגל ל-512, 65536/512=128 -> חסום ל-64 warps
           -> occupancy מקסימלי (מאוגרים) = 100%   ["בריא"]

   heavy: 128 אוגרים, 0 smem, 0 דליפה
           128 אוגרים -> 32*128=4096, 65536/4096=16 warps
           -> occupancy מקסימלי (מאוגרים) = 16/64 = 25%   ["לחוץ"]
```
...
static float timeKernel(void (*launch)(const float*, float*, int),
    ...
    launch<<<blocks, threads>>>(d_in, d_out, n);            // חימום
...
```bash
nvcc -O2 -arch=sm_90a -Xptxas -v -o rp_free regpressure.cu   # בלי הגבלה
...
טבלת סיכום:
   kernel      אוגרים/thread   occupancy   runtime
...
```text
maxrreg   אוגרים בפועל   occupancy   runtime
   24          24          100%       6.02 ms   <- דליפה כבדה
   28          28          100%       5.31 ms   <- דליפה
   32          32          100%       4.71 ms   <- דליפה
   40          40           80%       3.24 ms   <- דליפה קלה
   48          48           66%       2.15 ms
   64          64           50%       1.72 ms   <- מינימום (ללא דליפה)
   96          96           33%       1.78 ms
  128         128           25%       1.83 ms
  168         168           18%       2.05 ms
  255         168*          18%       2.05 ms   *ה-kernel לא צריך יותר מ-168
```
...
```text
   קצה שמאל (24-40 אוגרים):  דליפה ל-local memory -> גישות GPU RAM בלולאה החמה -> איטי
   קצה ימין (128-255 אוגרים): occupancy 25% ומטה -> לא מספיק warps להסתיר latency -> איטי
   אמצע (~64 אוגרים):         בלי דליפה, occupancy 50% - מספיק להסתרה. המהיר ביותר.
```
...
# rp_32   (-maxrregcount=32, 32 אוגרים)
...
```text
   מדד                     rp_free (128)   rp_32 (32)     מסקנה
   Registers Per Thread        128            32          תואם ל-ptxas -v
   Achieved Occupancy          24%            88%         העלינו occupancy...
   Local Memory                 0            488 B        ...אבל נכנסנו לדליפה
   SM Throughput               71%            30%         ...וה-throughput צנח
   סיבת stall דומיננטית      Wait/NotSel   Long Scoreboard  הדליפה = המתנה ל-GPU RAM
```
