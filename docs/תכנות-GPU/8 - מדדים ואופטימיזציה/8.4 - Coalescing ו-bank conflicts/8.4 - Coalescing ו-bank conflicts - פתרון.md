...
int main(int argc, char** argv) {
    ...
    for (int s : strides) {
        ...
        double usefulBytes = ((double)N / s) * sizeof(float);   // N/stride אלמנטים
...
__global__ void rowsum_bad(const float* __restrict__ A,
    ...
    for (int col = 0; col < N; ++col)
        acc += A[(size_t)row * N + col];      // threads שכנים במרחק N*4 בייט
    s[row] = acc;
}

// block אחד לשורה; threadIdx.x רץ על העמודות -> קריאה מאוחדת
__global__ void rowsum_good(const float* __restrict__ A,
    ...
    for (int col = threadIdx.x; col < N; col += blockDim.x)
        acc += A[(size_t)row * N + col];      // threads שכנים -> כתובות רצופות
    red[threadIdx.x] = acc;
    __syncthreads();
...
speedup    :  x13.3
max |diff| :  0.0        (שתי הגרסאות מסכימות)
```
...
__global__ void transposeNaive(const float* __restrict__ in,
    ...
    if (x < width && y < height)
        tile[threadIdx.y][threadIdx.x] = in[(size_t)y * width + x];  // מאוחד מ-global
    __syncthreads();
    x = blockIdx.y * TILE + threadIdx.x;
    y = blockIdx.x * TILE + threadIdx.y;
    if (x < height && y < width)
        out[(size_t)y * height + x] = tile[threadIdx.x][threadIdx.y]; // עמודה -> conflict
}
```
...
```text
כתובת = threadIdx.x * 32 + threadIdx.y
בנק   = (threadIdx.x * 32 + threadIdx.y) mod 32
      = ((threadIdx.x * 32) mod 32) + threadIdx.y
      = 0 + threadIdx.y
      = threadIdx.y      <- קבוע לכל 32 ה-threads של ה-warp!
```
...
```cpp
    __shared__ float tile[TILE][TILE + 1];   // 32x33 במקום 32x32
```

**החישוב ביד (סעיף 2).** עכשיו stride השורה הוא 33 מילים:

```text
כתובת = threadIdx.x * 33 + threadIdx.y
...
__global__ void bcast(const float* g, float* out) {
    ...
    __syncthreads();
    float v = data[0];                  // כל ה-warp -> אותה כתובת בדיוק
...
```text
    l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_ld.sum   (התנגשות 2-way)
```
...
דפוס                          | תוצאה           | קנס
------------------------------+-----------------+--------------
בנקים שונים (data[tid])       | מקבילי          | פי 1 (אין)
אותו בנק, אותה כתובת (data[0])| שידור broadcast | פי 1 (אין)
אותו בנק, כתובות שונות        | התנגסות N-way   | פי N (מס' כתובות)
```
...
--------------+--------------------+---------------------
coalescing    | חי כאן (128B burst)| חסר משמעות
bank conflict | חסר משמעות         | חי כאן (32 בנקים)
```
