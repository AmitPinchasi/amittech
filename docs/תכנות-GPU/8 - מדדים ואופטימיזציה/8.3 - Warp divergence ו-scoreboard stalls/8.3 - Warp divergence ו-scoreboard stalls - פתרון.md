...
```nasm
LDG.E R4, [R2.64]                        // load של data[idx]
FSETP.GT.AND P0, PT, R4.reuse, 0.5, PT   // P0 = (data[idx] > 0.5)
FADD R0, R4, 2                           // Block B (else): מבוצע עבור כל ה-threads
@P0 FMUL R0, R4, 4                        // Block A (if): מותנה ב-P0
FMUL R5, R0, R0                           // התכנסות: data[idx]*data[idx]
...
- **סעיף 5:** המהדר בחר להריץ צד אחד (FADD) לכולם ולדרוס את הצד השני עם predicate יחיד, במקום predication סימטרי על שני הצדדים. FADD היא פקודת מחזור-בודד זולה על CUDA Core; המהדר מעדיף לבזבז אותה מאשר להוסיף predicate שני ומורכבות בקרת-זרימה נוספת.
...
```text
   gridDim   = ceil(1,000,003 / 256) = 3907 בלוקים
   threads   = 3907 x 256 = 1,000,192
   warps     = 1,000,192 / 32 = 31,256

   warps אחידים (כולם idx<n או כולם idx>=n) : 31,255
   warps מתפצלים (רוכבים על הגבול idx=1,000,003):     1
...
__global__ void mem_bound(const float* in, float* out, int n) {
    ...
    if (i < n) out[i] = in[i];                    // LDG ואז STG, כמעט בלי חישוב
}
...
__global__ void transcendental(const float* in, float* out, int n) {
    ...
    if (i < n) {
        float x = in[i];
        for (int k = 0; k < 64; k++)
            x = sinf(x) + sqrtf(fabsf(x)) + expf(x * 0.01f);   // SFU כבד
        out[i] = x;
    }
}
```
...
```text
        MUFU.SIN  R6, R0
        MUFU.RSQ  R7, R8      // מבסיס ל-sqrtf / rsqrt
        MUFU.EX2  R9, R10     // מבסיס ל-expf
```
...
```text
[B------:R-:W2:-:S04]  /*00f0*/  LDG.E R0, [R2.64] ;     // load a[i], מסמן scoreboard 2
[B------:R-:W2:-:S01]  /*0100*/  LDG.E R5, [R4.64] ;     // load b[i], מסמן scoreboard 2
[B--2---:R-:W-:Y:S08]  /*0150*/  IMAD R0, R0, R5, RZ ;   // a*b, חוסם על scoreboard 2
...
- **סעיף 1-2:** ב-kernel שמערבב `LDG` (long) ו-`MUFU` (short), ncu **לא תמיד** מפריד את שתי סיבות העצירה נקי. כפי שרואים בפלט, המהדר משתמש רק בקומץ scoreboards שונים (כאן 0, 1, 2 מתוך 6 האפשריים) וממחזר אותם לאורך ה-kernel.
...
למה זה עבד: יש רק 6 scoreboards ל-warp, ולכן בקוד עמוס המהדר **חייב** למחזר אותם. המיחזור הוא שיוצר את העמימות: scoreboard יחיד יכול לעקוב ברצף אחרי load ואחרי MUFU, וה-classifier של ncu מייחס את העצירה לפי ניחוש. ה-SASS, לעומת זאת, מראה את הפקודה המדויקת - ולכן הוא מקור-האמת.
