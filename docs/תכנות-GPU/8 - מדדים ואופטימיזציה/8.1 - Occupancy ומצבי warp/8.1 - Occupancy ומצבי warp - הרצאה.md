...
**פעיל - active.** warp נחשב פעיל מהרגע שה-threads שלו מתחילים לרוץ ועד הרגע שכל ה-threads בו יצאו מה-kernel. במילים אחרות, active פירושו **תושב (resident) על ה-SM עם משאבים מוקצים** - אוגרים בקובץ-האוגרים, מקום ב-shared memory - מרגע ההשקה ועד הסיום. אוסף ה-warps הפעילים הוא **המאגר** שממנו ה-scheduler בוחר מועמדים בכל מחזור. הדקות המכרעת: **warp פעיל לא בהכרח מבצע פקודות.** הוא יכול להיות פעיל ותקוע ובטל בו-זמנית. ב-H100 (compute capability 9.0) המקסימום הוא **64 warps פעילים ל-SM = 2048 threads ל-SM** (64 x 32).
...
```text
   warp כשיר  <=>  כל הארבעה מתקיימים:
   1. הפקודה הבאה כבר נשלפה (fetched)
   2. יחידת הביצוע הדרושה פנויה (Tensor Core / CUDA Core / LSU)
   3. כל התלויות של הפקודה נפתרו (אוגרי מקור מוכנים)
   4. אין מחסום סנכרון (barrier) שחוסם את ההתקדמות
```
...
```text
   ...
   warp 0       תקוע       כשיר   נבחר   תקוע    תקוע     (פעיל לכל אורך הזמן)
   warp 1       נבחר       תקוע   תקוע   כשיר    נבחר
   warp 2       כשיר       נבחר   תקוע   תקוע    כשיר
   warp 3       תקוע       תקוע   כשיר   נבחר    תקוע
                  │          │      │
                  │          │      └── warp 0 נבחר: כשיר -> קיבל פקודה
                  │          └── רק warp 1,3 כשירים; warp 2 נבחר
                  └── כל הארבעה פעילים, אך רק 1,2 כשירים; warp 1 נבחר
```
...
```text
   מחזור פעיל  ?  יש לפחות warp אחד תושב על ה-SM
                  ├── כן, ולפחות אחד כשיר   -> מחזור פעיל
                  ├── כן, אבל כולם תקועים    -> מחזור פעיל (!)
                  └── לא, אף warp לא תושב     -> מחזור לא-פעיל (מוחרג)
```
...
עכשיו ההגדרה המרכזית. **occupancy הוא היחס בין מספר ה-warps הפעילים למספר המרבי של warps פעילים על ההתקן.**

```text
   occupancy  =  warps פעילים  /  מקסימום warps פעילים
```
...
- **תפוסה תיאורטית (theoretical occupancy)** - החסם העליון ל-occupancy, הנגזר **מתצורת ההשקה ומיכולות ההתקן**. זהו תקרה סטטית: בהינתן כמה אוגרים ו-shared memory כל block צורך, וכמה threads יש ב-block, החומרה יכולה להכניס לכל היותר כך-וכך warps ל-SM. מחושב לפני הריצה, ללא הרצה בכלל.
...
וה-kernel שנשיק, עם פרמטרי ההשקה הבאים:
...
```text
   (1) מגבלת אוגרים:
       אוגרים ל-block = 32 threads x 8 אוגרים = 256 אוגרים
       blocks = 65,536 / 256 = 256 blocks

   (2) מגבלת shared memory:
       blocks = 228 KB / 12 KB = 19 blocks

   (3) מגבלת חומרה (blocks ל-SM):
       blocks = 32

   (4) מגבלת warps:
       warps ל-block = 1;  64 / 1 = 64 blocks

   צוואר-הבקבוק = min(256, 19, 32, 64) = 19 blocks
```
...
ה-kernel הזה **חסום-shared-memory**, ומקורק ל-occupancy תיאורטי של כ-30%. שימו לב מה זה אומר: אם היינו רוצים occupancy גבוה יותר, מגדיל האוגרים בכלל לא יעזור (הם לא הצוואר) - צריך להוריד את ה-12 KB ל-block. לו היינו מורידים ל-6 KB, המגבלה הייתה `228/6 = 38`, וזה כבר נחסם על-ידי מגבלת החומרה `min(38, 32) = 32` blocks = 32 warps = 50% occupancy. זהו כל האופי של אופטימיזציית occupancy: מזהים את המשאב-המגביל ומשחררים אותו - לא נוגעים בשאר.

הנוסחה הכללית שכדאי לחקוק:

```text
   max blocks ל-SM = min( blocks לפי אוגרים,
                          blocks לפי shared memory,
                          מקסימום blocks בחומרה,
                          blocks לפי מגבלת warps )
   theoretical occupancy = (max blocks x warps-ל-block) / max-warps-ל-SM
```
...
// kernel לדוגמה: 12 KB shared memory סטטי (= 3072 floats)
...
int main(void) {
    ...
    int blockSize   = 32;
    size_t dynSmem  = 0;                  // אין shared memory דינמי כאן
    ...
    // איזה block size ימקסם occupancy?
...
וה-API מאשש את החשבון הידני: **19 blocks, 19 warps, כ-30% occupancy**, חסום-shared-memory. שימו לב שה-`myKernel` כאן צורך 12 KB shared memory כ-`tile` סטטי, בדיוק כמו בדוגמה. מספר האוגרים בפועל תלוי בגוף ה-kernel ובאופטימיזציות ה-ptxas; כדי לראות אותו הריצו `nvcc --ptxas-options=-v` (יידפיס `Used N registers`), וכדי לכפות תקרת אוגרים השתמשו ב-`__launch_bounds__` או ב-`-maxrregcount`. אם תרצו לשנות את ה-occupancy, שחקו עם `blockSize` וב-`dynSmem` וראו את `blocksPerSM` זז.
...
```text
   64 warps x 32 threads = 2048 threads על ה-SM
   65,536 אוגרים / 2048 threads = 32 אוגרים ל-thread   (ב-100% occupancy)

   לעומת:
   16 warps x 32 = 512 threads
   65,536 / 512 = 128 אוגרים ל-thread                  (ב-25% occupancy)
```
...
- ל-**kernel חסום-חישוב** -> ממקסמים את ניצול צנרות החישוב (pipe / Tensor Core utilization).
- ל-**kernel חסום-זיכרון** -> ממקסמים את ניצול רוחב-הפס של הזיכרון.
...
```text
  ...
  No Eligible                           %        73.10   <- warp scheduler רעב
...
```text
  ...
  Stall Long Scoreboard                      cycle        11.90   <- המתנה ל-load גלובלי
...
- מצב ה-warp מתואר בארבעה שמות-תואר **לא-בלעדיים**: **פעיל** (תושב מרגע ההשקה עד שכל ה-threads יצאו), **כשיר** (פעיל וגם מוכן להנפיק - ארבעה תנאים: פקודה נשלפה, יחידה פנויה, תלויות נפתרו, אין barrier), **תקוע** (פעיל אך לא יכול להנפיק - תלות ביצוע/זיכרון/צנרת), ו**נבחר** (הכשיר שה-scheduler הנפיק לו במחזור זה).
...
- ה-occupancy = warps פעילים / מקסימום warps פעילים; יש **תיאורטי** (חסם עליון מתצורת ההשקה וממגבלות ההתקן, מחושב לפני הריצה) ו**נמדד** (מה שקרה בפועל, ממוצע על מחזורים פעילים, כמעט תמיד נמוך מהתיאורטי).
- בדוגמת ה-H100 (64 warps, 32 blocks, 65,536 אוגרים, 228 KB smem ל-SM) עם kernel של 32 threads/block, 8 אוגרים/thread ו-12 KB smem/block: מגבלות ה-blocks הן אוגרים 256, smem 19, חומרה 32 -> `min = 19` -> חסום-smem -> תפוסה ~30%.
...
- ה-occupancy הוא **אמצעי** להסתרת latency מספקת, לא **מטרה**: ל-kernel חסום-חישוב ממקסמים ניצול צנרות החישוב, ל-kernel חסום-זיכרון ממקסמים ניצול רוחב-הפס - ואת ה-occupancy מסתכלים רק כדי לוודא שעברנו את סף חוק Little.
