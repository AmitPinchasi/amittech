...
צרו קובץ `occ.cu` עם מקרו הבדיקה ו-kernel נסיוני שצורך shared memory מדיד. נשתמש ב-shared memory **דינמי** כדי שנוכל לשנות את גודלו בזמן ההשקה בלי לקמפל מחדש:

```bash
nvcc -O2 -arch=sm_90a -o occ occ.cu      # החליפו ל-sm_75 על T4
```
...
// kernel פשוט שצורך shared memory דינמי (גודלו נקבע בהשקה)
extern __shared__ float smem[];
__global__ void work(const float* in, float* out, int n, int iters) {
    int t = blockIdx.x * blockDim.x + threadIdx.x;
    float acc = 0.0f;
    for (int k = 0; k < iters; k++)
        acc += in[(t + k) % n] * 1.0009f;   // עבודה תלויה, latency של זיכרון
...
בלי קוד. נתונה סקציית Scheduler Statistics מדוח ncu אמיתי של kernel חסום-latency:
...
5. אבחון: האם ה-kernel הזה חסום-latency בגלל occupancy נמוך מדי, או שיש מספיק warps והבעיה במקום אחר? מה הייתם ממליצים - להעלות occupancy, או לא? הצדיקו לפי חוק Little.
