...
// תרגיל 1: כל thread מדפיס את ה-warp rank וה-lane שלו
__global__ void reportWarps() {
    ...
    // מדפיסים רק נתיב 0 של כל warp כדי לא להציף את הפלט
    if (lane == 0) {
...
// תרגיל 2: הצגת המסכה של 32 הנתיבים
__global__ void showMask() {
    unsigned active = __activemask();
    // כל נתיב שה-threadIdx.x שלו זוגי תורם 1 להצבעה
...
int main(int argc, char** argv) {
    ...
    if (which == 1) {
        dim3 block(100);            // בכוונה לא כפולה של 32
        reportWarps<<<1, block>>>();
    } else if (which == 2) {
        showMask<<<1, 32>>>();      // warp בודד
    }
...
```cpp
// גישה מאוחדת: thread i קורא את אלמנט i -> 32 נתיבים = 32 אלמנטים רצופים
__global__ void copyCoalesced(const float* in, float* out, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) out[i] = in[i];
}

// גישה מפוזרת: אותו חישוב בדיוק, אבל דילוג של stride בין נתיבים סמוכים
__global__ void copyStrided(const float* in, float* out, int n, int stride) {
    int t = blockIdx.x * blockDim.x + threadIdx.x;
    int i = (long long)(t) * stride % n;   // ערבול הכתובת שכל נתיב ניגש אליו
    if (t < n) out[t] = in[i];
}
```
...
__global__ void divergent(float* out, int mode) {
    ...
    float x = 0.0f;
    int iters;
    if (mode == 0)      iters = 100;                 // כולם שווים - אין divergence
    else                iters = (lane < 16) ? 10 : 200; // חצי warp שונה מהחצי
