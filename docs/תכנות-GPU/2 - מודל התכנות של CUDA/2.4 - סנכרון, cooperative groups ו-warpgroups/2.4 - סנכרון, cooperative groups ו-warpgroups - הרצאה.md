...
__global__ void stencil(const float* in, float* out, int n) {
    ...
    tile[tid] = (i < n) ? in[i] : 0.0f;   // 1. global -> shared (כל thread תא אחד)
    __syncthreads();                       // 2. מחסום: כל ה-tile נטען

    // 3. compute: עכשיו מותר לקרוא תאים של שכנים שכתב thread אחר
...
```cpp
// שגוי! deadlock אם n אינו כפולה של blockDim.x
if (i < n) {
    tile[tid] = in[i];
    __syncthreads();   // רק חלק מה-threads נכנסים לענף ומגיעים לכאן
}

// נכון: כל thread מגיע למחסום, וההגנה על הגבול נעשית בטעינה עצמה
...
```text
  רמה בהיררכיה     ערוץ תקשורת            כלי סנכרון          עלות
  ------------     -----------            -----------          ----
  warp (32)        אוגרים דרך shuffle     __syncwarp          ~מחזורים בודדים
  block (CTA)      shared memory (L1)     __syncthreads        עשרות מחזורים
  grid             global memory          atomics (אין מחסום)  מאות מחזורים
```
...
__inline__ __device__ float warpReduceSum(float val) {
    // עץ צמצום: 16, 8, 4, 2, 1 - חמישה צעדים ל-warp שלם
    for (int offset = 16; offset > 0; offset >>= 1)
        val += __shfl_down_sync(0xffffffff, val, offset);
    return val;   // התוצאה המלאה יושבת ב-lane 0
}
```

```text
  צמצום warp עם __shfl_down_sync (מוצג ל-8 lanes; ב-warp אמיתי 32 -> 5 צעדים)
  lane:   0    1    2    3    4    5    6    7
  התחלה:  a0   a1   a2   a3   a4   a5   a6   a7
  off=4:  a0+a4 a1+a5 a2+a6 a3+a7  .    .    .    .
  off=2:  +a2.. +a3..   .    .
  off=1:  SUM    .
          ^ הסכום המלא של כל השמונה יושב ב-lane 0
```
...
__global__ void reduceShared(const float* in, float* out, int n) {
    ...
    // צמצום עץ ב-shared: בכל שלב מחצית ה-threads מסכמת זוג
    for (int s = blockDim.x / 2; s > 0; s >>= 1) {
        if (tid < s) sdata[tid] += sdata[tid + s];
        __syncthreads();                    // מחסום אחרי כל שלב - הכרחי
    }
...
__global__ void reduceShfl(const float* in, float* out, int n) {
    ...
    val = warpReduceSum(val);               // צעד 1: כל warp מצטמצם, תוצאה ב-lane 0

    __shared__ float warpSums[32];          // לכל היותר 1024/32 = 32 warps ל-block
    int lane = threadIdx.x % warpSize;
    int wid  = threadIdx.x / warpSize;
    if (lane == 0) warpSums[wid] = val;     // כל warp כותב תא אחד
    __syncthreads();                        // המחסום היחיד בכל ה-kernel

    // צעד 2: ה-warp הראשון מצמצם את הסכומים החלקיים
...
__global__ void reduceCG(const float* in, float* out, int n) {
    cg::thread_block block = cg::this_thread_block();          // ה-block כולו
    cg::thread_block_tile<32> warp = cg::tiled_partition<32>(block);  // חלוקה ל-warps

    int i = blockIdx.x * blockDim.x + threadIdx.x;
    float val = (i < n) ? in[i] : 0.0f;

    val = cg::reduce(warp, val, cg::plus<float>());            // צמצום warp נקי, בלי מסכות

    __shared__ float warpSums[32];
    if (warp.thread_rank() == 0)
        warpSums[warp.meta_group_rank()] = val;               // meta_group_rank = אינדקס ה-warp ב-block
    block.sync();                                              // שקול ל-__syncthreads
...
- `cg::tiled_partition<N>(g)` - מחלקת קבוצה לאריחים (tiles) בגודל `N` בזמן קומפילציה (32 = warp, אבל גם 16, 8, 4...); כל אריח הוא קבוצה עצמאית עם `.sync()` משלו.
...
## סנכרון חוצה-grid - grid_group וההשקה הקואופרטיבית
...
dim3 grid(numBlocksPerSm * numSm);      // ה-grid המקסימלי שנכנס בבת אחת

void* args[] = { &d_in, &d_out, &n };
CUDA_CHECK(cudaLaunchCooperativeKernel((void*)myKernel, grid, blockSize, args, 0, 0));
```

ה-`CUDA_CHECK` הוא מאקרו בדיקת שגיאות שנשתמש בו לאורך הקורס - הוא עוטף כל קריאת CUDA, בודק את קוד השגיאה המוחזר, ומדפיס קובץ+שורה אם משהו נכשל. במקרה של השקה קואופרטיבית הוא קריטי, כי `cudaErrorCooperativeLaunchTooLarge` היא בדיוק השגיאה שנקבל אם ביקשנו grid גדול מדי. הכלל: `grid.sync()` הוא כלי נדיר ויקר; לרוב עדיף לפצל ל-שני kernels נפרדים (סנכרון בין השקות הוא מחסום גלובלי "חינם"), ולהשתמש ב-grid group רק כשהאלגוריתם באמת דורש מספר סבבים על אותם נתונים תושבים.
...
הסיבה שכל זה קיים היא הוראה אחת: `wgmma.mma_async` (warpgroup matrix multiply-accumulate, asynchronous - כפל-וצבירת-מטריצות אסינכרוני ברמת warpgroup). במקום ש-32 threads של warp בודד ישתפו פעולה על אריח מטריצה קטן, 128 threads של warpgroup שלם מבצעים **אריח כפל-מטריצות גדול** בהוראה אחת. שלושה יתרונות נובעים מכך:

- **ריווי ה-Tensor Cores:** אריחים גדולים יותר מזינים את רוחב הפס האריתמטי האדיר של ה-Tensor Cores טוב יותר - הוראה אחת שמעסיקה את כל ארבעת ה-Tensor Cores של ה-SM.
...
- **אסינכרוניות:** ה-`wgmma` הן אסינכרוניות - כפל המטריצה מתקדם ברקע בעוד ה-warpgroup מבצע עבודה אחרת (למשל טוען את האריח הבא), מה שמאפשר חפיפה מלאה בין חישוב לזיכרון.
...
- הדפוס הקנוני הוא global -> shared -> __syncthreads -> compute: טוענים אריח מ-global אל shared, מסנכרנים, ורק אז מעבדים - אחרת נוצר race condition.
