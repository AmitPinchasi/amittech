...
// --- כאן ייכנסו ה-kernels מהתרגילים ---

int main() {
    const int n = 1 << 24;                 // ~16.7 מיליון אלמנטים
    const int block = 256;
    ...
    double ref = 0.0;
    for (int i = 0; i < n; ++i) { h_in[i] = 1.0f; ref += h_in[i]; }  // סכום ידוע = n
    ...
    // דוגמת השקה (החליפו את שם ה-kernel בכל תרגיל):
    reduceShared<<<grid, block, block * sizeof(float)>>>(d_in, d_partial, n);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());

    // צירוף הסכומים החלקיים של ה-blocks על ה-CPU (בתרגיל 4 נעשה זאת ב-atomic)
...
הידור: `nvcc -arch=sm_90 reduce.cu -o reduce` והרצה `./reduce`.
...
1. עטפו כל השקת kernel ב-`cudaEvent` (`cudaEventRecord` לפני ואחרי, `cudaEventElapsedTime`) והדפיסו זמן ריצה במילישניות. הריצו כל kernel 100 פעמים בלולאה וקחו ממוצע (אחרי warmup של השקה אחת).
...
2. במקום שכל block יכתוב לתא `out[blockIdx.x]` ואז נסכם על ה-CPU, תנו ל-thread מספר 0 של כל block לבצע `atomicAdd(out, blockSum)` אל **תא גלובלי יחיד**. אתחלו את התא ל-0 עם `cudaMemset` לפני ההשקה.
...
נשתמש ב-Cooperative Groups כדי לכתוב צמצום נקי ברמת אריח בגודל כלשהו.
...
2. ב-kernel, צרו אריח בגודל 32 עם `tiled_partition<32>` וקראו לפונקציה. ואז נסו אריח בגודל **16** (`tiled_partition<16>`) והשוו את התוצאה - כל אריח של 16 מצטמצם בנפרד.
3. הדפיסו מ-thread אחד לכל אריח את `g.thread_rank()`, `g.size()`, ו-`g.meta_group_rank()` כדי לראות איך ה-block מתחלק.
4. הסבירו: מה היתרון של `tiled_partition` על פני שימוש ישיר ב-`__shfl_down_sync` עם מסכה ידנית? מתי אריח קטן מ-32 שימושי?

רמז: `tiled_partition<N>` דורש ש-`N` יהיה חזקה של 2 עד 32. הקבוצה נושאת את המסכה בתוכה, אז אין סכנה לטעות במסכה כשעובדים עם אריחים חלקיים. אריחים קטנים שימושיים כשכל קבוצה קטנה של threads מעבדת יחידת עבודה נפרדת (למשל 16 threads לכל שורה של מטריצה קטנה).
...
רמז: warpgroup ממפה warp אחד לכל תת-מחיצה, ולכל תת-מחיצה Tensor Core משלה - לכן 4. block של 256 = 8 warps = 2 warpgroups (warp-ranks 0-3 ו-4-7). ה-wgmma אסינכרוני, ולכן ה-warpgroup יכול לטעון את האריח הבא בזמן שכפל המטריצה הנוכחי רץ ברקע - חפיפה שהיא לב הביצועים של kernels כמו FlashAttention.
