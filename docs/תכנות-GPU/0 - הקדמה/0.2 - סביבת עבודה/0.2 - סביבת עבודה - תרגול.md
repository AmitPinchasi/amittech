...
1. בתוך `main`, קראו ל-`cudaGetDeviceCount` (עטוף ב-`CUDA_CHECK`) ולולאה על כל התקן.
2. לכל התקן קראו ל-`cudaGetDeviceProperties`, והדפיסו לפחות: השם, ה-compute capability (`major.minor`), מספר ה-SM (`multiProcessorCount`), ה-`warpSize`, `maxThreadsPerBlock`, `sharedMemPerBlock`, וגודל הזיכרון הגלובלי (`totalGlobalMem`).
...
רמז: kernel תמיד מחזיר `void` - הוא לא "מחזיר" ערך אלא כותב (כאן, מדפיס). בלי `cudaDeviceSynchronize()` המארח יכול לצאת לפני שה-GPU רוקן את חוצץ ה-`printf`, אז הפלט עלול להיעלם. הסדר בין blocks לעולם אינו מובטח.
...
רמז: `cudaGetErrorString` מתרגם `cudaError_t` לטקסט קריא. אינדקס התקן לא חוקי מחזיר `cudaErrorInvalidDevice`.
