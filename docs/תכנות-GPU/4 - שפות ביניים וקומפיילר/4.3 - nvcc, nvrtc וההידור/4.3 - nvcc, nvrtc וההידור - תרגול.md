...
# למשל:  NVIDIA H100 80GB HBM3, 9.0   -> sm_90 / compute_90
...
### תרגיל 3 - הידור מותנה: התקן מול מארח
...
1. כתבו פונקציה אחת עם `__host__ __device__` שמתנהגת שונה בשני העולמות:
   ...
   #ifdef __CUDA_ARCH__
       return __CUDA_ARCH__;   // מתקמפל לקוד GPU
   #else
       return -1;              // מתקמפל לקוד CPU
   #endif
   }
   __global__ void probe(int* out) { out[0] = where_am_i(); }
   ```
2. קראו ל-`where_am_i()` גם מהמארח (ב-`main`, והדפיסו), וגם דרך ה-kernel `probe` (העתיקו את `out[0]` חזרה והדפיסו).
3. קמפלו עם `-arch=sm_90` והריצו. איזה ערך הדפיס המארח ואיזה ה-kernel?
4. הסבירו: למה אותה פונקציה בדיוק החזירה שני ערכים שונים?

רמז: במעבר המארח `__CUDA_ARCH__` **אינו מוגדר**, ולכן ה-`#ifdef` נכשל והמסלול הוא `return -1`. במעבר ההתקן הוא מוגדר לערך של הארכיטקטורה (`900` ל-`sm_90`), ולכן ה-kernel מחזיר `900`. זו הדגמה של הכלל מההרצאה: `__CUDA_ARCH__` מוגדר רק בקוד התקן. המארח ידפיס `-1`, ה-kernel ידפיס `900`.

### תרגיל 4 - הידור נפרד וקישור התקן
...
3. עכשיו קמפלו **עם** הידור נפרד וקישור התקן:
   ```bash
...
4. (בונוס) בצעו את שלב הקישור האחרון בשני שלבים נפרדים - `-dlink` ואז קישור מארח עם `g++` - כדי לראות את שלב קישור-ההתקן במפורש.

רמז: בלי `-dc`, כל קובץ מהודר כתוכנית-שלמה עצמאית, וההפניה של `apply` ל-`square` שבקובץ אחר נשארת לא-פתורה. השגיאה תהיה בסגנון `ptxas fatal : Unresolved extern function` או `nvlink error`. הדגל `-dc` (שווה ל-`-rdc=true -c`) מייצר קוד התקן ניתן-להזזה, ושלב הקישור האחרון מפעיל את `nvlink`. הפלט הצפוי הוא הריבועים: `0 1 4 9 16 25 36 49`.

### תרגיל 5 - הידור בזמן ריצה עם nvrtc

1. כתבו תוכנית שמהדרת kernel **ממחרוזת** בזמן ריצה עם `nvrtc`, ומשיקה אותו דרך ה-Driver API. השתמשו ב-kernel של `saxpy` (`out = a*x + y`).
2. השלבים בקוד: `nvrtcCreateProgram` -> `nvrtcCompileProgram` (עם `--gpu-architecture=compute_90`) -> תמיד משכו ומדפיסים את יומן ההידור -> `nvrtcGetPTX`. אחר כך Driver API: `cuInit` -> `cuCtxCreate` -> `cuModuleLoadDataEx` (כאן ה-driver עושה JIT) -> `cuModuleGetFunction` -> `cuLaunchKernel`.
