...
// ב. MUFU (פונקציה מיוחדת) - expf רץ על ה-SFU
...
// ג. HMMA - כפל מטריצות 16x16x16 על Tensor Cores
...
שתי צורות ההידור שנשתמש בהן לאורך התרגול - בינארי-מארח (ל-`cuobjdump`) ו-cubin ישיר (ל-`nvdisasm`):

```bash
# בינארי מארח שלם, עם מידע-שורות
nvcc -O2 -arch=sm_90a -lineinfo -c -o kernels.o kernels.cu

# cubin ישיר (הקלט של nvdisasm), עם מידע-שורות
...
2. פרקו את הפונקציה `wgemm` ואתרו את פקודות ה-`HMMA`. שימו לב לצורת הפקודה, למשל `HMMA.16816.F32` - המספרים הם ממדי האריח (M, N, K) שה-Tensor Core מעבד בפקודה אחת.
...
רמז: אם אתם על T4 (`sm_75`), ה-`wgemm` עדיין מקמפל ומייצר HMMA (Tensor Cores קיימים מ-Volta), אבל ה-`.16816` עשוי להיות `.1688` בגלל ממדי אריח שונים לפי דור. כפל מטריצות `16x16x16` הוא `16*16*16 = 4096` פעולות כפל-חיבור; `HMMA` יחיד מבצע אריח שלם, בעוד על CUDA Cores היו נדרשות אלפי `FFMA`. זה בדיוק היתרון של Tensor Cores שראינו ב-1.5.
...
4. מדוע פקודות בודדות עשויות להתמפות לשורות מקור "לא צפויות" (למשל פקודת `IMAD` שממופה לשורת ההכרזה של הלולאה)? מה עשה המהדר?

רמז: `nvdisasm -g` דורש cubin שקומפל עם `-lineinfo` (או `-G`). אם לא רואים מספרי שורות, שכחתם את הדגל בהידור. אופטימיזציות כמו reordering ו-instruction scheduling מפזרות פקודות שמקורן בשורה אחת על פני הפלט, ולכן המיפוי לא תמיד רציף - זו הסיבה שקוראים SASS לצד המקור ולא במקומו.
...
רמז: מ-Hopper ואילך, טעינות global עוברות דרך **מתאר זיכרון** (memory descriptor) שיושב ב-Uniform Register, ולכן תראו `desc[UR4][...]` ב-sm_90a אבל לא ב-sm_70. המסלול האחיד (uniform datapath) בשל יותר בדורות החדשים. זה בדיוק העניין ש-SASS צמוד-ארכיטקטורה: אותו PTX (ואותו מקור) עובר JIT/הידור ל-SASS שונה לכל דור. אם רוצים לראות שה-PTX **זהה** בין הדורות, הריצו `cuobjdump --dump-ptx` על שתיהן והשוו.
...
3. הוסיפו `-maxrregcount=32` להידור, פרקו שוב, וראו כיצד מספר ה-`STL`/`LDL` **גדל** (יותר spilling) בזמן שמספר האוגרים לכל thread קטן. הסבירו את הטרייד-אוף מול ה-occupancy.
