...
```text
pwndbg> cyclic -l 0x6161616161616166   # the value found in RSP at crash time
72
```
...
- **מה היה חסר עם PIE?** עם PIE הכתובות `JMPREL`, `SYMTAB`, `STRTAB`, `PLT0` והאזור הכתיב הן יחסיות לבסיס אקראי של הבינארי. היינו צריכים קודם לדלוף את בסיס הבינארי (לא בסיס libc), ורק אז לחשב את כל הכתובות היחסית אליו ולבנות את אותה chain.
