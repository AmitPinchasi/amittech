...
```bash
gcc -o demo demo.c
# gcc יתריע: warning: implicit declaration of function 'gets' - זה צפוי ותקין
echo "hello" | ./demo
```
...
- הדגל `-fno-stack-protector` הוא זה שגרם ל-Stack Canary להיעלם. בלעדיו gcc מדליק canary אוטומטית על פונקציות עם חוצצים.
...
```bash
ldd ./demo_default        # מאתר את הנתיב ל-libc.so.6
...
```bash
cat /proc/sys/kernel/randomize_va_space      # בדרך כלל 2 (מלא)
echo 0 | sudo tee /proc/sys/kernel/randomize_va_space
cat /proc/sys/kernel/randomize_va_space      # עכשיו 0
```

בדיקה חזותית שהכתובות התייצבו - הריצו פעמיים ורואים אותה כתובת מחסנית:

```bash
./demo_nopie_static &                       # או כל בינארי
...
```bash
ldd --version | head -1
# לדוגמה: ldd (Ubuntu GLIBC 2.35-0ubuntu3.8) 2.35
```
...
context.binary = ELF('./demo_default')   # קובע amd64 אוטומטית
...
למה זה עבד: `context.binary = ELF(...)` קרא מהקובץ שזה בינארי amd64 והדפיס את פרופיל ההגנות שלו, `process` הפעיל אותו כתת-תהליך, ו-pwntools ניהל את הצינור (stdin/stdout) בשבילנו. זה בדיוק השלד שנרחיב בכל שאר הקורס - רק שבמקום `sendline` תמים נשלח payload שדורס את כתובת החזרה.
