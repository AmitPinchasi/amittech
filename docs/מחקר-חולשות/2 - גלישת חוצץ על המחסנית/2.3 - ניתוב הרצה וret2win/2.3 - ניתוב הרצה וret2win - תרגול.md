...
בתרגול הזה תבנו בעצמכם בינארים פגיעים, תנתבו את ההרצה שלהם לפונקציה מנצחת, ותלמדו להעביר ארגומנטים גם ב-64 ביט וגם ב-32 ביט. זה בדיוק הבסיס לאתגר `bof` מ-pwnable.kr (שנפתור בהמשך הפרק) ולכל אתגרי ה-`ret2win` הקלאסיים. עבדו לפי הסדר - כל תרגיל בונה על הקודם.
...
הערה על הסביבה: השאירו את ההגנות כמו שהן - NX פעיל, בלי קנרי, בלי PIE. לא צריך לכבות ASLR כי אנחנו לא משתמשים בכתובות libc, רק בכתובות קבועות מתוך הבינארי.
...
3. מצאו את ה-offset עד כתובת החזרה של `vuln` בכל בינארי, בשיטת ה-`cyclic` מ-2.2. רמז: `cyclic 200` לקלט, ואז `cyclic_find(...)` על הערך שדרס את מצביע ההוראות.

רמז: ה-offset של `warmup` ו-`keys` שונה כי החוצצים בגודל שונה (64 מול 80). אל תניחו, תמדדו.
...
2. הרכיבו מטען: ריפוד עד ה-offset, ואז כתובת `win`.
3. הריצו וּודאו שאתם רואים את שורת הדגל.

רמז: כאן `win` לא קוראת לשום פונקציית libc כבדה, אז לרוב לא תיתקלו בבעיית יישור. אם בכל זאת יש קריסה, הציצו כבר עכשיו בסעיף היישור בהרצאה.
...
2. בנו ידנית שרשרת: `[pop rdi] [0xc0ffee] [win]`.
...
3. אם ה-shell לא נפתח למרות ששני האוגרים נכונים - זו כמעט בוודאות בעיית היישור מה-`movaps`. הוסיפו גאדג'ט `ret` בודד ליישור לפני הקריאה ונסו שוב.
4. כשמשיג shell, הריצו `cat` על משהו כדי לוודא שהוא אינטראקטיבי.

רמז: אחרי שתעבדו ידנית, נסו לשכתב את כל השרשרת בעזרת אובייקט `ROP` של pwntools עם `rop.call('win', [0xc0ffee, 0x1337])`, ואל תשכחו את `rop.find_gadget(['ret'])` ליישור.
...
רמז: `long` ב-32 ביט הוא 4 בתים. סלוט החזרה המזויף יכול להיות ערך דמה כי לא נחזור מ-`win` אחרי ה-shell, אבל הוא חייב להיות שם כדי לשמור על היישור של הארגומנטים.
...
2. קראו את תיאור האתגר `bof` מ-pwnable.kr. הוא דומה ברוח אבל שונה במנגנון: שם דורסים ערך של משתנה מקומי כדי לעבור בדיקה, לא כתובת חזרה. חשבו: איזה חלק מהידע מהתרגול הזה חוזר שם, ומה שונה?

רמז: ב-`bof` אין פונקציית `win` שקופצים אליה - הגלישה משנה ארגומנט שנבדק ב-`if`. אבל שיטת מציאת ה-offset וסידור המטען זהים לחלוטין למה שעשיתם כאן.
