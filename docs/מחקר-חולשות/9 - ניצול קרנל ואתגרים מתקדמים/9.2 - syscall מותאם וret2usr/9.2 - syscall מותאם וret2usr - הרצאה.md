...
![תרשים זרימה שמשווה בין דרייבר שבודק כתובת מהמשתמש עם access_ok ו-copy_to_user לבין דרייבר פגיע שכותב ישירות לכתובת שהמשתמש בחר בלי שום בדיקה](../../../images/illustrations/vuln-9-2-boundary-check.svg)
...
![תרשים זרימה של ret2usr: זרימת הבקרה בקרנל נחטפת, ואם SMEP כבוי הקפיצה לפונקציה במרחב המשתמש מצליחה ומריצה commit_creds(prepare_kernel_cred(0)) ומחזירה אותנו כ-root, אבל אם SMEP דלוק המעבד מסרב להריץ עמוד משתמש והקרנל קורס](../../../images/illustrations/vuln-9-2-ret2usr-flow.svg)
