...
בתרגול הזה תבנו שרשראות ROP בעצמכם, שלב אחרי שלב: קודם קריאה לפונקציה עם ארגומנט שאתם שולטים בו, ואז שרשרת מלאה שקוראת ל-`system("/bin/sh")` ופותחת shell. תעבדו גם ידנית (כתובות גאדג'ט מפורשות) וגם עם אובייקט ה-`ROP` של pwntools, כדי להרגיש את שתי הדרכים. עבדו לפי הסדר - כל תרגיל בונה על הקודם.
...
// לא נקראת בזרימה הרגילה - רק כדי ש-system ו-/bin/sh יהיו בבינארי
void never_called() {
    system("/bin/sh");
}
...
קמפלו ל-64 ביט, בלי קנרי ובלי PIE:

```bash
gcc -fno-stack-protector -no-pie -o rop_lab rop_lab.c
```

הערה על הסביבה: השאירו את ההגנות כמו שהן - NX פעיל, בלי קנרי, בלי PIE. לא צריך לכבות ASLR, כי כל הכתובות שנשתמש בהן (הקוד, ה-PLT, המחרוזת) קבועות בבינארי לא-PIE.
...
3. מצאו את ה-offset עד כתובת החזרה של `vuln` בשיטת ה-`cyclic` מ-2.2.

רמז: החוצץ הוא 64 בתים ו-`saved rbp` הוא 8 בתים ב-64 ביט, אז ה-offset אמור לצאת סביב 72. אל תניחו, תמדדו עם `cyclic 200` ו-`cyclic_find`.
...
2. מצאו גם גאדג'ט `ret` בודד (תצטרכו אותו ליישור בתרגיל 4): `ROPgadget --binary rop_lab | grep ': ret$'`.
...
### תרגיל 3 - שרשרת ROP עם ארגומנט מבוקר
...
1. בנו ידנית שרשרת: ריפוד עד ה-offset, ואז `[pop rdi] [0xdeadbeefcafe] [secret]`.
2. השתמשו ב-`flat({offset: [...]})` כדי לבנות את המטען.
...
### תרגיל 4 - שרשרת ROP ל-shell

עכשיו השלב הגדול: `system("/bin/sh")` שפותח shell אמיתי.

1. בנו ידנית שרשרת: `[pop rdi] [כתובת /bin/sh] [system@plt]`, עם ריפוד עד ה-offset.
...
רמז: זו בדיוק מלכודת היישור מההרצאה. הסדר הנכון עם התיקון הוא `[ret] [pop rdi] [binsh] [system]`. אם תשכחו את ה-`ret`, יש סיכוי טוב לקריסה על `movaps`.
...
2. בנו: `rop = ROP(elf)`, ואז `rop.raw(rop.find_gadget(['ret'])[0])` ליישור, ואז `rop.call('system', [binsh])`.
3. הדפיסו את `rop.dump()` והשוו לכתובות שמצאתם ידנית בתרגיל 4. הן אמורות להתאים.
4. שלחו `flat({offset: rop.chain()})` וּודאו shell.

רמז: `rop.call('system', [binsh])` מוצא את `pop rdi ; ret` בעצמו. זכרו את גאדג'ט ה-`ret` ליישור לפני הקריאה.
