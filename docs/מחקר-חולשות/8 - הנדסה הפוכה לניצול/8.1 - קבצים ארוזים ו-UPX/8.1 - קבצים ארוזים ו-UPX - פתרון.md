...
strings secret.packed | grep -i flag   # (ריק - נעלם)
strings secret.packed | grep -i upx    # מגלה את UPX! ואת החתימות
```
...
```bash
readelf -S ./flag     # אזהרה / "There are no sections in this file."
readelf -l ./flag     # שני מקטעי LOAD גדולים, entry בתוך השני
```
...
readelf -S ./flag.unpacked   # .text .rodata .data ... חזרו כל הסקשנים
strings ./flag | wc -l        #  מעט מאוד
strings ./flag.unpacked | wc -l  # הרבה יותר
```
...
<main>:
    ...
    mov    rdx,QWORD PTR [rip+...] ; טוען את המצביע הגלובלי flag
    mov    rax,QWORD PTR [rbp-0x8]
    mov    rsi,rdx                 ; מקור ה-strcpy = המחרוזת שעליה מצביע flag
    mov    rdi,rax                 ; יעד = החוצץ שהוקצה
    call   <strcpy>
```
...
Program stopped at the entry point (בתוך ה-stub של UPX)
...
pwndbg> info proc mappings
    0x400000   0x4c8000   ...   האזור של הבינארי המקורי
...
grep -abo 'UPX!' secret.corrupt        # מציג את ההיסטים של כל 'UPX!'
# משבשים בית אחת מההופעות (למשל בהיסט השני) בעזרת עורך hex או printf:
...
```bash
./secret.corrupt          # רץ כרגיל, מדפיס את ההודעה
