...
בתרגול הזה נבנה binary פגיע משלנו, נריץ אותו מול glibc מודרני (2.31 ומעלה), וננצל אותו עד shell בלי לגעת ב-`__free_hook` - בדיוק כמו שנצטרך לעשות על 2.34+. המטרה: לתרגל את המסלול המלא, מדליפה, דרך tcache poisoning, ועד אחת המטרות המודרניות מההרצאה.
...
void del(void) {
    printf("idx: "); int i = get_int();
    if (i < 0 || i >= MAX) return;
    free(notes[i]);          // באג: אין notes[i] = NULL
}
...
הנחות ההגנה שתראו ב-`checksec`: NX enabled, No canary, No PIE, Partial RELRO. ASLR של המערכת דולק - אל תכבו אותו, כל הרעיון הוא לעקוף אותו עם דליפות.

כדי לשלוט בגרסת glibc, הריצו את הבינארי מול libc ספציפי. הכי נוח עם pwninit או patchelf:

```bash
# מורידים libc של הגרסה הרצויה (למשל 2.31 מ-Ubuntu 20.04, או 2.35 מ-22.04)
pwninit --bin ./notes --libc ./libc.so.6         # יוצר notes_patched ו-ld
# או ידנית:
patchelf --set-interpreter ./ld-linux-x86-64.so.2 --replace-needed libc.so.6 ./libc.so.6 ./notes
```

הבאג: `del` משחרר בלי לאפס את המצביע. זה נותן לכם Use-After-Free מלא - אתם יכולים לצפות ב-chunk ששוחרר (דליפה), לערוך אותו (הרעלת fd), ואפילו לשחרר אותו פעמיים.
...
### תרגיל 2 - דליפת libc ודליפת heap

בנו את שלב הדליפות של ה-exploit ב-pwntools.
...
רמז: ודאו שהמטרה מיושרת ל-16 בתים, אחרת glibc 2.32+ יזרוק `unaligned tcache chunk detected`.
...
3. הרעילו שוב כדי להקצות מעל ה-saved RIP, וכתבו לשם שרשרת ret2libc (`pop rdi; ret; "/bin/sh"; system`).
...
רמז 2: שרשרת ROP צריכה גאדג'ט `ret` ליישור לפני `system` (בעיית movaps).

מסלול ב - FSOP בעת יציאה:

1. הרעילו tcache כדי לכתוב על `_IO_2_1_stdout_` ולהשיג דליפה (או השתמשו בדליפה מתרגיל 2), ולאחר מכן בנו `FILE` מזויף.
2. השתמשו ב-`FileStructure()` של pwntools כדי לזייף `stdout` שמדליף זיכרון, או בנו מבנה house of apple2 מלא ל-RCE.
3. דרסו את `_IO_list_all` שיצביע ל-`FILE` המזויף, וצאו מהתוכנית (אפשרות 5) כדי להפעיל את השרשרת.
