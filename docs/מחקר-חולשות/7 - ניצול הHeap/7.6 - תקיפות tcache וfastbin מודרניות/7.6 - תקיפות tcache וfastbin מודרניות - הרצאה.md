...
- החל מ-glibc 2.32 יש safe-linking: שדה ה-`fd` נשמר מעורבב, `fd = target XOR (chunk_addr >> 12)`. כדי לזייף אותו צריך לדעת את כתובת ה-chunk (leak heap), ואז לכתוב `target XOR (chunk_addr >> 12)`.
- החל מ-2.32 יש גם בדיקת alignment: הכתובת שמוחזרת חייבת להיות aligned ל-16 בתים, אחרת `malloc` יזרוק `malloc(): unaligned tcache chunk detected`. לכן כשבוחרים מטרה `T` צריך שתהיה aligned ל-16.
...
2. מחשבים את כתובת הreturn address שאנחנו רוצים לדרוס: `target = stack_leak - delta`. את ה-`delta` הקבוע (המרחק בין `environ` לבין ה-saved RIP של הפונקציה שתחזור) מוצאים פעם אחת ב-gdb, והוא זהה בכל הרצה כי המרחק בין שתי כתובות על אותה מחסנית קבוע.
3. tcache poisoning שני: מקצים chunk מעל `target`, וכותבים לשם chain ROP.
...
### הleak המהירה דרך _IO_2_1_stdout_
...
לקבל RCE דרך FSOP קשה יותר, כי מאז glibc 2.24 יש בדיקת vtable: `_IO_vtable_check` מוודא שמצביע ה-vtable נופל בתוך המקטע `__libc_IO_vtables`. אי אפשר יותר סתם להצביע את ה-vtable לכתובת שרירותית. הפתרון המודרני, שנקרא house of apple2 ועובד מצוין על 2.34 ו-2.35, exploits vtable **חוקי** אבל מסלול שבסופו קפיצה דרך מצביע ש**איננו** נבדק.
...
- אם יש לנו leak של ה-guard, מחשבים `mangled` ישירות.
...
```text
before:                                after inserting p2:
p1->bk_nextsize = target - 0x20  --->  *(target) = &p2   (heap address)
```
...
- מטרה 1 - environ: קוראים כתובת מחסנית, כותבים chain ROP על saved RIP. אמין ובלתי תלוי בגרסה.
- מטרה 2 - FSOP: מזייפים `_IO_FILE`. לleak - `_IO_2_1_stdout_`. ל-RCE על 2.34+ - house of apple2 דרך `_IO_wfile_jumps` וה-`_wide_vtable` שלא נבדק.
