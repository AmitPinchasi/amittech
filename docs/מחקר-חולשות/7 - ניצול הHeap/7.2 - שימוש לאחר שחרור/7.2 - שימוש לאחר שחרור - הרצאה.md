...
![תרשים המראה שלושה שלבים של מחזור נתח משוחרר: הבלוק בשימוש עם מצביע שלנו, אחרי free הבתים הראשונים הופכים למצביע fd והמצביע שלנו נשאר תלוי, ואז הקצאה חדשה באותו גודל מחזירה את אותו בלוק עם תוכן שהתוקף שולט בו](../../../images/illustrations/vuln-7-2-uaf-reuse-steps.svg)
...
![תרשים של קריאה וירטואלית ב-C++: מהאובייקט אל ה-vptr שלו, ומה-vptr אל טבלת ה-vtable שבה כל slot מחזיק מצביע לפונקציה אחרת](../../../images/illustrations/vuln-7-2-vtable-dispatch.svg)
...
```text
fake vptr = (_ZTV5Human + 16) - 8 = _ZTV5Human + 8
```
...
- **גודל האובייקט הוא 0x18 = 24 בתים.** ה-string במחלקה הודר עם ה-ABI הישן (COW string) שהוא מצביע יחיד, אז: vptr (8) + int age עם padding (8) + string (8) = 24. לכן `new char[24]` נופל לאותה מחלקת גודל בדיוק כמו האובייקט (chunk של 0x20), וממחזר אותו.
