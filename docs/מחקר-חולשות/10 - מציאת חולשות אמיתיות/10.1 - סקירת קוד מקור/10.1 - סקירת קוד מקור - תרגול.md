## תרגול - סקירת קוד של מנהל פתקים פגיע
...
```c
// notes.c - קמפלו עם: gcc -fno-stack-protector -no-pie -o notes notes.c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
// קורא שורה מהקלט הסטנדרטי עד תו שורה חדשה
...
- רמז 2: אם `read_line` פגומה, אז **כל** קורא שלה פגיע. מי הקוראים? (`add_note` עבור title, `read_int` עבור tmp, ו-`main` עבור choice).
- רמז 3: מה קורה אם בתפריט הראשי, בתגובה ל-`> `, תקלידו 200 תווים? לאיזה חוצץ הם נכנסים, וכמה מקום יש לו?
...
```c
int len = read_int();        // len בשליטת התוקף. איזה ערכים הוא יכול לקבל?
n->body = malloc(len + 1);   // מה קורה כאן אם len == -1? אם len == 2147483647?
n->len  = len;
read(0, n->body, len);       // הפרמטר השלישי של read הוא size_t. מה קורה אם len שלילי?
n->body[len] = '\0';
```
...
יש כאן יותר ממחלקת באג אחת. נסו למנות שתיים לפחות (רמז: אחת מהן קשורה לגלישת שלם, אחת לסימן).
