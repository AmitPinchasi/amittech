...
בתרגול הזה תכתבו harness של libFuzzer לספרייה קטנה, תבנו אותה עם AddressSanitizer, ותנו ל-fuzzer למצוא בשבילכם גלישת heap אמיתית - בלי לקרוא את הקוד ולחפש את הבאג ביד. אחר כך תלמדו לקרוא את דוח הקריסה, לשחזר אותו, ולמזער אותו. עבדו לפי הסדר; כל תרגיל בונה על הקודם.
...
צרו את הקבצים הבאים. זו ספריית ניתוח (parser) לפורמט בינארי זעיר בשם TinyParse. הפורמט:

```text
בתים 0..3 : מספר קסם (magic) - התווים "TP01"
בית 4     : מספר רשומות n
ואז n רשומות, כל אחת: [בית type][בית len][len בתים ערך]
```
...
int tp_parse(const uint8_t *data, size_t size) {
    if (size < 5) return -1;
    if (memcmp(data, "TP01", 4) != 0) return -1;   // שער ה-magic
    uint8_t n = data[4];
    size_t off = 5;
    for (uint8_t i = 0; i < n; i++) {
        ...
        off += 2;
        if (off + len > size) return -1;           // לא קוראים מעבר לקלט
        if (type == 0x02) {                        // רשומת "שם"
            char *name = malloc(16);               // חוצץ קבוע של 16 בתים
            memcpy(name, data + off, len);         // מעתיקים len בתים לתוכו
            free(name);
        }
        off += len;
    }
...
3. באיזו שורה בקובץ `tinyparse.c` נמצאת הפעולה הפוגעת, ובאיזו שורה הוקצה החוצץ שנדרס? מצאו את שתי השורות במחסנית הקריאות.
...
3. הסתכלו בקלט הממוזער (למשל עם `xxd`). מהו הבית שמגדיר את האורך שגורם לגלישה, ומה ערכו המינימלי כדי לחרוג מ-16?
