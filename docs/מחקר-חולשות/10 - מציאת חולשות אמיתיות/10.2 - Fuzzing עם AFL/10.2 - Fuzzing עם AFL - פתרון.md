...
3. הערך `len` הוא `uint8_t`, כלומר עד 255. השורה `if (off + len > n) len = n - off;` מגבילה רק את הקריאה מ**הקלט** (שלא נקרא מעבר לסוף הקובץ), אבל **לא** את הכתיבה ל-`name`. לכן אם הקובץ מכיל רשומת-שם עם `len` גדול מ-32 ומספיק בתים אחריו, `memcpy` תכתוב עד 255 בתים לתוך buffer של 32 - overflow מחסנית קלאסית.
