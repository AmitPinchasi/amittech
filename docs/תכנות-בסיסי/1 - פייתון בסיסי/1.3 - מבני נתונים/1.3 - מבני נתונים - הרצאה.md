...
![איור אנימציה המראה גישה לאותיות במחרוזת לפי מקום, כאשר הספירה של האינדקסים מתחילה מאפס](../../../images/illustrations/1-3-string-indexing.svg)
...
- בדוגמה למטה, הפעולה ( )split. מוחקת את הרווחים הנצאים במשתנה string, שומרת את כל התוים הנמצאים בין רווח לרווח ושומרת אותם כelements נפרדים ברשימה. 
```python
...
- גישה לelements ברשימה לפי מיקום ברשימה:
...
![איור אנימציה המראה כיצד חיתוך רשימה אוסף את האיברים שבמקומות 1 עד 3 ולא כולל את האיבר שבמקום 4](../../../images/illustrations/1-3-list-slicing.svg)
...
- קבלת אורך רשימה (כמה elements נימצאים ברשימה):
```python
my_list = [1, 2, 3, 4, 5]
length = len(my_list)
...
## מילון
- מילון ב-Python הוא מבנה נתונים שמאפשר לאחסן זוגות של keys וערכים. כל key במילון הוא ייחודי ומצביע על ערך מסוים (value). המילון ב-Python מוגדר באמצעות סוגריים מסולסלים `{}` ומפרידים בין key לערך שלו על ידי " : ".
...
- גישה לערך של key במילון:
```python
name = my_dict['name']
```

- שינוי ערך של key במילון:
```python
my_dict['age'] = 26
```

- מחיקת key מהמילון + גישה לkey שמחקנו
```python
removed_value = my_dict.pop('city')
```

- גישה לרשימה של הkeys של המילון
```python
keys = my_dict.keys()
```
...
בנוסף, הם מסודרים תמיד. הelements של סטים יהיו מסודרים לפי הערך שלהם לא כמו רשימות, למשל אם נגדיר את הסט הבא: `{2, 1, 3}` הוא יתורגם ל `{3, 2, 1}`.
...
- הוספת elements לסטים:
- בדרך זו תוכלו להוסיף elements לסטים שלכם.
```python
set1 = {1, 2, 3, 4, 5}
set1.add(6)
set1.update([7, 8, 9])
...
- פעולות נפוצות על 2 סטים במקביל:
	...
	- פעולת intersection (חיתוך) - מחזיר סט של רק הelements שזהיים בין שני הסטים.
	- פעולת difference - מחזיר סט של הelements של הסט הראשון השונים מן הelements של הסט השני (לא מחזיר את הסט של הelements הנמצאים בסט השני וואינם נמצאים בראשון).
	- פעולת symmetric_difference - מחזיר סט של הelements השונים בין שני הסטים.
```python
set1 = {1, 2, 3, 4, 5}
set2 = {3, 4, 5, 6, 7}
...
טאפלים הם בדיוק כמו רשימות, רק שהם "לא ניתנים לשינוי" או באנגלית "immutable, מה זה אומר?
 זה אומר שאחרי שאנחנו מגדירים טאפל, אנחנו לא יכולים לשנות את הelements בטאפל, לא להוסיף elements, ולא למחוק.
...
- בדיקת האם element בטאפל:
- יחזיר אמת או שקר בהתאם להאים הelement באמת נמצא בטופל או לא.
```python
tuple1 = (1, 2, 3, 4, 5)
...
|יחודיות|הelements יכולים לחזור אחד על השני|הelements לא יכולים לחזור אחד על השני|הelements יכולים לחזור אחד על השני|
|דוגמה מתי להשתמש|כמעט כל הזמן|כאשר אנחנו רוצים לסדר רשימה או למחוק את הelements החוזרים|כאשר אנחנו רוצים לעשות פיצול|

## סיקואנס
- סיקואנס או באנגלית "sequence" היא property שקיימת לdatatypes, הproperty הזו קיימת לdatatypes כמו רשימה, מחרוזת ומילון.
```python
l = ["h", "e", "l", "l", "o"]
s = "hello"
d = {
...
## מיוטאבילטי 
- מיוטאבילטי או באנגלית "mutability" היא property בפייתון של datatypes שמציינת האם האובייקט ניתן לשינוי אחרי שנוצר.
...
דוגמאות:
- אובייקטים מיוטאבילים: רשימה, מילון, וסטים. אפשר להוסיף, להוריד ולשנות elements באובייקטים האלו.
