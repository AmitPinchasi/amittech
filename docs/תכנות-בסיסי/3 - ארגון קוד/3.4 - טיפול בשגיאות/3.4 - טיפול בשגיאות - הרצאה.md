...
![איור אנימציה שמראה שגיאה שקורית בפונקציה פנימית ועולה במעלה שרשרת הקריאות, בדיוק השרשרת שהסטאק טרייס מציג](../../../images/illustrations/3-4-stack-trace.svg)
...
![תרשים זרימה אנימטיבי של try, except, else, finally: כשקורית שגיאה רץ הבלוק except, כשלא קורית שגיאה רץ הבלוק else, והבלוק finally רץ תמיד](../../../images/illustrations/3-4-try-except-flow.svg)

## משפחות של שגיאות
- בתמונה הבאה קיימים המון שגיאות בפייתון.
  ![דיאגרמת היררכיית שגיאות בפייתון: Exception מוביל ל-StandardError, ומשם למשפחות שגיאות כמו ArithmeticError (הכולל ZeroDivisionError, OverflowError, FloatingPointError), LookupError (הכולל IndexError, KeyError), EnvironmentError (הכולל OSError, IOError), וכן NameError, ValueError, ImportError ועוד](../../../images/Pasted image 20240204160329.png)
