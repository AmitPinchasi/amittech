...
**פונקציה מובנית `map`**
- הפונקציה המובנית `map` מבצעת פעולה שאנחנו מגדירים לה על כל הelements ברשימה שאנחנו נותנים לה.
...
- מוציא elements מרשימה בהתבסס על תנאי.
- בדרך כלל אנחנו מעבירים לפונקציה `filter` כפרמטר, תנאי שמטרתו להוציא elements מסויימים מרשימה אם הוא נכון.
```python
...
- הפונקציה filter דומה לmap, רק שמטרתה להוציא elements מהרשימה.
...
![איור שמראה שהדקורטור מקבל את הפונקציה say_hello כפרמטר ומחזיר פונקציה חדשה בשם wrapper שעוטפת אותה בקוד נוסף](../../../images/illustrations/3-3-decorator-wrap.svg)
...
for item in my_list:
	print(item)
```
- הבעיה, שהקוד הזה יהיה מאוד איטי, בגלל שקודם כל אנחנו צריכים ליצור רשימה שעלולה להיות גדולה מאוד, ורק אחרכך אנחנו נוכל לבצע עלייה פעולות, כמו למשל להדפיס את כל הelements שלה.
- במקרה הזה, דוגמה לקוד יותר יעיל יהיה: בזמן שאנחנו יוצרים elements ברשימה, נדפיס אותם אחד אחרי השני. 
- כך שבזמן שאנחנו יוצרים את כל הelements ברשימה, נבצע עלייה את הפעולות. למשל, כאשר אנחנו יוצרים רשימה, נדפיס כל element שיצרנו.
...
- דמיינו רשימה שיוצרת את עצמה בזמן שאנחנו עוברים עלייה עם לולאה ומדפיסים את הelements שלה, זה דוגמה לגנרטור.
...
for square in squares:
    print(square)
```
-  למעשה, כאשר הקוד שלנו עובר עם for על הגנרטור squares_numbers, בכל יצירה של element, הוא גם מדפיס אותו.
- כלומר, הקוד שלנו יקפוץ בין הfor שיש בפונקציה square_numbers לfor שיש למטה סיכרונית.

![איור אנימציה של גנרטור: הקוד קופץ בין הלולאה לפונקציית הגנרטור, ובכל סיבוב yield מחזיר ערך אחד בלבד להדפסה](../../../images/illustrations/3-3-generator-yield.svg)
